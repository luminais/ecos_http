...
#endif
#if 0 //luminais mark  (use 239.x.x.x)
#define RTL_IGMP_NOT_PROCESS_RESERVEDADDR 1
#endif
...
#define	IF_DEBUG(l)	if (mrouted_debug && mrouted_debug & (l))
...
//#undef LOG
#define LOG(level, function, str, args...) do { printf("[%15s]\t{%28s}\t\t%s,\t\t","DHCP",function, level); \
				printf(str, ## args); \
				printf("\n"); } while(0)
/*#define log(level, function, str, args...) do { printf("[%15s]\t{%28s}\t\t%s,\t\t","DHCP",function, level); \
					printf(str, ## args); \
...
#define LOG_INFO	""
#define LOG_DEBUG	""
#define LOG(level, function, str, args...) do {;} while(0)
#endif
#endif
/* main.c */
...
/* igmp.c */
extern int 		init_igmp __P((void));
extern void 	exit_igmp __P((void));
...
/* config.c */
extern int		config_vifs_from_kernel __P((void));
//extern int get_ifaddrs __P((void));
//extern void buildIfVc __P((void));
...
extern void 	exit_ktable __P((void));
...
extern void 		update_lclgrp __P((vifi_t, u_int32, u_int32));
extern struct gtable * find_lclgrp __P((vifi_t, u_int32, u_int32)); 

extern int		delete_lclgrp __P((int, u_int32, u_int32, int));
extern int 		SetExpireTimer __P((int,vifi_t, u_int32,u_int32, int));
extern int 		DeleteExpireTimer __P((int));
...
extern void		k_reinit_dvmrp __P((void));
...
#ifdef 	ECOS_DBG_STAT
extern int dbg_igmpproxy_index;
#endif
