    ...
    struct gtable *g;
{
	...
	u_int32 src;
	if(g==NULL){
		//diag_printf("gtable is Null[%s]:[%d].\n",__FUNCTION__,__LINE__);
		return;
	}	

	 v   = &uvifs[vifi];
    src = v->uv_lcl_addr;
	DBG_PRINT("grp:%x.src:%x.[%s]:[%d].\n",g->gt_mcastgrp,src,__FUNCTION__,__LINE__);
	k_join(g->gt_mcastgrp,src );
...
exit_ktable()
{
	...
	while(g)
	{
		...
		//diag_printf("g->gt_mcastgrp:%x",g->gt_mcastgrp);
    ...
    u_int32 old_parent_gw;
{
	...
	while ((pt = *ptnp)) {
		...
		diag_printf("update_table_entry deleting prune for (%s %s) from %s on vif %d -%s%s",
			RT_FMT(r, s1), inet_fmt(g->gt_mcastgrp, s2),
	...
	//IF_DEBUG(DEBUG_CACHE)
	diag_printf("updating cache entries (%s %s) old gm:%x",
	    RT_FMT(r, s1), inet_fmt(g->gt_mcastgrp, s2),
	    g->gt_grpmems);
	...
	//IF_DEBUG(DEBUG_CACHE)
	diag_printf( "updating cache entries (%s %s) new gm:%x",
	    RT_FMT(r, s1), inet_fmt(g->gt_mcastgrp, s2),
	    g->gt_grpmems);
...
{
	struct stable  *s=NULL;
	struct stable  *s_next;
	struct stable  *s_new=NULL;
	int find=0;
	s=g->gt_srctbl;
	while(s)
	{
		...
		if(src==s->st_origin){
			find=1;
		if(s->st_timerid)	
			s->st_timerid = DeleteSrcExpireTimer(s->st_timerid);
		s->st_timerid = SetSrcExpireTimer(IGMP_GROUP_MEMBERSHIP_INTERVAL, igmp_down_if_idx, g->gt_mcastgrp,src,0);	
			break;
		}
		s=s_next;
	...
	if(find==0)
	{
		DBG_PRINT("new src!grp:%x,src:%x.[%s]:[%d].\n",g->gt_mcastgrp,src,__FUNCTION__,__LINE__);
		if((s==NULL)||(src!=s->st_origin))
		{
			DBG_PRINT("new src![%s]:[%d].\n",__FUNCTION__,__LINE__);
			s_new = (struct stable *)malloc(sizeof(struct stable));
			if (s_new == NULL){
			    log(LOG_ERR, 0, "ran out of memory");
				return;
			}	
			...
			#endif
			s_new->st_origin	=src;
			s_new->st_timerid = SetSrcExpireTimer(IGMP_GROUP_MEMBERSHIP_INTERVAL, igmp_down_if_idx, g->gt_mcastgrp,src,0);
			time(&s_new->st_ctime);
			s_new->st_pktcnt = 0;
			s_new->st_savpkt = 0;
			s_new->st_next=g->gt_srctbl;
			g->gt_srctbl= s_new;
			g->gt_srccnt++;
			DBG_PRINT("new src!time:%d.%d.g->gt_srccnt:%d.[%s]:[%d].\n",time(0),s_new->st_ctime,g->gt_srccnt,__FUNCTION__,__LINE__);
		}
		
		
	}else
	{
		DBG_PRINT("old src!grp:%x.src:%x.time:%d.%d.[%s]:[%d].\n",g->gt_mcastgrp,src,time(0),s->st_ctime,__FUNCTION__,__LINE__);
		time(&s->st_ctime);
		#if 0
		if (s->st_ctime == 0) {
		    /* An old source which we're keeping around for statistics */
...
#ifdef DEBUG_MFC
		    md_log(MD_DUPE, src, g->gt_mcastgrp);
#endif
		    /* Ignore kernel->mrouted retransmissions */
		    if (time(0) - s->st_ctime > 5)
			log(LOG_WARNING, 0, "kernel entry already exists for (%s %s)",
				inet_fmt(src, s1), inet_fmt(g->gt_mcastgrp, s2));
		    k_add_rg(src, g);
		    return;
		}
		#endif
...

{
	...
	while (p) {
		if(p->st_origin == src) {
			*q = p->st_next;	
			if(p->st_timerid)	
				p->st_timerid = DeleteSrcExpireTimer(p->st_timerid);
			free(p);
...
/*
 * Time out record of a group membership
 */
static void
exprie_mbrentry(arg)
	void *arg;
{
	...
	flag = ep->flag;
	vifi=ep->vifi;
    mcastgrp = ep->mcastgrp;
	src=ep->src;
	
	DBG_PRINT("DEL group mbr entry.flag:%x.vifi:%x.mcastgrp:%x.[%s]:[%d].\n",flag,vifi,mcastgrp,__FUNCTION__,__LINE__);
	/*
	 * Group has expired
	 * delete all kernel cache entries with this group
	 */
	g=find_lclgrp(vifi, mcastgrp,src);
	if(g)
	{
		//if (g->gt_query)
		//	DeleteExpireTimer(g->gt_query);
	
		ret=delete_lclgrp(vifi, mcastgrp,src,flag);
	}
 ...
 */
int
SetExpireTimer(delaytime,vifi, mcastgrp,src, flag)
	int delaytime;
	vifi_t vifi;
	u_int32 mcastgrp;
	u_int32 src;
...
{
	ep_t *ep;
	DBG_PRINT("delaytime:%d.vifi:%xmcastgrp:%x.src:%x.flag:%x.[%s]:[%d].\n",
		delaytime,vifi,mcastgrp,src, flag,__FUNCTION__,__LINE__);
	...
	dbg_stat_add(dbg_igmpproxy_index, DBG_TYPE_MALLOC,DBG_ACTION_ADD, 0);
#endif
	ep->mcastgrp = mcastgrp;
	ep->src =src;
	ep->vifi = vifi;
	ep->flag =flag;	
	return timer_setTimer(delaytime, exprie_mbrentry, ep);
}



int
DeleteSrcExpireTimer(id)
	int id;
{
	timer_clearTimer(id);
	return 0;
 ...
 */
int
SetSrcExpireTimer(delaytime,vifi, mcastgrp,src, flag)
	int delaytime;
	vifi_t vifi;
	u_int32 mcastgrp;
	u_int32 src;
	int flag;
{
	return SetExpireTimer(delaytime,vifi, mcastgrp,src, flag);
}

struct gtable *
find_lclgrp(vifi, mcastgrp,src)
    vifi_t vifi;
    u_int32 mcastgrp;
	u_int32 src;
{
    struct rtentry *r;
    struct gtable *g=NULL;
	//struct gtable **gt,*prev_gt;
	struct gtable *g_next=NULL;
    int find=0;
   
    DBG_PRINT("group %s joined on vif %d.[%s]:[%d].\n",inet_fmt(mcastgrp,s1), vifi,__FUNCTION__,__LINE__);
	g = kernel_table;
 ...
 */
void
update_lclgrp(vifi, mcastgrp,src)
    vifi_t vifi;
    u_int32 mcastgrp;
	u_int32 src;
{
	...
	while(g)
	{
		...
		if (g->gt_mcastgrp == mcastgrp )
		{
			...
			update_srctbl(g,src);
			if (g->gt_timerid)
				g->gt_timerid = DeleteExpireTimer(g->gt_timerid);
			g->gt_timerid=SetExpireTimer(IGMP_GROUP_MEMBERSHIP_INTERVAL, igmp_up_if_idx, mcastgrp,src,1);
			DBG_PRINT("find the group(%x),src(%x) in kernel table,,g->gt_timerid:%d![%s]:[%d].\n",mcastgrp,src,g->gt_timerid,__FUNCTION__,__LINE__);
			break;	
		}
		g = g_next;
    }
	if(find==0){
		DBG_PRINT("new group![%s]:[%d].\n",__FUNCTION__,__LINE__);
		if (g== NULL || g->gt_mcastgrp != mcastgrp) 
		{
			g = (struct gtable *)malloc(sizeof(struct gtable));
			if (g == NULL){
			    log(LOG_ERR, 0, "ran out of memory");
				return ;
			}	
				...
				dbg_stat_add(dbg_igmpproxy_index, DBG_TYPE_MALLOC,DBG_ACTION_ADD, 0);
			#endif
			bzero(g, sizeof(struct gtable));
			g->gt_mcastgrp	    = mcastgrp;
			...
			g->gt_route	    = r;
			g->gt_rexmit_timer = 0;
			NBRM_CLRALL(g->gt_prunes);
			g->gt_prune_rexmit = PRUNE_REXMIT_VAL;
			VIFM_SET(vifi, g->gt_grpmems);
			
	   		g->gt_gnext = kernel_table;
		    g->gt_gprev = NULL;
		    kernel_table = g;
			if (g->gt_gnext)
		    	g->gt_gnext->gt_gprev = g;
			
			update_srctbl(g,src);
			
			g->gt_timerid=SetExpireTimer(IGMP_GROUP_MEMBERSHIP_INTERVAL, vifi, mcastgrp,src,1);
			DBG_PRINT("new group %x g->gt_timerid:%d.![%s]:[%d].\n",mcastgrp,g->gt_timerid,__FUNCTION__,__LINE__);
		}
		update_kernel(g);
 ...
 */
int 
delete_lclgrp(vifi, mcastgrp,src,flag)
    vifi_t vifi;
    u_int32 mcastgrp;
	u_int32 src;
	int flag;
{
    ...
    DBG_PRINT("group %s left on vif %d,[%s]:[%d]",inet_fmt(mcastgrp, s1), vifi,__FUNCTION__,__LINE__);
	...
	while(g)
	{
		...
		if (g->gt_mcastgrp == mcastgrp ) {
			...
			DBG_PRINT("srccnt:%x.g->gt_mcastgrp:%x,g->gt_timerid:%x.src:%x.[%s]:[%d].\n",srccnt,g->gt_mcastgrp,g->gt_timerid,src,__FUNCTION__,__LINE__);

			if(srccnt==0||flag){
				
				k_leave(g->gt_mcastgrp,src );
				k_del_rg(src, g);
				st=g->gt_srctbl;
				while(st){
					st_nxt = st->st_next;
