    ...
    struct listaddr *n;
{
    register struct rtentry *r;
    u_int adj_metric;
	diag_printf("origin:%x,mask:%x,metric%x,src:%x,vifi:%x[%s]:[%d].\n",
		origin,mask, metric, src, vifi,__FUNCTION__,__LINE__);
    /*
...
{
    struct rtentry *rt;
	diag_printf("src:%x.[%s]:[%d].\n",src,__FUNCTION__,__LINE__);
