...
static void generate_ssid(CTX_Tp pCtx)
{
	...
	strncat(pCtx->SSID, (char *)randSSID, strlen((char *)randSSID));
}
#endif
...
#else
	byteVal = 1;
	pMsg = add_tlv((unsigned char *)&out[idx], TAG_NETWORK_INDEX, 1, &byteVal);
#endif
#if 0 //defined(CONFIG_CUTE_MAHJONG)
	strcpy(tmpbuf, pCtx->SSID);
	//CMJ wlan idx maybe wrong when set it from interfacename, it can use the wlan0 phy_band_select for it is selective mode.
        apmib_get_ext(MIB_WLAN_PHY_BAND_SELECT, (void *)&size, 0, 0);

	//if((size == 2) && (pCtx->config_state == CONFIG_STATE_UNCONFIGURED) )
	if((size == 2))
	{
		strcat(tmpbuf, "-5G");
	}
	pMsg = add_tlv(pMsg, TAG_SSID, strlen(tmpbuf), tmpbuf);
	TX_DEBUG("SSID=%s\n",tmpbuf);
#else
...
extern struct eth_drv_sc rltk819x_wlan1_sc0;	//interface ; todo 
#endif
//#endif
...
#endif
#ifdef CONFIG_RTL_DUAL_PCIESLOT_BIWLAN
extern struct eth_drv_sc  rltk819x_wlan1_vxd_sc0;
#endif
...
#ifdef FOR_DUAL_BAND
	if(pCtx->is_ap){
		if(pCtx->InterFaceComeIn == COME_FROM_WLAN1 || pCtx->wps_on_wlan1 == 1){
			return rltk819x_send_wlan(&rltk819x_wlan1_sc0, data, size);
		}
		else if(pCtx->InterFaceComeIn == COME_FROM_WLAN0){
...
#endif		
	{
		if(!strcmp(pCtx->wlan_interface_name,WLAN0_INTF_NAME))
			return rltk819x_send_wlan(&rltk819x_wlan_sc0, data, size);
#ifdef RTLPKG_DEVS_ETH_RLTK_819X_WLAN_REPEATER_MODE
		else if(!strcmp(pCtx->wlan_interface_name,WLAN0_VXD_INTF_NAME))
			return rltk819x_send_wlan(&rltk819x_wlan0_vxd_sc0, data, size);
#endif

#ifdef CONFIG_RTL_DUAL_PCIESLOT_BIWLAN
		else if(!strcmp(pCtx->wlan_interface_name,WLAN1_INTF_NAME))
			return rltk819x_send_wlan(&rltk819x_wlan1_sc0, data, size);
#ifdef RTLPKG_DEVS_ETH_RLTK_819X_WLAN_REPEATER_MODE
		else if(!strcmp(pCtx->wlan_interface_name,WLAN1_VXD_INTF_NAME))
			return rltk819x_send_wlan(&rltk819x_wlan1_vxd_sc0, data, size);
#endif
#endif
...
#ifdef FOR_DUAL_BAND
	if(pCtx->InterFaceComeIn==COME_FROM_WLAN0)
		size = add_credential_attr(pCtx, pSta, (char *)tmpbuf, pCtx->is_ap);
	else if(pCtx->InterFaceComeIn==COME_FROM_WLAN1)
		size = add_credential_attr2(pCtx, pSta, (char *)tmpbuf, pCtx->is_ap);
#else
		size = add_credential_attr(pCtx, pSta, (char *)tmpbuf, pCtx->is_ap);
#endif
