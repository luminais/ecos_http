	      ...
	      unsigned int iface)
{
  ...
  if (!nowild)
...
#ifdef HAVE_IPV6
	{
	  ...
	  //diag_printf("%s:%d####iface=%d\n",__FUNCTION__,__LINE__,iface);

	 //char tmpbuf11[128];
	  //inet_ntop(AF_INET6, source->addr.addr6.s6_addr, tmpbuf11, 128);			
	  //diag_printf("%s:%d ####p.ipi6_addr=%s\n", __FUNCTION__,__LINE__,tmpbuf11);
  ...
  while (sendmsg(fd, &msg, 0) == -1)
    {
      ////diag_printf("%s:%d####send msg fails!\n",__FUNCTION__,__LINE__);
      if (retry_send())
	continue;
			 ...
			 struct dns_header *header, size_t plen, time_t now, struct frec *forward)
{
  ...
  if (!daemon->servers)
  {
    diag_printf("%s:%d ####no server to use!\n",__FUNCTION__,__LINE__);
    forward = NULL;
  }
    ...
    {
    //diag_printf("%s:%d #### gotname=%u\n",__FUNCTION__,__LINE__,gotname);
	
      if (gotname)
	flags = search_servers(now, &addrp, gotname, daemon->namebuff, &type, &domain, &norebind);

	  //diag_printf("%s:%d ####flags=%d\n",__FUNCTION__,__LINE__,flags);
...
#if 0  //print server address for debug
		if (start->addr.sa.sa_family == AF_INET6)
		{			
			inet_ntop(AF_INET6, start->addr.in6.sin6_addr.s6_addr, tmpbuf11, 128);			
			diag_printf("%s:%d ####server_addr=%s\n", __FUNCTION__,__LINE__,tmpbuf11);
		}
		else
		{
			inet_ntoa_r(start->addr.in.sin_addr, tmpbuf11);
			diag_printf("%s:%d ####server_addr=%s\n", __FUNCTION__,__LINE__,tmpbuf11);
		}
#endif		
		if (sendto(fd, (char *)header, plen, 0,&start->addr.sa, sa_len(&start->addr)) == -1)
		{
			  ////diag_printf("%s:%d ####sendto fail!\n",__FUNCTION__,__LINE__,fd);
			  if (retry_send())
			    continue;
		}
	      else
		{
		  ////diag_printf("%s:%d ####sendto success!\n",__FUNCTION__,__LINE__);
...
//#ifdef CONFIG_RTL_819X	
	 //diag_printf("%s:%d####if_index=%d\n",__FUNCTION__,__LINE__,if_index);
	 if(if_index==0)
	 {
	  	if_index=1;
