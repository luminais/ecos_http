...
#ifndef CONFIG_RTL_819X	  
	memset(source_addr->in6.sin6_addr.s6_addr, 0, sizeof(u_int8_t)*16);
#else
...
static void read_file(char *file, FILE *f, int hard_opt)	
{
  ...
  //diag_printf("%s:%d####hard_opt=%d\n",__FUNCTION__,__LINE__,hard_opt);
  while (fgets(buff, MAXDNAME, f))
      ...
      if (option == 0)
	{
	  ...
	  {	  
	  //diag_printf("%s:%d####start=%s\n",__FUNCTION__,__LINE__,start);
	    if (strcmp(opts[i].name, start) == 0)
	      {	      
		  ////diag_printf("%s:%d####name=%s val=%c\n",__FUNCTION__,__LINE__,opts[i].name,opts[i].val);
		option = opts[i].val;
		break;
	      }
	  }
...
void read_opts(char *compile_opts)
{
  ...
  conffile = opt_string_alloc(CONFFILE);
  //diag_printf("%s:%d####conffile=%s\n",__FUNCTION__,__LINE__,conffile);
