...
static int make_sock(union mysockaddr *addr, int type, int dienow)
{
  ...
  if ((fd = socket(family, type, 0)) == -1)
    ...
    err:
	diag_printf("%s:%d####create socket fails!\n",__FUNCTION__,__LINE__);
...
static struct listener *create_listeners(union mysockaddr *addr, int do_tftp, int dienow)
{
  ...
  if (fd != -1 || tcpfd != -1)
    {
      //diag_printf("%s:%d####fd=%d tcpfd=%d\n",__FUNCTION__,__LINE__,fd,tcpfd);
...
void create_bound_listeners(int dienow)
{
  ...
  for (iface = daemon->interfaces; iface; iface = iface->next)
	...
	(new = create_listeners(&iface->addr, 0, dienow)))
      {
	new->iface = iface;
  ...
  for (if_tmp = daemon->if_addrs; if_tmp; if_tmp = if_tmp->next)
    if (!if_tmp->used && (new = create_listeners(&if_tmp->addr, 0, dienow)))
      {
	new->iface = NULL;
	new->next = daemon->listeners;
	daemon->listeners = new;
	...
	while(tries--)
	  {
		...
		memset(addr.in6.sin6_addr.s6_addr, 0, sizeof(u_int8_t)*16);
#else
		addr.in6.sin6_addr = in6addr_any; 
#endif
...
int local_bind(int fd, union mysockaddr *addr, char *intname, int is_tcp)
{
  ...
  if (intname[0] != 0 && setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, intname, IF_NAMESIZE) == -1)
  {
  	diag_printf("%s:%d ####bind interface %s fails!\n",__FUNCTION__,__LINE__,intname);
    return 0;
  }
#endif

  //diag_printf("%s:%d ####intname=%s\n",__FUNCTION__,__LINE__,intname);
