...
struct lib1x_nal_intfdesc *  lib1x_nal_initialize( u_char * intdev , u_char * ouraddr, u_char inttype)
{
	...
	/* 1. Create a socket */
	/*linux and ecos defined EAP_BY_QUEUE*/
#ifndef EAP_BY_QUEUE
	...
	// desc->pf_sock = pcap_fileno(desc->pcap_desc);
	 ...
	 else
	 {
		...
		desc->udpsock = socket( AF_INET, SOCK_DGRAM, IPPROTO_UDP);\
		if ( desc->udpsock == -1 )
			lib1x_message(MESS_ERROR_FATAL,"lib1x_nal_initialize: Could not open Radius Authentication UDP socket !");

		desc->acctsock = socket( AF_INET, SOCK_DGRAM, IPPROTO_UDP);\
		if ( desc->acctsock == -1 )
			...
			int addrlen, int udpsock_type)
{
	...
	myaddr.sin_addr.s_addr = htonl(INADDR_ANY);//htonl("192.168.3.151");
...
#if 0
		flags = fcntl(desc->udpsock, F_GETFL ,0);
		flags |= O_NONBLOCK;
		if ( fcntl( desc->udpsock, F_SETFL, flags) != 0 )
			lib1x_message( MESS_ERROR_FATAL, "lib1x_nal_connect : FCNTL failed on UDP socket %d",errno );
#endif
		break;
...
#if 0
		flags = fcntl(desc->acctsock, F_GETFL,0 );
		flags |= O_NONBLOCK;
		if ( fcntl( desc->acctsock, F_SETFL, flags) != 0 )
			lib1x_message( MESS_ERROR_FATAL, "lib1x_nal_connect : FCNTL failed on UDP socket %d ",errno );
#endif
		break;
...
int lib1x_nal_msg_receive()
{
	msg_hdr_t *msg_hdr;
	 rtl_1x_global_config_t *pConf;
	Dot1x_Authenticator *auth;
	struct lib1x_nal_intfdesc * descSupp;
#ifdef RTL_RADIUS_2SET
	struct lib1x_nal_intfdesc * descSvr2;
#endif
	int suppid;
	struct lib1x_nal_intfdesc * descSvr;
	int i;
...
#endif
	{
	/*First fetch wlan event*/
#ifdef HAVE_SYSTEM_REINIT
		msg_hdr = cyg_mbox_timed_get(auth0_mbox_hdl, cyg_current_time() + msec_to_tick(500));
#else
		msg_hdr = (msg_hdr_t *)cyg_mbox_get(auth0_mbox_hdl);
#endif
		if(msg_hdr == NULL)
		{
			int i;
		...
		pConf = get_auth_interface(msg_hdr->msg_buf);
		if(pConf == NULL){
			free(msg_hdr);
			continue;
		}
		...
		auth = &pConf->RTLAuthenticator;
		descSvr = auth->GlobalTxRx->network_svr;
		descSupp = auth->GlobalTxRx->network_supp;
#ifdef RTL_RADIUS_2SET
		descSvr2 = auth->GlobalTxRx->network_svr2;
#endif
		char *src = msg_hdr->msg_buf+RTL_1X_INTERFACE_LEN;
		memcpy(auth->GlobalTxRx->RecvBuf,src,msg_hdr->msg_len-RTL_1X_INTERFACE_LEN);
		
		auth->GlobalTxRx->network_supp->packet.data = auth->GlobalTxRx->RecvBuf+FIFO_HEADER_LEN;
		auth->GlobalTxRx->network_supp->packet.caplen = msg_hdr->msg_len-FIFO_HEADER_LEN-RTL_1X_INTERFACE_LEN;
		free(msg_hdr);
#ifdef EAP_BY_QUEUE
...
#ifdef HAVE_SYSTEM_REINIT
	while(msg_hdr = (msg_hdr_t *)cyg_mbox_tryget(auth0_mbox_hdl)){
		free(msg_hdr);
	}
	auth_mbox_quitting = 0;
#endif

}

int lib1x_nal_mutil_receive()
{
	...
	FD_ZERO ( &fsRead);
	for(i = 0;i < rtl_1x_interface_num;i++){
		auth = &rtl_1x_global_config[i].RTLAuthenticator;
		descSvr = auth->GlobalTxRx->network_svr;
#ifdef RTL_RADIUS_2SET
		descSvr2 = auth->GlobalTxRx->network_svr2;
#endif
...
do_auth:
	for(i = 0; i < rtl_1x_interface_num;i++){
			lib1x_do_authenticator( &rtl_1x_global_config[i].RTLAuthenticator );
	}
#endif
}
	return 1; // david
...
int lib1x_nal_receive(Dot1x_Authenticator * auth)
{
	...
	msg_hdr_t *msg_hdr;
/*first: try get mssage from wifi driver -- leo*/
	while(1)
	{
				extern cyg_handle_t auth0_mbox_hdl;
				msg_hdr = (msg_hdr_t *)cyg_mbox_tryget(auth0_mbox_hdl);
				if(msg_hdr == NULL)
					goto recive_from_udp;
				memcpy(auth->GlobalTxRx->RecvBuf,msg_hdr->msg_buf+WLAN_INTF_LEN,msg_hdr->msg_len-WLAN_INTF_LEN);
				
				auth->GlobalTxRx->network_supp->packet.data = auth->GlobalTxRx->RecvBuf+FIFO_HEADER_LEN;
				auth->GlobalTxRx->network_supp->packet.caplen = msg_hdr->msg_len-FIFO_HEADER_LEN;
				free(msg_hdr);	
...
BOOLEAN lib1x_nal_send( struct lib1x_nal_intfdesc * desc,  char * packet , int size)
{
	...
	else
	{
		...
		//num_sent = sendto(desc->udpsock, (void*) packet, size, 0,(struct sockaddr *)&radaddr,sizeof(struct sockaddr_in));
...
#ifdef HAVE_SYSTEM_REINIT
void lib1x_nal_deinitialize( struct lib1x_nal_intfdesc *desc,  u_char inttype)
{
	if(!desc)
		return;
	if ( inttype == LIB1X_IT_PKTSOCK )
	{
		if(desc->libnet_desc){
			close(desc->libnet_desc->fd);
			free(desc->libnet_desc);
		}
