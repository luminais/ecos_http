		...
		{ register int nwritten;
			...
			{ if (soap_valid_socket(soap->socket))
					...
					if (soap->ssl)
						...
						if( soap->conn )
						{
							...
							if(nwritten==0)
								...
								diag_printf("call sslWrite(), n=%d, nwritten:%d, status:%d\n", n, nwritten, status );
						}else
#endif
#ifdef WITH_UDP
						...
						if (nwritten <= 0)
						{
							...
							{
								diag_printf("sslwrite error( errno:%d )\n", soap_socket_errno );
						...
						soap->start_send_time.tv_sec==0 && soap->start_send_time.tv_usec==0 )
				{
					...
					time(&soap->start_send_time.tv_sec); // because gettimeofday here cannot get the correct seconds since 1970-01-01 00:00:00 +0000 (UTC)
					diag_printf("set start_send_time={%d,%d}\n", soap->start_send_time.tv_sec, soap->start_send_time.tv_usec );
				}
...
//							diag_printf("<%s:%d>\n", __FUNCTION__, __LINE__);
							...
							if(r<=0) diag_printf("call ssRead(), n=%d, return=%d, status=%d\n", n, r, status );
							if(r>0)
							{
								return (size_t)r;
							}else if(r<0)
...
//							diag_printf("<%s:%d>soap->socket=%d\n", __FUNCTION__,__LINE__, soap->socket);
							...
							if( r>0 && soap->get_state_for_tr143 )
							{
								if( soap->start_recv_time.tv_sec==0 && soap->start_recv_time.tv_usec==0 )
								{
									...
									time(&soap->start_recv_time.tv_sec); // because gettimeofday here cannot get the correct seconds since 1970-01-01 00:00:00 +0000 (UTC)
									diag_printf("set start_recv_time={%d,%d}\n", soap->start_recv_time.tv_sec, soap->start_recv_time.tv_usec );
								}
//								diag_printf("test_bytes_rx=%d, r=%d, %d\n", soap->test_bytes_rx, r, soap->test_bytes_rx+r );
...
#ifndef WITH_LEAN
//			diag_printf("<%s:%d> soap->count=%d\n",__FUNCTION__,__LINE__,soap->count);
//			diag_printf("<%s:%d> soap->buflen=%d\n",__FUNCTION__,__LINE__,soap->buflen);
			//msleep(10);
...
//			diag_printf("<%s:%d> soap->count=%d\n",__FUNCTION__,__LINE__, soap->count);
	...
	SOAP_FMAC1
		...
		{
//	        diag_printf("<%s:%d>\n", __FUNCTION__,__LINE__);
#ifndef WITH_LEANER //not used, defined in stdsoap.h
			if (soap->mode & SOAP_ENC_DIME)
			{
//	            diag_printf("<%s:%d> soap->buflen=%d\n", __FUNCTION__,__LINE__,soap->buflen);
//	            diag_printf("<%s:%d> soap->dime.buflen=%d\n", __FUNCTION__,__LINE__,soap->dime.buflen);
			    if (soap->dime.buflen)
				{
//	                diag_printf("<%s:%d>\n", __FUNCTION__,__LINE__);
				    char *s;
...
//	            diag_printf("<%s:%d> soap->dime.chunksize=%d\n", __FUNCTION__,__LINE__,soap->dime.chunksize);
...
again:
	...
	SOAP_FMAC1
		...
		{ register struct soap_nlist *np = soap->nlist;
			...
			diag_printf("<%s:%d><SOAP_SYNTAX_ERROR>id1:%s, id2:%s\n", __FUNCTION__, __LINE__, id1,id2 );
	...
	SOAP_FMAC1
		...
		{ 
			diag_printf("enter soap_ssl_client_context: openssl\n");
			soap->keyfile = keyfile;
			soap->password = password;
			soap->cafile = cafile;
			soap->capath = capath;
			...
			diag_printf("soap_ssl_client_context: openssl - 1\n");
	...
	SOAP_FMAC1
		...
		{ /* Note: for MT systems, the main program MUST call soap_ssl_init() before any threads are started */
			//bbbbb;
			diag_printf("========enter soap_ssl_init: openssl========\n");
			if (!ssl_init_done)
			{ ssl_init_done = 1;
				SSL_library_init();
#ifndef WITH_LEAN
...
{ 
	diag_printf("enter ssl_auth_init, openssl, ssl_init_done=%d\n", ssl_init_done);
	if (!ssl_init_done)
		soap_ssl_init();
	//printf("===1===\n");
	if (!soap->ctx)
...
#endif
	//printf("ssl_auth_init returns: openssl\n");
	return SOAP_OK;
}
#endif
...
{
	//aaaaaa;
	//printf("enter soap_ssl_accept: openssl========\n");
	BIO *bio;
	int i, r;
	if (!soap_valid_socket(soap->socket))
...
{ /* Note: for MT systems, the main program MUST call soap_ssl_init() before any threads are started */
	...
	{
		ssl_init_done = 1;
		//printf("soap_ssl_init::matrixSslOpen!\n");
		/*
		...
		if (matrixSslOpen() < 0)
		{
			diag_printf("matrixSslOpen failed!\n");
		}
	}
}
#endif
...
{ soap_int32 iadd = -1;
	...
	if (!(host = gethostbyname((void*)addr)))
		soap->errnum = h_errno;
#else
	//if (!(host = gethostbyname(addr))) //by cairui
	//soap->errnum = h_errno;
#endif
#endif
	if (!host)
...
//	else
//		diag_printf("[%s:%d] tcp_connect, fd=%d\n", __FUNCTION__, __LINE__, fd);
	...
	for (;;)
	{
		...
		if(soap->get_state_for_tr143)
		{
			...
			time(&soap->tcp_open_request_time.tv_sec); // because gettimeofday here cannot get the correct seconds since 1970-01-01 00:00:00 +0000 (UTC)
			//fprintf( stderr, "set tcp_open_request_time={%d,%d}\n", soap->tcp_open_request_time.tv_sec, soap->tcp_open_request_time.tv_usec );
		}
#endif //_SUPPORT_TR143_
		//diag_printf("<%s:%d> fd=%d\n", __FUNCTION__,__LINE__, fd);
		//diag_printf("<%s:%d> soap=%p\n", __FUNCTION__,__LINE__, soap);
		//sleep(1);
#ifdef WITH_IPV6
		if (connect((SOAP_SOCKET)fd, res->ai_addr, res->ai_addrlen))
#else
			//diag_printf("<%s:%d> soap->peer.sin_port=%d\n", __FUNCTION__,__LINE__, soap->peer.sin_port);
	...
	if(soap->get_state_for_tr143)
	{
		...
		time(&soap->tcp_open_response_time.tv_sec); // because gettimeofday here cannot get the correct seconds since 1970-01-01 00:00:00 +0000 (UTC)
...
soap_bind(struct soap *soap, const char *host, int port, int backlog)
{
...
#endif
#ifndef WITH_LEAN
	if (soap->bind_flags && setsockopt((SOAP_SOCKET)soap->master, SOL_SOCKET, soap->bind_flags, (char*)&set, sizeof(int))) //bind_flags: SO_REUSEADDR
	{ soap->errnum = soap_socket_errno;
...
#if 1
	//diag_printf("==================Will set SO_REUSEADDR, soap->socket=%d, soap->master=%d==================\n", soap->socket,soap->master);
	if (soap->bind_flags && setsockopt((SOAP_SOCKET)soap->master, SOL_SOCKET, SO_REUSEADDR, (char*)&set, sizeof(int)))
	{
		//diag_printf("=================set SO_REUSEADDR failed===================\n");
		soap->errnum = soap_socket_errno;
		soap_set_receiver_error(soap, tcp_error(soap), "setsockopt SO_REUSEADDR failed in soap_bind()", SOAP_TCP_ERROR);
		return SOAP_INVALID_SOCKET;
	}	
	//diag_printf("==================Will set SO_REUSEPORT==================\n");
	if (soap->bind_flags && setsockopt((SOAP_SOCKET)soap->master, SOL_SOCKET, SO_REUSEPORT, (char*)&set, sizeof(int)))
	{
		//diag_printf("=================set SO_REUSEPORT failed===================\n");
		soap->errnum = soap_socket_errno;
		soap_set_receiver_error(soap, tcp_error(soap), "setsockopt SO_REUSEPORT failed in soap_bind()", SOAP_TCP_ERROR);
		return SOAP_INVALID_SOCKET;
	}
	//diag_printf("==================Will set SO_LINGER==================\n");
	struct linger so_linger;
	so_linger.l_onoff = 1;
	so_linger.l_linger = 0;
	if (setsockopt((SOAP_SOCKET)soap->master, SOL_SOCKET, SO_LINGER, &so_linger, sizeof(so_linger)))
	{
		//diag_printf("=================set SO_LINGER failed===================\n");
		soap->errnum = soap_socket_errno;
		soap_set_receiver_error(soap, tcp_error(soap), "setsockopt SO_LINGER failed in soap_bind()", SOAP_TCP_ERROR);
		return SOAP_INVALID_SOCKET;
	}
#if defined(CONFIG_RTL_819X) && defined(CONFIG_RTL_SORAPIDRECYCLE)
	//diag_printf("==================Will set SO_RAPIDRECYCLE==================\n");
	if (setsockopt((SOAP_SOCKET)soap->master, SOL_SOCKET, SO_RAPIDRECYCLE, (char*)&set, sizeof(int)))
	{
		//diag_printf("=================set SO_RAPIDRECYCLE failed===================\n");
		soap->errnum = soap_socket_errno;
		soap_set_receiver_error(soap, tcp_error(soap), "setsockopt SO_RAPIDRECYCLE failed in soap_bind()", SOAP_TCP_ERROR);
		return SOAP_INVALID_SOCKET;
	}
#endif
#endif
	...
	soap->peer.sin_port = htons((short)port);
	soap->errmode = 0;
	//diag_printf("[%s:%d] soap->peer.sin_addr.s_addr=%d\n", __FUNCTION__, __LINE__, soap->peer.sin_addr.s_addr);
	//diag_printf("[%s:%d] soap->peer.sin_port=%d\n", __FUNCTION__, __LINE__, soap->peer.sin_port);
	//diag_printf("[%s:%d] soap_bind, bind sockfd=%d\n", __FUNCTION__, __LINE__, soap->master);
	if (bind((SOAP_SOCKET)soap->master, (struct sockaddr*)&soap->peer, soap->peerlen))
	{ 
		//diag_printf("[%s:%d] soap_bind, Could not bind to host: %d\n", __FUNCTION__, __LINE__, soap_socket_errno);
		soap->errnum = soap_socket_errno;
		DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Could not bind to host\n"));
		soap_closesock(soap);
		soap_set_receiver_error(soap, tcp_error(soap), "bind failed in soap_bind()", SOAP_TCP_ERROR);
...
#endif
	//diag_printf("soap_bind, listen sockfd=%d\n", soap->master);
...
{
	//diag_printf("[%s:%d] enter tcp_accept, in stdsoap2.cpp\n", __FUNCTION__, __LINE__);
	int fd;
	//diag_printf("[%s:%d] ta: 1, s=%d, a=%p, *n=%d\n", __FUNCTION__, __LINE__, s, a, *n);
	//diag_printf("[%s:%d] aaa\n", __FUNCTION__, __LINE__);
	//diag_printf("[%s:%d] aaa\n", __FUNCTION__, __LINE__);
	//diag_printf("[%s:%d] aaa\n", __FUNCTION__, __LINE__);
	//diag_printf("[%s:%d] aaa\n", __FUNCTION__, __LINE__);
...
{
	//diag_printf("[%s:%d] enter soap_accept, in stdsoap2.cpp\n", __FUNCTION__, __LINE__);
	...
	if (soap_valid_socket(soap->master))
	{
		for (;;)
		{
...
#endif
#endif
			//diag_printf("[%s:%d] sa: 1, s=%d, a=%p, *n=%d\n", __FUNCTION__, __LINE__, soap->master,  (struct sockaddr*)&soap->peer, n);
			soap->socket = soap->faccept(soap, soap->master, (struct sockaddr*)&soap->peer, &n);
			//diag_printf("sa: 2, soap->socket=%d\n", soap->socket);
			...
			if (soap_valid_socket(soap->socket))
			{
				...
				//diag_printf("[%s:%d] soap->port: %d\n", __FUNCTION__, __LINE__, soap->port);
...
{ register int i;
	...
	{ for (mp = soap->mht[i]; mp; mp = mq)
		{ mq = mp->next;
			if (mp->live)
				diag_printf("%s(%d): malloc() = %p not freed (memory leak or forgot to call soap_end()?)\n", mp->file, mp->line, mp->ptr);
			free(mp);
		}
		soap->mht[i] = NULL;
	}
...
{ register int h = soap_hash_ptr(p);
	...
	{ if (mp->live)
		{ free(p);
			...
			diag_printf("%s(%d): free(%p) double free of pointer malloced at %s(%d)\n", file, line, p, mp->file, mp->line);
	}
	else
		diag_printf("%s(%d): free(%p) pointer not malloced\n", file, line, p);
}
#endif
...
{ if (!soap)
	...
	{ register char **q;
		for (q = (char**)&soap->alist; *q; q = *(char***)q)
		{
			...
			{ diag_printf("Data corruption in dynamic allocation (see logs)\n");
...
soap_outstring(struct soap *soap, const char *tag, int id, char *const*p, const char *type, int n)
{
	...
	if( tag && p && *p && *tag=='-' )
	{
		...
		if( (pS!=NULL) && (pE!=NULL) )
		{
//			diag_printf("<SendFileString></SendFileString> found, pasering...\n" );
			pS+=strlen( flag_h );
...
{
	diag_printf( "<%d>cookie:\n", lineno );
	if(p)
	{
		diag_printf("\tname:%s\n", p->name );
		diag_printf("\tvalue:%s\n", p->value );
		diag_printf("\tdomain:%s\n", p->domain );
		diag_printf("\tpath:%s\n", p->path );
		diag_printf("\texpire:%d\n", p->expire );
		diag_printf("\tversion:%d\n", p->version );
		diag_printf("\tsecure:%d\n", p->secure );
		diag_printf("\tsession:%d\n", p->session );
		diag_printf("\tenv:%d\n", p->env );
		diag_printf("\tmodified:%d\n", p->modified );
#ifdef _COOKIE_RFC2965_ /*jiunming*/
		diag_printf("\tport:%s\n", p->port );
		diag_printf("\tdiscard:%d\n", p->discard );
#endif
	}
}
...
{ 
//	diag_printf("enter soap_begin_recv\n");
	soap_wchar c;
	soap->error = SOAP_OK;
	soap_free(soap);
	soap_set_local_namespaces(soap);
	...
	*soap->endpoint = '\0';
	...
	{ soap->mode &= ~SOAP_IO;
		soap->error = soap->fparse(soap);// call http_parse()
...
//		        diag_printf("<%s:%d>, val=%s\n", __FUNCTION__,__LINE__, val);
				soap->length = soap_strtoul(val, NULL, 10);
//		        diag_printf("<%s:%d>, soap->length=%d\n", __FUNCTION__,__LINE__, soap->length);
                sleep(1);
             }
	...
	SOAP_FMAC1
		...
		{ if (soap_check_state(soap))
			diag_printf("Error: soap struct not initialized\n");
			else if (soap->error)
