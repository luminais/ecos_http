...
typedef struct MD5Context EVP_MD_CTX;

void TR069_MD5_Transform(MD5_uint32 buf[4], MD5_uint32 const in[16]);
void MD5_byteReverse(unsigned char *buf, unsigned longs)
{
    MD5_uint32 t;
    do {
 ...
 */
void cwmp_MD5_Init(struct MD5Context *ctx)
{
 ...
 * of bytes.
 */
void cwmp_MD5_Update(struct MD5Context *ctx, unsigned char const *buf, unsigned len)
{
    MD5_uint32 t;
	...
	TR069_MD5_Transform(ctx->buf, (MD5_uint32 *) ctx->in);
	buf += t;
	len -= t;
    }
 ...
 */
void cwmp_MD5_Final(unsigned char digest[16], struct MD5Context *ctx)
{
    unsigned count;
    unsigned char *p;
...
#else
      cwmp_MD5_Update( ctx, (void*)buf, (unsigned int)len );
#endif
      break;
