...
//#include <sys/time.h>/*erqing_fang 2013-08-21*/
#include <sys/ioctl.h>
#include <sys/wait.h>

#include <ctype.h>
//#include <err.h>/*erqing_fang 2013-08-21*/
#include <limits.h>
#include <netdb.h>
#include <stdio.h>
#include <stdlib.h>
...
#if defined(CONFIG_RTL_819X)	//erqing_fang 2013-08-21
#include "sysexits.h"
#include "time.h"
#else
#include <sys/time.h>
...
//add static by erqing_fang 2013-08-23
static int 		lineno = -1;

static int 		s;				/* main RAW socket 	   */
static int 		do_resolv=0;			/* Would try to resolv all */
static int		do_acct=0;			/* Show packet/byte count  */
static int		do_time=0;			/* Show time stamps        */
static int		do_quiet=0;			/* Be quiet in add and flush  */
static int		do_force=0;			/* Don't ask for confirmation */

/*erqing_fang 2013-08-21*/
extern int optind;
extern char *optarg;
extern int	getopt(int argc, char* const * argv, const char* opts);
//=============================================//
#if defined(CONFIG_RTL_819X)	//erqing_fang 2013-08-23
#include <cyg/infra/testcase.h>
#include <cyg/infra/diag.h>
void Quit_ip6fw (const char* msg)
{
	printf ("%s", msg);
}

#define warn printf//diag_printf
#define warnx printf//diag_printf
...
show_ip6fw(struct ip6_fw *chain)
{
	...
	switch (chain->fw_flg & IPV6_FW_F_COMMAND)
	{
		...
		default:
			...
			#if defined(CONFIG_RTL_819X)//erqing_fang 2013-08-22
			return;
			#endif
	}
	...
	if (mb==128 && do_resolv) {
		...
		if (mb!=128) {
			#ifdef RTL_EXTEND_IPV6FIREWALL_SUPPORT_IP6_RANGE//erqing_fang 2013-09-12			
            if (chain->fw_flg & IPV6_FW_F_SRCIP_RANGE)
            {
				printf(inet_ntop(AF_INET6,&chain->fw_src,ntop_buf,sizeof(ntop_buf)));
                printf("-");
				printf(inet_ntop(AF_INET6,&chain->fw_smsk,ntop_buf,sizeof(ntop_buf)));
            }
            else
			#endif
			if (mb == 0) {
	...
	if (chain->fw_prot == IPPROTO_TCP || chain->fw_prot == IPPROTO_UDP) {
		comma = " ";
		for (i = 0; i < nsp; i++) {
			print_port6(chain->fw_prot, chain->fw_pts[i], comma);
	...
	if (mb==128 && do_resolv) {
		...
		#ifdef RTL_EXTEND_IPV6FIREWALL_SUPPORT_IP6_RANGE//erqing_fang 2013-09-12
		if (chain->fw_flg & IPV6_FW_F_DSTIP_RANGE)
		{
			printf(inet_ntop(AF_INET6,&chain->fw_dst,ntop_buf,sizeof(ntop_buf)));
            printf("-");
			printf(inet_ntop(AF_INET6,&chain->fw_dmsk,ntop_buf,sizeof(ntop_buf)));
		}
		else
		#endif
		if (mb!=128) {
	...
	if (chain->fw_prot == IPPROTO_TCP || chain->fw_prot == IPPROTO_UDP) {
		comma = " ";
		for (i = 0; i < ndp; i++) {
			print_port6(chain->fw_prot, chain->fw_pts[nsp+i], comma);
...
#define	PRINTFLG(x)	{if (_flg_printed) printf(",");\
	...
	printf("\n");
	#if !defined(CONFIG_RTL_819X)//erqing_fang 2013-08-29
	if (do_resolv)
		endservent();
	#endif
}
...
"    show [number ...]\n"
"    zero [number ...]\n"
"  rule:  action proto(smac/dmac) src dst extras...\n"
"    action:\n"
...
lookup_host (host, ipaddr)
	...
	u_char *ipaddr;
{
    struct addrinfo *addrs=NULL, hints,*aip;
...
//    hints.ai_flags = AI_NUMERICHOST;
    ...
    err = getaddrinfo(host, "7000", &hints, &addrs);
	if(err<0||addrs==NULL)
	{
		return -1;
	}
	if(err==0){
		if(addrs->ai_family==AF_INET6){
			sinp6=(struct sockaddr_in6*)addrs->ai_addr;
			memcpy(ipaddr,&sinp6->sin6_addr,sizeof(sinp6->sin6_addr));
		}
		if(addrs)
			freeaddrinfo(addrs);
		return 0;
...
//void
int		//void=>int erqing_fang 2013-08-28
...
//		if (lookup_host(*av, ipno, AF_INET6) != 0){//erqing_fang 2013-08-27
		if(lookup_host(*av,ipno) != 0){
			printf("hostname \"%s\" unknown\n",*av);//erqing_fang 2013-08-28
			//show_usage("hostname \"%s\" unknown\n", *av);
			#if defined(CONFIG_RTL_819X)
			return -1;
			#endif
		}
		#ifdef CONFIG_RTL_819X
        *(p-1) = md;/*  restore parameters erqing_fang 2013-09-13 */
		#endif
		switch (md) {
			case '/':
				if (atoi(p) == 0) {
				...
				case '-':
					if(!inet_pton(AF_INET6,p,mask)){
					//if(inet_ntop(AF_INET6,mask,p,sizeof(p))){
					//if (!inet_aton(p, mask)){
						show_usage("bad end address ``%s''", p);
				#if defined(CONFIG_RTL_819X)	//erqing_fang 2013-09-13
					return -1;
				#endif
					}
					range_flag = 1;
	...
	char **av;
{
	...
	while (ac && isdigit(**av)) {
		...
		if (i) {
			exitval = 1;
			warn("failed to delete rule %u\n",rule.fw_number);
//			warn("rule %u: setsockopt(%s)\n", rule.fw_number, "IPV6_FW_DEL");
		}
	}
	if (exitval != 0){
	...
	char **av;
{
	...
	int retval = fill_ip6(&rule.fw_src, &rule.fw_smsk, &ac, &av);//erqing_fang 2013-09-13
	if(retval<0)
		return;
	#ifdef RTL_EXTEND_IPV6FIREWALL_SUPPORT_IP6_RANGE
	if (retval == 1)
		rule.fw_flg |= IPV6_FW_F_SRCIP_RANGE;
	#endif
	#if 0//erqing_fang 2013-09-13
//	fill_ip6(&rule.fw_src, &rule.fw_smsk, &ac, &av);//erqing_fang 2013-08-28
	if(fill_ip6(&rule.fw_src, &rule.fw_smsk, &ac, &av)<0){
		return;
	}
	#endif
	...
	retval = fill_ip6(&rule.fw_dst, &rule.fw_dmsk, &ac, &av);//erqing_fang 2013-09-13
	if(retval<0)
		return;
	#ifdef RTL_EXTEND_IPV6FIREWALL_SUPPORT_IP6_RANGE
	if (retval == 1)
		rule.fw_flg |= IPV6_FW_F_DSTIP_RANGE;
	#endif

//	fill_ip6(&rule.fw_dst, &rule.fw_dmsk, &ac, &av);//erqing_fang 2013-09-13
...
badviacombo:
	...
	if (i==-1){
			#if defined(CONFIG_RTL_819X)	//erqing_fang 2013-08-29
			errx(EX_UNAVAILABLE, "Failed to add rule.\n");
			return;
			#else
	...
	char **av;
{
	...
	if (!ac) {
		...
		if (setsockopt(s,IPPROTO_IPV6,IPV6_FW_ZERO,NULL,0)<0){
				#if defined(CONFIG_RTL_819X)//erqing_fang 2013-08-23
				errx(EX_UNAVAILABLE, "setsockopt(%s)", "IPV6_FW_ZERO");
				return;
				#else
				errx(EX_UNAVAILABLE, "setsockopt(%s)", "IPV6_FW_ZERO");
				#endif
...
{
	int 		ch;
	int i;//erqing_fang 2013-08-22
	extern int 	optind;

	/* init optind to 1 */
	optind = 1;
	if ( ac == 1 || ac == 0) {//erqing_fang 2013-08-23
		show_usage(NULL);
		#if defined(CONFIG_RTL_819X)//erqing_fang 2013-08-22
		return -1;
		#endif
	...
	if (!strncmp(*av, "add", strlen(*av))) {
		...
		if ( do_flush ) {
			if (setsockopt(s,IPPROTO_IPV6,IPV6_FW_FLUSH,NULL,0) < 0){
				#if defined(CONFIG_RTL_819X)/*erqing_fang 2013-08-21*/
				errx(EX_UNAVAILABLE, "setsockopt(%s)", "IPV6_FW_FLUSH");
				return -1;
				#endif
			}
			if (!do_quiet)
...
int
#if defined(CONFIG_RTL_819X)&&defined(HAVE_IPV6FIREWALL)	//erqing_fang 2013-08-21
ip6fw_init_main(ac, av)
#else
main(ac, av)
#endif
	int	ac;
...
#define	WHITESP		" \t\f\v\n\r"
	...
	if (ac > 1 && av[ac - 1][0] == '/' && access(av[ac - 1], R_OK) == 0) {
		...
		ip6fw_main(ac,av);
#endif
	close(s);//erqing_fang 2013-08-23
	return 0;
}

#if defined(CONFIG_RTL_819X)&&defined(IP6FW)	//erqing_fang 2013-08-23
#include <cyg/kernel/kapi.h>
unsigned char ip6fw_stack[36*1024];
cyg_handle_t ip6fw_thread_handle;
cyg_thread ip6fw_thread_obj;

void create_ip6fw_thread(void)
{
	/* Create the thread */
	cyg_thread_create(9,
		      ip6fw_init_main,
	...
	cyg_thread_resume(ip6fw_thread_handle);
}
#endif
