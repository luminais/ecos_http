...
// added by rock /////////////////////////////////////////
// fix mgbin use X86 defined offset problem
#ifdef VOIP_SUPPORT
#undef HW_SETTING_OFFSET
#define HW_SETTING_OFFSET 0x6000
#undef DEFAULT_SETTING_OFFSET
#define DEFAULT_SETTING_OFFSET 0x8000
#endif

#ifdef ECOS_FLASH_SIZE
#define FLASH_SIZE ECOS_FLASH_SIZE
#else
#define FLASH_SIZE 0x200000
#endif

#if defined(HAVE_CERT_MNG)
#ifndef CERT_SIZE
#ifndef CERT_MNG_SIZE
#define CERT_SIZE               0x2000
#else
#define CERT_SIZE               CERT_MNG_SIZE
#endif
#endif
#endif
...
static void showHelp(void)
{
	...
	printf("%s Ver %s.\n\n", copyright, version);
	printf("usage: mgbin [-s] [-c] [-a] -o outputfile bootcode config webpages kernel root\n");
...
int main(int argc, char** argv)
{
	...
	for (i=BOOT_CODE; i<=ROOT ; i++) {
		if (sector[i].filename[0]) {
...
#ifndef HAVE_CERT_MNG
				if((sbuf.st_size+CODE_IMAGE_OFFSET)>FLASH_SIZE)
#else
				if((sbuf.st_size+CODE_IMAGE_OFFSET)>(FLASH_SIZE-CERT_SIZE))
#endif
				{
					printf("\necos.bin oversize \n");
					exit(1);
			...
			if (is_all) 
				checksum = calculateChecksum(buf, sbuf.st_size, checksum);			
			else {
