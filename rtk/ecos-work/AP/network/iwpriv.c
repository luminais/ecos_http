...
set_private_cmd(int		skfd,		/* Socket */
		unsigned char *		args[],		/* Command line args */
		...
		int		priv_num)	/* Number of descriptions */
{
...
#ifdef __ECOS
	buffer = (u_char*)malloc(4096*sizeof(u_char));
	if(buffer == NULL)
	{
		diag_printf("[%s:%d]Failed to malloc buffer!!\n",__FUNCTION__,__LINE__);
		return -1;
	}
#endif
  ...
  if((count >= 1) && (sscanf((char *)args[0], "[%i]", &temp) == 1))
    {
      subcmd = temp;
      args++;
      count--;
  ...
  if((priv[k].set_args & IW_PRIV_TYPE_MASK) &&
	  ...
	  if(i < count)
	    {
	      ...
	      wrq.u.data.length = strlen((char *)args[i]) + 1;
	      if(wrq.u.data.length > (priv[k].set_args & IW_PRIV_SIZE_MASK))
		wrq.u.data.length = priv[k].set_args & IW_PRIV_SIZE_MASK;
	...
	case IW_PRIV_TYPE_FLOAT:
	  ...
	  for(; i < wrq.u.data.length; i++) {
	    double		freq;
	    if(sscanf((char *)args[i], "%lg", &(freq)) != 1)
	      {
	    ...
	    if(strchr((char *)args[i], 'G')) freq *= GIGA;
	    if(strchr((char *)args[i], 'M')) freq *= MEGA;
	    if(strchr((char *)args[i], 'k')) freq *= KILO;
	    sscanf((char *)args[i], "%i", &temp);
  ...
  if((priv[k].get_args & IW_PRIV_TYPE_MASK) &&
     (priv[k].get_args & IW_PRIV_SIZE_MASK))
    {
      ...
      //设置CE功率时,上层调用rf_pwr命令会导致一直打印,这里开启CE功率时过滤掉打印
	 if(0 != strcmp("rf_pwr",cmdname))
	 {
	    ...
	    char *	ifname)		/* Dev name */
{
  ...
  /* Do it */
  ret = set_private_cmd(skfd, args + 1, count - 1, ifname, (char *)args[0],
			priv, number);
	  ...
	  char *	ifname)		/* Dev name */
{
  ...
  if(k < 4)
    ...
    /* ...or as an integer */
    if(sscanf((char *)args[i], "%i", (int *) &ptype) != 1)
      {
	//iw_usage();
	goto err;
      }
 ...
 * The main !
 */
int iwpriv_main(unsigned int argc, unsigned char *argv[])
{
  int skfd;		/* generic raw socket desc.	*/
  int goterr = 0;

  //sanity check, no argument is not supported
  if (argc == 0)
  	return(-1);
  ...
  /*if(argc == 0)
    ...
    /* All */
    /*if((!strncmp(argv[0], "-a", 2)) || (!strcmp(argv[0], "--all")))
      iw_enum_devices(skfd, &print_priv_all, NULL, 0);
    else*/
      /* Help */
      /*if((!strncmp(argv[0], "-h", 2)) || (!strcmp(argv[0], "--help")))
	iw_usage();
      else*/
	/* Version */
	/*if (!strcmp(argv[0], "-v") || !strcmp(argv[0], "--version"))
	  ...
	  if(argc == 1) {
	    print_priv_info(skfd, (char *)argv[0], NULL, 0);
	  }
	  else
	    ...
	    /*if((!strncmp(argv[1], "-a", 2)) ||
	       (!strcmp(argv[1], "--all")))
	      print_priv_all(skfd, argv[0], NULL, 0);
	    else*/
#if 0
	      /* Roaming */
		...
		if(!strncmp(argv[2], "port", 4))
		  ...
		  goterr = set_private(skfd, argv + 1, argc - 1, (char *)argv[0]);
