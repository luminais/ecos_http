...
struct in_addr myip[2];
struct in_addr bcast_ip[2] ;
extern char interface_lan_or_wan_record;
...
int read_socket(int fd,char *buf,int len)
{
  ...
  if (ret <= 0)
    Debug(1,"read socket failed. ERRNO=%d\n",errno);
  else
    {
...
int read_max_data(int fd,char *buffer,int bufsize,int maxtime)
{
	...
	/* Query the system how many bytes are ready to be read */
	tmp_ioctl = ioctl(fd, FIONREAD, &nready);
...
{
  //printf("enter receive_nmb Client=%d buffer=%d BUFFER_SIZE=%d timeout=%d\n",Client,buffer,BUFFER_SIZE,timeout);	
...
****************************************************************************/
BOOL send_pkt(char *buf,int len,struct in_addr *ip,int port,int type)
{
  BOOL ret;
  int out_fd;
...
****************************************************************************/
#define TIOCNOTTY 8// for test 2013.5.17
...
****************************************************************************/
BOOL get_broadcast(struct in_addr *if_ipaddr, struct in_addr *if_bcast,char * if_name)
{
  ...
  while (count--)
    {
      ...
      show_nmb(outbuf);
      if (!send_pkt(outbuf,nmb_len(outbuf),&bcast_ip[interface_lan_or_wan_record],137,SOCK_DGRAM))
	{
	  NeedSwap = saved_swap;
	  return False;
	}
