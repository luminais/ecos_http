...
//#define HAVE_SETITIMER	1	// use setitimer() --> compiler error 
//#define HAVE_TIMERCREATE	1	// use timer_create() series --> should hack eCos to make it work 
#define HAVE_EXTERNAL_TICK	1	// external tick periodically 

#if !defined( HAVE_SETITIMER ) && !defined( HAVE_TIMERCREATE ) &&	\
	!defined( HAVE_EXTERNAL_TICK )
#error "Without timer support"
#endif
...
schedule_timer(void)
{
	...
	if (tm != &timers_head)
	{
#if defined( HAVE_SETITIMER )	// use itimerval
		struct itimerval next;
	       
	        memset(&next, 0, sizeof(next));
	       
	        timersub(&tm->expires, &tv, &next.it_value);
#elif defined( HAVE_TIMERCREATE )	// use itimerspec
		struct itimerspec next;
		struct timeval tv_it;
		struct sigevent se;
		
		static timer_t timerid = -1;
		
		if( timerid < 0 ) {	// create only once 
		...
		timersub(&tm->expires, &tv, &tv_it);
		
		TIMEVAL_TO_TIMESPEC( &tv_it, &next.it_value );
#endif
...
#elif defined( HAVE_TIMERCREATE )
		if ((next.it_value.tv_sec > 0) || 
				((next.it_value.tv_sec == 0) && (next.it_value.tv_nsec > 0)))
		{
			dlog(LOG_DEBUG, 4, "calling alarm: %ld secs, %ld nsecs", 
					next.it_value.tv_sec, next.it_value.tv_nsec);

			if(timer_settime(timerid, 0/*TIMER_ABSTIME*/, &next,  NULL))
				flog(LOG_WARNING, "schedule_timer setitimer for %ld.%ld failed: %s",
					next.it_value.tv_sec, next.it_value.tv_nsec, strerror(errno));
		}
#endif
...
#elif defined( HAVE_EXTERNAL_TICK )
	// do nothing 
#endif // HAVE_SETITIMER || HAVE_TIMERCREATE
}
...
void 
radvd_tick_alarm_handler( void )
{
#if defined( HAVE_EXTERNAL_TICK )
	alarm_handler( 0 );
#endif
}
...
#if defined( HAVE_SETITIMER ) || defined( HAVE_EXTERNAL_TICK )
...
#elif defined( HAVE_TIMERCREATE )
	timersub(&tm->expires, &tv, &tv_it);
	TIMEVAL_TO_TIMESPEC(&tv_it, &diff.it_value );
	dlog(LOG_DEBUG, 5, "check_time_diff, difference: %ld sec + %ld nsec",
