...
open_icmpv6_socket(void)
{
	...
	if (err < 0)
	{
		...
		//return (-1);		// ignore error, because XXX 2292bis says: 
							// "An attempt to set IPV6_CHECKSUM
...
int set_ipv6_mreq_allrouters( int sock, const char *ifname, int optname )
{
	// ifname may be 'eth0'
	// optname = IPV6_JOIN_GROUP or IPV6_LEAVE_GROUP
	struct ipv6_mreq mreq;
	int err;
	
	mreq.ipv6mr_multiaddr = in6addr_linklocal_allrouters;
	mreq.ipv6mr_interface = if_nametoindex( ifname );
	
	err = setsockopt(sock, IPPROTO_IPV6, optname, &mreq,
			 sizeof(mreq));
	if (err < 0)
	{
		flog(LOG_ERR, "setsockopt(%d): %s", optname, strerror(errno));
		return (-1);
	}	
