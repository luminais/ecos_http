...
static const char rcsid[] = "@(#)$Id: resend.c,v 1.1.1.1 2010-04-30 02:12:05 Exp $";
#endif
...
char	*datain;
{
	ether_header_t	*eh = NULL;
	ether_header_t	*new_eh = NULL;
	char	dhost[6];
	ip_t	*ip;
	...
	while ((i = (*r->r_readip)((char *)pbuf, sizeof(pbuf), NULL, NULL)) > 0)
		...
		if (!(opts & OPT_RAW)) {
			len = ntohs(ip->ip_len);
			new_eh = (ether_header_t *)realloc((char *)eh, sizeof(*eh) + len);
			if(NULL == new_eh)
			{
				free(eh);
				return -2;
