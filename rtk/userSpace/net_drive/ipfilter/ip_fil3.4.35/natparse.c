...
static const char rcsid[] = "@(#)$Id: natparse.c,v 1.7 2010-08-25 10:20:46 Exp $";
#endif
	...
	*status = 100;		/* default to error */
	...
	if (ipn.in_redir & (NAT_MAP|NAT_MAPBLK)) {
		...
		if (!*cpp) {
			...
			if (*cpp && !strcasecmp(*cpp, "autofw")) {
				/* Set autofw flag */
				ipn.in_flags |= IPN_AUTOFW;
				cpp++;	/* Skip "autofw" */
				if (strcasecmp(*cpp, "port")) {
				...
				if (isdigit(**cpp) && (s = strchr(*cpp, '-')))
					*s++ = '\0';
					...
					s = NULL;
				if (!portnum(*cpp, &ipn.in_autofwpmin, linenum))
					return NULL;
				cpp++;
				if (!strcmp(*cpp, "-")) {
					cpp++;
				...
				if (s) {
					if (!portnum(s, &ipn.in_autofwpmax, linenum))
						return NULL;
				}
				else
					ipn.in_autofwpmax = ipn.in_autofwpmin;
				if (!strcasecmp(*cpp, "tcp"))
					ipn.in_autofwp |= IPN_TCP;
				else if (!strcasecmp(*cpp, "udp"))
					ipn.in_autofwp |= IPN_UDP;
				else if (!strcasecmp(*cpp, "tcpudp"))
					ipn.in_autofwp |= IPN_TCPUDP;
