...
#if !defined(lint)
static const char rcsid[] = "@(#)$Id: printnat.c,v 1.6 2010-08-25 10:20:46 Exp $";
#endif
...
#if (defined(NetBSD) && (NetBSD >= 199905) && (NetBSD < 1991011)) || (__FreeBSD_version \
	>= 300000) || defined(OpenBSD)
...
int opts;
{
	...
	if ((nat->nat_flags & IPN_TCPUDPICMP) != 0)
...
int opts;
{
	...
	if (np->in_redir == NAT_REDIRECT) {
		...
		if (np->in_flags & IPN_AUTOFW) {
			printf(" autofw port %d-%d", np->in_autofwpmin, np->in_autofwpmax);
			if ((np->in_autofwp & IPN_TCPUDP) == IPN_TCPUDP)
				printf(" tcp/udp");
			else if ((np->in_autofwp & IPN_TCP) == IPN_TCP)
				printf(" tcp");
			else if ((np->in_autofwp & IPN_UDP) == IPN_UDP)
				printf(" udp");
		}
