...
dhcpd_osl_dns_list(DHCPD_IFC *ifp, char *buf)
{
	 ...
	 */
	 ...
	 dhcpd_nvram_get("lan_dns",ifp->unit,buf);
...
dhcpd_osl_init_config(DHCPD_IFC *ifp)
{
	...
	else
	{
		...
		inet_aton(nvram_get(tmp),&lan_ip);
		inet_aton(nvram_get(tmp),&(ifp->ipaddr));		
		snprintf(tmp, sizeof(tmp), "lan%d_netmask", ifp->unit);
		inet_aton(nvram_get(tmp),&lan_mask);
		inet_aton(nvram_get(tmp),&(ifp->netmask));
	...
	if (dhcpd_nvram_get("dhcp_start", ifp->unit, buf)){
		if (sscanf(buf, "%u.%u.%u.%u", &bufip[0], &bufip[1], &bufip[2], &bufip[3]) != 4){
			use_default = 1;
			goto pool_default;
		}
...
pool_default:
	...
	if(lease_time_user_value == 0){//为了获得WAN口DNS
		...
		#ifndef __CONFIG_AUTO_CONN_SERVER__
