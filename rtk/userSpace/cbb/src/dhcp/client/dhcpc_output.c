...
unsigned short
tenda_dhcp_chksum(unsigned char *ptr_dhcp, unsigned short udp_len)
{
	
	
	struct dhcphdr *dhcp = (struct dhcphdr *)ptr_dhcp;
	...
	//diag_printf ("%s %d dhcp->flags=%2x= udp_len_temp=%2x=\n", __FUNCTION__, __LINE__, dhcp->flags, ntohs(udp_len));
	
	//dhcp->flags = 0x0080;
	
	packet_len = sizeof(packet.ip) + ntohs(udp_len);
	
	packet.ip.ip_p = IPPROTO_UDP;
	packet.ip.ip_src.s_addr = INADDR_ANY;
	packet.ip.ip_dst.s_addr = INADDR_BROADCAST;
	packet.ip.ip_len = udp_len;
	
	packet.udp.uh_sport = htons(DHCP_CLIENT_PORT);
	packet.udp.uh_dport = htons(DHCP_SERVER_PORT);
	packet.udp.uh_ulen = udp_len; /* cheat on the psuedo-header */
	
	
	memcpy(&(packet.dhcp), dhcp, ntohs(udp_len) - sizeof(packet.udp));
	packet.udp.uh_sum = dhcp_chksum((unsigned short *)&packet, packet_len);
	
	
	
	//diag_printf ("%s %d  packet.udp.uh_sum=%2x=\n", __FUNCTION__, __LINE__, packet.udp.uh_sum);
	return packet.udp.uh_sum;
