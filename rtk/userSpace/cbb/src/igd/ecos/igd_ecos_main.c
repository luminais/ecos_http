 ...
 *
 * $Id: igd_ecos_main.c 241182 2011-02-17 21:50:03Z gmo $
 */
#include <stdio.h>
...
/*
add by lqz
目前存在IPTV等使用UPNP协议导致TCP套接字耗尽。
原因:客户端请求添加端口映射，如果存在该规则，则返回失败，错误码为713，
	此时客户端会一直发送请求，而UPNP线程优先级最低，无法快速的处理请求导致。
目前解决方案：将优先级由10调整为8，因为10目前是最低的.
*/
#define IGD_PRIORITY	8
#define IGD_STACKSIZE	16384
#define MAX_NO_BRIDGE 1	//llm add, 解决段错误
...
//roy+++,2010/10/12
		...
		ifname = nvram_safe_get(strcat_r(prefix, "pptp_ifname", name));
	}
	else if (strcmp(proto, "l2tp") == 0) {
		ifname = nvram_safe_get(strcat_r(prefix, "l2tp_ifname", name));
	}
//+++
...
igd_read_config(int *igd_port, int *igd_adv_time, IGD_NET **netlist)
{
	...
	for (i = 0, num = 0; i < MAX_NO_BRIDGE; i++) {
		if (i == 0)
	...
	for (i = 0, num = 0; i < MAX_NO_BRIDGE; i++) {
		...
		if (value == NULL)
			continue;
#ifdef REALTEK
		//llm add, realtek上bridge0没地址，取接口地址先写死为eth0
		value = "eth0";
#endif
