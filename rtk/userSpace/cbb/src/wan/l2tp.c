...
wan_l2tp_start(int unit)
{
	...
	cprintf("%s::Connecting to L2TP server, please wait...\n", __FUNCTION__);

	snprintf(prefix, sizeof(prefix), "wan%d_", unit);
	wan_ifname = nvram_get(strcat_r(prefix, "ifname", tmp));

	ifconfig(wan_ifname, IFUP,
		nvram_safe_get(strcat_r(prefix, "l2tp_ipaddr", tmp)),
		nvram_safe_get(strcat_r(prefix, "l2tp_netmask", tmp)));
...
//end	
	...
	for (name = dns, p = name, i = 0;
	     ...
	     name = next, p = 0) {
		strtok_r(p, " ", &next);
		if(!CGI_same_net(inet_addr(name), inet_addr(nvram_safe_get(strcat_r(prefix, "l2tp_gateway", tmp))), inet_addr(nvram_safe_get(strcat_r(prefix, "l2tp_netmask", tmp)))))
		{//tenda modify
			route_add(wan_ifname, 0, name,
			nvram_safe_get(strcat_r(prefix, "l2tp_gateway", tmp)), "255.255.255.255");
		}
	}

	/* connect to peer */
	hostinfo = gethostbyname(nvram_safe_get(strcat_r(prefix, "l2tp_server_name", tmp)));
	if (hostinfo && (hostinfo->h_length == 4)) {
		...
		cprintf("L2TP: error looking up %s\n",
			nvram_safe_get(strcat_r(prefix, "l2tp_server_name", tmp)));
#if 0//roy modify,2010/11/04
		return -1;
#else
//server is ip address
		snprintf(ip,sizeof(ip),"%s",nvram_safe_get(strcat_r(prefix, "l2tp_server_name", tmp)));
#endif
	}
//roy +++
	if(strlen(nvram_safe_get(strcat_r(prefix, "l2tp_gateway", tmp)))> 6)//simple check
//+++
	/* Add a host route for l2tp server */
	if(!CGI_same_net(inet_addr(ip), inet_addr(nvram_safe_get(strcat_r(prefix, "l2tp_gateway", tmp))), inet_addr(nvram_safe_get(strcat_r(prefix, "l2tp_netmask", tmp)))))
	{//tenda modify
	...
	tpi_firewall_update_tunnel(pppname);
...
#ifdef __CONFIG_DHCPC__
		char *value;
		value = nvram_get(strcat_r(prefix, "bind_l2tp_static", tmp));
		if (value && atoi(value) == 0)
...
bound_l2tp(char *ifname)
{
	...
	if (iflib_getifaddr(ifname, &ip, &mask) == 0 &&
		...
		strcpy(gateway, nvram_safe_get(strcat_r(prefix, "l2tp_gateway", tmp)));
		/* Check ipaddr */
		...
		/* Stop l2tp, if changed */
		if (stop) {
			pppname = nvram_safe_get(strcat_r(prefix, "l2tp_ifname", tmp));
	...
	if ((value = getenv("ip")))
		nvram_set(strcat_r(prefix, "l2tp_ipaddr", tmp), value);
	if ((value = getenv("subnet")))
		nvram_set(strcat_r(prefix, "l2tp_netmask", tmp), value);
	if ((value = getenv("router")))
		nvram_set(strcat_r(prefix, "l2tp_gateway", tmp), value);
	if ((value = getenv("dns"))) {
...
/*
 * l2tpdhcpc [deconfig/bound] [ifname] ...
 */
int
l2tpdhcpc(int argc, char **argv)
{
	char *cmd = argv[1];
	...
	if (strstr(cmd, "bound"))
		return bound_l2tp(ifname);
	if (strstr(cmd, "deconfig"))
		return deconfig_l2tp(ifname);
