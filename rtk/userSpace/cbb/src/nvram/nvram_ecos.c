...
void set_ssid_mac(char* wl_prefix){
	...
	for(i=0;  wlanmac_value[i]!='\0' && i<9; ++i){
		if (wlanmac_value[i+11] != ':'){		
				wlanmac_value[l]=wlanmac_value[i+11];
				++l;
			}
	}
	wlanmac_value[4]='\0';
	...
	{
		snprintf(ssid_mac, sizeof(ssid_mac),"%s_5G_%s", prev_ssid, wlanmac_value);
	...
	{
		snprintf(ssid_mac, sizeof(ssid_mac),"%s_2.4G_%s", prev_ssid, wlanmac_value);
	}
