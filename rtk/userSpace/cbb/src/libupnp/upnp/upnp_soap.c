 ...
 *
 * $Id: upnp_soap.c 241182 2011-02-17 21:50:03Z gmo $
 */
...
soap_process(UPNP_CONTEXT *context)
{
	...
	upnp_syslog(LOG_INFO, "access service name:%s, control_url:%s", service->name,service->control_url);
	
	action_field = soapaction;
	if (action_field == NULL)
		return R_BAD_REQUEST;
	upnp_syslog(LOG_INFO, "soap action:%s", soapaction);
