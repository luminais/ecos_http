 ...
 *
 *	$Id: rxpkt.c,v 1.31 2010/04/08 04:46:31 davidhsu Exp $
 */
...
static int check_authenticator_attr(STA_CTX_Tp pSta, unsigned char *pMsg, int msg_len)
{
	...
	hmac_sha256(pSta->last_tx_msg_buffer, pSta->last_tx_msg_size+size, pSta->auth_key, BYTE_LEN_256B, (unsigned char *)tmp, &size);
...
//		wsc_debug_out("IV", pData, BYTE_LEN_128B);
	...
	Decrypt_aes_128_cbc(pSta->key_wrap_key, pData, (unsigned char *)out, (unsigned int *)&size, &pData[BYTE_LEN_128B], tag_len-BYTE_LEN_128B);

#ifdef DEBUG
	if (pCtx->debug2) {
		wsc_debug_out("Plaintext of EncryptedSettings:", (unsigned char *)out, size);		
	}
#endif

	pData = check_tag(pCtx, (unsigned char *)out, size, TAG_KEY_WRAP_AUTH, BYTE_LEN_64B, "KeyWrapAuthenticator", TYPE_BIN, &tag_len);
	if (pData == NULL)
		return -1;
...
static int decrypt_setting(CTX_Tp pCtx, STA_CTX_Tp pSta, unsigned char *pMsg, int msg_len)
{
	...
	int needReplace=0;
#endif	
	size = decrypt_attr(pCtx, pSta, pMsg, msg_len, (char *)tmpbuf);
	if (size < 0)
		return -1;

#ifdef SUPPORT_HANDLE_MULTI_CREDENTIAL		
	//debug_out("attr data:",tmpbuf,size);
	_DEBUG_PRINT("\n");	
	while(1){		
		PreCred =
		...
		if(PreCred==NULL){
			...
			_DEBUG_PRINT("	Encry :%d ; ",EnptyType);	
	...
	return GetNetworkProfile(pCtx, (unsigned char *)pData, size);
}
...
static int msgHandler_M7(CTX_Tp pCtx, STA_CTX_Tp pSta, unsigned char *pMsg, int msg_len)
{
	...
	/*check E-Hash1*/ 
	hmac_sha256((unsigned char*)pCtx->peer_pin_code, strlen(pCtx->peer_pin_code)/2, 
		pSta->auth_key, BYTE_LEN_256B, tmp, &size);
	...
	/*check E-Hash2*/ 
	hmac_sha256((unsigned char*)&pCtx->peer_pin_code[strlen(pCtx->peer_pin_code)/2],
		strlen(pCtx->peer_pin_code)/2, pSta->auth_key, BYTE_LEN_256B, tmp, &size);
...
static int msgHandler_M6(CTX_Tp pCtx, STA_CTX_Tp pSta, unsigned char *pMsg, int msg_len)
{
	...
	int size=0;
	unsigned char tmpbuf[1024];
	unsigned char tmp1[200], tmp2[200];
	unsigned char tmp[200];
	DBFENTER;
	...
	pData = check_tag(pCtx, (unsigned char*)tmpbuf, size, TAG_R_SNONCE2, NONCE_LEN, "R-S2", TYPE_BIN, &tag_len);
	if (pData == NULL)
		return -1;
	memcpy(pSta->r_s2, pData, tag_len);

	/* check R-Hash1 */ 
	hmac_sha256((unsigned char*)pCtx->pin_code, strlen(pCtx->pin_code)/2,
		pSta->auth_key, BYTE_LEN_256B, tmp, &size);
	...
	/* check R-Hash2 */ 
	hmac_sha256((unsigned char*)&pCtx->pin_code[strlen(pCtx->pin_code)/2],
		strlen(pCtx->pin_code)/2, pSta->auth_key, BYTE_LEN_256B, tmp,&size);
...
static int msgHandler_M4(CTX_Tp pCtx, STA_CTX_Tp pSta, unsigned char *pMsg, int msg_len)
{
	...
	pData = check_tag(pCtx, (unsigned char*)tmpbuf, size,
		TAG_R_SNONCE1, NONCE_LEN, "R-S1", TYPE_BIN, &tag_len);
	...
	hmac_sha256((unsigned char*)pCtx->pin_code, strlen(pCtx->pin_code)/2,
		pSta->auth_key, BYTE_LEN_256B, tmp, (int *)&size);
	memcpy(tmpbuf, pSta->r_s1, NONCE_LEN);
	ptr = append((unsigned char*)&tmpbuf[BYTE_LEN_128B], tmp, BYTE_LEN_128B);
	BN_bn2bin(pSta->dh_enrollee->pub_key, tmp1);
	ptr = append(ptr, tmp1, PUBLIC_KEY_LEN);
	BN_bn2bin(pSta->dh_registrar->p, tmp2);
	ptr = append(ptr, tmp2, PUBLIC_KEY_LEN);
	size = (int)(((unsigned long)ptr) - ((unsigned long)tmpbuf));
	hmac_sha256((unsigned char*)tmpbuf, size, 
		pSta->auth_key, BYTE_LEN_256B, tmp, (int *)&size);
...
int pktHandler_wsc_done(CTX_Tp pCtx, STA_CTX_Tp pSta)
{
...
#endif
	//	unsigned char tmp[40];	// 2011-0830 add
#ifdef P2P_SUPPORT
	...
	if (pCtx->is_ap) {
		...
		sprintf(tmp, "echo %02X:%02X:%02X:%02X:%02X:%02X > %s", 
			pSta->addr[0],pSta->addr[1],pSta->addr[2],pSta->addr[3],pSta->addr[4],pSta->addr[5],
		...
		if (ReportWPSstate(pCtx->wlan_interface_name, &Report2wlan) < 0) {
			WSC_DEBUG("\n	Report p2p WPS state fail \n");
		}
		
		WSC_DEBUG("Report P2P GO WPS success\n");				
#endif
		DEBUG_PRINT("<<WPS protocol done>>)\n");
...
int pktHandler_upnp_select_msg(CTX_Tp pCtx, STA_CTX_Tp pSta, struct WSC_packet *packet)
{
	...
	if (pCtx->use_ie) {
		len = build_beacon_ie(pCtx, selectedReg, passid, method, (unsigned char *)tmpbuf);
		...
		len = build_probe_rsp_ie(pCtx, selectedReg, passid, method, (unsigned char *)tmpbuf);
