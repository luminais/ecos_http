 ...
 *
 *	$Id: wsc.h,v 1.56 2010/06/10 08:18:34 pluswang Exp $
 */
/*================================================================*/
...
#define WSC_DEBUG(fmt, args...) printf("\n[wsc]%s %d:"fmt,__FUNCTION__ , __LINE__ , ## args)
...
#define SC_DEBUG(fmt, args...) printf("[sc]%s %d:"fmt,__FUNCTION__ , __LINE__ , ## args)		
...
#define AUTO_LOCK_DOWN	// for all let it be default
...
#ifdef USE_MUTEX
#ifdef __ECOS
#define WSC_pthread_mutex_init(x, y); 	       cyg_mutex_init(x);
#define WSC_pthread_mutex_destroy(x); 	cyg_mutex_destroy(x);
#define WSC_pthread_mutex_lock(x);		cyg_mutex_lock(x);
#define WSC_pthread_mutex_unlock(x);	       cyg_mutex_unlock(x);
#define WSC_pthread_mutex_t			       struct cyg_mutex_t
#else
...
#ifdef WPS2DOTX
#define VERSION_STR					"v2.14-wps2.0"
#else
#define VERSION_STR					"v2.14-wps1.0"
#endif
...
#define WEB_PID_FILENAME			("/var/run/webs.pid")
	...
	#define LED_WSC_ERROR				TURNKEY_LED_WSC_NOP
	#define LED_WSC_SUCCESS			TURNKEY_LED_WSC_SUCCESS
...
enum { 
		NOT_USED=-1, 
		PROTOCOL_START=99, PROTOCOL_PBC_OVERLAPPING=11,
...
#define WLAN0_INTF_NAME "wlan0"
#define WLAN0_VXD_INTF_NAME "wlan0-vxd0"
#define WLAN1_INTF_NAME "wlan1"
#define WLAN1_VXD_INTF_NAME "wlan1-vxd0"
...
typedef struct context 
{
	...
	unsigned char wlan_interface_name[40];
	...
	unsigned char nonce_enrollee[NONCE_LEN];	
	unsigned char assigned_ssid[WSC_MAX_SSID_LEN+1];
...
#define nvram_safe_get(name) (nvram_get(name) ? : "")	//hong add
/*================================================================*/
...
extern int wlioctl_get_mib(unsigned char *interfacename , char* mibname ,int *result );
...
extern int wlioctl_set_led(unsigned char *interface, int flag);
extern int wlioctl_set_wsc_ie(unsigned char *interface, char *data, int len, int id, int flag);
...
extern int get_acl_allow_mib(CTX_Tp pCtx , unsigned char *interfacename);
...
extern int IssueDisconnect(unsigned char *interface, unsigned char *pucMacAddr, unsigned short reason);
