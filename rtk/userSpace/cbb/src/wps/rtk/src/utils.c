 ...
 *
 *	$Id: utils.c,v 1.33 2010/06/10 08:18:34 pluswang Exp $
 */
...
extern void wps_led_control(int value);
extern int run_clicmd(char *command_buf);

int wlioctl_set_led(unsigned char *interface, int flag)
{
	...
	if (flag == TURNKEY_LED_WSC_END) { // for turnkey LED_WSC_END
		...
		/*when return from normal wps operation(normal blink) 
		and   auto-lock-down not timeout yet  then still blink error-state*/
...
#ifdef __CONFIG_SHARE_WLAN_WPS_GPIO__
    /* wps process done, wifi led  follow wifi status */
    if(flag == TURNKEY_LED_WSC_END)
    {
        printf("WIFI led mode end\n");
        #ifdef FOR_DUAL_BAND
        wps_led_control(enable);
        
        memset(buf, 0x0, sizeof(buf));
        sprintf(buf, "wlan0 led 4");
        run_clicmd(buf);
        
        memset(buf, 0x0, sizeof(buf));
        sprintf(buf, "wlan1 led 4");
        run_clicmd(buf);
        #else
        wps_led_control(enable);
    ...
    else if ((flag == TURNKEY_LED_WSC_START) || (flag == TURNKEY_LED_PBC_OVERLAPPED)) /* disable wifi driver led */
    {
        printf("wps led mode start\n");
        #ifdef FOR_DUAL_BAND
        memset(buf, 0x0, sizeof(buf));
        sprintf(buf, "wlan0 led 5");
        run_clicmd(buf);
        memset(buf, 0x0, sizeof(buf));
        sprintf(buf, "wlan1 led 5");
        run_clicmd(buf);
...
int IssueDisconnect(unsigned char *interfacename, unsigned char *pucMacAddr, unsigned short reason)
{
	int skfd;
	int retVal = 0;
	struct iwreq wrq;
...
int wlioctl_set_wsc_ie(unsigned char *interfacename, char *data, int len, int id, int flag)
{
	int skfd;
	int retVal=0;
	struct iwreq wrq;
...
#ifdef FOR_DUAL_BAND		
#ifdef VAP_WPS
	if(!strcmp(interfacename , "wlan0-va0")){
#else
	if(!strcmp(interfacename , "wlan0")){
#endif		
...
#ifdef VAP_WPS
	}else if(!strcmp(interfacename , "wlan1-va0")){
#else
	}else if(!strcmp(interfacename , "wlan1")){
#endif	
	...
	if (ioctl(skfd, SIOCGIWIND, &wrq) < 0)
	{
		printf("%s %d failed, interfacename is %s\n",__FUNCTION__,__LINE__, interfacename);
		retVal = -1;
	}
...
int wlioctl_get_button_state(char *interface, int *pState)
{
	...
	if ((fp = fopen("/proc/gpio", "r")) != NULL) {
		...
		*pState = 0;
#else
#ifdef CONFIG_RTL_8196CS //no wps button and led on 96cs
	*pState = 0;
#else
...
int wlioctl_scan_reqest(char *interface, int *pStatus)
{
	...
	strcpy((char *)wrq.ifr_name, interface);	
...
int wlioctl_get_mib(	unsigned char *interfacename , char* mibname ,int *result )
{
	...
	{
		printf("ioctl[SIOCGIWRTLGETMIB], interface name is %s\n", interfacename);
		close(skfd);
#ifdef ECOS_DBG_STAT
		dbg_stat_add(dbg_wsc_index, DBG_TYPE_SOCKET, DBG_ACTION_DEL, 0);
#endif
...
#ifdef DEBUG
	if (pCtx->debug2) {
		wsc_debug_out("DH shared key", (unsigned char*)pSta->dh_shared_key, size);
		wsc_debug_out("DH SHA256 key", (unsigned char*)pSta->dh_digest_key, BYTE_LEN_256B);
...
//	apmib_set(MIB_WLAN_SSID, (void *)&tmp_flash_param.SSID);
    nvram_set("wl1_ssid", (void *)&tmp_flash_param.SSID);

	/*if vxd client mode, also change the repeater ssid show in web*/
	if(!strcmp(pCtx->wlan_interface_name,WLAN0_VXD_INTF_NAME))
	{
//		apmib_set(MIB_REPEATER_SSID1,(void *)&tmp_flash_param.SSID);
		if((-1 != pCtx->join_idx) && pCtx->join_idx <pCtx->ss_status.number && pCtx->ss_status.number!=0xff)
		{
			int channel;
//			SetWlan_idx(WLAN0_INTF_NAME);
			channel=pCtx->ss_status.bssdb[pCtx->join_idx].ChannelNumber;
...
//			apmib_set(MIB_WLAN_WPA_PSK, (void *)&tmp_flash_param.WPA_PSK);
            nvram_set("wl1_wpa_psk", (void *)&tmp_flash_param.WPA_PSK);
#endif
			if (strlen(pCtx->network_key) == 64)
...
//	apmib_set(MIB_WLAN_WSC_CONFIGURED, (void *)&tmp_flash_param.WSC_CONFIGURED);
    if(1 == tmp_flash_param.WSC_CONFIGURED)
...
//#endif
	...
	if(pCtx->both_band_ap == 1)
	{
		if(pCtx->InterFaceComeIn == COME_FROM_WLAN0){
			//write_param_to_flash2(pCtx, is_local);            
        }
#if 0
		apmib_save_idx();
...
//#endif
	if(pCtx->is_ap){
		strcpy(interface_name,pCtx->wlan_interface_name2);
	}
	else{
		strcpy(interface_name,pCtx->wlan_interface_name);
	}
//	SetWlan_idx(interface_name);
	if (is_local)
		strcpy(tmp_flash_param.SSID, pCtx->SSID2);
	else
		strcpy(tmp_flash_param.SSID, (char *)pCtx->assigned_ssid);
#ifdef HAVE_APMIB
//	apmib_set(MIB_WLAN_SSID, (void *)&tmp_flash_param.SSID);
    nvram_set("wl0_ssid", (void *)&tmp_flash_param.SSID);

	if(!strcmp(interface_name,WLAN1_VXD_INTF_NAME))
	{
//		apmib_set(MIB_REPEATER_SSID2,(void *)&tmp_flash_param.SSID);
		if((-1 != pCtx->join_idx) && pCtx->join_idx <pCtx->ss_status.number && pCtx->ss_status.number!=0xff)
		{
			int channel;
...
#endif
	if (is_local) {
		if (pCtx->auth_type_flash2 & WSC_AUTH_WPAPSK || pCtx->auth_type_flash & WSC_AUTH_WPA2PSK)
			is_psk = 1;			
	}
	else {
		if (pCtx->assigned_auth_type & WSC_AUTH_WPAPSK || pCtx->assigned_auth_type & WSC_AUTH_WPA2PSK)
...
//	apmib_set(MIB_WLAN_WPA_AUTH, (void *)&tmp_flash_param.WPA_AUTH);
	...
	if (is_psk) {
		if (is_local) {
			strcpy((char *)tmp_flash_param.WPA_PSK, pCtx->network_key2);
#ifdef HAVE_APMIB
//			apmib_set(MIB_WLAN_WPA_PSK, (void *)&tmp_flash_param.WPA_PSK);
            nvram_set("wl0_wpa_psk", (void *)&tmp_flash_param.WPA_PSK);
...
//			apmib_set(MIB_WLAN_PSK_FORMAT, (void *)&tmp_flash_param.PSK_FORMAT);
	...
	if (is_local) {
		strcpy(tmp_flash_param.WSC_PSK, pCtx->network_key2);
	}
	else {
		strcpy(tmp_flash_param.WSC_PSK, (char *)pCtx->assigned_network_key);
	}
...
//#endif
	if(pCtx->both_band_ap == 1)
	{
		if(pCtx->InterFaceComeIn == COME_FROM_WLAN1)
        {
            //write_param_to_flash(pCtx, is_local);
        }      
...
int get_acl_allow_mib(CTX_Tp pCtx , unsigned char *interfacename)
{
	/*WPS2.0  if acl/allow mode ; stop WPS2 for AP mode*/
	int mibValue=0;
	if(wlioctl_get_mib(interfacename , "aclmode" ,&mibValue)==0){
		if(mibValue==1){
			//UTIL_DEBUG("acl/allow mode\n\n");
			return mibValue;
		}else{
			return 0;			
		}
...
void func_on_wlan_tx(CTX_Tp pCtx , unsigned char *interfacename)
{
	...
	if(wlioctl_get_mib(interfacename , "func_off" ,&func_off_mibValue)==0){
		...
		if(func_off_mibValue > 1){
			WSC_DEBUG("%s's func_off_mibValue = %x\n",interfacename , func_off_mibValue);
		}
	}
...
//			system("reboot");
	...
	//set wsc_enable to zero for re-connect.
	if(!pGlobalCtx->is_ap)
	{
#ifdef HAVE_HTTPD
		extern int addClientWpsProfile();
		addClientWpsProfile();
#endif
		pGlobalCtx->start = 0;
		client_set_WlanDriver_WscEnable(pGlobalCtx, 0);
		
	}
	if(pGlobalCtx->config_by_ext_reg == CONFIG_BY_EXTERNAL_REGISTRAR)
		//if the DUT is configured by win7 PC, reboot for some WIN7 cant add wlan device to DUT otherwise unplug/plug ethernet cable
...
void reset_sta_UPnP(CTX_Tp pCtx, STA_CTX_Tp pSta)
{
	...
	if (pCtx->sta_invoke_reg == pSta && pCtx->registration_on >= 1) {
		...
		if (pCtx->pin_assigned || pCtx->pb_pressed) {
			UTIL_DEBUG("pCtx->pin_assigned = %d; pCtx->pb_pressed = %d\n", pCtx->pin_assigned, pCtx->pb_pressed);
			UTIL_DEBUG("reset_ctx_state\n");						
...
void clear_SetSelectedRegistrar_flag(CTX_Tp pCtx)
{
	...
	_DEBUG_PRINT("Clear SetSelectedRegistrar flag!\n");
...
void hmac_sha256(unsigned char *text, int text_len, 
	unsigned char *key, int key_len, unsigned char *digest, int *digest_len)
{
    unsigned int    outlen;
	...
	)
{
	...
	{	
		...
		hmac_sha256(text_buf, 4+text_len+4, key, key_len, md, (int *)&md_len);
