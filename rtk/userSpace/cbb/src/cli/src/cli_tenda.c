...
static void lqq(int argc, char *argv[])
{
	elinksdk_main();
}
CLI_NODE("elinksdk",	lqq,	"elinksdk");

extern void test_elink(char* name,char op,char*value);
extern int process_data_getrssiinfo(buff,ID); 
static void elinktest(int argc, char *argv[])
{
	if(argc < 3)
	{
		test_elink(argv[1],0,NULL);
	}else
	{
		process_data_getrssiinfo("{\"type\":\"getrssiinfo\",\"get\":{\"mac\":[\"683e342671bb\",\"BCA92020D8B7\"]}}}","asdasdadasdasdasdasdasdsd"); 
		//test_elink(argv[1],1,"{\"type\":\"cfg\",\"set\":{\"wifiswitch\":{\"status\":\"ON\"},\"ledswitch\":{\"status\":\"ON\"},\"wifitimer\":[{\"weekday\":\"1\",\"time\":\"09:00\",\"enable\":\"0\"}]}}");
	}
}
CLI_NODE("elinktest",	elinktest,	"elinksdk");