...
pppoe_osl_ppp_open(struct pppoe_softc *sc)
{
 ...
 /*lq pppoe向ppp传递信息*/
	...
	if(param->unit == 1)
	{
		conf.synchro_type = param->synchro_type;
		conf.start_time = param->start_time;
		conf.time_out = param->time_out;
		strcpy(conf.service_name,sc->service_name);
	}
 #endif
	/* Start ppp daemon */
	pid = ppp_start(&conf);
...
#ifdef __CONFIG_PPPOE_SERVER__
SYNCHRO_TYPE 	pppoe_synchro_type 	= AUTO_SYNCHRO;
SYNCHRO_STATUS 	pppoe_synchro_status = SYNCHROING; 
/*获取同步类型*/
SYNCHRO_TYPE get_synchro_type()
{
	return pppoe_synchro_type;
}
/*设置同步类型*/
void set_synchro_type(SYNCHRO_TYPE type)
{
	pppoe_synchro_type = type;
}
/*获取同步状态*/
SYNCHRO_STATUS get_synchro_status()
{
	return pppoe_synchro_status;
}
/*设置同步状态*/
void set_synchro_status(SYNCHRO_STATUS type)
{
	pppoe_synchro_status = type;
}

/*lq 设置和获取同步时间，页面请求时做返回*/
unsigned long long synchro_config_start_time(unsigned long long time,int operate)
{
	static unsigned long long start_time=0;
	if(operate == SET_SYBCHRO_TIME)
	{
		start_time = time;
	}
	else if(operate == GET_SYBCHRO_TIME)
	{
		return start_time;
	}
	else
		printf("fun=%s   line=%d   operate type error\n",__FUNCTION__,__LINE__);
	return start_time;
}
#endif
...
#ifdef __CONFIG_PPPOE_SERVER__
    /*lq 初始化设置启动时间，超时时间*/
		strcpy(param->ethname, nvram_safe_get("lan_ifname"));
		param->synchro_type = get_synchro_type();
		unsigned long long sec;
		sec = cyg_current_time();
		sec = sec  / 100;
		param->start_time = sec;
		param->time_out = 90;
		synchro_config_start_time(sec,SET_SYBCHRO_TIME);
		set_synchro_status(SYNCHROING);
#endif
	}
	else
	{	
...
pppoe_main(struct pppoe_ctx *ctx)
{
	...
	nvram_commit();
#endif	
   /*lq 系统刚启动就进行同步，cpu占用率比较高的时候，
   灯显示频率会有所偏差，所以这里等待5s，等完全启动后再进行启动*/
	if(strcmp(ctx->pppname,"ppp1") == 0)
	{
