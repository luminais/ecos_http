...
#define SPEEDTEST_WEB_UPDATE_TC_TIME  30
int g_speedtest_web_update_tc_timeout = 0;
int g_speedtest_reboot_check = 0;
#endif
 ...
 */
	if (initWebs() < 0) {
		HTTP_DBGPRINT(HTTP_DEBUG_ERROR, "%s: initWeb()<0 \n", __FUNCTION__);
		return;
	}
 ...
 */
	...
	while (!finished) 
	{
		...
		/*	Yp add,use the sleep to avoid the thread who's priority is lower than 
		 *	the httpd thread turn to "RUN" status.
		 *	Judge the alllowable loop times during the limited time.
	     */
		time_for_sleep++;
		if(0 == time_for_sleep % 100)
		{
			time_for_sleep = 0;	
			if(100 >= cyg_current_time() - now)
				cyg_thread_delay(10);
		...
		if (resultReady || resultSelect) 
		{
			...
			#ifdef __CONFIG_SPEEDTEST_IMPROVE__
			/*  重新使能TC统计超期时间 */
			g_speedtest_web_update_tc_timeout = SPEEDTEST_WEB_UPDATE_TC_TIME;
			#endif
