...
static void socketAcceptv6(socket_t *sp)
{
	socket_t 			*nsp;
	size_t			len;
 ...
 */
	...
	if ((newSock = accept(sp->sock, (struct sockaddr *) &addr.sa_in, (int *) &len)) < 0)
	{
		return;
	}

	//printf("httpd accetp [%s]\n",inet_ntoa(addr.sin_addr));
	struct linger optval1;
	optval1.l_onoff = 1;
	optval1.l_linger = 60;
	ret = setsockopt(newSock,SOL_SOCKET,SO_LINGER,&optval1,sizeof(struct linger));
	if(ret<0)
	{
		perror("setsockopt");
		return;
	}
	ret = setsockopt(newSock, SOL_SOCKET, SO_KEEPALIVE, (char *)&yes, sizeof(yes));	//heli 2005-08-03
	if(ret<0)
	{
		perror("setsockopt");
		return;
 ...
 */
	...
	nsp->timeOut = SOCKET_CONNECT_TIMEOUT;
 ...
 */
	if (sp->accept != NULL)
	{
		inet_ntop(AF_INET6, &addr.sa_in.sin6_addr, pString, sizeof(pString));
		if ((sp->accept)(nid, pString, ntohs(addr.sa_in.sin6_port), sp->sid) < 0)
		{
			socketFree(nid);
		}
	}
...
int socketOpenConnectionv6(char *host, int port, socketAccept_t accept, int flags )
{
	socket_t	*sp;
	int	sid, rc;
	usockaddr addr;
 ...
 */
	...
	memset( &addr, 0, sizeof(usockaddr) );

	addr.sa.sa_family = AF_INET6;
	sp->sock  = socket( addr.sa.sa_family, SOCK_STREAM, 0 );
	if ( sp->sock  < 0 )
	{
		perror( "socket" );
		socketFree(sid);
	...
	if ( setsockopt( sp->sock , SOL_SOCKET, SO_REUSEADDR, (char*) &rc, sizeof(rc) ) < 0 )
	{
   	...
   	addr.sa_in.sin6_family = AF_INET6;
	addr.sa_in.sin6_port   = htons((short) (port & 0xFFFF));
	addr.sa_in.sin6_len   = sizeof(struct sockaddr_in6);
	inet_pton(AF_INET6, ":", (char *)&addr.sa_in.sin6_addr);

	if ( bind(sp->sock, (struct sockaddr *)&addr.sa_in, sizeof( addr.sa_in) ) < 0 )
	{
		printf("1111  sp->sock, &addr.sa, sizeof(addr.sa_in) ) < 0 \n ");
		perror( "bind" );
		socketFree(sid);
		return -1;
	}

	if (listen(sp->sock, 10) < 0)
	{
		socketFree(sid);
		return -1;
	}
...
void socketCloseConnection(int sid)
{
	...
	if ((sp = socketPtr(sid)) == NULL) {
		HTTP_DBGPRINT(HTTP_DEBUG_ERROR, "%s:--socketPtr(sid)) == NULL-- \n", __FUNCTION__);
		return;
	}
	socketFree(sid);
}
...
extern int checkAcl(unsigned int ip,int port);

#ifdef __CONFIG_APCLIENT_DHCPC__
extern int gpi_apclient_dhcpc_enable_by_mib();
#endif
...
#ifdef __CONFIG_APCLIENT_DHCPC__
	if(1 != gpi_apclient_dhcpc_enable_by_mib())
#endif
	{
		HTTP_DBGPRINT(HTTP_DEBUG_ERROR, "%s: clinet ip %s, port %d\n", __FUNCTION__, inet_ntoa(addr.sin_addr), sp->port);
//roy +++ for remote web manage
		if(! checkAcl(addr.sin_addr.s_addr,sp->port))
		{
			close(newSock);
			return;
		}
...
#ifdef ECOS
	fd_set			fdReadFds, fdWriteFds, fdExceptFds;
#endif /* ECOS */
/*
...
#ifdef ECOS
    /* Don't use balloc */
    readFds = (fd_mask *)&fdReadFds;
	writeFds = (fd_mask *)&fdWriteFds;
	exceptFds = (fd_mask *)&fdExceptFds;
 ...
 */
	...
	HTTP_DBGPRINT(HTTP_DEBUG_ERROR, "%s:--nEvents = %d -- \n", __FUNCTION__,nEvents);
	if (nEvents > 0) {
		if (all) {
			sid = 0;
		}
 ...
 */
 		/* pxy add, check whether "sp" eq NULL, prevent exception*/
		if (socketList && sid < socketMax && sp && socketList[sid] == sp) {
			sp->currentEvents = 0;
		}
	}
	return 1;
