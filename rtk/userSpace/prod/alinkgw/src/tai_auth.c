...
int ChangePassword(const char *json_in, char *buff, unsigned int buf_sz)
{
	...
	{
		if(strcmp(nvram_safe_get(LAN_HTTP_LOGIN_PASSWD),"") != 0)
		{
	...
	else
	{
		...
		if(strcmp(nvram_safe_get(LAN_HTTP_LOGIN_PASSWD),encode) != 0)
		{
			printf("current match err.\n");
			free(encode);
			encode = NULL;
	...
	if(strcmp(new_pwd,"") == 0 )
	{
		_SET_VALUE(LAN_HTTP_LOGIN_PASSWD, "");
	...
	else
	{
		...
		_SET_VALUE(LAN_HTTP_LOGIN_PASSWD, encode);
