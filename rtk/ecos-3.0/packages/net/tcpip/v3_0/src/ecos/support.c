...
#ifdef RTLPKG_DEVS_ETH_RLTK_819X_WLAN
#if STACK_SIZE < 8192
#undef STACK_SIZE
...
void cyg_kmem_print_stats( void )
{
    ...
    diag_printf( "   mbufs %lu, clusters %lu, free clusters %lu\n",
    ...
    diag_printf( "   Failed to get %lu times\n"
                 "   Waited to get %lu times\n"
                 "   Drained queues to get %lu times\n",
...
show_network_tables(pr_fun *pr)
{
    ...
    (*pr)("\n");
    for (ifp = ifnet.tqh_first; ifp != 0; ifp = ifp->if_list.tqe_next) {
        (*pr)("%s[%d]: ifq_maxlen=%d ifq_len=%d ifq_drops=%d\n", 
