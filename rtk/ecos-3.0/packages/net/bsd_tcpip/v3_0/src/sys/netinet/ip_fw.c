...
#if defined(CONFIG_RTL_819X)&&defined(MALLOC_DECLARE)	//jwj:20120612
...
//HF test
static u_int32_t dyn_max = 2048 ;
//static u_int32_t dyn_max = 512 ;
...
extern void CleanupAliasData(void);
...
#define ADJUST_CHKSUM_TCP(port_mod, port_org, chksum) \
	do { \
		int accumulate = 0; \
		accumulate += (port_org); \
		accumulate -= (port_mod); \
		...
		if (accumulate < 0) { \
			...
			chksum = ((unsigned short) ~accumulate); \
		} else { \
			accumulate = (accumulate >> 16) + (accumulate & 0xffff); \
			accumulate += accumulate >> 16; \
			chksum = ((unsigned short) accumulate); \
...
static int
tcpopts_match(struct tcphdr *tcp, struct ip_fw *f, struct ifnet *rif, struct ifnet *oif)
#else
static int
	...
	for (; cnt > 0; cnt -= optlen, cp += optlen) {
		...
		default:
			...
			case TCPOPT_MAXSEG:
				...
				/*ugly patched...*/
				//lq 在ip_output得时候修改mss
				if((oif && rif) ||(oif && !rif))
				{
					//if(oif->if_type == 0x17 /*IFT_PPP*/)
					if((oif->if_mtu - 40)<ntohs(*(unsigned short *)(&cp[2])))
					{
					//diag_printf("%s %d  mtu  %d\n",__FUNCTION__,__LINE__,oif->if_mtu);
						ADJUST_CHKSUM_TCP(htons(oif->if_mtu - 40),*(unsigned short *)(&cp[2]),tcp->th_sum);
						*(unsigned short *)(&cp[2])=htons(oif->if_mtu - 40);
					}
				}
				/*if mss incoming is max than input interface mtu. then the reply packet maybe oversized
				  * FIX it here
				  */
				if(rif && (oif==NULL))
				{
					if((rif->if_mtu - 40)<ntohs(*(unsigned short *)(&cp[2])))
					{
					//diag_printf("%s %d  mtu  %d\n",__FUNCTION__,__LINE__,rif->if_mtu);
						ADJUST_CHKSUM_TCP(htons(rif->if_mtu - 40),*(unsigned short *)(&cp[2]),tcp->th_sum);
						*(unsigned short *)(&cp[2])=htons(rif->if_mtu - 40);
					}
				}
				/*MSS patch end...*/
				break;
...
#if defined(CONFIG_RTL_NAPT_GARBAGE_COLLECTION)
void rtl_removeDynRule(struct ipfw_flow_id *pkt_in, struct ipfw_flow_id *pkt_out)
{
	int i;
	...
	i = hash_packet(pkt_out);

	for (prev=NULL, q = ipfw_dyn_v[i] ; q != NULL ; )
	{
		if ( pkt_out->proto == q->id.proto)
		{
			switch (q->type)
			{
				default:        /* bidirectional rule, no masks */
#if 1
					if (pkt_out->src_ip == q->id.src_ip&&pkt_out->dst_ip == q->id.dst_ip&&
					    pkt_out->src_port == q->id.src_port&&pkt_out->dst_port == q->id.dst_port)
#else
					if ((pkt_out->src_ip == q->id.src_ip&&pkt_out->dst_ip == q->id.dst_ip&&
					     pkt_out->src_port == q->id.src_port&&pkt_out->dst_port == q->id.dst_port) ||
					    (pkt_out->src_ip == q->id.dst_ip&&pkt_out->dst_ip == q->id.src_ip &&
					     pkt_out->src_port == q->id.dst_port&&pkt_out->dst_port == q->id.src_port ))
#endif
					{
						old_q = q ;
						if (prev != NULL)
	...
	/*To be here rarely*/
	i = hash_packet(pkt_in);

	for (prev=NULL, q = ipfw_dyn_v[i] ; q != NULL ; )
	{
		if ( pkt_in->proto == q->id.proto)
		{
			switch (q->type)
			{
				default:        /* bidirectional rule, no masks */
#if 1
					if (pkt_in->src_ip == q->id.src_ip&&pkt_in->dst_ip == q->id.dst_ip&&
					    pkt_in->src_port == q->id.src_port&&pkt_in->dst_port == q->id.dst_port)
#else
					if ((pkt_in->src_ip == q->id.src_ip&&pkt_in->dst_ip == q->id.dst_ip&&
					     pkt_in->src_port == q->id.src_port&&pkt_in->dst_port == q->id.dst_port) ||
					    (pkt_in->src_ip == q->id.dst_ip&&pkt_in->dst_ip == q->id.src_ip &&
					     pkt_in->src_port == q->id.dst_port&&pkt_in->dst_port == q->id.src_port ))
#endif
					{
						old_q = q ;
						if (prev != NULL)
...
static void rtl_removeLinkByIpfw(struct ip_fw_chain *chain)
{
	int i ;
	struct ipfw_dyn_rule *prev, *q, *old_q ;

	if (ipfw_dyn_v == NULL || dyn_count == 0 ||chain == NULL)
		return ;
             ...
             struct ip_fw_chain *chain)
{
	...
	if (r == NULL)
	{
		DEB(printf ("sorry cannot allocate state\n");)
		return ;
	}
 ...
 */
#if defined(CONFIG_RTL_819X)&&!defined(CONFIG_RTL_NAPT_GARBAGE_COLLECTION)
static int
#else
static void
#endif
install_state(struct ip_fw_chain *chain)
{
	...
	if (q != NULL)
	{
		...
		DEB(printf(" entry already present, done\n");)
#if defined(CONFIG_RTL_819X)&&!defined(CONFIG_RTL_NAPT_GARBAGE_COLLECTION)
		return 0;
#else
		return ;
	...
	{
//	 diag_printf("dyn_count is %d, dyn_max is %d\n", dyn_count, dyn_max);
		if (last_log == time_second)
		{
#if defined(CONFIG_RTL_819X)&&!defined(CONFIG_RTL_NAPT_GARBAGE_COLLECTION)
			return -1;
		...
		DEB(printf(" Too many dynamic rules, sorry\n");)
#if defined(CONFIG_RTL_819X)&&!defined(CONFIG_RTL_NAPT_GARBAGE_COLLECTION)
		return -1;
#else
		return;
...
int check_url(struct ip *ip,struct ip_fw *f,int proto)
{
	struct udphdr *udp;
	struct tcphdr *tcp;
	u_int8_t * data;
	struct dns_head *dns_p;
	int count;
	u_int8_t len;
	u_int8_t url_len;
	u_char url[MAX_URL_LEN];

	if(proto==IPPROTO_UDP)
	{
		udp =(struct udphdr*)((u_int32_t *)ip + ip->ip_hl);
		data = (u_int8_t *)udp + UDP_HDR_LENGTH;
	}
	else if(proto==IPPROTO_TCP)
	{
		tcp = (struct tcphdr*)((u_int32_t *)ip + ip->ip_hl);
		data = (u_int8_t *)((u_int32_t *)tcp + tcp->th_off);
	}
	else
		return 1;
	dns_p = (struct dns_head *)data;
	data = data + sizeof(struct dns_head);
	count=0;
	/*dns query?*/
	if(!(dns_p->flag&0x8000))
		while(count<dns_p->questions)
		{
			count++;
			url_len = 0;
			memset(url,0,MAX_URL_LEN);
			/*change query name in dns  to standard format*/
			while(*data!=0)
			{
				len = (*data);
				data++;
				while(len && url_len<MAX_URL_LEN-1)
				{
					url[url_len] = *data;
					data++;
					url_len++;
				...
				if(*data!=0 && url_len<MAX_URL_LEN-1)
				{
					url[url_len] = '.';
					url_len++;
				}
          ...
          struct sockaddr_in **next_hop)
{
...
#if defined (DUMMYNET) && defined (CONFIG_RTL_QOS)&&defined (KLD_ENABLED)
	int pipematch=0;
	int queuematch=0;
#endif
	...
	u_char eth_addr[ETHER_ADDR_LEN];
	...
	if (offset == 0)
	{
		...
		switch (proto)
		{
			...
			case IPPROTO_ICMP:
				...
				src_port = ((struct icmp *) ((u_int32_t *)ip + ip->ip_hl))->icmp_id;
				dst_port = ((struct icmp *) ((u_int32_t *)ip + ip->ip_hl))->icmp_id;
#endif
				break ;
...
#undef PULLUP_TO
...
#if defined (DUMMYNET) && defined (CONFIG_RTL_QOS)&&defined (KLD_ENABLED)
		if(((*flow_id)->rule->fw_flg&IP_FW_F_COMMAND)==IP_FW_F_PIPE)
		{
			//diag_printf("pipematched,[%s]:[%d].\n",__FUNCTION__,__LINE__);
			pipematch=1;
		}
		if(((*flow_id)->rule->fw_flg&IP_FW_F_COMMAND)==IP_FW_F_QUEUE)
		{
			//diag_printf("queuematched,[%s]:[%d].\n",__FUNCTION__,__LINE__);
			queuematch=1;
		}
#endif
	...
	again:
...
#if defined (DUMMYNET) && defined (CONFIG_RTL_QOS)&&defined (KLD_ENABLED)
		/*only match one pipe*/
		if( pipematch&& ((f->fw_flg & IP_FW_F_COMMAND)==IP_FW_F_PIPE))
		{
			//diag_printf("qos pipe has matched already\n[%s]:[%d].\n",__FUNCTION__,__LINE__);
			continue;
		}
		/*only match one queue*/
		if( queuematch&& ((f->fw_flg & IP_FW_F_COMMAND)==IP_FW_F_QUEUE))
		{
			//diag_printf("qos queue has matched already\n[%s]:[%d].\n",__FUNCTION__,__LINE__);
			continue;
		}
#endif
		...
		if(f->fw_flg & IP_FW_F_SMAC)
		{
			if(memcmp(f->ether_src,(*m)->m_pkthdr.ether_src,ETHER_ADDR_LEN))
				continue;
		}

		if(f->fw_flg & IP_FW_F_DMAC)
		{
			/*arp resolve dest ip*/
			la = arplookup(dst_ip.s_addr, 1, 0);
			if(la!=NULL && la->la_rt!=NULL)
			{
				sdl = SDL(la->la_rt->rt_gateway);
				if( sdl->sdl_family == AF_LINK && sdl->sdl_alen != 0)
				{
					bcopy(LLADDR(sdl), eth_addr, sdl->sdl_alen);
					/*dest mac match?*/
					if(memcmp(f->ether_dst,eth_addr,ETHER_ADDR_LEN))
						continue;
				}
				else
					continue;
		...
		if(f->fw_flg & IP_FW_F_URL)
		{
			if((proto==IPPROTO_UDP||proto==IPPROTO_TCP) && ntohs(dst_port)==DNS_PORT)
			{
				if(check_url(ip,f,proto)==0)
					continue;
			}
			else
				continue;
		...
		if (f->fw_flg & IP_FW_F_SRCIP_RANGE)
		{
			if (((f->fw_flg & IP_FW_F_INVSRC) != 0) ^ ((src_ip.s_addr < f->fw_srcipaddr_start) || (src_ip.s_addr > f->fw_srcipaddr_end)))
				continue;
			//diag_printf("%s %d got match srcip %s \n", __FUNCTION__, __LINE__, inet_ntoa(src_ip));
		}
		else
#endif
...
#ifdef RTL_EXTEND_FIREWALL_SUPPORT_IP_RANGE
		if (f->fw_flg & IP_FW_F_DSTIP_RANGE)
		{
			if (((f->fw_flg & IP_FW_F_INVDST) != 0) ^ ((dst_ip.s_addr < f->fw_dstipaddr_start) || (dst_ip.s_addr > f->fw_dstipaddr_end)))
				continue;
			//diag_printf("%s %d got match dstip %s \n", __FUNCTION__, __LINE__, inet_ntoa(dst_ip));
		}
		else
#endif
		...
		if (f->fw_prot == IPPROTO_IP
		    ...
		    || f->fw_prot == IPPROTO_DMAC
		    || f->fw_prot == IPPROTO_URL_FILTER)
		{
			/*Check url*/
		...
		switch (proto)
		{
			case IPPROTO_TCP:
			{
				...
				if (f->fw_tcpopt != f->fw_tcpnopt &&
#ifdef CONFIG_RTL_819X
				    !tcpopts_match(tcp, f, rif, oif)
#else
				    !tcpopts_match(tcp, f)
#endif
				   )
...
check_ipfw_struct(struct ip_fw *frwl)
{
	 ...
	 */
#ifdef RTL_EXTEND_FIREWALL_SUPPORT_IP_RANGE
	if ((frwl->fw_flg & IP_FW_F_SRCIP_RANGE) || (frwl->fw_flg & IP_FW_F_DSTIP_RANGE))
	{
		if ((frwl->fw_srcipaddr_start > frwl->fw_srcipaddr_end) ||
		    (frwl->fw_dstipaddr_start > frwl->fw_dstipaddr_end))
		{
...
#if defined(CONFIG_RTL_819X)	//jwj:20120612
						dst->chain = (struct ip_fw_chain *)p->chain->rule->fw_number ;
#else
