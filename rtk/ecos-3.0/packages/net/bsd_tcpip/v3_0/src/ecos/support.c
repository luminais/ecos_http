...
static unsigned char net_clusters_area[NET_CLUSTERS_SIZE] __attribute__ ((aligned(HAL_DCACHE_LINE_SIZE)));
#else
...
void 
cyg_net_mbuf_free(caddr_t addr)
{
    START_STATS();
    mbstat.m_mbufs--;
...
void dump_cluster_stats(int type)
{
	int i=0;
	union	mcluster *p;
	
	diag_printf("Free clusters:\n");
	p = mclfree;
	while (p) {
		if (type==0) {
			diag_printf("%d:%p %d\n", i, p, mclrefcnt[mtocl(p)]);
		}
		else {
			if (mclrefcnt[mtocl(p)])
				diag_printf("%d:%p %d\n", i, p, mclrefcnt[mtocl(p)]);
		}
		p = ((union mcluster *)(p))->mcl_next;
		i++;
	}
	
	diag_printf("Clusters:\n");
	diag_printf("total=%d free=%lu(%d)\n", (NET_CLUSTERS_SIZE/MCLBYTES), mbstat.m_clfree, i);
	for (i=0; i<((NET_CLUSTERS_SIZE/MCLBYTES)+1); i++) {
		//diag_printf("%d:%p(%p) %d\n", i, &net_clusters_area[MCLBYTES*i], cltom(i), net_clusters_refcnt[i]);
		if (type==0) {
			diag_printf("%d:%p %d\n", i, &net_clusters_area[MCLBYTES*i], net_clusters_refcnt[i]);
		}
		else {
			if ((net_clusters_refcnt[i]<0) || (net_clusters_refcnt[i]>1))
				diag_printf("%d:%p %d\n", i, &net_clusters_area[MCLBYTES*i], net_clusters_refcnt[i]);
		}
	}
	/*dump ppp reserved cluster!!*/
#ifdef CYGPKG_PPP
	dump_ppp_cluster_stats();
#endif
}
...
#if 0
    p = (unsigned char *)((((long)(&net_mbufs_area) + MSIZE - 1) / MSIZE)  * MSIZE);
    cyg_mempool_fix_create(p,
                           ((((&(net_mbufs_area[NET_MBUFS_SIZE])) - p) / MSIZE)  * MSIZE),
                           MSIZE,
                           &net_mbufs,
                           &net_mbufs_pool);
#else	
...
/*获取内核剩余内存，用于app在线升级时判断*/
int get_kern_free_mem()
{
...
/*  获取mbuf 剩余内存, 用于mbuf 内存耗尽时的判断*/
int get_kern_mbuf_free_mem()
{
	cyg_mempool_info info;
...
/*获取misc剩余内存，用于app在线升级时判断*/
int get_kern_misc_free_mem()
{
	cyg_mempool_info info;
...
/*mbuftl is aligned on MCLBYTES. assume x is aligned on MCLBYTES*/
int
cyg_mtocl(u_long x)
{
    int res;
    res = (((u_long)(x) - (u_long)mbutl) / MCLBYTES);
    return res;
}
...
{
    struct mbuf *res;
    res = (struct mbuf *)((caddr_t)((u_long)mbutl + ((u_long)(x) * MCLBYTES)));
    return res;
}
...
cyg_netint(cyg_addrword_t param)
{
	...
	spl = cyg_splinternal();
	if(curisr & (1<<NETISR_RTL_DRV))
	{
		curisr &= ~(1<<NETISR_RTL_DRV);
...
#if 1  // Bridge code not available yet
#if NBRIDGE > 0
    bridgeattach(0);
#endif
#endif
...
void show_arp_tables(pr_fun *pr)
{
    cyg_scheduler_lock();
    (*pr)("ARP tables\n");
    (*pr)("IP     MAC\n");	
...
show_network_tables(pr_fun *pr)
{
    ...
    (*pr)("\n");
    for (ifp = ifnet.tqh_first; ifp; ifp = ifp->if_link.tqe_next) {
        (*pr)("%s%d[%d]: ifq_maxlen=%d ifq_len=%d ifq_drops=%d\n", 
        	ifp->if_name, ifp->if_unit, ifp->if_index,
        	ifp->if_snd.ifq_maxlen, ifp->if_snd.ifq_len, ifp->if_snd.ifq_drops);
    }
    {
    	extern struct	ifqueue arpintrq;
    	extern	struct	ifqueue	ipintrq;
	(*pr)("arpintrq: ifq_maxlen=%d ifq_len=%d ifq_drops=%d\n", 
        	arpintrq.ifq_maxlen, arpintrq.ifq_len, arpintrq.ifq_drops);
	(*pr)("ipintrq: ifq_maxlen=%d ifq_len=%d ifq_drops=%d\n", 
        	ipintrq.ifq_maxlen, ipintrq.ifq_len, ipintrq.ifq_drops);
    }
    */
    /* for debug, end */
    cyg_scheduler_unlock();
 ...
 * Function name: show_network_tables2
 * Description: The following function is used to update(add) arp tables of tenda_arp
 * Date: 2015.12.18
 */
 
//roy +++,2010/09/25
void
show_network_tables2(int (*mtenda_routing_entry)(struct radix_node *, void *),void *pr)
{
    int i, error;
    struct radix_node_head *rnh;
    ...
    for (i = 1; i <= AF_MAX; i++) {
        if ((rnh = rt_tables[i]) != NULL) {
            error = rnh->rnh_walktree(rnh, mtenda_routing_entry, pr);
        }
    }
    cyg_scheduler_unlock();
}
...
#if defined(CYGPKG_NET_FREEBSD_GET_MEMINFO)
void mem_chain_upgrade_malloc_cluster(void** buf,int* len){
	union mcluster* buf2 = NULL;	
	MCLALLOC(buf2,M_DONTWAIT);	
	*buf = buf2->mcl_buf;
	*len = 	MCLBYTES;
}

void mem_chain_upgrade_free_cluster(void *buf){
	union  mcluster* p;
	p = (union  mcluster*)((char*)buf - ((union  mcluster*)0)->mcl_buf);	
	MCLFREE(p);
}

void mem_chain_upgrade_malloc_miscpoll(void** buf,int len){
	char* buf2 = NULL;
	buf2 = (char*)cyg_net_malloc(len,M_DEVBUF,M_NOWAIT);
	*buf = buf2;
}

void mem_chain_upgrade_free_miscpoll(void* buf){
	cyg_net_free(buf,M_DEVBUF);
}

void mem_chain_upgrade_get_meminfo(int* misc_max,int* cluster_max,int* cluster_block){
	cyg_mempool_info info;

    cyg_mempool_var_get_info( net_mem, &info );
	*misc_max = info.maxfree;

    cyg_mempool_fix_get_info( net_clusters, &info );
    *cluster_max = info.freemem;
    *cluster_block = info.blocksize;
}
