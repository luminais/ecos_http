...
//#include <netinet/fastpath/rtl_if.h>
#include <rtl/rtl_if.h>
#include <net/if_var.h>
#include <netinet/ip_flow.h>

#include <net/if_arp.h>
#include <net/if_bridge.h>
#include "../libalias/alias_local.h"
/*
	Virtual Rome Driver API & System (Light Rome Driver Simulator)
*/
#if 0//defined (DUMMYNET)
...
int fast_pppoe_xmit(struct sk_buff *skb, struct ifnet *ipf);
...
struct l2tp_info
{
	...
	//__u32 daddr;
	//__u32 saddr;
	__u16 peer_tid;                   /* Tunnel ID */
	__u16 peer_cid;                   /* Caller ID */
	__u16 tid;                   /* Tunnel ID */
	__u16 cid;                   /* Caller ID */
	__u16 valid;
	/*data. the valid let mac header start from 2 offset then iph is aligned on 4 bytes boundary*/
       unsigned char mac_header[ETH_HLEN];
	struct iphdr iph;
	struct l2tp_ext_hdr l2tp_data_hdr;
};
...
struct nat_info
{
	...
	int		rule_nr;
	struct ip_fw_l *rule;
	#else  //always define CONFIG_RTL_QOS if DUMMYNET is defined
	int		pipe_nr;
	void	*rule;
	#endif
};
...
#ifdef DOS_FILTER
#if defined(__ECOS)
	extern int filter_enter(struct mbuf *m, struct ifnet *ifp, struct ether_header *eh);
	extern void filter_addconnect(unsigned int ipaddr);
	extern void filter_delconnect(unsigned int ipaddr);
#else
	extern int filter_enter(void *skb);
	extern int filter_init(void);
	extern void filter_exit(void);
...
//	extern int pptp_tcp_finished;
	extern int fast_pptp_to_lan( struct mbuf  **pm);
	extern int fast_pptp_to_wan(struct mbuf **pbuf, struct ifnet  *ifp);
	extern int fast_pptp_to_lan_skb(void *pskb);
	extern int fast_pptp_to_wan_skb(void *skb,struct ifnet  *ifp);
	...
	extern int fast_l2tp_to_wan(void *skb, struct ifnet *ifp);
...
#if (defined(__ECOS) && defined(CYGNUM_RAM_SIZE_0x00800000))
#define	NAPT_TABLE_LIST_MAX	1024
#define	NAPT_TABLE_ENTRY_MAX	1024
#define	PATH_TABLE_LIST_MAX	1024
#else
#if defined(CONFIG_RTL_8198) || defined(CONFIG_RTL_92D_SUPPORT) ||defined(CONFIG_RTL_8196D) ||defined(CONFIG_RTL_8197D) ||defined(CONFIG_RTL_8196E) || defined(CONFIG_RTL_8197F) || defined(CONFIG_RTL_8881A)
...
#define		FLAG_QOS_ENTER		1
#define		FLAG_QOS_CHECK		2
...
#if 1//defined (DUMMYNET)
	uint8 			flag;
#if 1//defined (CONFIG_RTL_QOS)
	int 		rule_nr;
	struct ip_fw_l  *rule;
#else //for code independence, CONFIG_RTL_QOS is always defined if DUMMYNET is defined
	int			pipe_nr;
	void*			rule;
#endif
...
#define FASTPATH_ADJUST_CHKSUM_IPLEN_FRAG(ip_len_mod, ip_len_org, fragoffset_mod, fragoffset_org, chksum) \
	do { \
		s32 accumulate = 0; \
		accumulate += (ip_len_org); \
		accumulate -= (ip_len_mod); \
		accumulate += (fragoffset_org); \
		accumulate -= (fragoffset_mod); \
		accumulate += (chksum); \
		if (accumulate < 0) { \
			accumulate = -accumulate; \
			accumulate = (accumulate >> 16) + (accumulate & 0xffff); \
			accumulate += accumulate >> 16; \
			chksum = ((uint16) ~accumulate); \
		} else { \
			accumulate = (accumulate >> 16) + (accumulate & 0xffff); \
			accumulate += accumulate >> 16; \
			chksum = ((uint16) accumulate); \
...
int fast_path_post_process_xmit_check(struct ip *iph, struct tcphdr *tcphupuh, struct sk_buff *m, struct ifnet *ifp);
...
int ip_finish_output3(struct ifnet *ifp, struct mbuf *m, 
	struct sockaddr *dst, struct rtentry *rt, struct nat_info *,struct route*);
#else
int ip_finish_output3(struct ifnet *ifp, struct mbuf *m, struct sockaddr *dst, struct rtentry *rt);
#endif
#if defined (CONFIG_RTL_FAST_PPPOE)
int ip_finish_output4(struct sk_buff *skb);
#endif
__IRAM_GEN int enter_fast_path(struct ifnet *ifp, struct mbuf  *m, struct ether_header *eh);
...
int rtl_ip_route_input(struct sk_buff  *skb, unsigned int daddr, unsigned int saddr, u8 tos);
...
unsigned int rtl_get_ct_ip_by_dir(void *ct_ptr, enum ip_conntrack_dir dir, int flag);
unsigned short rtl_get_ct_port_by_dir(void *ct_ptr, enum ip_conntrack_dir dir, int flag);
...
#ifdef __ECOS
extern long fp_jiffies;
long rtl_getTimeStamp(void);

#define RTLNIPQUAD(addr) \
	((unsigned char *)&addr)[0], \
	((unsigned char *)&addr)[1], \
	((unsigned char *)&addr)[2], \
	((unsigned char *)&addr)[3]
...
extern int arpresolve(register struct arpcom *ac, register struct rtentry *rt, struct mbuf *m, register struct sockaddr *dst, register unsigned char *desten, struct rtentry *);
#endif

#define RTL_FASTPATH_FRAGMENT_SUPPORT 1
#if defined(RTL_FASTPATH_FRAGMENT_SUPPORT)
#define HH_DATA_MOD     16
#define HH_DATA_RESERVE_LEN     128

int fast_pppoe_xmit_check(struct sk_buff *skb, struct ifnet  *ifp);
int fast_pppoe_xmit2(struct ifnet  *ifp, struct sk_buff *skb, struct sockaddr *dst, struct rtentry *rt);
int fast_l2tp_to_wan_check(struct ifnet  *ifp, struct sk_buff *skb);
int fast_l2tp_to_wan2(struct ifnet  *ifp, struct sk_buff *skb, struct sockaddr *dst, struct rtentry *rt);
int fast_pptp_to_wan_check(struct ifnet  *ifp, struct sk_buff *skb);
int fast_pptp_to_wan_skb2(struct ifnet  *ifp, struct sk_buff *skb, struct sockaddr *dst, struct rtentry *rt);
#endif
