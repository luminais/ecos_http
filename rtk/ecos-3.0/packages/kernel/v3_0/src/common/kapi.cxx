...
cyg_bool_t cyg_thread_dump_info( cyg_handle_t threadh,
                                cyg_uint16 id, cyg_uint8 flag) __THROW
{
    cyg_bool_t result = true;
    Cyg_Thread *thread = (Cyg_Thread *)threadh;
    CYG_CHECK_DATA_PTRC( thread );
    ...
    if( thread->get_unique_id() == id)
    {
	 ...
	 {	 	
		...
		if(flag == 1)
			regs=thread->get_saved_context();
		else			
			regs=thread->rtl_get_saved_context();
		dump_stacktrace(regs);
	 }
    }
    else result = false;
