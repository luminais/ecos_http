...
unsigned int Cyg_HardwareThread::get_thread_entry()
{
	return (unsigned int )&Cyg_HardwareThread::thread_entry;
}

externC unsigned int rtl_get_thread_entry()
{
	return Cyg_HardwareThread::get_thread_entry();
}
...
#if 0
    /*HF debug*/
    if(current->stack_base == (unsigned int)idle_thread_stack) {
		diag_printf("%s %d stack_base %x\n",__FUNCTION__,__LINE__,current->stack_base);
		dump_stack();
    }	
    /*HF*/
#endif
...
idle_thread_main( CYG_ADDRESS data )
{
	...
	extern void softdog_clear_time(void);
	softdog_clear_time();
#endif
#if 0
