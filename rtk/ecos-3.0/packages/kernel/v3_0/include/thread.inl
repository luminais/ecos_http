...
)
{
    entry_point = e_point;
    entry_data  = e_data;
#if defined(CYGDBG_KERNEL_DEBUG_GDB_THREAD_SUPPORT) || defined(CONFIG_RTL_819X)
    saved_context = 0;
#endif
...
inline HAL_SavedRegisters *Cyg_HardwareThread::rtl_get_saved_context()
{
    HAL_SavedRegisters *regs;
    HAL_THREAD_GET_SAVED_REGISTERS( stack_ptr, regs );
    return regs;
}
