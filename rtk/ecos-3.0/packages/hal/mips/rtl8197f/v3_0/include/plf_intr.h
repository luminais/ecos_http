...
#define CYGNUM_HAL_INTERRUPT_NUM0			32	// INT = 0~31
#define CYGNUM_HAL_INTERRUPT_NUM1			64	// INT = 32~63

#define CYGNUM_HAL_INTERRUPT_HW1              64
#define CYGNUM_HAL_INTERRUPT_HW2              65
#define CYGNUM_HAL_INTERRUPT_HW3              66
#define CYGNUM_HAL_INTERRUPT_HW4              67
#define CYGNUM_HAL_INTERRUPT_HW5              68
#define CYGNUM_HAL_SPURIOUS_INTERRUPT         69
...
{                                                                       \
    if( (_vector_) < CYGNUM_HAL_INTERRUPT_NUM0 )                        \
    {                                                                   \
        REG32(BSP_GIMR) &= ~(1 << (_vector_));                          \
    }                                                                   \
    else if( (_vector_) < CYGNUM_HAL_INTERRUPT_NUM1 )                   \
    {                                                                   \
        REG32(BSP_GIMR2) &= ~(1 << ((_vector_) - CYGNUM_HAL_INTERRUPT_NUM0)); \
    }                                                                   \
...
{									\
	...
	REG32(BSP_TC1DATA) = (0xFFFFFFFF >> BSP_TCD_OFFSET) << BSP_TCD_OFFSET;		\
	REG32(BSP_TCCNR) |= (BSP_TC1EN | BSP_TC1MODE_TIMER);		\
	REG32(BSP_TCIR) &= ~BSP_TC1IE;					\
}
#else
#define HAL_CLOCK_INITIALIZE( _period_ )				\
{									\
