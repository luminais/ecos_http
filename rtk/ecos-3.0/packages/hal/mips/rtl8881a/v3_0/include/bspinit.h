...
#define IMEM1_SIZE		0x2000
	...
	la	$8, __iram
	la	$9, 0x0fffe000	# 8K-byte(IMEM0_SIZE) alignment
	and	$8, $8, $9
	mtc3	$8, $0	# IW bas
	nop
	nop
	add	$8, $8, (IMEM0_SIZE-1) # 0x1fff
	mtc3	$8, $1	# IW top
	nop
	nop
	...
	nop
	nop
	add	$8, $8, (IMEM1_SIZE-1)
	mtc3	$8, $3	# IW top
	nop
	nop

	#--- Refill IMEM1 with a 0->1 transition
	mtc0	$0, $20, 1 # CCTL
	nop
	nop
	li	$8,0x00000010 # IMEM1 Fill
	mtc0	$8, $20, 1
	nop
	nop
skip_imem_init:
...
#define WORD_SZIE		4
	...
	la	$9, 0x0ffff000	# 4K-byte(DMEM0_SIZE) alignment
	and	$8, $8, $9
	mtc3	$8, $4	# DW bas
	nop
	nop
	add	$8, $8, (DMEM0_SIZE-1) # 0x0fff
	mtc3	$8,$5	# DW top
	nop
	nop
