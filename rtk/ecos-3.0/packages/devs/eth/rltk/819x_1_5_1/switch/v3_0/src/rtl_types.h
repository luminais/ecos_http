...
#define CONFIG_RTL_8881A_CUTE_MAHJONG_ESD
#endif
#endif
/* ===============================================================================
		print macro
...
#define __IRAM_SECTION_	__attribute__ ((section(".iram-fwd")))
#else
#define __IRAM_SECTION_
#endif
#ifdef CYGSEM_HAL_DMEM_SUPPORT
#define __DRAM_SECTION_	__attribute__ ((section(".dram-fwd")))
#else
#define __DRAM_SECTION_
#endif
...
#if defined(CONFIG_RTL8196B) && !defined(CONFIG_RTL_8198_NFBI_BOARD)
...
#ifdef __ECOS
#ifdef CONFIG_RTL_8196CS
#define CONFIG_POCKET_ROUTER_SUPPORT		1
#endif
#endif
...
#define DYN_THR_AGG_fcOFF				0xa0
#define DYN_THR_AGG_sharedON			0x88
#define DYN_THR_AGG_sharedOFF			0x70
...
#define RTL_swNic_send(a,b,c,d,e)		New_swNic_send(a,b,c,d,e)
#define RTL_swNic_receive(a,b,c,d)	New_swNic_receive(a,b,c,d)
#define RTL_swNic_txDone()			New_swNic_txDone()
#define RTL_swNic_freeRxBuf()		New_swNic_freeRxBuf()
#define RTL_dumpRxRing()			New_dumpRxRing()
#define RTL_dumpTxRing()			New_dumpTxRing()
//#define RTL_dumpMbufRing()
#else
#define RTL_swNic_init(a,b,c,d)		swNic_init(a,b,c,d)
#define RTL_swNic_send(a,b,c,d,e)		swNic_send(a,b,c,d,e)
#define RTL_swNic_receive(a,b,c,d)	swNic_receive(a,b,c,d)
#define RTL_swNic_txDone()			swNic_txDone()
#define RTL_swNic_freeRxBuf()		swNic_freeRxBuf()
#define RTL_dumpRxRing()			dump_rx_desc_own_bit()
#define RTL_dumpTxRing()			dump_tx_desc_own_bit()
//#define RTL_dumpMbufRing(())			rtl_dumpMbufRing()
#endif

#ifdef __ECOS
#define SMP_LOCK_ETH(x)				save_and_cli(x)
#define SMP_UNLOCK_ETH(x)			restore_flags(x)

#define SMP_LOCK_ETH_XMIT(x)		save_and_cli(x)
#define SMP_UNLOCK_ETH_XMIT(x)		restore_flags(x)

#define SMP_LOCK_ETH_RECV(x)		save_and_cli(x)
#define SMP_UNLOCK_ETH_RECV(x)		restore_flags(x)

#define SMP_LOCK_ETH_BUF(x)			save_and_cli(x)
#define SMP_UNLOCK_ETH_BUF(x)		restore_flags(x)

#define SMP_LOCK_ETH_HW(x)			save_and_cli(x)
#define SMP_UNLOCK_ETH_HW(x)		restore_flags(x)
...
#define RTL_TRAP_BR_SUBNET_PKT_FROM_WAN_TO_CPU
#endif
