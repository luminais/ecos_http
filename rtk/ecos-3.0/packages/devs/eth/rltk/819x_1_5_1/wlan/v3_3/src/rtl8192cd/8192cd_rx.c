...
#endif
    //td_dump_8023_skb(priv,pskb,__func__,NULL);
...
/* Hotspot 2.0 Release 1 */
	...
	#if defined(CONFIG_RTL_QOS_RATE_LIMIT_CHECK)
	ret_check = rtl_qos_rate_limite_check(1, priv->dev->name, pskb);
	if (ret_check == 0){
		dev_kfree_skb_any(pskb);
		return ;
	}
...
#endif
        if(1 == priv->pshare->rf_ft_var.td_dev_probe_enable && (0 == pfrinfo->to_fr_ds || 2 == pfrinfo->to_fr_ds))//BSPHZX++
        {
            tenda_dev_probe_process(priv, pfrinfo);
        }
#ifdef CONFIG_RTK_MESH
	pfrinfo->is_11s = 0;
