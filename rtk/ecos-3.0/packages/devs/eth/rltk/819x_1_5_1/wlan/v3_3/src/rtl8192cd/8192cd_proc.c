...
extern unsigned int        all_max_power[];
...
#endif			
{
    ...
    PRINT_SINGL_ARG("    td_width_enable: ", priv->pmib->dot11nConfigEntry.td_width_enable, "%d");
	if (COEXIST_ENABLE) {
#ifdef CLIENT_MODE
		if (OPMODE & WIFI_STATION_STATE) {
...
//	PRINT_BITMAP_ARG("    has_2r_sta", priv->pshare->has_2r_sta);
    ...
    PRINT_SINGL_ARG("    td_acs_enable: ", priv->pmib->dot11nConfigEntry.td_acs_enable, "%u");
    PRINT_SINGL_ARG("    td_acs_expire: ", priv->pmib->dot11nConfigEntry.td_acs_expire, "%u");
...
#endif			
{
	...
	if (is_MCS_rate(pstat->rx_rate)) 
	{
		...
		rate_index = pstat->rx_rate - HT_RATE_ID;
		PRINT_ONE(rate_index, "%d", 0);
		if(rate_index < 0)//该值概率性出现-126
		{
			PRINT_ONE("error", " %s", 1);
			CHECK_LEN_B;
		}
		...
		{
			rate = (unsigned char *)MCS_DATA_RATEStr[pstat->rx_bw&0x01][pstat->rx_splcp&0x01][rate_index];
			PRINT_ONE(rate, " %s", 1);
			CHECK_LEN_B;
		}
	}
...
//add by luo
int a_rate[] = {6, 9, 12, 18, 24, 36, 48, 54, 0, 1, 0, 0, 0, 2, 5, 11};//5 is 5.5,because is not support float
int b_rate[] = {6, 9, 12, 18, 24, 36, 48, 54, 0, 1, 2, 5, 11, 0, 0, 0};//5 is 5.5,because is not support float

static int output_power(unsigned int *power_result)
{
	int i,j;
	int temp[16];
	j = 0;
	...
	for(i=0;i<j;i++)
	{
		...
		{
			printf("%2d.5  ",(temp[i]-1)/2);
		}
	}
	printf("\n");
	printf("RATE:  ");
	printf("MCSO  MCS1  MCS2  MCS3  MCS4  MCS5  MCS6  MCS7  MCS8  MCS9  MCS10  MCS11  MCS12  MCS13  MCS14  MCS15\n");
	
	printf("POWER: ");
	for(i=16;i<32;i++)
	{		
		if(0 == power_result[i]%2)
		{
			if(i>16+9)
			{
				printf("%5d  ",power_result[i]/2);
			}
			else
			{
				printf("%4d  ",power_result[i]/2);
			}
		}
		else
		{
			if(i>16+9)
			{
				printf("%3d.5  ",(power_result[i]-1)/2);
			}
			else
			{
				printf("%d.5  ",(power_result[i]-1)/2);
			}
		}
	}
	printf("\n\n");
...
static int rtl8192cd_proc_power(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
	struct net_device *dev = PROC_GET_DEV();
    struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	...
	unsigned int temp = 0;
	unsigned int all_index_power[64];
	unsigned int per_line_index_power[16];
	//A tianxian
	per_line_index_power[0] = RTL_R32(rTxAGC_A_Rate18_06);//0xe00
	per_line_index_power[1] = RTL_R32(rTxAGC_A_Rate54_24);//oxe04
	per_line_index_power[2] = RTL_R32(rTxAGC_A_CCK1_Mcs32);//0xe08
	per_line_index_power[3] = RTL_R32(rTxAGC_A_CCK11_2_B_CCK11);//0x86c
	per_line_index_power[4] = RTL_R32(rTxAGC_A_Mcs03_Mcs00);//0xe10
	per_line_index_power[5] = RTL_R32(rTxAGC_A_Mcs07_Mcs04);//0xe14
	per_line_index_power[6] = RTL_R32(rTxAGC_A_Mcs11_Mcs08);//0xe18
	per_line_index_power[7] = RTL_R32(rTxAGC_A_Mcs15_Mcs12);//0xe1c

	//B tianxian
	per_line_index_power[8] = RTL_R32(rTxAGC_B_Rate18_06);//0x830
	per_line_index_power[9] = RTL_R32(rTxAGC_B_Rate54_24);//0x834
	per_line_index_power[10] = RTL_R32(rTxAGC_B_CCK5_1_Mcs32);//0x838 
	per_line_index_power[11] = RTL_R32(rTxAGC_A_CCK11_2_B_CCK11);//0x86c
	per_line_index_power[12] = RTL_R32(rTxAGC_B_Mcs03_Mcs00);//0x83c
	per_line_index_power[13] = RTL_R32(rTxAGC_B_Mcs07_Mcs04);//0x848
	per_line_index_power[14] = RTL_R32(rTxAGC_B_Mcs11_Mcs08);//0x84c
	per_line_index_power[15] = RTL_R32(rTxAGC_B_Mcs15_Mcs12);//0x868

	for(i=0;i<16;i++)
	{
		temp = per_line_index_power[i];
		while(temp)
		{
			all_index_power[j++] = temp & 0xff;
			temp = temp >> 8;
		}	
	}
	...
	//output max_power
	printf("------------------max power----------------------\n");
	output_power(all_max_power);
	//output index_power
	printf("------------------index power-----------------------\n");
	output_power(all_index_power);
...
/*获取elink 所需的无线包统计信息*/
int rtl8192cd_elink_wlan_stats(char*name,struct net_device_elink_stats *stats)		
{
	extern struct rtl8192cd_priv *wlan_search_priv(char *name);
	struct rtl8192cd_priv *priv = wlan_search_priv(name);
	
	stats->tx_packets = priv->net_stats.tx_packets;
	stats->tx_bytes = priv->net_stats.tx_bytes;
	stats->tx_errors = priv->net_stats.tx_errors;
	stats->tx_drops = priv->ext_stats.tx_drops;
	stats->rx_packets = priv->net_stats.rx_packets;
	stats->rx_bytes = priv->net_stats.rx_bytes;
	stats->rx_errors = priv->net_stats.rx_errors;
	stats->rx_data_drops = priv->ext_stats.rx_data_drops;	
...
int g_smart_led_status = 1;
EXPORT_SYMBOL(g_smart_led_status);
...
#endif
{
	...
	int flag;
        ...
        else if (flag == 3) //only used for wifi button 
        {
            g_smart_led_status = 1;
        }
		else if (flag == 4) //  add flag 4 for shared wifi/wps gpio do not turn on wifi led while smart led set led off 
        {
            if (g_smart_led_status == 1)
            {
                if (IS_DRV_OPEN(priv))
                {
                    control_wireless_led(priv, 2);
...
#ifdef USE_OUT_SRC
static char *steer_msg = NULL;
#define STEER_MSG_LEN	80*24

static int proc_get_steer_cmd(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
{
    int len = 0;
#if defined(__KERNEL__) && !defined(CONFIG_RTL_PROC_NEW)
    ...
    //allocate memory to phydm_msg
    if (NULL == steer_msg) {
        steer_msg = rtw_zmalloc(STEER_MSG_LEN);
        if (NULL == steer_msg) {
            return -EFAULT;
        }
        sta_steer_read_proc();
    ...
    PRINT_ONE(steer_msg, "%s", 1); //print phydm_smg to buf for proc file
#if defined(__KERNEL__) && !defined(CONFIG_RTL_PROC_NEW)
    size = pos; // size is for the length check macro CHECK_LEN
    CHECK_LEN; //if phydm_msg is larger than length, it must be written in proc file several times 
#endif
    rtw_mfree(steer_msg, STEER_MSG_LEN); //free memory
    steer_msg = NULL;
...
static int proc_set_steer_cmd(struct file *file, const char *buffer,
				unsigned long count, void *data)
{
    ...
    if (buffer && !copy_from_user(tmp, buffer, 64)) { //read cmd from proc file
		if (NULL == steer_msg) {
			...
			memset(steer_msg, 0, STEER_MSG_LEN);
		}
        
        steer_cmd(tmp, count, 1, steer_msg, STEER_MSG_LEN);
        //phydm_cmd(ODMPTR, tmp, count, 1, steer_msg, STEER_MSG_LEN);
		if (strlen(steer_msg) == 0) {
			rtw_mfree(steer_msg, STEER_MSG_LEN);
			steer_msg = NULL;
		}
    }
    return count;
...
static int rtl8192cd_proc_acs_score(struct seq_file *s, void *data)
{
    struct net_device *dev = PROC_GET_DEV();
    struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	int i;
    ...
    if(priv->ss_req_ongoing)	{
		...
		PRINT_ONE(" SiteSurvey score : ", "%s", 1);
        PRINT_ONE("    ====================", "%s", 1);
		for(i = 0; i < priv->available_chnl_num; i++){
			unsigned char ch = priv->available_chnl[i];
			PRINT_SINGL_PART_ARG("\tChannel:",	ch, "%-4d");
			PRINT_SINGL_PART_ARG("\tScore:",	priv->chnl_ss_score[ch], "%-4d");
			PRINT_SINGL_ARG("\tLevel:", priv->chnl_ss_score_level[ch], "%-4d");
		}
		PRINT_ONE("    ====================", "%s", 1);
	}
	return 0;
...
static int rtl8192cd_proc_ch_util(struct seq_file *s, void *data)
{
    struct net_device *dev = PROC_GET_DEV();
    struct rtl8192cd_priv *priv = GET_DEV_PRIV(dev);
	int i;
    ...
    if(priv->ss_req_ongoing)	{
		...
		PRINT_ONE(" SiteSurvey channel utilization: ", "%s", 1);
        PRINT_ONE("    ====================", "%s", 1);
		for(i = 0; i < priv->available_chnl_num; i++){
			unsigned char ch = priv->available_chnl[i];
			PRINT_SINGL_PART_ARG("\tChannel:",	ch, "%-4d");
			PRINT_SINGL_PART_ARG("\tBSS:",	priv->chnl_ss_ch_bss[ch], "%-4d");	
			PRINT_SINGL_PART_ARG("\tBusy:",	priv->chnl_ss_ch_busy[ch], "%-4d");
			PRINT_SINGL_ARG("\tPercent:",	priv->chnl_ss_ch_busy_percent[ch], "%-4d");
		}
		PRINT_ONE("    ====================", "%s", 1);
	}
	return 0;
    ...
    RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_acs_score);
    RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_ch_util);
#endif // CONFIG_RTL_PROC_NEW
...
void MDL_DEVINIT rtl8192cd_proc_init(struct net_device *dev)
{
    ...
    RTK_CREATE_PROC_READ_ENTRY(p, "acs_score", rtl8192cd_proc_acs_score);
    RTK_CREATE_PROC_READ_ENTRY(p, "ch_util", rtl8192cd_proc_ch_util);
}
...
void /*__devexit*/MDL_EXIT rtl8192cd_proc_remove (struct net_device *dev)
{
    ...
    if (rtl8192cd_proc_root != NULL) {
		...
		remove_proc_entry( "acs_score", rtl8192cd_proc_root );
...
static struct _proc_table_ proc_table[] =
{
    ...
    {"power",			rtl8192cd_proc_power},//add luo
    ...
    {"acs_score",			rtl8192cd_proc_acs_score,NULL},
    {"ch_util",			rtl8192cd_proc_ch_util,NULL},
#ifdef THERMAL_CONTROL
	{"therm_control",				rtl8192cd_proc_thermal_control}
#endif
#ifdef CONFIG_TENDA_WLAN_STA_STEER
    {"steer",	proc_get_steer_cmd,proc_set_steer_cmd},
#endif
};
