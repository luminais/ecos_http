...
#define RTL_BB_R32(addr)        (*(volatile unsigned long *)(addr))
#define RTL_BB_W32(addr, l)     ((*(volatile unsigned long*)(addr)) = (l))
#define BSP_PIN_MUX_SEL17       0xB8000844UL
#define BSP_PEFGH_DIR           0xb8003524UL        /* Port EFGH direction */
#define BSP_PEFGH_DAT           0xb8003528UL        /* Port EFGH data */
...
static void set_sw_LED0(struct rtl8192cd_priv *priv, int flag)
{
    ...
    if ((GET_CHIP_VER(priv) == VERSION_8188E)||(GET_CHIP_VER(priv) == VERSION_8192E)) {
        ...
        RTL_BB_W32(BSP_PIN_MUX_SEL17, (RTL_BB_R32(BSP_PIN_MUX_SEL17) | BIT28));//mux
        RTL_BB_W32(BSP_PEFGH_DIR, (RTL_BB_R32(BSP_PEFGH_DIR) | BIT1));//dir
        #endif
        if(flag) /* led off*/
        {
            #ifdef __CONFIG_SHARE_WLAN_WPS_GPIO__
            RTL_BB_W32(BSP_PEFGH_DAT, (RTL_BB_R32(BSP_PEFGH_DAT) | BIT1));//data
            #else
...
void enable_sw_LED(struct rtl8192cd_priv *priv, int init)
{
    ...
    #endif    
	{
        if (LED_TYPE == LEDTYPE_SW_LED2_GPIO10_LINKTXRX)
            ...
            diag_printf("LED2SV & LED1SV ctrl fem\n");
...
//BSPLJF++ for share led for wps and wifi
void enable_share_LED(struct rtl8192cd_priv *priv)
{
        set_sw_LED0(priv, LED_ON);
}
