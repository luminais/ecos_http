...
struct Dot11nConfigEntry {
    ...
    unsigned int    td_width_enable;        // add by jack, for 20/40 autowidth
    unsigned char	td_acs_enable;			// liuke add, for auto-channel enable
    unsigned int    td_acs_expire;		// liuke add, for auto-channel expire time
	unsigned int    td_auto_antiInterferance; //BSPLJF++ for auto antiInterferance
};
...
struct tenda_priv_mib {
    int debug;
};
/* end add by dengxingde */
