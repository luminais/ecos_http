...
//#endif
	...
	unsigned int        td_width_bad;
    unsigned int        td_width_good_lmt;
    unsigned int        td_width_good;
    unsigned int        td_width_all;
    unsigned int        td_width_loop_thd;
    unsigned int        td_width_dbg;
#endif
//BSPLJF++ 自动抗干扰覆盖优先机制
    unsigned int        td_good_FA_count;
    unsigned int        td_bad_FA_count;
    unsigned int        td_FA_UpperBound;
    unsigned int        td_FA_LowerBound;
    unsigned int        td_FA_UpperBound_20M;
    unsigned int        td_FA_LowerBound_20M;
    unsigned int        td_FA_count_all;
    unsigned int        td_interferance_dbg;
    unsigned int        td_FA_among_count; 
	unsigned char		pwr_by_rate;
 	//add bufferL
    unsigned int        td_dev_probe_enable;    

    unsigned char       td_acs_dbg;

//BSPLJF++ rx fifo overflow
    unsigned int        td_rxfifo_count;
    unsigned int        td_wifi_restart_count;
    unsigned int        td_rxpacket_pre;
    unsigned char       td_rxfifoO_enable;
...
#endif
	unsigned char 		td_dig_sel; //自动调节灵敏度的范围
...
/*cfg p2p cfg p2p*/	
    ...
    /* add by dengxingde,2017-6-10, for tenda priv mib */
    struct tenda_priv_mib tenda_mib;
...
typedef struct rtl8192cd_priv {
    ...
    unsigned int            auto_channel_change;    // 0: do not change current channel, 1:change current channel
	unsigned int			auto_channel_backup;
	unsigned long			up_time;
	unsigned int			auto_channel_step;
	unsigned int			auto_channel_expire_time;		
...
//	unsigned char			*pBeaconErp;			// ptr of ERP field in beacon buf
    ...
    unsigned int            chnl_ss_score[MAX_CHANNEL_NUM]; // record  sitesurvey chanel scores, Tenda add
    unsigned int            chnl_ss_score_level[MAX_CHANNEL_NUM];   // record  sitesurvey chanel scores level, Tenda add
	unsigned int            chnl_ss_ch_bss[MAX_CHANNEL_NUM]; // record  sitesurvey chanel bss number , Tenda add         
    unsigned int            chnl_ss_ch_busy[MAX_CHANNEL_NUM]; // record  sitesurvey chanel busy score, Tenda add
    unsigned int            chnl_ss_ch_busy_percent[MAX_CHANNEL_NUM]; // record  sitesurvey chanel busy utilization , Tenda add     
