...
)
{
	...
	/*buferLÐÞ¸ÄÒÔÂú×ãmcs0ºÍmcs5µÄÒÇÆ÷²âÊÔ*/
	pDM_Odm->TH_L2H_ini_mode2 = 10;
	...
	if (pDM_Odm->SupportICType & ODM_IC_11AC_GAIN_IDX_EDCCA) {		/*8814a no need to find pwdB lower bound, maybe*/
		...
		if (pDM_Odm->SupportICType & ODM_RTL8822B && priv->pshare->rf_ft_var.adaptivity_enable == 1)
			ODM_SetBBReg(pDM_Odm, 0x8f4, BIT21, 0x1);
	}
...
)
{
	...
	if (pDM_Odm->SupportICType & (ODM_RTL8821C | ODM_RTL8822B | ODM_RTL8814A))
		ODM_SetBBReg(pDM_Odm, 0x8a0, BIT1 | BIT0 , 0);
