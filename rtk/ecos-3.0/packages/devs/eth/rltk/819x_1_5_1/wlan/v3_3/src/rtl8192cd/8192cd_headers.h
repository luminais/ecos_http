...
EXTERN int _free_stainfo_(struct rtl8192cd_priv *priv, struct	stat_info *pstat);
#define free_stainfo(priv,pstat)    \
    do {TD_DBG(priv, TD_DBG_FREE_STA, "[%s] free_sta["MACSTR"] %s__%d\n", \
            priv->dev->name,MAC2STR(pstat->hwaddr),__func__,__LINE__);  \
        _free_stainfo_(priv,pstat);     \
    }while(0)
...
EXTERN void tenda_dev_probe_process(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo);//BSPHZX++
...
#ifdef CONFIG_TENDA_WLAN_DBG
EXTERN void _issue_deauth_(struct rtl8192cd_priv *priv,	unsigned char *da, int reason);
EXTERN void _issue_disassoc_(struct rtl8192cd_priv *priv,	unsigned char *da, int reason);

#define issue_deauth(priv,da,reason)  \
    do {TD_DBG(priv, TD_DBG_DEAUTH, "[TX][%s]["MACSTR"] deauth reason %d %s__%d\n", \
            priv->dev->name,MAC2STR(da),reason,__func__,__LINE__);    \
        _issue_deauth_(priv, da, reason);             \
    }while(0)

#define issue_disassoc(priv,da,reason)  \
    do{ TD_DBG(priv, TD_DBG_DISASSOC, "[TX][%s]["MACSTR"] disassoc reason %d %s__%d\n",    \
            priv->dev->name,MAC2STR(da),reason,__func__,__LINE__);  \
        _issue_disassoc_(priv, da, reason);         \
    }while(0)
...
EXTERN int rtl8192cd_autochannel_sel(struct rtl8192cd_priv *priv, unsigned int change);
