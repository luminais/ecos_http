...
/*cfg p2p cfg p2p*/
	...
	*/
	...
	// construct operation and basic rates set
	{
		...
		priv->auto_channel_expire_time = priv->pmib->dot11nConfigEntry.td_acs_expire;
		...
		if(1){
			int k;
			printk("available channels[%s]: ",priv->pmib->dot11dCountry.dot11CountryString);
			for(k = 0; k < priv->available_chnl_num; k++){
				printk("%d ", priv->available_chnl[k]);
			}
			printk("\n");
		}
...
int wifi5g_status_flag = 0;
int wifi2g_status_flag = 0;
extern int steer_timer_up;
...
SMP_LOCK(flags);
...
#endif
	{
        ...
        if ((LED_TYPE >= LEDTYPE_SW_LINK_TXRX) && (LED_TYPE < LEDTYPE_SW_MAX))
		{   
		    if (!wifi2g_status_flag && wifi5g_status_flag)
            {      
		        enable_share_LED(priv);
                panic_printk("2.4G close!!! But enable_wifi_LED\n");
            }
            else if (wifi2g_status_flag && !wifi5g_status_flag)
            { 
                enable_share_LED(priv);
                panic_printk("5G close!!! But enable_wifi_LED\n");
            }
		...
		if (priv->pshare->aidarray[i]) {
...
#endif
			{
				if (priv->pshare->aidarray[i]->used == TRUE) {
                    ...
                    if (_free_stainfo_(priv, &(priv->pshare->aidarray[i]->station)) == FAIL) {
                        struct stat_info *pstat_tmp = &(priv->pshare->aidarray[i]->station);
                        TD_DBG(priv,TD_DBG_FREE_STA,"[%s] free_sta["MACSTR"]fails %s__%d\n",
                            priv->dev->name,MAC2STR(pstat_tmp->hwaddr),__func__,__LINE__);
                    }
#else	
...
#ifdef __CONFIG_SHARE_WLAN_WPS_GPIO__
    if (GET_CHIP_VER(priv) == VERSION_8822B)
        wifi5g_status_flag = 1;        
    if (GET_CHIP_VER(priv)==VERSION_8197F)
        wifi2g_status_flag = 1;
#endif
...
//	if (IS_VXD_INTERFACE(priv) && !GET_ROOT_PRIV(priv)->pmib->miscEntry.func_off) 
    ...
    if (OPMODE & WIFI_AP_STATE) {
        ...
        bss.dev = dev;
        bss.priv = priv;
        bss.ch = priv->pmib->dot11RFEntry.dot11channel;
        strncpy(bss.ssid, priv->pmib->dot11StationConfigEntry.dot11DesiredSSID,32);
        bss.key_type = priv->pmib->dot1180211AuthEntry.dot11EnablePSK;
        if (priv->pmib->dot1180211AuthEntry.dot11EnablePSK) {
            strncpy(bss.keys,priv->pmib->dot1180211AuthEntry.dot11PassPhrase,65);
        }
        steer_update_bss_info(&bss, 0);
    }
#endif
...
extern void td_auto_width_clear_status(struct rtl8192cd_priv *priv);
...
#if __CONFIG_SHARE_WLAN_WPS_GPIO__
    if (GET_CHIP_VER(priv) == VERSION_8822B)
        wifi5g_status_flag = 0;
    if (GET_CHIP_VER(priv)==VERSION_8197F)
        wifi2g_status_flag = 0;
#endif
...
#endif // SDIO_AP_OFFLOAD
	//add by linlinglong, 无线wan重启时,不强行
	if (strcmp(dev->name, "wlan0-va0"))
		td_auto_width_clear_status(priv);
...
#ifdef CONFIG_TENDA_WLAN_DBG
    td_wlan_dbg_init(&priv->tenda_mib);
#endif
	...
	for (i=0; i<sizeof(wlan_device)/sizeof(struct _device_info_); i++) {
		if (wlan_device[i].priv && (wlan_device[i].priv != priv)) {
			if (wlan_device[i].priv->pmib->dot11OperationEntry.opmode & WIFI_AP_STATE) {
                ...
                TD_DBG(wlan_device[i].priv, TD_DBG_FREE_STA, "[%s] del_sta["MACSTR"] %s__%d\n",
                    wlan_device[i].priv->dev->name,MAC2STR(pstat->hwaddr),__func__,__LINE__);
		...
		for (i=0; i<RTL8192CD_NUM_VWLAN; i++) {
			if (GET_ROOT(priv)->pvap_priv[i] && IS_DRV_OPEN(GET_ROOT(priv)->pvap_priv[i]) && (GET_ROOT(priv)->pvap_priv[i] != priv) &&
				...
				if (GET_ROOT(priv)->pvap_priv[i]->pmib->dot11OperationEntry.opmode & WIFI_AP_STATE) {
					sprintf((char *)tmpbuf, "%02x%02x%02x%02x%02x%02x",
						pstat->hwaddr[0],pstat->hwaddr[1],pstat->hwaddr[2],pstat->hwaddr[3],pstat->hwaddr[4],pstat->hwaddr[5]);
                    TD_DBG(GET_ROOT(priv)->pvap_priv[i], TD_DBG_FREE_STA, "[%s] del_sta["MACSTR"] %s__%d\n",
                        priv->dev->name,MAC2STR(pstat->hwaddr),__func__,__LINE__);
