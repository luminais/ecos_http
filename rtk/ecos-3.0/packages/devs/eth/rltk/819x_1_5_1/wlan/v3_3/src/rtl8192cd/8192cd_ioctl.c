...
#include "./WlanHAL/Data/8197F/ce_offset.h"
#endif
...
#define RTL8192CD_IOCTL_FFLUSH_ONLINE_TABLE		0x8BAE
#define ACL_allow 1
#define ACL_deny 2
#endif//tsananiu//d
...
#define SIOCDEVPROBERES	   0x8BFD // BSPHZX++ get device probe reasult, tenda add
#define SIOCDEVPROBENUM	   0x8BFE // BSPHZX++get device probe reasult, tenda add
...
#define SIOCGICHANNELSCORE		0x8BF9// get channel score, tenda add
...
#define _OFFSET_TENDA(field)	((int)(long *)&(((struct tenda_priv_mib *)0)->field))
#define _SIZE_TENDA(field)	sizeof(((struct tenda_priv_mib *)0)->field)
...
struct iw_priv_args privtab[] = {
    ...
    { RTL8192CD_IOCTL_FFLUSH_ONLINE_TABLE, IW_PRIV_TYPE_CHAR | 40, IW_PRIV_TYPE_BYTE | 128, "fflush_acl_list" },
#endif//tsananiu//
#endif
...
#endif
    { SIOCGICHANNELSCORE, IW_PRIV_TYPE_INT | 0, IW_PRIV_TYPE_INT | 0, "get_ch_score" },
};
...
typedef struct _tenda_dev_probe_info {
    unsigned char               mac[MACADDRLEN];
    int                         rssi;
    unsigned char               isAP;	
	int 					    network;
	char 						ssid[WLAN_SSID_MAXLEN+1];		
} web_tenda_dev_probe_info; 
#if defined(CONFIG_PROLINK_PLDT) || defined(CONFIG_WLAN_UPLINKAP_SSIDSCAN)
...
static struct iwpriv_arg mib_table[] = {
	...
	{"oprates",		INT_T,		_OFFSET(dot11StationConfigEntry.dot11SupportedRates), _SIZE(dot11StationConfigEntry.dot11SupportedRates), 0xff0},
	{"basicrates",	INT_T,		_OFFSET(dot11StationConfigEntry.dot11BasicRates), _SIZE(dot11StationConfigEntry.dot11BasicRates), 0xff0},
...
//PRINT_SINGL_ARG("    dgaf_disable:  ",priv->dgaf_disable, "%d");
	...
	{"td_rxfifoO_enable",			RFFT_T,	_OFFSET_RFFT(td_rxfifoO_enable), _SIZE_RFFT(td_rxfifoO_enable), 1},
	...
	/*腾达接收灵敏度的大小，BSPLJF++*/	
  	{"td_dig_sel",		RFFT_T,	_OFFSET_RFFT(td_dig_sel), _SIZE_RFFT(td_dig_sel), 0},
	...
	{"bws_thd", 	RFFT_T, _OFFSET_RFFT(bws_Thd), _SIZE_RFFT(bws_Thd), 800},	
    ...
    {"td_width_dbg", 	RFFT_T, _OFFSET_RFFT(td_width_dbg), _SIZE_RFFT(td_width_dbg), 0},
    {"td_width_enable",    INT_T,		_OFFSET(dot11nConfigEntry.td_width_enable), _SIZE(dot11nConfigEntry.td_width_enable), 0},
#endif
    {"td_FA_UpperBound", 	RFFT_T, _OFFSET_RFFT(td_FA_UpperBound), _SIZE_RFFT(td_FA_UpperBound), 768},
    {"td_FA_LowerBound", 	RFFT_T, _OFFSET_RFFT(td_FA_LowerBound), _SIZE_RFFT(td_FA_LowerBound), 256},
    {"td_FA_UpperBound_20M", 	RFFT_T, _OFFSET_RFFT(td_FA_UpperBound_20M), _SIZE_RFFT(td_FA_UpperBound_20M), 600},
    {"td_FA_LowerBound_20M", 	RFFT_T, _OFFSET_RFFT(td_FA_LowerBound_20M), _SIZE_RFFT(td_FA_LowerBound_20M), 256},
    {"td_auto_antiInterferance",    INT_T,		_OFFSET(dot11nConfigEntry.td_auto_antiInterferance), _SIZE(dot11nConfigEntry.td_auto_antiInterferance), 0},
    {"td_interferance_dbg", 	RFFT_T, _OFFSET_RFFT(td_interferance_dbg), _SIZE_RFFT(td_interferance_dbg), 0},
	{"td_dev_probe_enable", RFFT_T, _OFFSET_RFFT(td_dev_probe_enable), _SIZE_RFFT(td_dev_probe_enable), 1},
...
/* add by dengxingde,2016-02-02, for tenda private mib config ioctl */
	{"td_debug",            TD_PRIV_INT_T, _OFFSET_TENDA(debug),  _SIZE_TENDA(debug),0},  
	
    {"td_acs_dbg",  RFFT_T, _OFFSET_RFFT(td_acs_dbg), _SIZE_RFFT(td_acs_dbg), 0},
    {"td_acs_enable",    BYTE_T,        _OFFSET(dot11nConfigEntry.td_acs_enable), _SIZE(dot11nConfigEntry.td_acs_enable), 0},
    {"td_acs_expire",    INT_T,     _OFFSET(dot11nConfigEntry.td_acs_expire), _SIZE(dot11nConfigEntry.td_acs_expire), ACS_DEFAULT_EXPIRE_TIME},
...
int set_mib(struct rtl8192cd_priv *priv, unsigned char *data)
{
	...
	case INT_T:
        ...
        /* update acs recheck time immediately */
        if(strcmp(entry->name, "td_acs_expire") == 0){
            priv->auto_channel_expire_time = int_val;
        }
...
/* WPS2DOTX   */	
    ...
    case TD_PRIV_INT_T:
        ...
        memcpy(((unsigned char *)&priv->tenda_mib)+entry->offset, (unsigned char *)&int_val, sizeof(int));
        break;
	default:
...
#endif
    case TD_PRIV_INT_T:
        memcpy(data, ((unsigned char *)&priv->tenda_mib)+entry->offset, sizeof(int));
        PRINT_INFO("tenda mib data: %d\n", *((int *)data));
#ifdef CONFIG_TENDA_WLAN_DBG        
        if (entry->offset == _OFFSET_TENDA(debug)) {
            td_wlan_dbg_help();
        }
#endif
        break;
...
/*删除所有在线客户端*/
static int del_all_sta (struct rtl8192cd_priv *priv)
{
	struct stat_info *pstat;
	struct list_head *phead, *plist;
#ifndef SMP_SYNC
...
static void get_bss_info(struct rtl8192cd_priv *priv, bss_info_2_web *pBss)
{
    ...
    pBss->wide = priv->pshare->is_40m_bw;//fh add auto bandwidth
...
#endif // WDS
static int get_dev_probe_info(struct rtl8192cd_priv *priv, web_tenda_dev_probe_info *pSs)//BSPHZX++
{
	int i = 0,count_num = 0;
	int count = 0;
    web_tenda_dev_probe_ioctl p[MAX_DEV_PROBE_NUM];

	memset((char *)pSs, '\0', count * sizeof(web_tenda_dev_probe_info));
    count = tenda_dev_probe_ioctl(priv->pmib->dot11RFEntry.dot11channel, p);
    for(i = 0; i < count; i ++){
        memcpy(&pSs[i], &p[i], sizeof(web_tenda_dev_probe_ioctl));        
      //  diag_printf("\n ++ mac="MACSTR",rssi=%d, isAP=%d,network=%d, i=%d,ssid=%s\n",p[i].mac[0],p[i].mac[1],p[i].mac[2],p[i].mac[3],p[i].mac[4],p[i].mac[5],p[i].rssi,p[i].isAP,p[i].network,i,p[i].ssid);
    }
...
*****************************************************************************/
static int update_online_client(struct rtl8192cd_priv *priv)
{
	int i = 0;
	int j = 0;
	struct stat_info *pstat;
	int send_disasoc=1;
	char *cur_sta_info = (unsigned char *)sta_info;
	sta_info_2_web *tmp_sta_info = NULL;

	tmp_sta_info = (sta_info_2_web *)(cur_sta_info + sizeof(sta_info_2_web));
	get_sta_info(priv, (sta_info_2_web *)(cur_sta_info + sizeof(sta_info_2_web)), NUM_STAT);
	
	for(i = 0;i<NUM_STAT;i++)
	{
		if(tmp_sta_info[i].aid &&( tmp_sta_info[i].flags &STA_INFO_FLAG_ASOC ))
		{
			for(j = 0;j<priv->pmib->dot11StationConfigEntry.dot11AclNum;j++)
			{
				if(memcmp(priv->pmib->dot11StationConfigEntry.dot11AclAddr[j],tmp_sta_info[i].addr,MACADDRLEN) == 0)
				{
					break;	
				}
			}
			if((priv->pmib->dot11StationConfigEntry.dot11AclMode == ACL_deny
				&& j < priv->pmib->dot11StationConfigEntry.dot11AclNum)
				   ||  (priv->pmib->dot11StationConfigEntry.dot11AclMode == ACL_allow
					 && j >= priv->pmib->dot11StationConfigEntry.dot11AclNum))
			{
				pstat = get_stainfo(priv, tmp_sta_info[i].addr);
				del_station(priv, pstat, send_disasoc);
			}			
		}
	}
	return 0;
...
static void rtl8192cd_SSReq_AutoTest(struct rtl8192cd_priv *priv)
{
	...
	if (!ret)	// now, let's start site survey
	{
		...
		panic_printk("%s(%d) ss_req_ongoing = 1\n", __func__, __LINE__);
		start_clnt_ss(priv);
	}else{
		return ;
	}
...
int rtl8192cd_autochannel_sel(struct rtl8192cd_priv *priv, unsigned int change)
{
	INT8 ret = 0;
	...
	if (!ret)	// now, let's start site survey
	{
		...
		DEBUG_INFO("start_clnt_ss, trigger by %s, ss_ssidlen=0\n", (char *)__FUNCTION__);
		priv->auto_channel = 1;
        priv->auto_channel_change = change;
		start_clnt_ss(priv);
	}
	return ret;
}
...
static unsigned int rf_pwr_a[7]={0};
static unsigned int rf_pwr_b[8];
/* pwr_status 1 for high power */
static int pwr_status = 1;
static int restore_power = 0xf;
static int last_channel;

#define MIN_PWR_IDX 1
#define DEBUG_RF_ADJ
...
int dynamic_RF_pwr_adj(struct rtl8192cd_priv *priv, unsigned char *data)
{
	...
	int num;
    int path_b_reg[] = {rTxAGC_B_Rate18_06, rTxAGC_B_Rate54_24,
                        rTxAGC_B_CCK5_1_Mcs32, rTxAGC_B_Mcs03_Mcs00,
                        rTxAGC_B_Mcs07_Mcs04, rTxAGC_B_Mcs11_Mcs08, 
                        rTxAGC_B_Mcs15_Mcs12, rTxAGC_A_CCK11_2_B_CCK11};
        
    int path_a_reg[] = {rTxAGC_A_Rate18_06, rTxAGC_A_Rate54_24,
                        rTxAGC_A_CCK1_Mcs32, rTxAGC_A_Mcs03_Mcs00,
	...
	if (*ptr == '-') {
	    ...
	    panic_printk("0x%x: 0x%08x\n", path_a_reg[2], rf_pwr_a[2]);
	    panic_printk("0x%x: 0x%08x\n", path_b_reg[2], rf_pwr_b[2]);
	    panic_printk("0x%x: 0x%08x\n", path_b_reg[7], rf_pwr_b[7]);
	    panic_printk("0x%x: 0x%08x\n", path_a_reg[0], rf_pwr_a[0]);
	    panic_printk("0x%x: 0x%08x\n", path_a_reg[1], rf_pwr_a[1]);
	    panic_printk("0x%x: 0x%08x\n", path_a_reg[3], rf_pwr_a[3]);
	    panic_printk("0x%x: 0x%08x\n", path_a_reg[4], rf_pwr_a[4]);
	    panic_printk("0x%x: 0x%08x\n", path_a_reg[5], rf_pwr_a[5]);
	    panic_printk("0x%x: 0x%08x\n", path_a_reg[6], rf_pwr_a[6]);
	    panic_printk("0x%x: 0x%08x\n", path_b_reg[0], rf_pwr_b[0]);
	    panic_printk("0x%x: 0x%08x\n", path_b_reg[1], rf_pwr_b[1]);
	    panic_printk("0x%x: 0x%08x\n", path_b_reg[3], rf_pwr_b[3]);
	    panic_printk("0x%x: 0x%08x\n", path_b_reg[4], rf_pwr_b[4]);
	    panic_printk("0x%x: 0x%08x\n", path_b_reg[5], rf_pwr_b[5]);
	    panic_printk("0x%x: 0x%08x\n", path_b_reg[6], rf_pwr_b[6]);
	    ...
	    int offset_num = 0;
        panic_printk("path_offset:\n");
        for (offset_num = 0; offset_num < 8; offset_num++)
            panic_printk("0x%08x\n", path_offset[offset_num]);
        panic_printk("\n");
	    
		sprintf((char *)data, "[FAIL] No sign to know to add or subtract\n");
		return strlen((char *)data)+1;
	}
	num = sscanf(ptr, "%d,%d", &adj_value, &pwr_lvl);
    if(2 != num)
    {
        sprintf((char *)data, "[FAIL] not enough data\n");
        return strlen((char *)data)+1;
    }
    ...
    DEBUG_RF_ADJ("adj_value %d minus_sign %d pwr_status %d for rfe_type %d \n", 
            adj_value, minus_sign, pwr_status, priv->pmib->dot11RFEntry.rfe_type);

    if ((adj_value == 0) && (path_offset != NULL))
        {
            int i;
            unsigned char offset_byte0;
            unsigned char offset_byte1;
            unsigned char offset_byte2;
            unsigned char offset_byte3;
            unsigned int  offset;
            unsigned char high_pwr_cck_idx;
            char pwr_status_low = 0;
			int min_pwr_offset;
            
            if(0 == priv->drv_state)
            {
                DEBUG_RF_ADJ("CC++ pwr_adj skip for drv down\n");
                return strlen((char *)data)+1;
            }
			min_pwr_offset = ((path_offset[2] & 0x0000ff00) >> 8) - 12;
            byte0=PHY_QueryBBReg(priv, rTxAGC_A_CCK1_Mcs32, bTxAGCRateCCK);
            high_pwr_cck_idx = (rf_pwr_a[2] & 0x0000ff00) >> 8;
            DEBUG_RF_ADJ("current cck index %d high_pwr_cck_idx %d bw %d\n", byte0, high_pwr_cck_idx, priv->pshare->CurrentChannelBW);
            
            if (high_pwr_cck_idx != 0)
            {
                /* 判断当前功率是否为低功率，预留12个index的余量,低功率情况下高低功率差值是12个index，
                要确保高功率调整为低功率时也会更新当前最大功率*/
                if ((byte0 < high_pwr_cck_idx) && (byte0 < (high_pwr_cck_idx - 12 + (restore_power*2))))
                {
                    pwr_status_low = 1;

					if (last_channel != priv->pmib->dot11RFEntry.dot11channel)
					{
                        pwr_status_low = 0;
                        DEBUG_RF_ADJ("=============channel changed 0x%x\n", rf_pwr_a[2]);
					}
					if (byte0 + min_pwr_offset > high_pwr_cck_idx)
					{
                        pwr_status_low = 0;
                        DEBUG_RF_ADJ("============>pwr_ctrl bug detect 0x%x\n", rf_pwr_a[2]);
					}
                }
                
            }
            
            /* set low power, check current power idx */
            if((1 == minus_sign) && (0 == pwr_status_low) )
            {
                restore_power = pwr_lvl;
				last_channel = priv->pmib->dot11RFEntry.dot11channel;
                for (i = 0;i < 7; i++)
                {
                    //rf_pwr_a[]里面只保存最大功率
                    readVal = PHY_QueryBBReg(priv, path_a_reg[i], 0x7f7f7f7f);
                    ...
                    if (i == 2)
                    {
                        DEBUG_RF_ADJ("=============save current power 0x%x\n", rf_pwr_a[2]);
                    }
                    byte0 = (readVal & 0xff000000) >> 24;
                    byte1 = (readVal & 0x00ff0000) >> 16;
                    byte2 = (readVal & 0x0000ff00) >> 8;
                    ...
                    offset_byte0 = (path_offset[i] & 0xff000000) >> 24;
                    offset_byte1 = (path_offset[i] & 0x00ff0000) >> 16;
                    offset_byte2 = (path_offset[i] & 0x0000ff00) >> 8;
                    offset_byte3 = (path_offset[i] & 0x000000ff);
                    offset_byte0 -= (pwr_lvl*2);
                    offset_byte1 -= (pwr_lvl*2);
                    offset_byte2 -= (pwr_lvl*2);
                    offset_byte3 -= (pwr_lvl*2);

                    /* 增加边界检查，在偏移大于index的情况下将寄存器设置为12 */  
                    if (byte0 > offset_byte0)
                    {
                        byte0 -= offset_byte0;
                    }
                    else
                    ...
                    writeVal = (byte0<<24) | (byte1<<16) |(byte2<<8) | byte3;
                    DEBUG_RF_ADJ("write reg 0x%x val 0x%x\n", path_a_reg[i], 
                    writeVal);
                    if (path_a_reg[i] == rTxAGC_A_CCK1_Mcs32)
                    {
                        PHY_SetBBReg(priv, path_a_reg[i], bTxAGCRateCCK, byte2);
                    }
                    else
                    {
                        PHY_SetBBReg(priv, path_a_reg[i], 0x7f7f7f7f, writeVal);
                    }
                }
                for (i = 0;i < 8; i++)
                {
                    //rf_pwr_b[]里面只保存最大功率
                    readVal = PHY_QueryBBReg(priv, path_b_reg[i], 0x7f7f7f7f);
                    byte0 = (readVal & 0xff000000) >> 24;
                    byte1 = (readVal & 0x00ff0000) >> 16;
                    byte2 = (readVal & 0x0000ff00) >> 8;
                    byte3 = (readVal & 0x000000ff);
                    ...
                    writeVal = (byte0<<24) | (byte1<<16) |(byte2<<8) | byte3;
                    PHY_SetBBReg(priv, path_b_reg[i], 0x7f7f7f7f, writeVal);
                }
                /* low power status */
                pwr_status = 0;
            }
            else if ((minus_sign == 0) && (pwr_status == 0))
            {
             /* restore */
             for (i = 0;i < 7; i++)
             {
	...
	PHY_SetBBReg(priv, rTxAGC_A_CCK1_Mcs32, bTxAGCRateCCK, byte0);
        for (index=0; index< 8; index++)
        {
            ...
            readVal = PHY_QueryBBReg(priv, path_b_reg[index], 0x7f7f7f7f);
            byte0 = (readVal & 0xff000000) >> 24;
            byte1 = (readVal & 0x00ff0000) >> 16;
            byte2 = (readVal & 0x0000ff00) >> 8;
            byte3 = (readVal & 0x000000ff);
            ...
            writeVal = (byte0<<24) | (byte1<<16) |(byte2<<8) | byte3;
            PHY_SetBBReg(priv, path_b_reg[index], 0x7f7f7f7f, writeVal);
        }
        }
...
//static unsigned char       High_TxAgcCCK[4][4]  = {0};
static unsigned char       High_TxAgcOFDM[4][8] = {0};
static unsigned char       High_TxAgcMCS[4][32]; 
static unsigned char       High_TxAgcVHT[4][40];
static unsigned char       High_power_channel;
static unsigned char       restore_power_ac=0xf;
static unsigned char       adj_pwr_idx;

#define ADJ_PATH_A 0
#define ADJ_PATH_B 1

#define MIN_PWR_IDX_AC 1

int dynamic_RF_pwr_adj_11ac(struct rtl8192cd_priv *priv, unsigned char *data)
{
    unsigned char *ptr = data;
    int index, minus_sign, adj_value,pwr_lvl;
    int rate;
    int pwr_status_low = 0;
    char adj_pwr;
    int num;
    ...
    if (*ptr == '-') {
        ...
        for (rate=TX_AGC_OFDM_6M;rate<=TX_AGC_OFDM_54M;rate++)
        {
        	panic_printk("%x,",High_TxAgcOFDM[ADJ_PATH_A][rate-TX_AGC_OFDM_6M]);
        }
        panic_printk("\n");
        panic_printk("HT MCS 1SS:     ");
        for (rate=TX_AGC_HT_NSS1_MCS0;rate<=TX_AGC_HT_NSS1_MCS7;rate++)
        {
        	panic_printk("%x,",High_TxAgcMCS[ADJ_PATH_A][rate-TX_AGC_HT_NSS1_MCS0]);
        }
        panic_printk("\n");
        panic_printk("HT MCS 2SS:     ");
        for (rate=TX_AGC_HT_NSS2_MCS8;rate<=TX_AGC_HT_NSS2_MCS15;rate++)
        {
        	panic_printk("%x,",High_TxAgcMCS[ADJ_PATH_A][rate-TX_AGC_HT_NSS1_MCS0]);
        }
        panic_printk("\n");
        panic_printk("VHT MCS 1SS:    ");
        for (rate=TX_AGC_VHT_NSS1_MCS0;rate<=TX_AGC_VHT_NSS1_MCS9;rate++)
        {
        	panic_printk("%x,", High_TxAgcVHT[ADJ_PATH_A][rate-TX_AGC_VHT_NSS1_MCS0]);
        }
        panic_printk("\n");
        panic_printk("VHT MCS 2SS:    ");
        for (rate=TX_AGC_VHT_NSS2_MCS0;rate<=TX_AGC_VHT_NSS2_MCS9;rate++)
        {
        	panic_printk("%x,", High_TxAgcVHT[ADJ_PATH_A][rate-TX_AGC_VHT_NSS1_MCS0]);
        }
        ...
        for (rate=TX_AGC_OFDM_6M;rate<=TX_AGC_OFDM_54M;rate++)
        {
        	panic_printk("%x,",High_TxAgcOFDM[ADJ_PATH_B][rate-TX_AGC_OFDM_6M]);
        }
        panic_printk("\n");
        panic_printk("HT MCS 1SS:     ");
        for (rate=TX_AGC_HT_NSS1_MCS0;rate<=TX_AGC_HT_NSS1_MCS7;rate++)
        {
        	panic_printk("%x,",High_TxAgcMCS[ADJ_PATH_B][rate-TX_AGC_HT_NSS1_MCS0]);
        }
        panic_printk("\n");
        panic_printk("HT MCS 2SS:     ");
        for (rate=TX_AGC_HT_NSS2_MCS8;rate<=TX_AGC_HT_NSS2_MCS15;rate++)
        ...
        for (rate=TX_AGC_VHT_NSS1_MCS0;rate<=TX_AGC_VHT_NSS1_MCS9;rate++)
        {
        	panic_printk("%x,", High_TxAgcVHT[ADJ_PATH_B][rate-TX_AGC_VHT_NSS1_MCS0]);
        }
        panic_printk("\n");
        panic_printk("VHT MCS 2SS:    ");
        for (rate=TX_AGC_VHT_NSS2_MCS0;rate<=TX_AGC_VHT_NSS2_MCS9;rate++)
        ...
        for (rate=TX_AGC_OFDM_6M;rate<=TX_AGC_OFDM_54M;rate++)
        {
        	panic_printk("%d,",offset_OFDM[rate-TX_AGC_OFDM_6M]);
        }
        panic_printk("\n");
        panic_printk("HT MCS :     ");
        for (rate=TX_AGC_HT_NSS1_MCS0;rate<=TX_AGC_HT_NSS2_MCS15;rate++)
        {
        	panic_printk("%d,",offset_MCS[rate-TX_AGC_HT_NSS1_MCS0]);
        }
        panic_printk("\n");
        panic_printk("VHT MCS :    ");
        for (rate=TX_AGC_VHT_NSS1_MCS0;rate<=TX_AGC_VHT_NSS2_MCS9;rate++)
        {
        	panic_printk("%d,", offset_VHT[rate-TX_AGC_VHT_NSS1_MCS0]);
        }
        panic_printk("\n");
        sprintf((char *)data, "[FAIL] No sign to know to add or subtract\n");
        return strlen((char *)data)+1;
    ...
    if(priv->pmib->dot11RFEntry.rfe_type == 6)
    {
        if (priv->pmib->dot11RFEntry.dot11channel >= 157)
        {    
            adj_pwr_idx = 5;
        }
        else
        ...
        DEBUG_RF_ADJ("pwr offset for rfe_type %d\n", 
    ...
    if((adj_value !=0) || (offset_OFDM == NULL))
    {
        sprintf((char *)data, "invalid adj_value %d for acphy\n", adj_value);
        return strlen((char *)data)+1;
    }
    if (0 == priv->drv_state)
    {
    ...
    DEBUG_RF_ADJ("High_TxAgcOFDM[A][TX_AGC_OFDM_6M] 0x%x \n", 
                    High_TxAgcOFDM[ADJ_PATH_A][TX_AGC_OFDM_6M - TX_AGC_OFDM_6M]);
    
    if (High_TxAgcOFDM[ADJ_PATH_A][TX_AGC_OFDM_6M - TX_AGC_OFDM_6M] !=0)
    {
        DEBUG_RF_ADJ("check low power current %d High_power %d \n",
            priv->pshare->phw->CurrentTxAgcOFDM[ADJ_PATH_A][TX_AGC_OFDM_6M - TX_AGC_OFDM_6M],
            High_TxAgcOFDM[ADJ_PATH_A][TX_AGC_OFDM_6M - TX_AGC_OFDM_6M]);
        DEBUG_RF_ADJ("restore_power_ac = %d\n", restore_power_ac);
        if (priv->pshare->phw->CurrentTxAgcOFDM[ADJ_PATH_A][TX_AGC_OFDM_6M - TX_AGC_OFDM_6M] < 
            (High_TxAgcOFDM[ADJ_PATH_A][TX_AGC_OFDM_6M - TX_AGC_OFDM_6M] - 
            adj_pwr_idx+(restore_power_ac*2)))//?
        {
            pwr_status_low = 1;
            if(priv->pmib->dot11RFEntry.dot11channel != High_power_channel)
            {
                DEBUG_RF_ADJ("CE power_ctrl channel changed\n");
                pwr_status_low = 0;
            }
			if ((priv->pshare->phw->CurrentTxAgcOFDM[ADJ_PATH_A][TX_AGC_OFDM_6M - TX_AGC_OFDM_6M] 
				+ offset_OFDM[TX_AGC_OFDM_6M - TX_AGC_OFDM_6M] - 12) 
				> High_TxAgcOFDM[ADJ_PATH_A][TX_AGC_OFDM_6M - TX_AGC_OFDM_6M])
			{
                DEBUG_RF_ADJ("CE power_ctrl bug detect \n");
                pwr_status_low = 0;
			}
        }
    }
    if ( (1 == minus_sign) && (0 == pwr_status_low))
    {
        High_power_channel = priv->pmib->dot11RFEntry.dot11channel;
        restore_power_ac = pwr_lvl;
        DEBUG_RF_ADJ(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>set low power for current OFDM6 = %d channel %d bw %d\n", 
        priv->pshare->phw->CurrentTxAgcOFDM[ADJ_PATH_A][TX_AGC_OFDM_6M-TX_AGC_OFDM_6M],
        High_power_channel, priv->pshare->CurrentChannelBW);
        for (rate=TX_AGC_OFDM_6M;rate<=TX_AGC_OFDM_54M;rate++)
        {
            High_TxAgcOFDM[ADJ_PATH_A][rate-TX_AGC_OFDM_6M] = 
            priv->pshare->phw->CurrentTxAgcOFDM[ADJ_PATH_A][rate-TX_AGC_OFDM_6M] + pwr_lvl*2;
            High_TxAgcOFDM[ADJ_PATH_B][rate-TX_AGC_OFDM_6M] = 
                priv->pshare->phw->CurrentTxAgcOFDM[ADJ_PATH_B][rate-TX_AGC_OFDM_6M] + pwr_lvl*2;

            adj_pwr = priv->pshare->phw->CurrentTxAgcOFDM[ADJ_PATH_A][rate-TX_AGC_OFDM_6M]- 
                offset_OFDM[rate - TX_AGC_OFDM_6M];
            adj_pwr += (pwr_lvl*2);
            
            if (adj_pwr > 0)
            {
                PHYSetTxPower88XX(priv,rate,rate,ADJ_PATH_A,adj_pwr);
            }
            else
            {
                adj_pwr = MIN_PWR_IDX_AC;
                PHYSetTxPower88XX(priv,rate,rate,ADJ_PATH_A,adj_pwr);
                DEBUG_RF_ADJ("--------------------------------------------------------------------->outof    range\n");
            }
            adj_pwr = priv->pshare->phw->CurrentTxAgcOFDM[ADJ_PATH_B][rate-TX_AGC_OFDM_6M]-
                offset_OFDM[rate - TX_AGC_OFDM_6M];
            adj_pwr += (pwr_lvl*2);
            if (adj_pwr > 0)
            {
                 PHYSetTxPower88XX(priv,rate,rate,ADJ_PATH_B,adj_pwr);
            }
            else
            {
                adj_pwr = MIN_PWR_IDX_AC;
        ...
        for (rate=TX_AGC_HT_NSS1_MCS0;rate<=TX_AGC_HT_NSS2_MCS15;rate++)
        {
            High_TxAgcMCS[ADJ_PATH_A][rate-TX_AGC_HT_NSS1_MCS0] = 
            priv->pshare->phw->CurrentTxAgcMCS[ADJ_PATH_A][rate-TX_AGC_HT_NSS1_MCS0] + pwr_lvl*2;
            High_TxAgcMCS[ADJ_PATH_B][rate-TX_AGC_HT_NSS1_MCS0] = 
            priv->pshare->phw->CurrentTxAgcMCS[ADJ_PATH_B][rate-TX_AGC_HT_NSS1_MCS0] + pwr_lvl*2;
            
            adj_pwr = priv->pshare->phw->CurrentTxAgcMCS[ADJ_PATH_A][rate-TX_AGC_HT_NSS1_MCS0]-
                offset_MCS[rate-TX_AGC_HT_NSS1_MCS0];
            adj_pwr += (pwr_lvl*2);	
            ...
            adj_pwr = priv->pshare->phw->CurrentTxAgcMCS[ADJ_PATH_B][rate-TX_AGC_HT_NSS1_MCS0]-
                offset_MCS[rate-TX_AGC_HT_NSS1_MCS0];
            adj_pwr += (pwr_lvl*2);
            if (adj_pwr > 0)
            {
        ...
        for (rate=TX_AGC_VHT_NSS1_MCS0;rate<=TX_AGC_VHT_NSS2_MCS9;rate++)
        {
            High_TxAgcVHT[ADJ_PATH_A][rate-TX_AGC_VHT_NSS1_MCS0] = 
            priv->pshare->phw->CurrentTxAgcVHT[ADJ_PATH_A][rate-TX_AGC_VHT_NSS1_MCS0]+pwr_lvl*2;
            High_TxAgcVHT[ADJ_PATH_B][rate-TX_AGC_VHT_NSS1_MCS0] = 
            priv->pshare->phw->CurrentTxAgcVHT[ADJ_PATH_B][rate-TX_AGC_VHT_NSS1_MCS0]+pwr_lvl*2;
            
            adj_pwr = priv->pshare->phw->CurrentTxAgcVHT[ADJ_PATH_A][rate-TX_AGC_VHT_NSS1_MCS0]-
                offset_VHT[rate-TX_AGC_VHT_NSS1_MCS0];
            adj_pwr += (pwr_lvl*2);
            if (adj_pwr > 0)
            {
                PHYSetTxPower88XX(priv,rate,rate,ADJ_PATH_A,adj_pwr);
            ...
            adj_pwr = priv->pshare->phw->CurrentTxAgcVHT[ADJ_PATH_B][rate-TX_AGC_VHT_NSS1_MCS0]-
                offset_VHT[rate-TX_AGC_VHT_NSS1_MCS0];
            adj_pwr += (pwr_lvl*2);
            if (adj_pwr > 0)
            {
    ...
    else if((0 == minus_sign) && (High_TxAgcOFDM[ADJ_PATH_A][TX_AGC_OFDM_6M - TX_AGC_OFDM_6M] != 0))
    {
        //restore;
        DEBUG_RF_ADJ("restore\n");
        DEBUG_RF_ADJ("ofdm :");
        for (rate=TX_AGC_OFDM_6M;rate<=TX_AGC_OFDM_54M;rate++)
        {
            DEBUG_RF_ADJ("rate %d idx %d\n", rate, High_TxAgcOFDM[ADJ_PATH_A][rate-TX_AGC_OFDM_6M]);
            adj_pwr = High_TxAgcOFDM[ADJ_PATH_A][rate-TX_AGC_OFDM_6M] - pwr_lvl*2;
            PHYSetTxPower88XX(priv,rate,rate,ADJ_PATH_A,adj_pwr);            
            adj_pwr = High_TxAgcOFDM[ADJ_PATH_B][rate-TX_AGC_OFDM_6M] - pwr_lvl*2;
            PHYSetTxPower88XX(priv,rate,rate,ADJ_PATH_B,adj_pwr);
        }
        DEBUG_RF_ADJ("mcs:");
        for (rate=TX_AGC_HT_NSS1_MCS0;rate<=TX_AGC_HT_NSS2_MCS15;rate++)
        {            
            DEBUG_RF_ADJ("rate %d idx %d\n", rate, High_TxAgcMCS[ADJ_PATH_A][rate-TX_AGC_HT_NSS1_MCS0]);            
            adj_pwr = High_TxAgcMCS[ADJ_PATH_A][rate-TX_AGC_HT_NSS1_MCS0] - pwr_lvl*2;
            PHYSetTxPower88XX(priv,rate,rate,ADJ_PATH_A,adj_pwr);
            adj_pwr = High_TxAgcMCS[ADJ_PATH_B][rate-TX_AGC_HT_NSS1_MCS0] - pwr_lvl*2;
            PHYSetTxPower88XX(priv,rate,rate,ADJ_PATH_B,adj_pwr);
        }
        DEBUG_RF_ADJ("vht:");
        for (rate=TX_AGC_VHT_NSS1_MCS0;rate<=TX_AGC_VHT_NSS2_MCS9;rate++)
        {
            DEBUG_RF_ADJ("rate %d idx %d\n", rate, High_TxAgcVHT[ADJ_PATH_A][rate-TX_AGC_VHT_NSS1_MCS0]);
            adj_pwr = High_TxAgcVHT[ADJ_PATH_A][rate-TX_AGC_VHT_NSS1_MCS0] - pwr_lvl*2;
            PHYSetTxPower88XX(priv,rate,rate,ADJ_PATH_A,adj_pwr);
            
            adj_pwr = High_TxAgcVHT[ADJ_PATH_B][rate-TX_AGC_VHT_NSS1_MCS0] - pwr_lvl*2;
            PHYSetTxPower88XX(priv,rate,rate,ADJ_PATH_B,adj_pwr);
        }
    }
    DEBUG_RF_ADJ("adj_RF pwr done\n");
    return 0;
...
adj_ac:
    ...
    if (1 == minus_sign)
    {
        DEBUG_RF_ADJ("set power -%d\n", adj_value);
        for (rate=TX_AGC_OFDM_6M;rate<=TX_AGC_OFDM_54M;rate++)
        {
            if (priv->pshare->phw->CurrentTxAgcOFDM[ADJ_PATH_A][rate-TX_AGC_OFDM_6M] > adj_value &&
                priv->pshare->phw->CurrentTxAgcOFDM[ADJ_PATH_B][rate-TX_AGC_OFDM_6M] > adj_value)
            {
                adj_pwr = priv->pshare->phw->CurrentTxAgcOFDM[ADJ_PATH_A][rate-TX_AGC_OFDM_6M]- 
                    adj_value;
                PHYSetTxPower88XX(priv,rate,rate,ADJ_PATH_A,adj_pwr);
        ...
        for (rate=TX_AGC_HT_NSS1_MCS0;rate<=TX_AGC_HT_NSS2_MCS15;rate++)
        {
            if (priv->pshare->phw->CurrentTxAgcMCS[ADJ_PATH_A][rate-TX_AGC_HT_NSS1_MCS0] > adj_value &&
                priv->pshare->phw->CurrentTxAgcMCS[ADJ_PATH_B][rate-TX_AGC_HT_NSS1_MCS0] > adj_value)
            {
                adj_pwr = priv->pshare->phw->CurrentTxAgcMCS[ADJ_PATH_A][rate-TX_AGC_HT_NSS1_MCS0]-
                    adj_value;
                PHYSetTxPower88XX(priv,rate,rate,ADJ_PATH_A,adj_pwr);
        ...
        for (rate=TX_AGC_VHT_NSS1_MCS0;rate<=TX_AGC_VHT_NSS2_MCS9;rate++)
        {
            if (priv->pshare->phw->CurrentTxAgcVHT[ADJ_PATH_A][rate-TX_AGC_VHT_NSS1_MCS0] > adj_value &&
                priv->pshare->phw->CurrentTxAgcVHT[ADJ_PATH_B][rate-TX_AGC_VHT_NSS1_MCS0] > adj_value)
            {
                adj_pwr = priv->pshare->phw->CurrentTxAgcVHT[ADJ_PATH_A][rate-TX_AGC_VHT_NSS1_MCS0]-
                    adj_value;
                PHYSetTxPower88XX(priv,rate,rate,ADJ_PATH_A,adj_pwr);
...
#endif
web_tenda_dev_probe_info web_messgae[MAX_DEV_PROBE_NUM + 1] ;
...
#define VERIFY_WRITE 1
...
#endif
					{	//if station
                        TD_DBG(priv, TD_DBG_FREE_STA, "[%s] del_sta[%s] %s__%d\n",
                            priv->dev->name,wrq->u.data.pointer,__func__,__LINE__);
	...
	case RTL8192CD_IOCTL_CLEAR_ACL_TABLE:
		...
		break;
		/*lq*/
	case  RTL8192CD_IOCTL_FFLUSH_ONLINE_TABLE:
		ret = update_online_client(priv);
		break;
	...
	case RTL8192CD_IOCTL_DEL_STA:
		if ((wrq->u.data.length > sizeof_tmpbuf) ||
			...
			break;
		{
		//----------- follow user for debug... -----------------//
		unsigned char macaddr[MACADDRLEN], tmpbuf1[3];
		for(i=0; i<MACADDRLEN; i++) {
			tmpbuf1[0] = tmpbuf[2*i];
			tmpbuf1[1] = tmpbuf[2*i+1];
			tmpbuf1[2] = 0;
			macaddr[i] = (unsigned char)_atoi((char *)tmpbuf1, 16);
		}
        TD_DBG(priv, TD_DBG_FREE_STA, "[%s] del_sta["MACSTR"] %s__%d\n",
            priv->dev->name,MAC2STR(macaddr),__func__,__LINE__);
		//----------- -------end...----------------------//
        }	
		if(*tmpbuf)
		{
...
//#ifdef CONFIG_WLAN_DUAL_BAND
	...
	case SIOCGIWRTLSSSCORE: //-- get site survey channel score --//
		...
		break;
#endif // CONFIG_PROLINK_PLDT
    case SIOCDEVPROBENUM:	
		dev_num = tenda_dev_probe_num_ioctl(priv->pmib->dot11RFEntry.dot11channel);  
		if (ioctl_copy_to_user((void *)wrq->u.data.pointer, &dev_num, sizeof(dev_num)))
			break;
		wrq->u.data.length = sizeof(dev_num);
		ret = 0;
		break;
    case SIOCDEVPROBERES:  
        memset(web_messgae, '\0', sizeof(web_tenda_dev_probe_info));
        returned = get_dev_probe_info(priv, (web_tenda_dev_probe_info *)web_messgae);
        if (returned > 0) 
        {
            if (ioctl_copy_to_user(wrq->u.data.pointer, web_messgae, sizeof(web_tenda_dev_probe_info)*(MAX_DEV_PROBE_NUM + 1)))
            		break;			
        }

       ret = 0;
        wrq->u.data.length = sizeof(web_tenda_dev_probe_info)*(MAX_DEV_PROBE_NUM + 1);
	...
	break;
    //去除F9获取自动信道评分
//    case SIOCGICHANNELSCORE:
//        diag_printf("---get SIOCGICHANNELSCORE---\n");
//        td_cal_channel_score(priv);
//        break;
...
/*--------------P2P related ioctl----------------------------------end*/
	...
	case SIOCSRFPWRADJ:
		...
		if(ioctl_copy_from_user(tmpbuf, (void *)wrq->u.data.pointer, wrq->u.data.length))
			break;
        if(IS_HARDWARE_TYPE_8822BE(priv))
        {
		    i = dynamic_RF_pwr_adj_11ac(priv, tmpbuf);
        }
        else
        {
...
*/
	...
	case SIOC92DAUTOCH:
	{
		...
		PCIeWakeUp(priv, POWER_DOWN_T0);
#endif
		ret = rtl8192cd_autochannel_sel(priv, 1);
		break;
	}
    case SIOCGICHANNELSCORE:
    {
        if (!(OPMODE & WIFI_AP_STATE))
        {
            DEBUG_ERR("can't do auto-channel scan for non-AP mode!\n");
            break;
        }
#ifdef UNIVERSAL_REPEATER
        if (IS_VXD_INTERFACE(priv)) 
        {
            DEBUG_ERR("can't do auto-channel scan for vxd!\n");
            break;
        }
#endif
#ifdef MBSSID
        if (GET_ROOT(priv)->pmib->miscEntry.vap_enable &&
            IS_VAP_INTERFACE(priv)) 
        {
            DEBUG_ERR("can't do auto-channel scan for vap!\n");
            break;
        }
#endif
        ...
        PCIeWakeUp(priv, POWER_DOWN_T0);
#endif
        ret = rtl8192cd_autochannel_sel(priv, 0);
        break;
    }
