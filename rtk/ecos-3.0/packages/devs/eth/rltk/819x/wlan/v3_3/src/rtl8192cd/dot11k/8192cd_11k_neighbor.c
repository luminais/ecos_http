...
int rm_construct_neighbor_report(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo, unsigned char channel)
{
    ...
    if(i == MAX_NEIGHBOR_REPORT)  /*not found and no empty slot, find the oldest neighbor and replace it*/
    {
        for(i = 0; i < MAX_NEIGHBOR_REPORT; i++)
        {
            ...
            {
                if(oldest_aging < priv->rm_neighbor_aging[i]) {
                    oldest_aging = priv->rm_neighbor_aging[i];
                    oldest_idx = i;
                }
            }
        }        
    ...
    memcpy(priv->rm_neighbor_ssid[i], ssid_ptr, ssid_len);
    priv->rm_neighbor_ssid[i][ssid_len] = 0;
    priv->rm_neighbor_aging[i] = 0;
...
#endif
{
    ...
    for (i = 0 ; i < MAX_NEIGHBOR_REPORT; i++)
    {
        ...
        PRINT_ONE("    Aging:", "%s", 0);
        PRINT_ONE(priv->rm_neighbor_aging[i], "%ld", 1);       
        j++;
    }
