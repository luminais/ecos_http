...
static int32 _rtl865x_delPpp(uint32 sessionId)
{
	...
	if(entry->refCnt > 1)
	{
		diag_printf("Please del other table entry which referenced me...ppp sessionId(%d)\n", sessionId);
    ...
    uint16 vid = entry->netif->vid;
    rtl8651_findAsicVlanIndexByVid(&vid);
    rtl8651_getAsicVlan(vid, &vlan);
    #else
	rtl8651_getAsicVlan(entry->netif->vid, &vlan);
    #endif
	retval = rtl865x_delFilterDatabaseEntry(RTL865x_L2_TYPEII, vlan.fid, &entry->server_mac);
...
static int32 _rtl865x_addPpp(uint8 *ifname, ether_addr_t *mac, uint32 sessionId, int32 type)
{
	...
	//diag_printf("%s(%d): ifname(%s),mac(%02x:%02x:%02x:%02x:%02x:%02x),sid(%d),type(%d)",__FUNCTION__,__LINE__,ifname,
	...
	if(netif == NULL)
		return TBLDRV_ENETIFINVALID;

	if(netif->if_type != IF_PPPOE)
		return TBLDRV_ELINKTYPESHOULDBERESET;
	...
	for(i = 0; i < 2; i++)
	{
		...
		*/
		if(rtl865x_Lookup_fdb_entry(vlan.fid, mac, fdb_type[i], &column,&fdbEntry) != SUCCESS)
		{	
			continue;
		}
		...
		rtl865x_addFilterDatabaseEntryExtension(vlan.fid, &l2temp_entry);		
//		retval = _rtl865x_addFilterDatabaseEntry((fdbEntry.nhFlag==0)?RTL865x_L2_TYPEI: RTL865x_L2_TYPEII, fid, mac, FDB_TYPE_FWD, fdbEntry.memberPortMask, fdbEntry.auth,fdbEntry.srcBlk);
		rtl865x_refleshHWL2Table(mac, FDB_DYNAMIC|FDB_STATIC, vlan.fid);
	}
...
static int32 _rtl865x_eventHandle_delNetif(void *para)
{
	...
	for(i = 0 ; i < PPP_NUMBER;i++,entry++)
	{
		if((entry->valid) && entry->netif == netif)
			diag_printf("%s(%d): BUG....deleted netif is referenced by ppp table!\n",__FUNCTION__,__LINE__);
	}
	return EVENT_CONTINUE_EXECUTE;
}
...
static int32 _rtl865x_eventHandle_delL2Fdb(void * param)
{
	...
	for(i = 0; i < PPP_NUMBER; i++,entry++)
	{
		if((entry->valid) && memcmp(&entry->server_mac,&fdbEntry->macAddr,sizeof(ether_addr_t)) == 0)
			diag_printf("%s(%d): BUG....deleted l2 fdb is referenced by ppp table!\n",__FUNCTION__,__LINE__);
	}
