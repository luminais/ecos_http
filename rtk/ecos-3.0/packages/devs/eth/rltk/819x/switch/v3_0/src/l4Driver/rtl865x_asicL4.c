        ...
        uint32 ageSec, int8 entryType, int8 isTcp, int8 isCollision, int8 isCollision2, int8 isValid ) 
{
	...
	if(forced == TRUE)
		return swTable_addEntry(TYPE_L4_TCP_UDP_TABLE, index, &entry);
	else if(swTable_addEntry(TYPE_L4_TCP_UDP_TABLE, index, &entry))
			return FAILED;
	return SUCCESS;
}
...
int32 rtl8651_setAsicNaptTcpUdpTable(int8 forced, uint32 index, rtl865x_tblAsicDrv_naptTcpUdpParam_t *naptTcpUdpp) 
{
	...
	if(forced == TRUE){
		swTable_addEntry(TYPE_L4_TCP_UDP_TABLE, index, &entry);
	}else if(swTable_addEntry(TYPE_L4_TCP_UDP_TABLE, index, &entry))
		return FAILED;
	return SUCCESS;
}
...
int32 rtl8651_getAsicRawNaptTable(uint32 index, void   *entry)
{
	...
	retval=swTable_readEntry(TYPE_L4_TCP_UDP_TABLE, index, entry);
...
int32 rtl8651_setAsicRawNaptTable( uint32 index, void *vEntry, int8 forced )
{
	...
	entry->collision = entry->collision2 = 1;
	if( forced == TRUE )
		return swTable_addEntry( TYPE_L4_TCP_UDP_TABLE, index, vEntry );
	else if( swTable_addEntry( TYPE_L4_TCP_UDP_TABLE, index, vEntry ) )
		return FAILED;
	return SUCCESS;
}
...
int32 rtl865x_initAsicL4(void)
{
	...
	/*clear asic table*/
#if 0 //!defined(CONFIG_RTL8196C_REVISION_B) && !defined(CONFIG_RTL_8198)
