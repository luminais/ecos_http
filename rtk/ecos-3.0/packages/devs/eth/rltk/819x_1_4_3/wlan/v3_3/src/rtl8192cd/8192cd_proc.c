...
//	if ((GET_CHIP_VER(priv)==VERSION_8192D) && (priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G)) {
	...
	if(priv->pmib->efuseEntry.enable_efuse)	{
		...
		for(k=0; k<priv->EfuseMapLen; k+=16)
			...
			PRINT_SINGL_ARG("    flash kfree enable:", priv->pmib->dot11RFEntry.kfree_enable, "%d");
			PRINT_SINGL_ARG("    current_kfree_on:", priv->pshare->kfree_on, "%d");
			PRINT_SINGL_ARG("    efuse kfree value: 0x", priv->pshare->kfree_value, "%x");
#endif
	}
#endif
...
#endif
#ifdef CH_LOAD_CAL
	PRINT_SINGL_ARG("	 chbusytime:	 ", priv->ext_stats.chbusytime, "%lu");
