...
static int rtl8192cd_init_sw(struct rtl8192cd_priv *priv)
{
	...
	if(GET_CHIP_VER(priv) == VERSION_8822B)
	{
		...
		priv->pshare->rf_ft_var.num_su_txbfee_entry = 2;
		priv->pshare->rf_ft_var.num_mu_txbfee_entry = 2;
	} else {
		priv->pshare->rf_ft_var.num_su_txbfee_entry = 2;
		priv->pshare->rf_ft_var.num_mu_txbfee_entry = 0;
	} 
#endif
...
#if defined(DOT11K) || defined(UCHAN_SUPPORT)
    if(priv->pmib->dot11StationConfigEntry.dot11RadioMeasurementActivated) {
...
#endif
#else
#if defined(CONFIG_RTL_819X) && !defined(CONFIG_RTL8196C_KLD) && !defined(__OSK__) && !(defined(CONFIG_RTL8672) && defined(CONFIG_OPENWRT_SDK))
