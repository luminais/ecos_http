...
// added for traffic control, by zhuhuan on 2016.03.01
...
/*lq tcp?㎝涩?*/
static struct pri_data pri_tcp_config[] =
{
	{
		.data_len = 80,
		.priority = 5,
...
/*lq udp?㎝涩?*/
static struct pri_data pri_udp_config[] =
{
	{
		.data_len = 320,
		.priority = 5,
...
void adjust_priority(int argc,char* argv[])
{
	...
	if(argc == 5)
	{
		...
		if(tcp_or_udp == 0)
		{
			if(index < 2)
			{
				pri_tcp_config[index].data_len = data_len;
				pri_tcp_config[index].priority = priority;
			}
			else
			{
				printf("index must <3!!!\n");
		...
		else
		{
			if(index < 1)
			{
				pri_udp_config[index].data_len = data_len;
				pri_udp_config[index].priority = priority;
			}
			else
			{
				printf("index must <1!!!\n");
	...
	if(argc == 2)
	{
		...
		else
		{
			...
			printf("\nindex      proto       data_len      priority\n");
			while(index < 2)
			{	
				printf("  %d         tcp     %9d   %9d\n",index,pri_tcp_config[index].data_len,pri_tcp_config[index].priority);
				index++;
			}
			printf("----------------------------------------------\n");
			printf("  %d         udp     %9d   %9d\n",0,pri_udp_config[0].data_len,pri_udp_config[0].priority);
		}
	}
}
/* udp and len < 254 */
/*add lq 誹?誹㎝????竚涩?*/
__IRAM_IN_865X int td_set_skb_priority(struct sk_buff *skb)
{
	...
	if(eh->ether_type == htons(ETHERTYPE_IP))
	{
		...
		{
			source = ntohs(*(unsigned short*)((char *) iph + (iph->ip_hl << 2)));
			if(source == 80 || source == 443)
			{
			     while(pri_tcp_config[index].data_len != 0)
			     {
			     		if(iph->ip_len < pri_tcp_config[index].data_len)
			     		{
			         		skb->cb[1] = pri_tcp_config[index].priority;
						return pri_tcp_config[index].priority;
			     		}
					index++;
			     }
			}
	       ...
	       {
		     while(pri_udp_config[index].data_len != 0)
		     {
		     		if(iph->ip_len < pri_udp_config[index].data_len)
		     		{
		         		skb->cb[1] = pri_udp_config[index].priority;
					return pri_udp_config[index].priority;
		     		}
				index++;
		     }
		}
	...
	BOOLEAN *bRtsEnable, BOOLEAN *bCts2SelfEnable, BOOLEAN *bHwRts, BOOLEAN *bErpProtect, BOOLEAN *bNProtect)
{
	...
	*bNProtect = FALSE;
	...
	if(!txcfg->pstat) //eric-mu
		return; 

#if (BEAMFORMING_SUPPORT == 1) && defined(CONFIG_WLAN_HAL_8822BE)
//Force Cts2Self for MU clients
	if((GET_CHIP_VER(priv)==VERSION_8822B) 
		&& priv->pmib->dot11RFEntry.txbf_mu
		&& is_support_bf(txcfg->pstat, TXBF_TYPE_MU)){
		...
		*bRtsEnable = TRUE;
		...
		{	// when HT MIMO Dynamic power save is set, RTS is needed
			*bRtsEnable = TRUE;
			...
			if (is_MCS_rate(txcfg->tx_rate) && (txcfg->pstat && /*(txcfg->pstat->IOTPeer!=HT_IOT_PEER_INTEL) ||*/ !txcfg->pstat->no_rts)) 
			{
					*bNProtect = 1;
					*bHwRts = TRUE;
...
void rtl88XX_signin_txdesc(struct rtl8192cd_priv *priv, struct tx_insn* txcfg, unsigned char convHdr)
{
	...
	for (i=0; i < txcfg->frg_num; i++)
	{
		...
		if(txcfg->fr_type == _SKB_FRAME_TYPE_) {
	       desc_data.tid = ((struct sk_buff *)txcfg->pframe)->cb[1];
...
#elif defined(CONFIG_WLAN_HAL_8197F)
			{
				...
				PRI_TO_QNUM(desc_data.tid, q_num, priv->pmib->dot11OperationEntry.wifi_specific);
#endif
				if((q_num == BE_QUEUE) && (txcfg->q_num == VI_QUEUE)) {
					desc_data.tid = 0x04; /* set tid as VI */
				}else if((q_num == VI_QUEUE) && (txcfg->q_num == VO_QUEUE)) {					
					desc_data.tid = 0x06; /* set tid as VO */
				}
			}
#endif
		}
		...
		if(*da == 0xff)
			desc_data.is_broadcast = 1;
		else
			desc_data.is_broadcast = 0;
...
__inline__ static int rtl8192cd_swq_enqueue(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info	*pstat)
{
	...
	if ((assoc_num == 1) && (7500000 < pstat->rx_avarage) &&  ((pstat->tx_avarage * 10) <  pstat->rx_avarage) && (priv->pshare->rssi_min > 40))  /* Rx Check : Rx > 60M && Rx > 10 * Tx && rssi > 40% */
	{
	        if (pstat->swq.swq_en[q_num]) {
	            if ((34375000 < pstat->rx_avarage) || (pstat->rx_avarage < 12500000)) { /* 275M < TP  || TP < 100M(100M solution) 2017/3/7合入原厂解决方案 */
...
#endif
	#if defined(CONFIG_RTL_QOS_RATE_LIMIT_CHECK)
	ret_check = rtl_qos_rate_limite_check(2, priv->dev->name, skb);
	if (ret_check ==  0){
		// download traffic control of wireless, added by zhuhuan on 2016.03.01
		goto free_and_stop;
	}
	else{
...
) //struct net_device *dev)
{
	...
	if (IS_SUPPORT_TX_AMSDU(priv) && amsdu && pstat && (pstat->aggre_mthd & AGGRE_MTHD_MSDU) && (pstat->amsdu_level > 0)
#ifdef SUPPORT_TX_MCAST2UNI
...
#elif defined(CONFIG_WLAN_HAL_8197F) //wishchen
			if((IS_HAL_CHIP(priv)) && (txcfg->q_num==BE_QUEUE) && priv->pshare->iot_mode_VO_exist) {
				txcfg->q_num = VI_QUEUE; /* use VI Q for BE pkt when testing VO+BE */
			}else if((IS_HAL_CHIP(priv)) && (txcfg->q_num==VI_QUEUE) && priv->pshare->iot_mode_BK_exist) {
				txcfg->q_num = VO_QUEUE; /* use VO Q for VI pkt when testing VI+BK */
			}
#endif
			if(
