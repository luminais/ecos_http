...
int td_flash_size = 0x100000;   //default 1M

#define MTD_SPI_DEBUG		3 //2
...
#define LDEBUG(args...) diag_printf(args)
#endif
//1
#if (MTD_SPI_DEBUG == 1)
#define NDEBUG(args...) diag_printf(args)
...
//#define MD25D80		0x514014
{0x514014, 0x00, SIZE2N_01MB, SIZE_064K, SIZE_004K, SIZE_256B, "MD25D80"
#if (SPI_DRIVER_MODE == 1)
, 40, ComSrlCmd_SE,  SpiRead_11110B, ComSrlCmd_NoneQeBit,  PageWrite_111002
#else
, 40, ComSrlCmd_SE,  SpiRead_11110B, ComSrlCmd_NoneQeBit,  PageWrite_111002
#endif
},

//#define KH25L80		0xc22014
{0xc22014, 0x00, SIZE2N_01MB, SIZE_064K, SIZE_004K, SIZE_256B, "KH25L80"
#if (SPI_DRIVER_MODE == 1)
, 40, ComSrlCmd_SE,  SpiRead_11110B, ComSrlCmd_NoneQeBit,  PageWrite_111002
#else
, 40, ComSrlCmd_SE,  SpiRead_11110B, ComSrlCmd_NoneQeBit,  PageWrite_111002
...
void spi_regist(unsigned char ucChip)
{
	...
	diag_printf("0x%x %s size=0x%x\n", spi_flash_info[ucChip].chip_id, spi_flash_info[ucChip].chip_name,
                spi_flash_info[ucChip].chip_size);
    td_flash_size = spi_flash_info[ucChip].chip_size;
}
...
unsigned int spiFlashReady(unsigned char ucChip)
{
	...
	while (1)
	{
		...
		{
			diag_printf("spiFlashReady: too many retry\n");
		}
	}
...
unsigned int ComSrlCmd_ComWriteSector(unsigned char ucChip, unsigned int uiAddr, unsigned char* pucBuffer, int doerase)
{
	unsigned int i, ui;
	unsigned char* puc = pucBuffer;
...
unsigned int ComSrlCmd_BufWriteSector(unsigned char ucChip, unsigned int uiAddr, unsigned int uiLen, unsigned char* pucBuffer, int doerase)
{
	//unsigned char pucSector[spi_flash_info[ucChip].sector_size];
	unsigned char mallocTimes = 3;//when malloc fail,malloc 3 times more.
	unsigned char* pucSector = (unsigned char*)malloc(spi_flash_info[ucChip].sector_size);
	if(pucSector == NULL){
		while(mallocTimes){
			pucSector = (unsigned char*)malloc(spi_flash_info[ucChip].sector_size);
			if(pucSector)
				break;
			mallocTimes--;
		}		
	...
	if(pucSector == NULL){
		diag_printf("malloc sector size fail when start address is not sector alienged\n");
		return 0;
	}
	memset(pucSector,0,spi_flash_info[ucChip].sector_size);
	...
	//write back
	ui = ComSrlCmd_ComWriteSector(ucChip, uiStartAddr, pucSector, doerase);
...
unsigned int ComSrlCmd_ComWriteData(unsigned char ucChip, unsigned int uiAddr, unsigned int uiLen, unsigned char* pucBuffer, int doerase)
{
	...
	{
		ComSrlCmd_BufWriteSector(ucChip, uiStartAddr, uiStartLen, puc, doerase);
	}
	else
	{	
		if(uiStartLen > 0)
		...
		{
			ComSrlCmd_ComWriteSector(ucChip, uiSectorAddr, puc, doerase);
			puc += spi_flash_info[ucChip].sector_size;
			uiSectorAddr += spi_flash_info[ucChip].sector_size;
		}
		if(uiEndLen > 0)
		{
			ComSrlCmd_BufWriteSector(ucChip, uiEndAddr, uiEndLen, puc, doerase);
		}
	}
