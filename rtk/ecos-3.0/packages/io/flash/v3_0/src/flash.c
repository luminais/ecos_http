...
#if defined(CYGNUM_IO_FLASH_MEM_CHAIN_UPGRADE)
...
#if defined(CYGNUM_IO_FLASH_MEM_CHAIN_UPGRADE)
__externC int 
mem_chain_support_cyg_flash_program(cyg_flashaddr_t flash_base, 
                  const void *ram_base, 
                  size_t len, 
                  cyg_flashaddr_t *err_address)
{  
  #if defined(CYGNUM_IO_FLASH_MEM_CHAIN_UPGRADE)
  #define MEM_CHAIN_UPGRADE_CACHE_SIZE	4
  char mem_chain_upgrade_cache[MEM_CHAIN_UPGRADE_CACHE_SIZE];
  char* mem_chain_ram;
  int	node_size,need_size,cache_size;
  memset(mem_chain_upgrade_cache,0,MEM_CHAIN_UPGRADE_CACHE_SIZE);
  #endif
  ...
  while (write_count > 0) {
...
#if defined(CYGNUM_IO_FLASH_MEM_CHAIN_UPGRADE)
	{
		if(mem_chain_upgrade_flash_write_count(block_size,&offset,ram,write_count,&this_write) == -1){
			diag_printf("cannot convert address\n");
...
#if defined(CYGNUM_IO_FLASH_MEM_CHAIN_UPGRADE)
	{
		mem_chain_ram =(char*) mem_chain_upgrade_mem_convert((char*)ram);
		if(mem_chain_ram ==  NULL){
			diag_printf("cannot convert address\n");
			return CYG_FLASH_ERR_PROGRAM;
		}
		//diag_printf("this_write:%d,offset:%d.\n",this_write,offset);
		
		if(this_write%4 == 0 || this_write == write_count){
#ifdef CONFIG_RTL_819X
			/*HF. avoid watchdog timeout*/
			HAL_ICACHE_SYNC();
#endif
			stat = dev->funs->flash_program(dev, addr, mem_chain_ram, this_write);
		}else if(this_write >4){
			//first write
			if(offset >= (this_write % 4))
				offset -= this_write % 4;
			else
				offset = (offset + block_size - this_write % 4);
			this_write -=  this_write % 4;
#ifdef CONFIG_RTL_819X
			/*HF. avoid watchdog timeout*/
			HAL_ICACHE_SYNC();
#endif
			...
			//second write
			if(write_count < 4)
				cache_size = write_count;
			else
				cache_size = 4;
			
			memcpy(mem_chain_upgrade_cache,mem_chain_ram,this_write);
			need_size = cache_size-this_write;
			
			mem_chain_ram = (char*) mem_chain_upgrade_mem_convert((char*)(ram+this_write));
			node_size = mem_chain_upgrade_node_size((char*)(ram+this_write));
			
			while(node_size < need_size){
				memcpy(mem_chain_upgrade_cache+cache_size-need_size,mem_chain_ram,node_size);
				need_size -= node_size;
				mem_chain_ram = (char*) mem_chain_upgrade_mem_convert((char*)(ram+cache_size-need_size));
				node_size = mem_chain_upgrade_node_size((char*)(ram+cache_size-need_size));
			}
			
			memcpy(mem_chain_upgrade_cache+cache_size-need_size,mem_chain_ram,need_size);
			offset += cache_size-this_write;
			offset = offset % block_size;
			this_write = cache_size;
			
			mem_chain_ram = mem_chain_upgrade_cache;
...
#if defined(CYGNUM_IO_FLASH_MEM_CHAIN_UPGRADE)
	{
		if (!dev->funs->flash_read && memcmp((void *)addr, mem_chain_ram, this_write) != 0) {
			stat = CYG_FLASH_ERR_DRV_VERIFY;
        	CHATTER(dev, "V");
      	}
	}
	...
	if(this_write > write_count)
		diag_printf("this_write:%d,write_count:%d.\n",this_write,write_count);
  ...
  if (len > (dev->end + 1 - flash_base)) {
    return mem_chain_support_cyg_flash_program(dev->end+1, ram, 
                             len - (dev->end + 1 - flash_base),
                             err_address);
  }
  return CYG_FLASH_ERR_OK;
