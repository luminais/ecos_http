...
eth_drv_init(struct eth_drv_sc *sc, unsigned char *enaddr)
{
    ...
    unit = 0;
#ifdef RTLPKG_DEVS_ETH_RLTK_819X_WRAPPER
    //for wlan0-wds0, wlan0-wds1, ...
    while (*np && !((*np >= '0') && (*np <= '9') && (*(np+1) != '-'))) *xp++ = *np++;
#else
...
// DEBUG
			...
			if (ifa->ifa_addr->sa_family == AF_LINK) {
				...
				if (sdl != NULL) {
					if ((ETHER_ADDR_LEN == sdl->sdl_alen)
						...
						bcopy(&ifr->ifr_hwaddr.sa_data, LLADDR(sdl), ETHER_ADDR_LEN);
					}
				}
			}
		}
	...
	case SIOCGIFSTATUS:
		...
		if ((sc->funs->control)(sc, ETH_DRV_GET_IF_STATUS, data, 0)) {
			error = EINVAL;
		}
		break;
#endif
	...
	if (ifr && (ifr->ifr_flags & IFF_UP) == 0 &&
            ...
            if (ifr && (ifr->ifr_flags & IFF_UP) != 0 &&
                 ...
                 */
                eth_drv_start(sc);
            } else if(ifr && (((ifr->ifr_flags & IFF_UP) && (ifp->if_flags & IFF_RUNNING)) ||
             ((ifr->ifr_flags & IFF_UP) ==0  && (ifp->if_flags & IFF_RUNNING) == 0))) {

		  /*Do nothing if down/up the downed or upped interface by ifconfig*/
		  
            }else if(ifp->if_flags & IFF_ALLMULTI){ 
            	/*Do nothing if add mutilcast interface for wlan interface duplicate open/close */
			}else {
		  /*
  ...
  //                       __FUNCTION__ );
	...
	if ((cmd >= SIOCDEVPRIVATE && cmd <= SIOCDEVPRIVATE + 15) ||
		...
		error = (sc->funs->control)(sc, cmd, ifr, 0);
		break;
	}
...
#if defined(CYGPKG_NET_FREEBSD_STACK)//||defined(CONFIG_RTL_MTU_IOCTL_SUPPORT)
	if(cmd == SIOCSIFMTU){
		error = 0;
		break;
	}
...
extern int g_eth_recv_out_mbuf;
...
#ifdef RTLPKG_DEVS_ETH_RLTK_819X_RX_ZERO_COPY
        sg_list[sg_len].buf = (CYG_ADDRESS)m;
        sg_len = 2;
#endif
    } else {
...
#ifdef RTLPKG_DEVS_ETH_RLTK_819X_RX_ZERO_COPY
        eh = (struct ether_header *)sg_list[0].buf;
        /*
...
extern __attribute__((nomips16)) void ecos_synch_eth_drv_dsr(void); // from ecos/timeout.c in net stack
#else
