...
void rtl_gpio_init(void)
{
   ...
   /*****配置B4,B7,C1,C3,为GPIO模式/wps_led、sys_led,reset/wps_btn,wifi_btn****/
    ...
    reg_data &= ~(0xff << 16); //H0 H1
    reg_data |= (0x3 << 20);
    reg_data |= (0x2 << 16);
    RTL_W32(BSP_PIN_MUX_SEL13,reg_data);
    printf("H0 mux REG:0X%X\n", RTL_R32(BSP_PIN_MUX_SEL13));
    reg_data = RTL_R32(BSP_PEFGH_CNR);
    reg_data &= ~(3 << 24);
    RTL_W32(BSP_PEFGH_CNR,reg_data);
    printf("H0 cnr REG:0X%X\n", RTL_R32(BSP_PEFGH_CNR));
    
    reg_data = RTL_R32(BSP_PEFGH_DIR);
    reg_data &= ~(1 << 24);
    reg_data |= (1 << 25);
    RTL_W32(BSP_PEFGH_DIR,reg_data);
    printf("H0 DIR REG:0X%X\n", RTL_R32(BSP_PEFGH_DIR));
    
    reg_data = RTL_R32(BSP_PEFGH_DAT);
    reg_data &= ~(1 << 25);
    RTL_W32(BSP_PEFGH_DAT,reg_data);
#endif
#else  /*CONFIG_GPIO_APIV2*/
    printf("Init GPIO in GPIO_APIV2\n");
#if defined(CONFIG_LAN_LED_GPIO_NR)
    set_gpio_direction(CONFIG_LAN_LED_GPIO_NR, 0);
#endif

#if defined(CONFIG_SYS_LED_GPIO_NR)
    set_gpio_direction(CONFIG_SYS_LED_GPIO_NR, 0);
#endif

#if defined(CONFIG_WAN_LED_GPIO_NR)
    set_gpio_direction(CONFIG_WAN_LED_GPIO_NR, 0);
#endif

#if defined(CONFIG_SYS_LED_GPIO_NR)
    set_gpio_direction(CONFIG_SYS_LED_GPIO_NR, 0);
    /*仅系统灯设置常亮*/
    set_gpio_value(CONFIG_SYS_LED_GPIO_NR, 0);
#endif

#if defined(CONFIG_WPS_LED_GPIO_NR)
    set_gpio_direction(CONFIG_WPS_LED_GPIO_NR, 0);
#endif

#if defined(CONFIG_24G_LED_GPIO_NR)
    set_gpio_direction(CONFIG_24G_LED_GPIO_NR, 0);
#endif

#if defined(CONFIG_5G_LED_GPIO_NR)
    set_gpio_direction(CONFIG_5G_LED_GPIO_NR, 0);
#endif

#if defined(CONFIG_RESET_BTN_GPIO_NR)
    set_gpio_direction(CONFIG_RESET_BTN_GPIO_NR, 1);
#endif

#if defined(CONFIG_WPS_BTN_GPIO_NR)
    set_gpio_direction(CONFIG_WPS_BTN_GPIO_NR, 1);
#endif

#if defined(CONFIG_WIFI_BTN_GPIO_NR)
    set_gpio_direction(CONFIG_WIFI_BTN_GPIO_NR, 1);
#endif
#endif /*CONFIG_GPIO_APIV2*/
