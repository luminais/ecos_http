...
static cyg_int32 buff[CYGNUM_IO_FLASH_TEST_LENGTH*2];
...
void flash_test( void )
{
    ...
    if ((stat = flash_init(diag_printf)) != 0) {
        ...
        diag_printf("FLASH driver init failed");
    }
...
/*
    if ((CYGNUM_IO_FLASH_TEST_OFFSET > (flash_end - flash_start)) ||
        ...
        diag_printf("FLASH test region outside physical limits");
    }
    if ((CYGNUM_IO_FLASH_TEST_LENGTH % flash_block_size) != 0) {
        diag_printf("FLASH test region must be integral multiple of erase block size");
    }
*/
...
/*    test_buf1 = (CYG_ADDRWORD)CYGMEM_SECTION_heap1;
    ...
    if (CYGMEM_SECTION_heap1_SIZE < (CYGNUM_IO_FLASH_TEST_LENGTH * 2)) {
        diag_printf("FLASH not enough heap space - reduce size of test region");
    }
*/    
    test_buf1 = (CYG_ADDRWORD)buff;
    test_buf2 = test_buf1 + CYGNUM_IO_FLASH_TEST_LENGTH;
    ...
    if (!ok) {
        diag_printf("FLASH erase failed");
        passed = false;
    }
    ...
    if (!ok) {
        diag_printf("FLASH write/verify failed");
    }
