...
extern  rtl_1x_global_config_t rtl_1x_global_config[RTL_1X_MAX_INTERFACE_SUPPORT];
extern int rtl_1x_interface_num ;
...
#ifdef __ECOS
cyg_handle_t auth_sys_clk;
cyg_handle_t auth_alarm_hdl;
cyg_handle_t auth_conter_hdl;
cyg_alarm auth_alarm_obj;
#endif
...
#ifdef __ECOS
void lib1x_timer_authenticator(cyg_handle_t alarm_handle,cyg_addrword_t data);	
#else
				...
				u_char * dev_svr, u_char * dev_supp)
{
	...
	//----endif
	{
		...
		memset( dev_txrx->device_svr, 0, IFNAMSIZ + 10);
		...
		dev_txrx->radsvraddr.sin_family = AF_INET;   // we need to 'connect' on udp to the server
		dev_txrx->radsvraddr.sin_port = htons( udp_svrport );//htons( 1812 );
		dev_txrx->radsvraddr.sin_addr.s_addr = inet_addr( svrip_addr );//inet_addr("192.168.3.100");
		...
		if (auth->use_2nd_rad) {
			...
			dev_txrx->radsvraddr2.sin_family = AF_INET;   // we need to 'connect' on udp to the server
	...
	memset( dev_txrx->device_supp, 0, IFNAMSIZ + 10);
	...
	//---------------------------------
        ...
        memset( dev_txrx->device_wlan0, 0, IFNAMSIZ + 10);
	//sc_yang
...
Global_Params *  lib1x_init_authenticator(Dot1x_Authenticator *auth, TxRx_Params * dev_txrx)
{
	...
	//-------------------------------------------------
		...
		global->akm_sm->ANonce_Octet_p = global->akm_sm->ANonce.Octet;
		global->akm_sm->SNonce_Octet_p = global->akm_sm->SNonce.Octet;
...
#ifdef __ECOS
#ifndef USE_ALARM 
unsigned char auth_timer_stack[8*1024];
cyg_handle_t auth_timer_thread_handle;
cyg_thread auth_timer_thread_obj;
#ifdef HAVE_SYSTEM_REINIT
static int auth_timer_quitting = 0;
#endif
void auth_timer_main()
{
	struct timeval	timeofday={0,0};
#ifdef HAVE_SYSTEM_REINIT
	while(!auth_timer_quitting)
#else
	while(1)
#endif
	{
		timeofday.tv_sec = 0;
		timeofday.tv_usec = LIB1X_BASIC_TIMER_UNIT;
		select(0,NULL,NULL,NULL,&timeofday);
		
		
		lib1x_timer_authenticator(0,0);		
	}
#ifdef HAVE_SYSTEM_REINIT
	auth_timer_quitting = 0;
#endif
}

void lib1x_start_auth_timer()
{
	cyg_thread_create(16,
					  ...
					  "auth_timer",
					  &auth_timer_stack,
					  sizeof(auth_timer_stack),
					  &auth_timer_thread_handle,
					  &auth_timer_thread_obj );
			/* Let the thread run when the scheduler starts */
			cyg_thread_resume(auth_timer_thread_handle);

}
#else 
int lib1x_init_authTimer()
{
	auth_sys_clk = cyg_real_time_clock();
	cyg_clock_to_counter(auth_sys_clk,&auth_conter_hdl);
	cyg_alarm_create(auth_conter_hdl,lib1x_timer_authenticator,
		0,&auth_alarm_hdl,&auth_alarm_obj);
	cyg_alarm_initialize(auth_alarm_hdl,cyg_current_time()+10,10);
	cyg_alarm_enable(auth_alarm_hdl);
}
#endif
#ifdef HAVE_SYSTEM_REINIT
void lib1x_stop_auth_timer()
{
#ifndef USE_ALARM
	cyg_thread_info tinfo;
	if(get_thread_info_by_name("auth_timer", &tinfo)){
		auth_timer_quitting = 1;
		while(auth_timer_quitting)
			cyg_thread_delay(10);
		cyg_thread_kill(auth_timer_thread_handle);
		cyg_thread_delete(auth_timer_thread_handle);
	}
#else
	cyg_alarm_delete(auth_alarm_hdl);
#endif
}
#endif
#else
...
#endif
{
...
#else
	u_long Counter , CounterReGroupKey;
#endif
...
#endif
	{
		...
		auth = &(pConf->RTLAuthenticator);
#ifdef RTL_WPA_CLIENT
		client = &(pConf->RTLClient);
#endif
...
#else
	auth->Timercounter++;
	if(auth->Timercounter == 0xffffffff)
		auth->Timercounter = 0;
	Counter = auth->Timercounter;
#endif
...
#endif
	{
		//diag_printf("%s-->%d:gk_sm= %d\n",__FUNCTION__,__LINE__,auth->gk_sm);
		if(auth->gk_sm->GResetCounter != 0)
		{
#ifndef RTL_1X_SUPPORT_MULTI_INTERFACE
			CounterReGroupKey = 0;
#else 
			auth->TimercounterReGroupKey = 0;
#endif
			auth->gk_sm->GResetCounter = FALSE;
		}
#ifndef RTL_1X_SUPPORT_MULTI_INTERFACE
		...
		if(CounterReGroupKey == 0xffffffff)
			CounterReGroupKey = 0;
#else
		auth->TimercounterReGroupKey++;
		if(auth->TimercounterReGroupKey == 0xffffffff)
			auth->TimercounterReGroupKey = 0;
		CounterReGroupKey = auth->TimercounterReGroupKey;
#endif
		 ...
		 u_short dstport )
{
	...
	if ( memcmp( eth->ether_shost, ether_src, ETHER_ADDRLEN ) != 0 )
	{
...
void lib1x_handle_eapsupp_key( Global_Params * global,  struct lib1x_packet * pkthdr)
{
	...
	//global->EAPOLMsgRecvd.Length = pkthdr->caplen - 14;
...
#ifdef HAVE_SYSTEM_REINIT
void lib1x_deinit_txrx(TxRx_Params *dev_txrx)
{
	...
	if(!dev_txrx)
		return;
#ifndef PSK_ONLY
	lib1x_nal_deinitialize(dev_txrx->network_svr,LIB1X_IT_UDPSOCK);
	free(dev_txrx->device_svr);
	free(dev_txrx->device_supp);
	
#ifdef RTL_WPA2_PREAUTH
	lib1x_nal_deinitialize(dev_txrx->network_ds,LIB1X_IT_PKTSOCK);
#else

#endif

#endif // !PSK_ONLY        
	lib1x_nal_deinitialize(dev_txrx->network_supp,LIB1X_IT_PKTSOCK);
#ifdef RTL_RADIUS_2SET
	lib1x_nal_deinitialize( dev_txrx->network_svr2, LIB1X_IT_UDPSOCK);
#endif
	free(dev_txrx->device_wlan0);
	close(dev_txrx->fd_control);
	free(dev_txrx);
}
void lib1x_deinit_auth(Dot1x_Authenticator * auth)
{
	if(!auth)
		return;
	lib1x_deinit_authenticator(auth->authGlobal->global);
	free(auth->authGlobal);
#ifdef RTL_WPA2
	INIT_LIST_HEAD(&auth->pmk_cache); // todo cleanlist
#endif

}
void lib1x_deinit_authGlobal(Dot1x_Authenticator *auth)
{
		if(!auth)
			return;
        free(auth->gk_sm->GNonce.Octet); 
		free(auth->gk_sm); 
	
}
void lib1x_deinit_authRSNConfig(Dot1x_Authenticator * auth)
{

        int retVal = 0;
	if(!auth)
		return;
	free(auth->RSNVariable.AuthInfoElement.Octet);
}

//------------------------------------------------------
//  lib1x_deinit_authenticator:
//	free the variable
//-------------------------------------------------------

void  lib1x_deinit_authenticator(Global_Params *global)
{
	...
	// 1. Deinit port timers
	free(auth_pae->port_timers); 
	// 2. Deinit Reauthentication State Machine
	free(auth_pae->reauth_sm);

	// 3. Deinit Backend Authentication State Machine
	free(auth_pae->bauth_sm); 

	// 4. Deinit Controlled Directions State Machine
	
	free(auth_pae->ctrl_sm);

	// 5. Key Receive State Machine
	free(auth_pae->krc_sm);

	// 6. Authenticator Key Transmit State Machine
	free(auth_pae->keyxmit_sm); 

	// 7. Accounting state machine
	free(auth_pae->acct_sm);
	// 8. DeInitialize Authenticator Port Access Entity .. state machine also !

	free(auth_pae->sendBuffer);
	...
	free(auth_pae->acct_sendBuffer);
	
	// free memory for eap buffer
	free(auth_pae->fromsupp.pkt);
	free(auth_pae->fromsvr.pkt) ;

	free(auth_pae->rinfo);
	free(auth_pae->rconst); 
...
#ifdef _RTL_WPA_UNIX
	free(global->akm_sm->SuppInfoElement.Octet);
	free(global->akm_sm->AuthInfoElement.Octet);
	//free(global->akm_sm->ANonce.Octet);
	free(global->akm_sm->ANonce_Octet_p);
	//free(global->akm_sm->SNonce.Octet);
	free(global->akm_sm->SNonce_Octet_p);
	free(global->RadiusKey.SendKey.Octet);
	free(global->RadiusKey.RecvKey.Octet);
    free(global->akm_sm); 
#endif
	free(global);
}
...
// reduce pre-alloc memory size -------------------------------------
void dealloc_supp(Dot1x_Supplicant *supp)
{
	if(!supp)
		return;
	lib1x_deinit_authenticator(supp->global);
	free(supp);
}
