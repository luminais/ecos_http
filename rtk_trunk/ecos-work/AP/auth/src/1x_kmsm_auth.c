...
--*/
{
	...
	{
		//global->EAPOLMsgRecvd.Length -= 14;
		switch(akm_sm->state)
		{
			case akmsm_PTKSTART:
...
//					syslog(LOG_AUTH|LOG_INFO, "%s: Authentication failled! (4-2: ERROR_NONEEQUL_REPLAYCOUNTER)\n", dev_supp); // david+2006-03-31, add event to syslog
				...
				}else
				{
					...
					wpa2_hexdump("Message 2:", global->EAPOLMsgRecvd.Octet + 14, global->EAPOLMsgRecvd.Length-14);
					wpa2_hexdump("ANonce:", global->akm_sm->ANonce.Octet, global->akm_sm->ANonce.Length);
					wpa2_hexdump("SNonce 2:", global->akm_sm->SNonce.Octet, global->akm_sm->SNonce.Length);
					wpa2_hexdump("PMK:", global->akm_sm->PMK, PMK_LEN);
					wpa2_hexdump("ADDR1:", global->EAPOLMsgRecvd.Octet, 6);
					wpa2_hexdump("ADDR2:", global->EAPOLMsgRecvd.Octet+6, 6);
#endif
...
int lib1x_akmsm_AuthenticationSuccess( Global_Params * global)
{
		...
		global->authSuccess && akm_sm->state == akmsm_AUTHENTICATION2)
	{
		...
		if( global->RadiusKey.Status == MPPE_SDRCKEY_AVALIABLE)
		{
			...
			//wpa2_hexdump("PMK:", akm_sm->PMK, PMK_LEN);
			...
			CalcPMKID(	akm_sm->PMKID,
					...
					global->theAuthenticator->global->TxRx->oursupp_addr,   // AA
					global->theAuthenticator->supp_addr); 			// SPA
			//wpa2_hexdump("OUR_ADDR:", global->theAuthenticator->global->TxRx->oursupp_addr, 6);
			//wpa2_hexdump("SUPP_ADDR:",global->theAuthenticator->supp_addr, 6);
