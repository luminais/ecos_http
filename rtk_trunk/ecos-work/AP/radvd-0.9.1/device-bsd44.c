...
#include <net/if_var.h>	// struct ia_ifa
#include <netinet6/in6_var.h>	// struct prf_ra 
#include <netinet6/nd6.h>	// struct nd_ifinfo
...
setup_deviceinfo(int sock, struct Interface *iface)
{
	...
	while(p < end)
	{
		    ...
		    (!memcmp(iface->Name, ((struct sockaddr_dl *)p)->sdl_data, nlen)))
		{
    		...
    		p += ((*p) > sizeof(struct sockaddr) ?(*p): sizeof(struct sockaddr));	
	}
...
static struct nd_ifinfo * if_name2ndinfo( const char *iface )
{
	extern struct nd_ifinfo *cyg_nd_ifinfo;	// hack: copy from '<netinet6/nd6.h>'
	
	unsigned int if_idx = if_nametoindex( iface );
	struct nd_ifinfo *ndi = &cyg_nd_ifinfo[ if_idx ];
...
{
	struct nd_ifinfo *ndi = if_name2ndinfo( iface );
	...
	//printf( "setting CurHopLimit (%u) for %s\n",
	//			hlim, iface );
...
set_interface_reachtime(const char *iface, uint32_t rtime)
{
	...
	ndi ->basereachable = rtime;

#define ND_MIN_RANDOM_FACTOR		512	/* 1024 * 0.5 */
#define ND_MAX_RANDOM_FACTOR		1536	/* 1024 * 1.5 */
#define ND_COMPUTE_RTIME(x) \
		(((ND_MIN_RANDOM_FACTOR * ((x >> 10) + !!( x & 0x1FF))) + (rand() & \
		((ND_MAX_RANDOM_FACTOR - ND_MIN_RANDOM_FACTOR) * (x >> 10)))) /1000)

	ndi ->reachable = ND_COMPUTE_RTIME( rtime );
	if(ndi ->reachable == ((rtime >> 1)/1000))
		ndi ->reachable += 1;
	
	//printf( "setting BaseReachableTime (%u) and reachable (%u) for %s\n",
	//			rtime, ndi ->reachable, iface );
...
set_interface_retranstimer(const char *iface, uint32_t rettimer)
{
	...
	ndi ->retrans = rettimer;
	
	//printf( "setting RetransTimer (%u) for %s\n",
	//				rettimer, iface);
