...
extern struct in_addr bcast_ip[2];
extern struct in_addr myip[2];
pstring myhostname="";
pstring mynetifname="";
int myttl = 0x3f480;
...
//record netbios names of eht0 and eth1
#define MAX_NETBIOS_NAME_NUM  10
#define MAX_NETBIOS_INTERFACE_NUM  2
typedef struct
{
	pstring myname[MAX_NETBIOS_INTERFACE_NUM][MAX_NETBIOS_NAME_NUM];
	char myname_idx[MAX_NETBIOS_INTERFACE_NUM];
	char myname_size[MAX_NETBIOS_INTERFACE_NUM];
	char interface_lan_or_wan_record;
}netbios_name_struct;
netbios_name_struct  netbios_name;
...
#define NBSERVER_THREAD_STACK_SIZE 0x00006000
#define NBSERVER_THREAD_PRIORITY 16
unsigned char nbserver_cmd_enable = 0;
cyg_uint8  nbserver_stack[NBSERVER_THREAD_STACK_SIZE];
cyg_handle_t nbserver_thread;
cyg_thread  nbserver_thread_object;
				...
				struct res_rec **recs, int count)
{
	...
	//printf("enter parse_alloc_res_rec\n");
...
BOOL register_name(char *inbuf,char *outbuf,char *name,struct in_addr *ip)
{
	...
	while (count--){
	 	show_nmb(outbuf);
	  	if (!send_pkt(outbuf,nmb_len(outbuf),&bcast_ip,137,SOCK_DGRAM)){
			return False;
	  	}
	  	if (receive_nmb(inbuf,30))
		{	
	...
	show_nmb(outbuf);
	return(send_pkt(outbuf,nmb_len(outbuf),&bcast_ip[netbios_name.interface_lan_or_wan_record],137,SOCK_DGRAM));
}
...
void reply_reg_request(char *inbuf,char *outbuf)
{
	...
	int i=0;
	for(i=0; i<netbios_name.myname_size[netbios_name.interface_lan_or_wan_record]; i++){
		if (name_equal(qname,netbios_name.myname[netbios_name.interface_lan_or_wan_record][(netbios_name.myname_idx[netbios_name.interface_lan_or_wan_record] + MAX_NETBIOS_NAME_NUM -i) % MAX_NETBIOS_NAME_NUM]))
			break;
	}
	if(i >= netbios_name.myname_size[netbios_name.interface_lan_or_wan_record])
	{
		return;
	}
	

	printf("Someones using my name (%s), sending negative reply\n",qname);
	...
	show_nmb(outbuf);
	send_pkt(outbuf,nmb_len(outbuf),&ip,137,SOCK_DGRAM);
...
void reply_name_query(char *inbuf,char *outbuf)
{
	...
	int tmpport;  
	char tmp_interface_lan_or_wan_record = netbios_name.interface_lan_or_wan_record;
		
	name_extract(inbuf,12,qname);

	/* if it's not my name then return */
	int i,j;
	for(j=0;j<MAX_NETBIOS_INTERFACE_NUM;j++){
		for(i=0; i<netbios_name.myname_size[j]; i++){
			if (name_equal(qname,netbios_name.myname[j][(netbios_name.myname_idx[j] + MAX_NETBIOS_NAME_NUM-i) % MAX_NETBIOS_NAME_NUM]))
				break;
		}
		if(i < netbios_name.myname_size[j])
			break;
	}
	if(j >= MAX_NETBIOS_INTERFACE_NUM)
		return;
	tmp_interface_lan_or_wan_record = j;
	/* record  new ip  */
	if(!myip[tmp_interface_lan_or_wan_record].s_addr){
		int   fd=0;
		struct ifreq ifr; 
		pstring interface_name = "";
		strcpy(interface_name, "eth0");
		if(tmp_interface_lan_or_wan_record>0){
		...
		if((ioctl(fd, SIOCGIFADDR, &ifr))>=0){  
	   		memcpy(&myip[tmp_interface_lan_or_wan_record].s_addr,&((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr.s_addr,4);
			get_broadcast(&myip[tmp_interface_lan_or_wan_record],&bcast_ip[tmp_interface_lan_or_wan_record],interface_name);
		}
		close(fd);
	}
	...
	CVAL(p,11) = 0;
	p += 12;
	memcpy(p,&myip[tmp_interface_lan_or_wan_record],4);
	p += 4;
	...
	tmpip = lastip;
	tmpport = lastport;
	send_pkt(outbuf,nmb_len(outbuf),&tmpip,tmpport,SOCK_DGRAM);
...
void reply_name_query_status(char *inbuf,char *outbuf)
{
	...
	//printf("enter reply_name_query_status\n");
	...
	CVAL(p,10) = 1; //namenum	
	p += 11;
	len = strlen(netbios_name.myname[netbios_name.interface_lan_or_wan_record][netbios_name.myname_idx[netbios_name.interface_lan_or_wan_record]]);
	pTemp = netbios_name.myname[netbios_name.interface_lan_or_wan_record][netbios_name.myname_idx[netbios_name.interface_lan_or_wan_record]];
	for(i=0;i<len;i++)
	{
	...
	tmpport = lastport;
	len = 247;
	send_pkt(outbuf,len,&tmpip,tmpport,SOCK_DGRAM);
}
...
void process(char *lookup)
{
	...
	int idx;
  	...
  	if (!reply_only && !register_name(InBuffer,OutBuffer,netbios_name.myname[netbios_name.interface_lan_or_wan_record][netbios_name.myname_idx[netbios_name.interface_lan_or_wan_record]]
		,&myip[netbios_name.interface_lan_or_wan_record]))
    {
      	return;
    }
  	if (*lookup)
...
BOOL init_structs(void )
{
	...
	///add 2013.5.23 
	memset(&ifr, 0x00, sizeof(ifr));  ;
	if(netbios_name.interface_lan_or_wan_record){
		strcpy(ifr.ifr_name, "eth0"); 
		if((ioctl(fd, SIOCGIFADDR, &ifr))>=0){
			sin = (struct sockaddr_in *)&ifr.ifr_addr;  
			memcpy(&myip[0].s_addr,&sin->sin_addr.s_addr,4);
			get_broadcast(&myip[0],&bcast_ip[0],"eth0");
		}
	}
	else{
		...
		if((ioctl(fd, SIOCGIFADDR, &ifr))>=0){
			sin = (struct sockaddr_in *)&ifr.ifr_addr;  
	   		memcpy(&myip[1].s_addr,&sin->sin_addr.s_addr,4);
			get_broadcast(&myip[1],&bcast_ip[1],"eth1");
		}
	}
	...
	if ((flag) & (IFF_RUNNING | IFF_UP)) {
		...
		sin = (struct sockaddr_in *)&ifr.ifr_addr;  
 		if (sin->sin_addr.s_addr){
   			memcpy(&myip[netbios_name.interface_lan_or_wan_record].s_addr,&sin->sin_addr.s_addr,4);
   		}
	} 
  ...
  /* Read the broadcast address from the interface */
	if (!got_bcast){
    	get_broadcast(&myip[netbios_name.interface_lan_or_wan_record],&bcast_ip[netbios_name.interface_lan_or_wan_record],mynetifname);
  	}
  	return True;
}
...
/****************************************************************************
  main program
****************************************************************************/
int nbserver_main(cyg_addrword_t data)
{
	extern FILE *dbf;
	...
	init_structs();
 	set_signal();
	if (open_sockets(is_daemon,port_num)){
		process(lookup);
      	close_sockets();
    }
	fclose(dbf);
...
int nbserver_start(unsigned int argc, unsigned char *argv[])
{
	int opt;
	extern char *optarg;
	pstring tmp_netbios_name = "";
	pstring tmp_interface_name = "";
	int i;

	printf("  enter nmbserver_startup\n");
  	
	if((argc == 1) && (!strcmp(argv[0],"-D"))){
		strcpy(tmp_interface_name,"eth0");
		strcpy(tmp_netbios_name,"rlx-ecos");
	}
	else{
		if(argc > MAX_NETBIOS_NAME_NUM * 2 + 3){
			printf("invalid input!\n Too many characters! The max num of nebios name is %d.\n",MAX_NETBIOS_NAME_NUM);
			return(-1);
		}
		if((!(argc%2)) || (argc<5) || strcmp(argv[0],"-i") || strcmp(argv[argc-1],"-D")){
			printf("invalid input!\n should be 'nmbserver -i  [eth0/eth1/lan/wan] -n [netbios-name] -n [netbios-name] ...... -D\n");
			return(-1);
		}
		for(i=0; i<(argc-3)/2; i++){
			if(strcmp(argv[2+i*2],"-n")) {
				printf("invalid input!\n should be 'nmbserver -i  [eth0/eth1/lan/wan] -n [netbios-name] -n [netbios-name] ...... -D\n");
				return(-1);
			}
		}
	}
	if((!strcmp(argv[1],"wan")) || (!strcmp(argv[1],"eth1")) ){
		netbios_name.interface_lan_or_wan_record = 1;
		strcpy(tmp_interface_name,"eth1");
	}
	else{
		netbios_name.interface_lan_or_wan_record = 0;
		strcpy(tmp_interface_name,"eth0");
	}
	memset(netbios_name.myname[netbios_name.interface_lan_or_wan_record],0,sizeof(netbios_name.myname[netbios_name.interface_lan_or_wan_record]));
	netbios_name.myname_size[netbios_name.interface_lan_or_wan_record] = 0;
	netbios_name.myname_idx[netbios_name.interface_lan_or_wan_record] = 0;
	for(i=0; i<(argc-3)/2; i++){
		strcpy(tmp_netbios_name,argv[3+i*2]);
		if(netbios_name.myname_size[netbios_name.interface_lan_or_wan_record]){
			netbios_name.myname_idx[netbios_name.interface_lan_or_wan_record]++;
			netbios_name.myname_idx[netbios_name.interface_lan_or_wan_record] %= MAX_NETBIOS_NAME_NUM;	
		}
		strcpy(netbios_name.myname[netbios_name.interface_lan_or_wan_record][netbios_name.myname_idx[netbios_name.interface_lan_or_wan_record]],tmp_netbios_name);
		if(netbios_name.myname_size[netbios_name.interface_lan_or_wan_record] < MAX_NETBIOS_NAME_NUM)
					netbios_name.myname_size[netbios_name.interface_lan_or_wan_record] ++;
	}
	printf("Interface:%s   Netbios name:", tmp_interface_name);
	for(i=0;i<netbios_name.myname_size[netbios_name.interface_lan_or_wan_record];i++){
		printf("%s ",netbios_name.myname[netbios_name.interface_lan_or_wan_record][((netbios_name.myname_idx[netbios_name.interface_lan_or_wan_record] + MAX_NETBIOS_NAME_NUM)-i)% MAX_NETBIOS_NAME_NUM]);
	}
	printf("\n");
	
	if(nbserver_cmd_enable){
	...
	{	
		is_daemon = True;
		strcpy(mynetifname,tmp_interface_name);
		
		nbserver_cmd_enable = 1;
		cyg_thread_create(NBSERVER_THREAD_PRIORITY,
		nbserver_main,
		0,
		"nbserver",
		nbserver_stack,
		sizeof(nbserver_stack),
		&nbserver_thread,
		&nbserver_thread_object);
		
		cyg_thread_resume(nbserver_thread);
		return(0);
	}
}
