...
function band_11N_enable(value)
{
	...
	if(value==7||value==11||value==71||value==75)
		return true;
	else
		return false;
}
...
function isBand2G(band_value,Band2G5GSupport)
{
	if(band_value==0
	||band_value==1
	||band_value==2
	||(band_value==7 && Band2G5GSupport==1)
	||band_value==8
	||band_value==9
	||band_value==10
	)
...
function isBand5G(band_value,Band2G5GSupport)
{
	if(band_value==3
	||(band_value==7 && Band2G5GSupport==2)
	||band_value==11
	||band_value==63
	||band_value==67
	||band_value==71
...
function updateChan(form, wlan_id)
{
	...
	if(isBand5G(band_value,Band2G5GSupport)){ // 3:5g_a 11:5g_an 7:n 2:PHYBAND_5G
		currentBand = 2;
	}
	else if(isBand2G(band_value,Band2G5GSupport)){
		currentBand = 1;
	}else if(band_value == 4 || band_value==5 || band_value==6 || band_value==14){
		currentBand = 3;
	}
...
if(form.name == "wizard")
{
	...
	if(Band2G5GSupport == 2 || wlBandMode == 3) // 2:PHYBAND_5G 3:BANDMODESIGNLE
	{
		...
		form.elements["band"+wlan_id].options[idx++] = new Option("5 GHz (A+AC)", "67", false, false);
...
function checkIPMask(field)
{
  ...
  if ( !checkMask(field.value,1) ) {
      	alert(util_gw_mask_invalid1 + util_gw_mask_rang);
	field.value = field.defaultValue;
	field.focus();
	return false;
  }
...
function checkIntSize(field,rangeStart,rangeEnd,msg)
{
	if (field != null)
	{
	     ...
	     if (!checkIntDigitValid(field.value)||
...
var ntp_server_array=new Array(7);
ntp_server_array[0]=new ntp_entry(ntp_server_north_america1,"198.123.30.132");
ntp_server_array[1]=new ntp_entry(ntp_server_north_america2,"209.249.181.22");
ntp_server_array[2]=new ntp_entry(ntp_server_Europe1,"85.12.35.12");
