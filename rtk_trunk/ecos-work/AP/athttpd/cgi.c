...
// ASP CGI Support
// ==============================================================================
#ifdef CYGOPT_NET_ATHTTPD_USE_ASP
cyg_int32 
cyg_httpd_exec_cgi_asp(void)
{
    ...
    switch (httpstate.method)
    {
	 ...
	 case CYG_HTTPD_METHOD_HEAD:
		asp_handle_GET(file_name,httpstate.inbuffer);
		break;
	 case CYG_HTTPD_METHOD_POST:
	 	asp_save_last_ip(&httpstate.host);
		asp_handle_POST(file_name,httpstate.post_data,httpstate.content_len);
		break;
	 default:
