...
#ifdef CONFIG_IRES_WEB_SUPPORT
#include "../html/cyg_ires_table.c"
#endif


#ifdef CONFIG_IRES_WEB_ADVANCED_SUPPORT
static cyg_httpd_ires_table_entry web_entry;
extern int httpd_find_ires_advanced(char *path_name, char **ptr, int *size);
cyg_httpd_ires_table_entry *
cyg_httpd_find_ires(char *path_name)
{
	if(httpd_find_ires_advanced(path_name,&web_entry.f_ptr,&web_entry.f_size)<0)
		return NULL;
	web_entry.f_pname=path_name;
	return &web_entry;
}
...
cyg_httpd_find_ires(char *path_name)
{
    ...
    {
        //diag_printf("%s %d path_name (%s)  f_pname(%s)\n",__FUNCTION__,__LINE__,path_name, entry->f_pname);
