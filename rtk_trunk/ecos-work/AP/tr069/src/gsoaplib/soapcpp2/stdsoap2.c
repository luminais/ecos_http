...
chunk_again:
	...
	/******************************************************************************/
#ifdef WITH_LEAN
	int32_t soap_get0(struct soap *m_soap)
	{ if (m_soap->bufidx >= m_soap->buflen && soap_recv(m_soap))
		return EOF;
		return (unsigned char)m_soap->buf[m_soap->bufidx];
	}
#endif

	/******************************************************************************/
#ifdef WITH_LEAN
	int32_t soap_get1(struct soap *m_soap)
	{ 
		if (m_soap->bufidx >= m_soap->buflen && soap_recv(m_soap))
			return EOF;
		return (unsigned char)m_soap->buf[m_soap->bufidx++];
...
soap_bind(struct soap *soap, const char *host, int port, int backlog)
{
	...
	printf("out soap_bind, soap->master=%d\n", soap->master);
	return soap->master;
}
#endif
#endif
...
tcp_accept(struct soap *soap, int s, struct sockaddr *a, int *n)
{
	...
	timeout_s.tv_sec = 0;
        ...
        FD_ZERO(&fd);
        FD_SET((SOAP_SOCKET)s, &fd);
        r_fd = select((SOAP_SOCKET)(s + 1), &fd, &fd, NULL, &timeout_s);// readfs, writefs, execptionfs
        printf("r_fd=%d\n", r_fd);
	...
	if(r_fd > 0)
	{
		printf("ta: 1\n");
		int ret;
		ret = (int)accept((SOAP_SOCKET)s, a, (SOAP_SOCKLEN_T*)n);
		printf("ta: ret =%d\n", ret);
		return ret;
	}else{
		printf("ta: return -1\n");
...
{
	printf("enter soap_accept, in stdsoap2.c\n");
	...
	*soap->endpoint = '\0';
	...
	SOAP_FMAC1
		...
		{ if (soap_check_state(soap))
			...
			printf("Error: soap struct not initialized\n");
			else if (soap->error)
	...
	SOAP_FMAC1
		...
		{
			printf("enter soap_register_plugin_arg\n");
			register struct soap_plugin *p;
			register int r;
