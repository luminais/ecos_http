...
void matrixX509FreeCert(sslRsaCert_t *cert)
{
	...
	while (curr) {
		...
		if (curr->extensions.keyUsage)	psFree(curr->extensions.keyUsage);/*patch from 1.8.6 version*/
...
static int32 matrixX509ValidateCertInternal(psPool_t *pool, sslRsaCert_t *subjectCert, 
						   sslRsaCert_t *issuerCert, int32 chain)
{
	printf("enter matrixX509ValidateCertInternal, issuerCert=%p\n", issuerCert);
	sslRsaCert_t	*ic;
...
*/
	if (issuerCert == NULL) {
		printf("Warning:  No CA to validate cert with\n", NULL);
		printf("\tPerforming self-signed CA test\n", NULL);
		ic = subjectCert;
	} else {
		ic = issuerCert;
	}
