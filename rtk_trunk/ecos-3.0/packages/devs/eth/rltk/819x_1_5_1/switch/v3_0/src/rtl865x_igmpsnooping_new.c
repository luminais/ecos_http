...
#if defined (CONFIG_RTL_HARDWARE_MULTICAST)||defined (CONFIG_RTL_IGMP_PROXY_KERNEL_MODE)
#include <rtl/rtl865x_eventMgr.h>
#endif


static struct rtl_multicastModule rtl_mCastModuleArray[MAX_MCAST_MODULE_NUM];    
#if 1//defined(__linux__) && defined(__KERNEL__)
//static struct timer_list igmpSysTimer;	/*igmp timer*/
static struct callout igmpSysTimer;	/*igmp timer*/
...
#elif defined(__ECOS)
static void rtl_multicastSysTimerExpired(void);
#endif
...
struct rtl_groupEntry* rtl_searchGroupEntry(uint32 moduleIndex, uint32 ipVersion,uint32 *multicastAddr)
{
	...
	int32 hashIndex=0;
	//diag_printf("module:%d,ipVersion=%d,multicastAddr:%x.[%s]:[%d].\n",moduleIndex,ipVersion,multicastAddr[0],__FUNCTION__,__LINE__);
	hashIndex=rtl_igmpHashAlgorithm(ipVersion, multicastAddr);
	
	if(ipVersion==IP_VERSION4)
	{
		
		if(rtl_mCastModuleArray[moduleIndex].rtl_ipv4HashTable)
	...
	{
		if(rtl_mCastModuleArray[moduleIndex].rtl_ipv6HashTable)
	...
	//diag_printf("not find group.[%s]:[%d].\n",__FUNCTION__,__LINE__);
	return NULL;
}
...
{	
	...
	#if 0
	rtl_process_m2uEntry(groupEntry,clientEntry);
	#endif
...
{	
	...
	if(groupEntry!=NULL)
	{
		//diag_printf("------%x,[%s]:[%d].\n",groupEntry->groupAddr[0],__FUNCTION__,__LINE__);
...
{	
	...
	{		
		portMask=(1<<clientEntry->portNum);
...
static uint32 rtl_getGroupSourceFwdPortMask(struct rtl_groupEntry * groupEntry,uint32 *sourceAddr, uint32 sysTime)
{
	...
	//diag_printf("fwdPortMask=%x[%s]:[%d].\n",fwdPortMask,__FUNCTION__,__LINE__);	
	return fwdPortMask;
}
...
static void rtl_checkGroupEntryTimer(struct rtl_groupEntry * groupEntry, struct rtl_groupEntry ** hashTable)
{
	...
	{	
		...
		{
			//strcpy(reportEventContext.devName,rtl_mCastModuleArray[moduleIndex].deviceInfo.devName);
			rtl865x_raiseEvent(EVENT_DEL_GROUP, &timerEventContext);
		}
...
static uint32 rtl_processQueries(uint32 moduleIndex,uint32 ipVersion, uint32 portNum, uint8* pktBuf, uint32 pktLen)
{
	...
	uint64 mask=1;
	uint32 fwdPortMask;
	mask=((mask<<MAX_SUPPORT_PORT_NUMBER)-1)&0xffffffff;
...
out:	
	...
	fwdPortMask=(~(1<<portNum) & ((uint32)mask));
...
static  uint32 rtl_processJoin(uint32 moduleIndex, uint32 ipVersion, uint32 portNum, uint32 *clientAddr, uint8 *pktBuf)
{
	...
	//diag_printf("=======join proess,moduleIndex:%x,portNum:%x[%s]:[%d].\n",moduleIndex,portNum,__FUNCTION__,__LINE__);
	if(ipVersion==IP_VERSION4)
	{
		if(pktBuf[0]==0x12)
		{ 
	...
	else
	{
		...
		groupAddress[2]=ntohl(((struct mldv1Pkt *)pktBuf)->mCastAddr[2]);
		groupAddress[3]=ntohl(((struct mldv1Pkt *)pktBuf)->mCastAddr[3]);
		//diag_printf("1grp:%x:%x:%x:%x\n",groupAddress[0],groupAddress[1],groupAddress[2],groupAddress[3]);
			
	}	
#endif	
	if(rtl_is_reserve_multicastAddr(groupAddress,ipVersion))
	{
		//diag_printf("2grp:%x:%x:%x:%x\n",groupAddress[0],groupAddress[1],groupAddress[2],groupAddress[3]);
		goto out;
	}
	hashIndex=rtl_igmpHashAlgorithm(ipVersion, groupAddress);
	...
	{
		//diag_printf("=======new group:%x[%s]:[%d].\n",groupAddress[0],__FUNCTION__,__LINE__);
		...
		{
			//diag_printf("[%s]:[%d].\n",__FUNCTION__,__LINE__);
			/*igmp proxy kernel mode support*/
			strcpy(reportEventContext.devName,rtl_mCastModuleArray[moduleIndex].deviceInfo.devName);
			rtl865x_raiseEvent(EVENT_ADD_GROUP, &reportEventContext);
		}
...
out:
	fwdPortMask=(((~allClientPortMask)| multicastRouterPortMask) & (~(1<<portNum))&((uint32)mask));
	//diag_printf("=======join proess,moduleIndex:%x,fwdPortMask:%x[%s]:[%d].\n",moduleIndex,fwdPortMask,__FUNCTION__,__LINE__);
	return fwdPortMask;
...
//	uint32 multicastRouterPortMask=rtl_getMulticastRouterPortMask(moduleIndex, ipVersion, rtl_sysUpSeconds);
	//diag_printf("====== Leave process.moduleIndex:%d,portNum:%d,[%s]:[%d].\n",moduleIndex,portNum,__FUNCTION__,__LINE__);
	if(ipVersion==IP_VERSION4)
	{
		groupAddress[0]=ntohl(((struct igmpv2Pkt *)pktBuf)->groupAddr);
	}
	...
	if(groupEntry!=NULL)
	{   
		//diag_printf("groupEntry exist.[%s]:[%d].\n",__FUNCTION__,__LINE__);	
		clientEntry=rtl_searchClientEntry( ipVersion, groupEntry, portNum, clientAddr);
		if(clientEntry!=NULL) 
		{
	...
	if((groupEntry!=NULL) && (groupEntry->clientList==NULL))
	{
		...
		if(ipVersion==IP_VERSION4)
		{
			...
			/*we only support ipv4 hardware multicast*/
			strcpy(reportEventContext.devName,rtl_mCastModuleArray[moduleIndex].deviceInfo.devName);
			rtl865x_raiseEvent(EVENT_DEL_GROUP, &reportEventContext);
		}
...
static uint32 rtl_processIgmpv3Mldv2Reports(uint32 moduleIndex, uint32 ipVersion, uint32 portNum,uint32 *clientAddr, uint8 *pktBuf)
{
	...
	mask=((mask<<MAX_SUPPORT_PORT_NUMBER)-1)&0xffffffff;
	//uint32 multicastRouterPortMask=rtl_getMulticastRouterPortMask(moduleIndex, ipVersion, rtl_sysUpSeconds);
	//diag_printf("======Igmpv3Mldv2 process.moduleIndex:%d,portNum:%d,[%s]:[%d].\n",moduleIndex,portNum,__FUNCTION__,__LINE__);
	if(ipVersion==IP_VERSION4)
	{
...
static void _rtl865x_configIgmpSnoopingExpire(int32 disableExpire)
{
	...
	if((rtl_mCastTimerParas.disableExpire==TRUE) && (disableExpire==FALSE) )
	{
		...
		//struct timezone tz;
		gettimeofday(&currentTimeVector,NULL);
		/*reset start time*/
...
int32 rtl_igmpMldProcess(uint32 moduleIndex, uint8 * macFrame,  uint32 portNum, uint32 *fwdPortMask)
{
	...
	*fwdPortMask=(~(1<<portNum)) & 0xFFFFFFFF;
	//diag_printf("=======moduleIndex:%d,portNum:%d.[%s]:[%d].\n",moduleIndex,portNum,__FUNCTION__,__LINE__);
...
int32 rtl_getMulticastDataFwdInfo(uint32 moduleIndex, struct rtl_multicastDataInfo *multicastDataInfo, struct rtl_multicastFwdInfo *multicastFwdInfo)
{
	...
	//diag_printf("udp pkt fwd.module:%d.[%s]:[%d].\n",moduleIndex,__FUNCTION__,__LINE__);
	if(moduleIndex>=MAX_MCAST_MODULE_NUM)
	{
		return FAILED;
	}
	...
	//diag_printf("============moduleIndex:%d,grp=%x.[%s]:[%d]!\n",moduleIndex,multicastDataInfo->groupAddr[0],__FUNCTION__,__LINE__);
	if(groupEntry==NULL)
	{
		//diag_printf("============groupEntry==NULL.[%s]:[%d]!\n",__FUNCTION__,__LINE__);
...
//ecos timer is different
#if 1//defined(__linux__) && defined(__KERNEL__)
#if defined(__linux__) && defined(__KERNEL__)

static void rtl_multicastSysTimerExpired(uint32 expireDada)
#elif defined(__ECOS)
static void rtl_multicastSysTimerExpired()
#endif

{
	...
	callout_reset(&igmpSysTimer,1*HZ,rtl_multicastSysTimerExpired,NULL);
}
...
#elif defined(__ECOS)
	callout_init(&igmpSysTimer);
	callout_reset(&igmpSysTimer,1*HZ,rtl_multicastSysTimerExpired,NULL);

//	init_timer(&igmpSysTimer, igmpSysTimer.data, (void*)rtl_multicastSysTimerExpired);
...
int igmp_show(void)
{
	...
	for(moduleIndex=0; moduleIndex<MAX_MCAST_MODULE_NUM ;moduleIndex++)
	{
		...
		{
			diag_printf("-------------------------------------------------------------------------\n");
			diag_printf("module index:%d, ",moduleIndex);
			#ifdef CONFIG_RTL_HARDWARE_MULTICAST
			diag_printf("device:%s, portMask:0x%x,",rtl_mCastModuleArray[moduleIndex].deviceInfo.devName,rtl_mCastModuleArray[moduleIndex].deviceInfo.portMask);
			#endif
			diag_printf("ipv4[0x%x]",rtl_mCastModuleArray[moduleIndex].ipv4UnknownMCastFloodMap);
			#if defined (CONFIG_RTL_MLD_SNOOPING)	
			diag_printf("ipv6[0x%x]\n",rtl_mCastModuleArray[moduleIndex].ipv6UnknownMCastFloodMap);
			#endif
			diag_printf("\n");
			diag_printf("igmp list:\n");
			groupCnt=0;	
			for(hashIndex=0;hashIndex<rtl_hashTableSize;hashIndex++)
				...
				{
					groupCnt++;
					diag_printf("    [%d] Group address:%d.%d.%d.%d",groupCnt,//hashIndex
					...
					{
						diag_printf(" <static> portMask:0x%x",groupEntryPtr->staticFwdPortMask);
					}
					#endif
					diag_printf("\n");
					...
					{	
						...
						clientCnt++;
						diag_printf( "        <%d>%d.%d.%d.%d\\port %d\\IGMPv%d\\",clientCnt,
						...
						diag_printf( "%s",(clientEntry->groupFilterTimer>rtl_sysUpSeconds)?"EXCLUDE":"INCLUDE");
						if(clientEntry->groupFilterTimer>rtl_sysUpSeconds)
						{
							diag_printf( ":%ds",clientEntry->groupFilterTimer-rtl_sysUpSeconds);
						}
						else
						{
							diag_printf( ":0s");
						...
						while(sourceEntryPtr!=NULL)
						{
							...
							{
								diag_printf( "%ds",sourceEntryPtr->portTimer-rtl_sysUpSeconds);
							}
							else
							{
								diag_printf( "0s");
			...
			for(hashIndex=0;hashIndex<rtl_hashTableSize;hashIndex++)
				...
				{
					groupCnt++;	
					diag_printf( "    [%d] Group address:%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x",groupCnt,
					...
					{	
						...
						clientCnt++;
						diag_printf( "        <%d>%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x\\port %d\\MLDv%d\\",clientCnt,
						...
						while(sourceEntryPtr!=NULL)
						{
							diag_printf( "%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x-%x%x%x%x%x%x%x%x:",
...
void igmp_cmd_op(int argc, char *argv[])
{
	#if NBRIDGE > 0
	...
	if (argc >= 2)
	{
		...
		if ((strcmp(argv[1], "read")==0))
		{
			...
			{
				#if defined (CONFIG_RTL_MLD_SNOOPING)
				diag_printf("\nigmp:%d  mld:%d\n",igmpsnoopenabled,mldSnoopEnabled);
				#else
				diag_printf("\nigmp:%d\n",igmpsnoopenabled);
				#endif
			}
			#if defined CONFIG_RTL_FREEBSD_FAST_PATH
			else if ((strcmp(argv[2], "fastfwd")==0))
			{
				#if NBRIDGE > 0
				diag_printf("\nipMulticastFastFwd:%d  needCheckMfc:%d\n",ipMulticastFastFwd,needCheckMfc);
				#else
				diag_printf("\n not support! \n");
				#endif
			}
			...
			#if defined (CONFIG_RTL_BRIDGE_QUERY_SUPPORT)				
				#if NBRIDGE > 0
				igmpQueryEnabled=rtl_get_igmpQueryEnabled();
				diag_printf("\nigmp query:%d \n",igmpQueryEnabled);
				#endif
				#if defined (CONFIG_RTL_MLD_SNOOPING)	
				mldQueryEnabled=rtl_get_mldQueryEnabled();
				diag_printf("mld query:%d \n",mldQueryEnabled);
				#endif
			#else
				diag_printf("\n not support! \n");
			#endif
		...
		else if ((strcmp(argv[1], "write")==0))
		{
			if(argc > 4)
			{
				...
				if ((strcmp(argv[2], "enable")==0))
				{
					igmpsnoopenabled=strtol(argv[3], NULL, 0);
					#if defined (CONFIG_RTL_MLD_SNOOPING)
					mldSnoopEnabled=strtol(argv[4], NULL, 0);
					diag_printf("igmp:%d  mld:%d\n",igmpsnoopenabled,mldSnoopEnabled);
					#else
					diag_printf("igmp:%d\n",igmpsnoopenabled);
					#endif
				}
				...
				{
					#if NBRIDGE > 0
					ipMulticastFastFwd=strtol(argv[3], NULL, 0);
					needCheckMfc=strtol(argv[4], NULL, 0);
					diag_printf("ipMulticastFastFwd:%d  needCheckMfc:%d\n",ipMulticastFastFwd,needCheckMfc);
					#else
					diag_printf("\n not support! \n");
					#endif
				}
				...
				#if defined (CONFIG_RTL_BRIDGE_QUERY_SUPPORT)
					#if NBRIDGE > 0
					igmpQueryEnabled=strtol(argv[3], NULL, 0);
					rtl_set_igmpQueryEnabled(igmpQueryEnabled);					
					#endif
					
					#if defined (CONFIG_RTL_MLD_SNOOPING)	
					mldQueryEnabled=strtol(argv[4], NULL, 0);
					rtl_set_mldQueryEnabled(mldQueryEnabled);
					
					diag_printf("igmpQueryEnabled:%d,mldQueryEnabled:%d\n",igmpQueryEnabled,mldQueryEnabled);
					#else
					diag_printf("igmpQueryEnabled:%d\n",igmpQueryEnabled);
					#endif
				#else
					diag_printf("\n not support! \n");
				#endif
				}
				else if((strcmp(argv[2], "unknown")==0))
				{
					ipVersion = strtoul(argv[3], NULL, 0);
					portMask = strtoul(argv[4], NULL, 0);
					for(moduleIndex=0; moduleIndex<MAX_MCAST_MODULE_NUM ;moduleIndex++)
					{
						if(rtl_mCastModuleArray[moduleIndex].enableSnooping==TRUE)
						{
							if(ipVersion == IP_VERSION4)
								rtl_setIpv4UnknownMCastFloodMap(moduleIndex,portMask);
						#if defined (CONFIG_RTL_MLD_SNOOPING)	
							else if(ipVersion == IP_VERSION6)
								rtl_setIpv6UnknownMCastFloodMap(moduleIndex,portMask);
						#endif
						}					
					}
				}
		...
		{
			diag_printf("eth igmp read enable/fastfwd/query/entry\n");
			diag_printf("eth igmp write enable igmpEnable mldEnable\n");
			diag_printf("eth igmp write fastfwd fastFwd checkMfc\n");
			diag_printf("eth igmp write query igmpQuery mldQuery\n");
			diag_printf("eth igmp write unknown ipVersion portMask\n");
		}
	}
	else
	{
...
int32 rtl_getGroupInfov6(uint32 * groupAddr,struct rtl_groupInfo * groupInfo)
{
	...
	for(moduleIndex=0; moduleIndex<MAX_MCAST_MODULE_NUM ;moduleIndex++)
	{
		if(rtl_mCastModuleArray[moduleIndex].enableSnooping==TRUE)
		{
			hashIndex=rtl_igmpHashAlgorithm(IP_VERSION6, groupAddr);
			if(rtl_mCastModuleArray[moduleIndex].rtl_ipv6HashTable != NULL)
				groupEntryPtr=rtl_mCastModuleArray[moduleIndex].rtl_ipv6HashTable[hashIndex];
			else
				groupEntryPtr = NULL;
