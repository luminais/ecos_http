 ...
 */
#ifdef __ECOS
#if defined(CONFIG_RTL_8196C) ||defined(CONFIG_RTL_8197F)
...
#if defined(CONFIG_RTL_VLAN_SUPPORT) || defined(CONFIG_RTL_DNS_TRAP)
...
#if defined(CONFIG_RTL819X_GPIO)
int gpio_simulate_mdc_mdio = 0;
#endif
#endif

#if defined(CONFIG_RTL_8197F)
int init_p0_for_8211f(void)
{
	unsigned int data;
...
void tenda_reset_ports(void)
{
	...
	for(i = 0; i < 5; i++) {
		statCtrlReg0 = 0;
		rtl8367b_getAsicPHYReg(i, 0x0, &statCtrlReg0);
		statCtrlReg0 |= RESTART_AUTONEGO;
		rtl8367b_setAsicPHYReg(i, 0x0, statCtrlReg0);
	}
#else
    for(i = 0; i < 5; i++) {
...
#if defined(CONFIG_RTL8196C_REVISION_B) || defined(CONFIG_RTL8198) || defined(CONFIG_RTL_819XD) || defined(CONFIG_RTL_8196E) || defined(CONFIG_RTL_8881A) ||defined(CONFIG_RTL_8197F)
...
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
		if (i == 0)
			wphyid = 8;//RTL8198C_PORT0_PHY_ID;
...
void rtl_add_Acl_for_br_subnet_from_wan(int index);
#endif
int32 swCore_init(int VlanCount)
{
	...
	*/
	...
	for (port=0; port<=CPU; port++)
		rtl8651_setAsicPortBasedFlowControlRegister(PHY0+port, rtl_portFCON, rtl_portFCOFF);
#endif
...
#elif defined(CONFIG_RTL8198) || defined(CONFIG_RTL_819XD) || defined(CONFIG_RTL_8196E) || defined(CONFIG_RTL_8881A) ||defined(CONFIG_RTL_8197F)
...
#if defined(CONFIG_RTL_8211F_SUPPORT)
	{
#if defined(CONFIG_RTL_8197F) && defined(CONFIG_RTL819X_GPIO)
		int ret;
...
#elif defined(CONFIG_RTL_8881A) && defined(CONFIG_RTL_8211E_SUPPORT)
	init_p0();
...
//	WRITE_MEM32( FFCR, READ_MEM32( FFCR ) | EN_MCAST | EN_UNUNICAST_TOCPU | EN_UNMCAST_TOCPU);
 ...
 /* Init PHY LED style */
	...
	*/	
	...
	#endif
	REG32(LEDCR) = (REG32(LEDCR) & ~LEDTOPOLOGY_MASK) | LEDMODE_DIRECT;	
#else
#ifdef BICOLOR_LED
	...
	for(port=0;port<MAX_PORT_NUMBER;port++)
	{
	...
	#endif
		{
			#if defined(CONFIG_RTL_8211F_SUPPORT)
			if (port == 0)
				rtl8651_restartAsicEthernetPHYNway(port+1, PORT0_RGMII_PHYID);	
			else
			#endif
	...
	REG32(SYS_CLK_MAG) |= (CM_ACTIVE_LX2_CLK | CM_ACTIVE_LX2_ARB);
    	REG32(CPUICR1) |= ((1 << CF_NIC_LITTLE_ENDIAN_OFFSET) | (1 << CF_TXRX_DIV_LX_OFFSET));
...
void rtl_add_Acl_for_br_subnet_from_wan(int index)
{
	...
	ap.srcIpAddr_ 		= ntohl(_br0_ip);
    ap.srcIpAddrMask_	= ntohl(_br0_mask);
	ap.dstIpAddr_ 		= ntohl(_br0_ip);
    ap.dstIpAddrMask_	= ntohl(_br0_mask);
...
void showAclRule(void)
{
...
#if defined(RTL_REFINE_ACL)	
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
	rtl_acl_param_t asic_acl2;
	uint32 acl_temp = 0;
	inv6_addr_t start_ip, end_ip;
#endif
#endif
	uint32 lan_end_index = 0, wan_end_index = 0;
	...
	lan_end_index += 2;
	#endif
	for(index=0; index<lan_end_index; index++)
	{
		...
		case RTL865X_ACL_IP_RANGE:
			diag_printf( " [%d] rule type: %s	 rule action: %s\n", asic_acl.aclIdx, "IP Range", actionT[asic_acl.actionType_]);
			diag_printf("\tdipU: %d.%d.%d.%d dipL: %d.%d.%d.%d\n", (asic_acl.dstIpAddr_>>24),
					((asic_acl.dstIpAddr_&0x00ff0000)>>16), ((asic_acl.dstIpAddr_&0x0000ff00)>>8),
					(asic_acl.dstIpAddr_&0xff), (asic_acl.dstIpAddrMask_>>24), ((asic_acl.dstIpAddrMask_&0x00ff0000)>>16),
					((asic_acl.dstIpAddrMask_&0x0000ff00)>>8), (asic_acl.dstIpAddrMask_&0xff)
					);
			diag_printf( "\tsipU: %d.%d.%d.%d sipL: %d.%d.%d.%d\n", (asic_acl.srcIpAddr_>>24),
					((asic_acl.srcIpAddr_&0x00ff0000)>>16), ((asic_acl.srcIpAddr_&0x0000ff00)>>8),
					(asic_acl.srcIpAddr_&0xff), (asic_acl.srcIpAddrMask_>>24), ((asic_acl.srcIpAddrMask_&0x00ff0000)>>16),
					((asic_acl.srcIpAddrMask_&0x0000ff00)>>8), (asic_acl.srcIpAddrMask_&0xff)
					);
		...
		case RTL865X_ACL_ICMP:
			diag_printf(" [%d] rule type: %s	 rule action: %s\n", asic_acl.aclIdx, "ICMP", actionT[asic_acl.actionType_]);
			diag_printf("\tdip: %d.%d.%d.%d dipM: %d.%d.%d.%d\n", (asic_acl.dstIpAddr_>>24),
					((asic_acl.dstIpAddr_&0x00ff0000)>>16), ((asic_acl.dstIpAddr_&0x0000ff00)>>8),
					(asic_acl.dstIpAddr_&0xff), (asic_acl.dstIpAddrMask_>>24), ((asic_acl.dstIpAddrMask_&0x00ff0000)>>16),
					((asic_acl.dstIpAddrMask_&0x0000ff00)>>8), (asic_acl.dstIpAddrMask_&0xff)
			...
			diag_printf("\tTos: %x   TosM: %x   type: %x	 typeM: %x	 code: %x	codeM: %x\n",
		...
		case RTL865X_ACL_ICMP_IPRANGE:
			diag_printf(" [%d] rule type: %s	 rule action: %s\n", asic_acl.aclIdx, "ICMP IP RANGE", actionT[asic_acl.actionType_]);
			diag_printf("\tdipU: %d.%d.%d.%d dipL: %d.%d.%d.%d\n", (asic_acl.dstIpAddr_>>24),
					((asic_acl.dstIpAddr_&0x00ff0000)>>16), ((asic_acl.dstIpAddr_&0x0000ff00)>>8),
					(asic_acl.dstIpAddr_&0xff), (asic_acl.dstIpAddrMask_>>24), ((asic_acl.dstIpAddrMask_&0x00ff0000)>>16),
					((asic_acl.dstIpAddrMask_&0x0000ff00)>>8), (asic_acl.dstIpAddrMask_&0xff)
		...
		case RTL865X_ACL_IGMP:
			diag_printf(" [%d] rule type: %s	 rule action: %s\n", asic_acl.aclIdx, "IGMP", actionT[asic_acl.actionType_]);
			diag_printf("\tdip: %d.%d.%d.%d dipM: %d.%d.%d.%d\n", (asic_acl.dstIpAddr_>>24),
					((asic_acl.dstIpAddr_&0x00ff0000)>>16), ((asic_acl.dstIpAddr_&0x0000ff00)>>8),
					(asic_acl.dstIpAddr_&0xff), (asic_acl.dstIpAddrMask_>>24), ((asic_acl.dstIpAddrMask_&0x00ff0000)>>16),
					((asic_acl.dstIpAddrMask_&0x0000ff00)>>8), (asic_acl.dstIpAddrMask_&0xff)
			...
			diag_printf("\tTos: %x   TosM: %x   type: %x	 typeM: %x\n", asic_acl.tos_, asic_acl.tosMask_,
		...
		case RTL865X_ACL_IGMP_IPRANGE:
			diag_printf(" [%d] rule type: %s	 rule action: %s\n", asic_acl.aclIdx, "IGMP IP RANGE", actionT[asic_acl.actionType_]);
			diag_printf("\tdip: %d.%d.%d.%d dipM: %d.%d.%d.%d\n", (asic_acl.dstIpAddr_>>24),
					((asic_acl.dstIpAddr_&0x00ff0000)>>16), ((asic_acl.dstIpAddr_&0x0000ff00)>>8),
					(asic_acl.dstIpAddr_&0xff), (asic_acl.dstIpAddrMask_>>24), ((asic_acl.dstIpAddrMask_&0x00ff0000)>>16),
					((asic_acl.dstIpAddrMask_&0x0000ff00)>>8), (asic_acl.dstIpAddrMask_&0xff)
		...
		case RTL865X_ACL_TCP:
			diag_printf(" [%d] rule type: %s	 rule action: %s\n", asic_acl.aclIdx, "TCP", actionT[asic_acl.actionType_]);
			diag_printf("\tdip: %d.%d.%d.%d dipM: %d.%d.%d.%d\n", (asic_acl.dstIpAddr_>>24),
					((asic_acl.dstIpAddr_&0x00ff0000)>>16), ((asic_acl.dstIpAddr_&0x0000ff00)>>8),
					(asic_acl.dstIpAddr_&0xff), (asic_acl.dstIpAddrMask_>>24), ((asic_acl.dstIpAddrMask_&0x00ff0000)>>16),
					((asic_acl.dstIpAddrMask_&0x0000ff00)>>8), (asic_acl.dstIpAddrMask_&0xff)
			...
			diag_printf("\tTos:%x  TosM:%x  sportL:%d  sportU:%d	dportL:%d  dportU:%d\n",
			...
			diag_printf("\tflag: %x  flagM: %x  <URG:%x> <ACK:%x> <PSH:%x> <RST:%x> <SYN:%x> <FIN:%x>\n",
		...
		case RTL865X_ACL_TCP_IPRANGE:
				diag_printf(" [%d] rule type: %s	 rule action: %s\n", asic_acl.aclIdx, "TCP IP RANGE", actionT[asic_acl.actionType_]);
				diag_printf("\tdipU: %d.%d.%d.%d dipL: %d.%d.%d.%d\n", (asic_acl.dstIpAddr_>>24),
					((asic_acl.dstIpAddr_&0x00ff0000)>>16), ((asic_acl.dstIpAddr_&0x0000ff00)>>8),
					(asic_acl.dstIpAddr_&0xff), (asic_acl.dstIpAddrMask_>>24), ((asic_acl.dstIpAddrMask_&0x00ff0000)>>16),
					((asic_acl.dstIpAddrMask_&0x0000ff00)>>8), (asic_acl.dstIpAddrMask_&0xff)
		...
		case RTL865X_ACL_UDP:
			diag_printf(" [%d] rule type: %s	 rule action: %s\n", asic_acl.aclIdx,"UDP", actionT[asic_acl.actionType_]);
			diag_printf("\tdip: %d.%d.%d.%d dipM: %d.%d.%d.%d\n", (asic_acl.dstIpAddr_>>24),
					((asic_acl.dstIpAddr_&0x00ff0000)>>16), ((asic_acl.dstIpAddr_&0x0000ff00)>>8),
					(asic_acl.dstIpAddr_&0xff), (asic_acl.dstIpAddrMask_>>24), ((asic_acl.dstIpAddrMask_&0x00ff0000)>>16),
					((asic_acl.dstIpAddrMask_&0x0000ff00)>>8), (asic_acl.dstIpAddrMask_&0xff)
		...
		case RTL865X_ACL_UDP_IPRANGE:
			diag_printf(" [%d] rule type: %s	 rule action: %s\n", asic_acl.aclIdx, "UDP IP RANGE", actionT[asic_acl.actionType_]);
			diag_printf("\tdipU: %d.%d.%d.%d dipL: %d.%d.%d.%d\n", (asic_acl.dstIpAddr_>>24),
					((asic_acl.dstIpAddr_&0x00ff0000)>>16), ((asic_acl.dstIpAddr_&0x0000ff00)>>8),
					(asic_acl.dstIpAddr_&0xff), (asic_acl.dstIpAddrMask_>>24), ((asic_acl.dstIpAddrMask_&0x00ff0000)>>16),
					((asic_acl.dstIpAddrMask_&0x0000ff00)>>8), (asic_acl.dstIpAddrMask_&0xff)
		...
		case RTL865X_ACL_SRCFILTER:
			diag_printf(" [%d] rule type: %s	 rule action: %s\n", asic_acl.aclIdx, "Source Filter", actionT[asic_acl.actionType_]);
			diag_printf("\tSMAC: %x:%x:%x:%x:%x:%x  SMACM: %x:%x:%x:%x:%x:%x\n",
			...
			diag_printf("\tsvidx: %d	 svidxM: %x   sport: %d   sportM: %x   ProtoType: %x\n",
		...
		case RTL865X_ACL_DSTFILTER:
			diag_printf(" [%d] rule type: %s	 rule action: %s\n", asic_acl.aclIdx, "Deatination Filter", actionT[asic_acl.actionType_]);
			diag_printf("\tDMAC: %x:%x:%x:%x:%x:%x  DMACM: %x:%x:%x:%x:%x:%x\n",
			...
			diag_printf("\tdvidx: %d	 dvidxM: %x  ProtoType: %x	 dportL: %d   dportU: %d\n",
		...
		case RTL865X_ACL_DSTFILTER_IPRANGE:
			diag_printf(" [%d] rule type: %s	 rule action: %s\n", asic_acl.aclIdx, "Deatination Filter(IP Range)", actionT[asic_acl.actionType_]);
			diag_printf("\tDMAC: %x:%x:%x:%x:%x:%x  DMACM: %x:%x:%x:%x:%x:%x\n",
					asic_acl.dstFilterMac_.octet[0], asic_acl.dstFilterMac_.octet[1], asic_acl.dstFilterMac_.octet[2],
					asic_acl.dstFilterMac_.octet[3], asic_acl.dstFilterMac_.octet[4], asic_acl.dstFilterMac_.octet[5],
					asic_acl.dstFilterMacMask_.octet[0], asic_acl.dstFilterMacMask_.octet[1], asic_acl.dstFilterMacMask_.octet[2],
			...
			case RTL865X_ACL_IPV6_RANGE:
				/* A ipv6 rule occupied two entry, 
				   function rtl8651_getAsicAclRule take one entry at a time.
				   So we need to call function rtl8651_getAsicAclRule again. */

				acl_temp = index;			 
				acl_temp++; 			/* move to the second entry index of ipv6 rule */
				//need refine?
				if (acl_temp<=lan_end_index)	/* it should be less than or equal to acl_end. */
				{
					memset(&asic_acl2, 0x00, sizeof(rtl_acl_param_t));
					if ( rtl8651_getAsicAclRule(acl_temp, &asic_acl2) == FAILED)
						diag_printf("=============%s(%d): get asic acl rule error!\n",__FUNCTION__, __LINE__);
					if ((!asic_acl2.ipv6EntryType_) && asic_acl.ipv6EntryType_)
					{
						...
						diag_printf(" [%d-%d] rule type: %s	rule action: %s\n", index, acl_temp, "IPv6", actionT[asic_acl.actionType_]);
						if (RTL865X_ACL_IPV6 == asic_acl.ruleType_)
						{
							...
							diag_printf("\tsip: %x:%x:%x:%x:%x:%x:%x:%x  sipM: %x:%x:%x:%x:%x:%x:%x:%x\n",
							...
							diag_printf("\tdip: %x:%x:%x:%x:%x:%x:%x:%x  dipM: %x:%x:%x:%x:%x:%x:%x:%x\n",
									htons(start_ip.v6_addr16[0]),htons( start_ip.v6_addr16[1]), 
									htons(start_ip.v6_addr16[2]), htons(start_ip.v6_addr16[3]),
									htons(start_ip.v6_addr16[4]), htons(start_ip.v6_addr16[5]),
									htons(start_ip.v6_addr16[6]), htons(start_ip.v6_addr16[7]),
						...
						else if (RTL865X_ACL_IPV6_RANGE == asic_acl.ruleType_)
						{
							...
							diag_printf("\tsipLB: %x:%x:%x:%x:%x:%x:%x:%x  sipUB: %x:%x:%x:%x:%x:%x:%x:%x\n",
									 htons(start_ip.v6_addr16[0]),	htons(start_ip.v6_addr16[1]), 
									 htons(start_ip.v6_addr16[2]),	htons(start_ip.v6_addr16[3]),
									 htons(start_ip.v6_addr16[4]),	htons(start_ip.v6_addr16[5]),
									 htons(start_ip.v6_addr16[6]),	htons(start_ip.v6_addr16[7]),
							...
							diag_printf("\tdipLB: %x:%x:%x:%x:%x:%x:%x:%x  dipUB: %x:%x:%x:%x:%x:%x:%x:%x\n",
								 htons(start_ip.v6_addr16[0]),	htons(start_ip.v6_addr16[1]), 
								 htons(start_ip.v6_addr16[2]),	htons(start_ip.v6_addr16[3]),
								 htons(start_ip.v6_addr16[4]),	htons(start_ip.v6_addr16[5]),
								 htons(start_ip.v6_addr16[6]),	htons(start_ip.v6_addr16[7]),
			...
			case RTL865X_ACL_PM: 
				...
				if (acl_temp<=lan_end_index)	/* it should be less than or equal to acl_end. */
				{
					...
					if ((!asic_acl2.ipv6EntryType_) && asic_acl.ipv6EntryType_)
					{
						...
						diag_printf(" [%d-%d] rule type: %s	rule action: %s\n", index, acl_temp, "Pattern match", actionT[asic_acl.actionType_]);
						diag_printf("\tsportU: %d sportL: %d dportU: %d dportL: %d protocol: %d\n",
						...
						diag_printf("\tInvert: %d   Combine1: %d	 Combine2: %d	IPtunnel1: %d IPtunnel2: %d IpfragApply: %d\n",
		...
		switch (asic_acl.actionType_)
		{
			...
			case RTL865X_ACL_REDIRECT_PPPOE:
			case RTL865X_ACL_MIRROR_KEEP_MATCH:
				diag_printf("\tnetifIdx: %d   pppoeIdx: %d   l2Idx:%d  ", asic_acl.netifIdx_, asic_acl.pppoeIdx_, asic_acl.L2Idx_);
				break;

			case RTL865X_ACL_PRIORITY:
				diag_printf("\tprioirty: %d   ", asic_acl.priority_) ;
				break;

			case RTL865X_ACL_DEFAULT_REDIRECT:
				diag_printf("\tnextHop:%d  ", asic_acl.nexthopIdx_);
				break;
			...
			case RTL865X_ACL_DROP_RATE_EXCEED_BPS:
			case RTL865X_ACL_LOG_RATE_EXCEED_BPS:
				diag_printf("\tratelimitIdx: %d  ", asic_acl.ratelimtIdx_);
				break;
#if defined(CONFIG_RTL_8198C) || defined(CONFIG_RTL_8197F)
            case RTL865X_ACL_VID:
				diag_printf("\taclvid: %d  ", asic_acl.aclvid_);
                break;
#endif
			default:
				;
	...
	wan_end_index += 2;
	#endif
	for(index=10; index<wan_end_index; index++)
	{
		rtl8651_getAsicAclRule(index, &asic_acl);
		diag_printf("%s\n", "	Lan Netif Acl Rules:");
		switch(asic_acl.ruleType_)
		...
		case RTL865X_ACL_DSTFILTER_IPRANGE:
			...
			case RTL865X_ACL_IPV6_RANGE:
				...
				acl_temp++; 			/* move to the second entry index of ipv6 rule */
				//need refine?
				if (acl_temp<=wan_end_index)	/* it should be less than or equal to acl_end. */
				{
					memset(&asic_acl2, 0x00, sizeof(rtl_acl_param_t));
					if ( rtl8651_getAsicAclRule(acl_temp, &asic_acl2) == FAILED)
						diag_printf("=============%s(%d): get asic acl rule error!\n",__FUNCTION__, __LINE__);
...
int32 add_acl_for_dns_trap(uint32 index)
{
	int ret = 0, id = 0;
	rtl_acl_param_t rule;			
...
void acl_rule_test(void)
{
	...
	#if 0
	//ip rule
	ap.actionType_ = RTL865X_ACL_DROP;
	ap.ruleType_ = RTL865X_ACL_IP;
	ap.srcIpAddr_ = (192 << 24)|(168<<16)|(1<<8)|121;
	ap.srcIpAddrMask_ = 0xFFFFFFFF;
	...
	//tcp rule
	ap.actionType_ = RTL865X_ACL_DROP;
	ap.ruleType_ = RTL865X_ACL_TCP;
	ap.srcIpAddr_ = (192 << 24)|(168<<16)|(1<<8)|121;
	ap.srcIpAddrMask_ = 0xFFFFFFFF;
	ap.un_ty.L3L4.is.tcp._dstPortUpperBound = 80;
	ap.un_ty.L3L4.is.tcp._dstPortUpperBound = 80;

	ap.un_ty.L3L4.is.tcp._srcPortUpperBound = 65535;
	ap.un_ty.L3L4.is.tcp._srcPortLowerBound = 0;
	ap.pktOpApp = RTL865XC_ACLTBL_ALL_LAYER;
	#endif
	...
	//udp rule
	ap.actionType_ = RTL865X_ACL_DROP;
	ap.ruleType_ = RTL865X_ACL_UDP;
	ap.srcIpAddr_ = (192 << 24)|(168<<16)|(1<<8)|121;
	ap.srcIpAddrMask_ = 0xFFFFFFFF;
	ap.un_ty.L3L4.is.udp._dstPortUpperBound = 1000;
	ap.un_ty.L3L4.is.udp._dstPortLowerBound = 1000;

	ap.un_ty.L3L4.is.udp._srcPortUpperBound = 65535;
	ap.un_ty.L3L4.is.udp._srcPortLowerBound = 0;
	ap.pktOpApp = RTL865XC_ACLTBL_ALL_LAYER;
	#endif

	ret = swCore_aclCreate(10, &ap);
