...
static int32 _rtl865x_addNaptConnection(rtl865x_napt_entry *naptEntry, rtl865x_priority *prio)
{
	...
	#if defined(CONFIG_RTL_REDUCE_MEMORY)
	nat_tuple.in_host_ip			= htonl(naptEntry->intIp);
	nat_tuple.in_host_port			= htons((uint16)(naptEntry->intPort));
	nat_tuple.ext_host_ip			= htonl(naptEntry->extIp);
	nat_tuple.ext_host_port			= htons((uint16)(naptEntry->extPort));
	nat_tuple.rem_host_ip			= htonl(naptEntry->remIp);
	nat_tuple.rem_host_port		= htons((uint16)(naptEntry->remPort));
	...
	in = rtl8651_naptTcpUdpTableIndex((uint8)naptEntry->protocol, htonl(naptEntry->remIp), htons((uint16)(naptEntry->remPort)), htonl(naptEntry->extIp), htons((uint16)(naptEntry->extPort)));
...
void rtl865x_asicNaptShow(void)
{
	...
	for(idx=0; idx<RTL8651_TCPUDPTBL_SIZE; idx++) {
		...
		if (asic_tcpudp.isValid == 1 || asic_tcpudp.isDedicated == 1 ) {
			...
			p_ip = (uint8 *)&(asic_tcpudp.insideLocalIpAddr);
