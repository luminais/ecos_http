...
#if defined(CONFIG_RTL_8197F) && defined(CONFIG_RTL_MLD_SNOOPING) && defined(CONFIG_RTL_HARDWARE_MULTICAST)
typedef struct rtl819x_mcast_fwd_descriptor6_s{
	char netifName[16];
	unsigned int descPortMask;
	char toCpu;
	unsigned int vid;
	unsigned int fwdPortMask;
	MC_LIST_ENTRY(rtl819x_mcast_fwd_descriptor6_s)	next;
	
}rtl819x_mcast_fwd_descriptor6_t;
typedef MC_LIST_HEAD(mcast_fwd_descriptor_head6_s, rtl819x_mcast_fwd_descriptor6_s)  mcast_fwd_descriptor_head6_t;
typedef struct rtl819x_tblDrv_mCastv6_s {
	unsigned int sip[4];
	unsigned int dip[4];
	uint16 	port;
	uint32	mbr;
	...
	TAILQ_ENTRY(rtl819x_tblDrv_mCastv6_s) nextMCast;	
} rtl819x_tblDrv_mCastv6_t;
struct rtl819x_multicastv6Table{
	struct freeList6_s {
		
		TAILQ_HEAD( _FreeMultiCastEntry6, rtl819x_tblDrv_mCastv6_s) freeMultiCast;
	} freeList;

	struct inuseList6_s {
		TAILQ_HEAD( _InuseMCast6, rtl819x_tblDrv_mCastv6_s) *mCastTbl;
	}inuseList;
};
typedef struct rtl819x_mCastv6Config_s{
	unsigned int externalPortMask;	
}rtl819x_mCastv6Config_t;
#endif
#endif
