...
#define DEBUG_PSK
#define PKS_PRINT(_fmt, arg...)    TD_DBG(priv,TD_DBG_PSK_STR,_fmt, ##arg)
#define DEBUG_HEX(a,b,c)   do{ \
    if(priv->tenda_mib.debug & TD_DBG_PSK_HEX)  \
        debug_out(a,b,c);   \
}while(0)
#else
#define PKS_PRINT   panic_printk
#define DEBUG_HEX(a,b,c) debug_out(a,b,c)
#endif
...
#ifdef DEBUG_PSK
	PKS_PRINT("PSK: ParseIE -> WPA UnicastCipher=%x\n", pInfo->UnicastCipher);
#endif
...
#ifdef DEBUG_PSK
	PKS_PRINT("PSK: ParseIE -> WPA2 UnicastCipher=%x\n", pInfo->UnicastCipher);
#endif
...
#ifdef DEBUG_PSK
	PKS_PRINT("PSK: Issue assoc-rsp [%x], mac:%02x:%02x:%02x:%02x:%02x:%02x\n",
...
#ifdef DEBUG_PSK
	PKS_PRINT("PSK: Remove PTK, mac: %02x:%02x:%02x:%02x:%02x:%02x\n",
...
#ifdef DEBUG_PSK
	PKS_PRINT("PSK: Set PTK, mac: %02x:%02x:%02x:%02x:%02x:%02x\n",
...
#ifdef DEBUG_PSK
	DEBUG_HEX(NULL, Set_Key.KeyMaterial, ulKeyLength);
#endif
...
#ifdef DEBUG_PSK
	PKS_PRINT("PSK: Set PORT [%x], mac: %02x:%02x:%02x:%02x:%02x:%02x\n",
...
#ifdef DEBUG_PSK
	DEBUG_HEX("PSK: Set RSNIE", priv->wpa_global_info->AuthInfoElement.Octet,
...
#ifdef DEBUG_PSK
	PKS_PRINT("PSK: disconnect sta, mac: %02x:%02x:%02x:%02x:%02x:%02x\n",
		   pstat->hwaddr[0], pstat->hwaddr[1], pstat->hwaddr[2],
		   pstat->hwaddr[3], pstat->hwaddr[4], pstat->hwaddr[5]);
#endif
...
#ifdef DEBUG_PSK
						PKS_PRINT("!!!!DO NOT disconnect STA under 1 client case!!!\n");
#endif
						SMP_UNLOCK_PSK_RESEND(flags);
						return;
					}
...
#ifdef DEBUG_PSK
		PKS_PRINT("PSK: Exceed max retry, disconnect sta\n");
#endif
...
send_packet:
...
#ifdef DEBUG_PSK
	{
		...
		PKS_PRINT("PSK: Send a EAPOL %s, len=%x\n", msg, pskb->len);
		DEBUG_HEX(NULL, pskb->data, pskb->len);
	}
#endif
...
#ifdef DEBUG_PSK
		DEBUG_HEX("PSK: Generated GTK=", pGblInfo->GTK[pGblInfo->GN], GTK_LEN);
#endif
...
#ifdef DEBUG_PSK
		PKS_PRINT("Not Eapol-key pkt (type %d), drop\n", eapol->packet_type);
#endif
#if defined(CONFIG_WLAN_STATS_EXTENTION)
		priv->ext_stats.total_psk_fail++;
#endif
...
#ifdef DEBUG_PSK
	PKS_PRINT("PSK: Rx a EAPOL %s, len=%x\n", STATE2RXMSG(pstat), pStaInfo->EAPOLMsgRecvd.Length);
	DEBUG_HEX(NULL, pstat->wpa_sta_info->EAPOLMsgRecvd.Octet, pStaInfo->EAPOLMsgRecvd.Length);
#endif
...
#ifdef DEBUG_PSK
		PKS_PRINT("PSK: Rx MIC errir report from client\n");
#endif
...
#ifdef DEBUG_PSK
			DEBUG_HEX("PSK: Generated PTK=", pStaInfo->PTK, PTK_LEN_TKIP);
#endif
...
#ifdef DEBUG_PSK
	DEBUG_HEX(NULL, pStaInfo->EAPOLMsgRecvd.Octet, pStaInfo->EAPOLMsgRecvd.Length);
#endif
...
#endif
#ifdef DEBUG_PSK
	DEBUG_HEX("PSK: PMK=", pGblInfo->PSK, PMK_LEN);
	if (priv->pmib->dot1180211AuthEntry.dot11PassPhraseGuest[0])
		DEBUG_HEX("PSK-Guest: PMK=", pGblInfo->PSKGuest, PMK_LEN);
#endif
...
#ifdef DEBUG_PSK
	PKS_PRINT("PSK: WPA unicast cipher= ");
	for (i = 0; i < pGblInfo->NumOfUnicastCipher; i++)
		PKS_PRINT("%x ", pGblInfo->UnicastCipher[i]);
	PKS_PRINT("\n");

#ifdef RTL_WPA2
	PKS_PRINT("PSK: WPA2 unicast cipher= ");
	for (i = 0; i < pGblInfo->NumOfUnicastCipherWPA2; i++)
		PKS_PRINT("%x ", pGblInfo->UnicastCipherWPA2[i]);
	PKS_PRINT("\n");
#endif

	PKS_PRINT("PSK: multicast cipher= %x\n", pGblInfo->MulticastCipher);
#endif
...
#ifdef DEBUG_PSK
	PKS_PRINT("PSK: Got evt:%s[%x], sta: %02x:%02x:%02x:%02x:%02x:%02x, msg_len=%x\n",
		   ID2STR(id), id,
...
// button 2009.05.21
	...
	case DOT11_EVENT_REASSOCIATION_IND:
		...
		if (OPMODE & WIFI_AP_STATE) {
			...
			if (len > 2 && msg != NULL) {
#ifdef DEBUG_PSK
				DEBUG_HEX("PSK: Rx Assoc-ind, RSNIE", msg, len);
#endif
...
#ifdef DEBUG_PSK
	DEBUG_HEX("PSK: Generate PMK_R0_Name=", pmk_r0_name, PMKID_LEN);
#endif
...
#ifdef DEBUG_PSK
	DEBUG_HEX("PSK: Generate PMK_R1=", pmk_r1, PMK_LEN);
#endif
...
#ifdef DEBUG_PSK
	DEBUG_HEX("PSK: Generate PMK_R1_Name=", pmk_r1_name, PMKID_LEN);
#endif
...
#ifdef DEBUG_PSK
	DEBUG_HEX("PSK: Generate pmk_r1_name=", pmk_r1_name, PMKID_LEN);
	DEBUG_HEX("ft_auto_generate_pmk: PMK_R0_Name= 0", pmk_r0_name, PMKID_LEN);
	DEBUG_HEX("ft_auto_generate_pmk: pmk_r1_name=", pmk_r1_name, PMKID_LEN);
	DEBUG_HEX("ft_auto_generate_pmk: PMK_R1=", pmk_r1, PMK_LEN);
#endif
...
#ifdef DEBUG_PSK
	DEBUG_HEX("ft_auto_generate_pmk: cur_pmk_r1=", cur_pmk_r1, PMK_LEN);
#endif
...
int ft_check_ft_auth(struct rtl8192cd_priv *priv, struct stat_info *pstat, unsigned char *pbuf, unsigned int limit, unsigned int *status)
{
	...
	if (IEEE8021X_FUN) {
		...
		if (!r1kh) {
			...
			DEBUG_HEX("PMK-R0: ", pStaInfo->cache_pmk_r0_id, PMKID_LEN);
