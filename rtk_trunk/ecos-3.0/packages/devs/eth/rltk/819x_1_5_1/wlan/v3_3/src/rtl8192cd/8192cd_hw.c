...
#endif
//fix 解决自动信道时，选择40M频宽，出现扩展方向不正确的问题
if(priv->pshare->CurrentChannelBW == HT_CHANNEL_WIDTH_20_40){
    if((channel < 5) && (priv->pshare->offset_2nd_chan != HT_2NDCH_OFFSET_ABOVE)){
        printk("==== %s(%d):ch(%d) Force ABOVE \n",__func__, __LINE__, channel);
        priv->pshare->offset_2nd_chan = HT_2NDCH_OFFSET_ABOVE;
        SwBWMode(priv, priv->pshare->CurrentChannelBW, priv->pshare->offset_2nd_chan);
    }else if((channel > 9) && (channel <=13) && (priv->pshare->offset_2nd_chan != HT_2NDCH_OFFSET_BELOW)){
        printk("==== %s(%d):ch(%d) Force BELOW \n",__func__, __LINE__, channel);
