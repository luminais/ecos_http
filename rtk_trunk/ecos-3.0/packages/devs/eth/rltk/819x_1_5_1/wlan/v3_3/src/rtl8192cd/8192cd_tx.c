...
#ifdef CONFIG_TENDA_WLAN_ISOLATE
int tenda_isolate_filter(struct rtl8192cd_priv *priv,struct sk_buff *skb, struct net_device *dev)
{
#define eamstr		"%02x:%02x:%02x:%02x:%02x:%02x"
#define eamac(a)	(a)[0], (a)[1], (a)[2],(a)[3], (a)[4], (a)[5]

    int drop = 0;
    struct rtl8192cd_priv *priv_sa = NULL, *priv_root = GET_ROOT(priv);
    struct stat_info	*sta_sa = NULL;
    unsigned char       *da;
    unsigned char       *sa;
    int i;

    if ((priv->tenda_mib.sta_isolate == 0)
        && (priv_root->tenda_mib.ssid_isolate == 0)){
        goto out;
    }
    ...
    sta_sa = get_stainfo(priv_root, sa);
    ...
    if (sta_sa) {
        ...
        for (i=0; i<RTL8192CD_NUM_VWLAN; i++) { 
            if(IS_DRV_OPEN(priv_root->pvap_priv[i])) {
                if (sta_sa == NULL) {
                    sta_sa = get_stainfo(priv_root->pvap_priv[i], sa);
                    if (sta_sa) {
                        priv_sa = priv_root->pvap_priv[i];
                        break;
                    }
                }
            }
    ...
    if (priv->tenda_mib.sta_isolate) {
        /* in the same vap ,we use sta isolate */
        if (priv == priv_sa)
            drop = 1;
    }

    if (priv_root->tenda_mib.ssid_isolate) {
        /* in diff vap, we use ssid isolate */
        if (priv != priv_sa)
            drop = 1;
    }

    if (priv_root->tenda_mib.debug & TD_DBG_ISOLATE && drop){
        printk("[%15s][DROP],len=[%d],src="eamstr",dst="eamstr" dev(%s)->(%s)\n",
            __func__,skb->len,eamac(sa),eamac(da),
            priv_sa->dev->name,dev->name);
    }
out:
    return drop;
}
...
//Check if crossband enabled && valid switch path available && this is not preferred interface
    ...
    //td_dump_8023_skb(priv, skb, __func__, NULL);
...
#endif	
                    pdesc_data->ampduDensity = 4;//BSPLJF++ synchronize AC9v3
#else
...
) //struct net_device *dev)
{
	...
	priv = GET_DEV_PRIV(dev);
#ifdef CONFIG_RTK_MESH
    td_dump_8023_skb(priv,skb,__func__,ptxcfg);
#else
    td_dump_8023_skb(priv,skb,__func__,NULL);
#endif
#if defined(CONFIG_RTK_MESH)
