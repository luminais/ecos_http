...
int del_station(struct rtl8192cd_priv *priv, struct stat_info *pstat, int send_disasoc)
{
	...
	if (!list_empty(&pstat->asoc_list))
	{
		...
		if (send_disasoc){
			issue_disassoc(priv, pstat->hwaddr, sta_steer_get_assoc_status());
		}
#else
		if (send_disasoc)
...
int	_free_stainfo_(struct rtl8192cd_priv *priv, struct stat_info *pstat)
#else
...
void cnt_assoc_num(struct rtl8192cd_priv *priv, struct stat_info *pstat, int act, char *func)
{
	...
	diag_printf("assoc_num%s(%d) in %s %02X%02X%02X%02X%02X%02X\n",
		act?"++":"--",
		priv->assoc_num,
		func,
		pstat->hwaddr[0],
...
void td_deauth_all_sta(struct rtl8192cd_priv *priv)
{
    struct stat_info *pstat;
    struct list_head *phead, *plist;
...
static int td_auto_width_first = 0;
void td_auto_width_clear_status(struct rtl8192cd_priv *priv)
{
    td_auto_width_first = 0;
    priv->pshare->rf_ft_var.td_width_all = 0;
    priv->pshare->rf_ft_var.td_width_bad = 0;
    priv->pshare->rf_ft_var.td_width_good = 0;
}
...
void checkBandwidth(struct rtl8192cd_priv *priv)
{
	...
	int CCA_counter	= ODMPTR->FalseAlmCnt.Cnt_CCA_all;
    int FA_counter = ODMPTR->FalseAlmCnt.Cnt_all;
    int FA_TOTAL_CNT = ODMPTR->FalseAlmCnt.Cnt_all;
    int CCA_TOTAL_CNT = ODMPTR->FalseAlmCnt.Cnt_CCA_all;
    ...
    if(IS_OUTSRC_CHIP(priv)) {
        ...
        CCA_counter	= ODMPTR->FalseAlmCnt.Cnt_CCA_all;
    }
#endif
...
/* add by jack, 20/40M autowidth */
    if (priv->pmib->dot11nConfigEntry.td_width_enable) {
        unsigned int td_loop_thd;
        int force_sw = 0;
        priv->pshare->rf_ft_var.td_width_all++;
        if (!priv->pshare->rf_ft_var.td_width_loop_thd) {
            priv->pshare->rf_ft_var.td_width_good_lmt = 600;
            priv->pshare->rf_ft_var.td_width_loop_thd = 120;
        }
        if (td_auto_width_first == 0) {
            td_loop_thd = 10;
            force_sw = 1;
        } else {
            td_loop_thd = priv->pshare->rf_ft_var.td_width_loop_thd;
            force_sw = 0;
        }
        if (FA_TOTAL_CNT > priv->pshare->rf_ft_var.bws_Thd
                || CCA_TOTAL_CNT > (priv->pshare->rf_ft_var.bws_Thd+100)) {
            priv->pshare->rf_ft_var.td_width_bad++;
        } else if (FA_TOTAL_CNT < priv->pshare->rf_ft_var.td_width_good_lmt
            && CCA_TOTAL_CNT < priv->pshare->rf_ft_var.td_width_good_lmt) {
            priv->pshare->rf_ft_var.td_width_good++;
        }
        if (priv->pshare->rf_ft_var.td_width_dbg) {
            diag_printf("FA(%d) CCA(%d) bad:%d good:%d all:%d good,bad_lmt(%d,%d) loop=%d is40M(%d) nosta(%d)\n",
                FA_TOTAL_CNT,CCA_TOTAL_CNT,
                priv->pshare->rf_ft_var.td_width_bad,
                priv->pshare->rf_ft_var.td_width_good,
                priv->pshare->rf_ft_var.td_width_all,
                priv->pshare->rf_ft_var.td_width_good_lmt,
                priv->pshare->rf_ft_var.bws_Thd,
                td_loop_thd,
                priv->pshare->is_40m_bw,
                assoc_num);
        }

        if (priv->pshare->rf_ft_var.td_width_all > 
                td_loop_thd){

            td_auto_width_first = 1;

            if (priv->pshare->rf_ft_var.td_width_bad * 10 
                    >= priv->pshare->rf_ft_var.td_width_all * 7){
                if (((0 == assoc_num) || force_sw)
                    && priv->pshare->is_40m_bw != HT_CHANNEL_WIDTH_20) {
                    diag_printf("bad:%d good:%d all:%d ,so turn 20M, force_sw(%d)\n",
                        priv->pshare->rf_ft_var.td_width_bad,
                        priv->pshare->rf_ft_var.td_width_good,
                        priv->pshare->rf_ft_var.td_width_all,
                        force_sw);
                    ...
                    /* deauth all sta */
                    if (force_sw) {
                        td_deauth_all_sta(priv);
                    }
                }
            } else if (priv->pshare->rf_ft_var.td_width_good * 10 
                    >= priv->pshare->rf_ft_var.td_width_all * 7) {
                if ((0 == assoc_num)
                    && priv->pshare->is_40m_bw == HT_CHANNEL_WIDTH_20) {
                    diag_printf("bad:%d good:%d all:%d ,so turn 40M\n",
                        priv->pshare->rf_ft_var.td_width_bad,
                        priv->pshare->rf_ft_var.td_width_good,
                        priv->pshare->rf_ft_var.td_width_all);
...
/* end add by jack */
    ...
    struct ht_info_elmt	*ht_ie = &priv->ht_ie_buf;
	if((ht_ie->info0 & (_HTIE_2NDCH_OFFSET_BL_ | _HTIE_STA_CH_WDTH_)) ||
		(ht_ie->info0 & (_HTIE_2NDCH_OFFSET_AB_ | _HTIE_STA_CH_WDTH_))){
		priv->pmib->dot11RFEntry.dot11channelBW = HT_CHANNEL_WIDTH_20_40;
	}else{
		priv->pmib->dot11RFEntry.dot11channelBW = HT_CHANNEL_WIDTH_20;
	}
		...
		if (r0kh->key_expire_to > 0 && --r0kh->key_expire_to == 0) {
			...
			if (pstat) {
                ...
                TD_DBG(priv, TD_DBG_FREE_STA, "[%s] del_sta["MACSTR"] %s__%d\n",
                    priv->dev->name,MAC2STR(sta_addr),__func__,__LINE__);
				del_sta(priv, tmpBuf);		
			}
