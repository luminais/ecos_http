...
static unsigned char td_cur_igi = 0;
static unsigned int td_cur_fa = 0;

void tenda_set_fa_igi(unsigned int fa, unsigned char igi)
{
...
void tenda_get_fa_igi(int * fa, int * igi)
{
	...
	)
{
	...
	//页面显示当前fa 及灵敏度
	td_cur_igi = CurrentIGI;
	td_cur_fa = pFalseAlmCnt->Cnt_all;
	...
	//1 Boundary Decision
	{
		...
		else
		{
			...
			if(priv->pshare->rf_ft_var.td_adaptivity_mode && priv->pshare->rf_ft_var.td_dig_max)
			{
				dm_dig_max  = priv->pshare->rf_ft_var.td_dig_max;//td_dig_max在sme中修改
			}
			else
			/*腾达adaptivity模式 end*/
			{
	    ...
	    if (priv->pshare->rf_ft_var.td_dig_sel)
            dm_dig_max = priv->pshare->rf_ft_var.td_dig_sel;	
