...
)
{
	...
	if (Channel >= 1 && Channel <= 14) {
		...
		TxLimit = PHY_GetTxPowerLimitOriginal(Adapter, RegPwrTblSel, BAND_ON_2_4G, (CHANNEL_WIDTH) BandWidth, ODM_RF_PATH_A, TxRate, Channel);
		...
		TxLimit = PHY_GetTxPowerLimitOriginal(Adapter, RegPwrTblSel, BAND_ON_5G, (CHANNEL_WIDTH) BandWidth, ODM_RF_PATH_A, TxRate, Channel);
...
)
{
	...
	if (Method == TSSI_MODE) {
		...
		case ODM_RF_PATH_A:
				...
				TxPowerIndex = PHY_GetTxPowerIndex_8814A(Adapter, (ODM_RF_RADIO_PATH_E) RFPath, TxRate, (CHANNEL_WIDTH) BandWidth, Channel);
