	...
	)
{
	...
	for (i = 0; i < MAC_REG_NUM_8197F; i++)
		MAC_backup[i] = ODM_Read4Byte(pDM_Odm, Backup_MAC_REG[i]);

	for (i = 0; i < BB_REG_NUM_8197F; i++)
...
{	
	...
	while (LOK_notready) {
		...
		if (delay_count >= 300) {
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("[IQK]S%d LOK timeout!!!\n", Path));
			/*_IQK_ResetNCTL_8197F(pDM_Odm);*/
				break;
			}
		}

		ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE, ("[IQK]S%d ==> delay_count = 0x%d\n", Path, delay_count));
	...
	if (!LOK_notready) {
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE, ("[IQK]0x8 = 0x%x, 0x58 = 0x%x\n", LOK_temp2, LOK_temp3));
	} else {
...
{	
	...
	while (fail) {
		...
		if ((*pDM_Odm->pBandWidth == 2) && (idx == TX_IQK)) /*80M*/
			ODM_delay_ms(WBIQK_delay_8197F);
		else
			ODM_delay_ms(IQK_delay_8197F);
		...
		while (notready) {
			...
			if (!notready)	{
				...
				delay_count++;
			if (delay_count >= 300) {
				ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("[IQK]S%d IQK timeout!!!\n", Path));
				/*_IQK_ResetNCTL_8197F(pDM_Odm);*/
				break;
				}
		}
	...
	if (!fail) {
		if (idx == TX_IQK)
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE, ("[IQK]S%d_IQC = 0x%x\n", Path, pIQK_info->IQC_Matrix[idx][Path]));
	}
...
{	
	...
	for (Path = 0; Path <= 1; Path++) {
		...
		_IQK_RFESetting_8197F(pDM_Odm, FALSE);

					
		_LOK_One_Shot_8197F(pDM_Odm, Path);
	}
	...
	for (Path = 0; Path <= 1; Path++) {
		...
		for (idx = 0; idx <= 1; idx++) {						/*ii = 0:TXK , 1: RXK*/
			...
			if (idx == TX_IQK) {
				...
				_IQK_RFESetting_8197F(pDM_Odm, TRUE);
			}
		
			/*if(idx == TX_IQK)*/
				/*_LOK_One_Shot_8197F(pDM_Odm, Path);*/
			_IQK_One_Shot_8197F(pDM_Odm, Path, idx);
	...
	)
{
	...
	if (pIQK_info->IQKtimes == 0x0) {
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("[IQK]=====>PHY_IQCalibrate_8197F_Init\n"));
		for (jj = 0; jj < 2; jj++) {
			for (ii = 0; ii < NUM; ii++) {
...
{
	u4Byte	MAC_backup[MAC_REG_NUM_8197F], BB_backup[BB_REG_NUM_8197F], RF_backup[RF_REG_NUM_8197F][2];
	u4Byte	Backup_MAC_REG[MAC_REG_NUM_8197F] = {0x520, 0x550}; 
	u4Byte	Backup_BB_REG[BB_REG_NUM_8197F] = {0x808, 0x90c, 0xc00, 0xcb0, 0xcb4, 0xcbc, 0xe00, 0xeb0, 0xeb4, 0xebc, 0x1990, 0x9a4}; 
	u4Byte	Backup_RF_REG[RF_REG_NUM_8197F] = {0x0, 0x8f, 0x65}; 
	u1Byte	chnlIdx = ODM_GetRightChnlPlaceforIQK(Channel);
	
	_IQK_BackupMacBB_8197F(pDM_Odm, MAC_backup, BB_backup, Backup_MAC_REG, Backup_BB_REG);
	_IQK_AFESetting_8197F(pDM_Odm, TRUE);
	_IQK_BackupRF_8197F(pDM_Odm, RF_backup, Backup_RF_REG);
	_IQK_ConfigureMACBB_8197F(pDM_Odm);
	_IQK_StartIQK_8197F(pDM_Odm, chnlIdx);
	/*_IQK_ResetNCTL_8197F(pDM_Odm);	for 3-wire to  BB use*/
	_IQK_AFESetting_8197F(pDM_Odm, FALSE);
	_IQK_RestoreMacBB_8197F(pDM_Odm, MAC_backup, BB_backup, Backup_MAC_REG, Backup_BB_REG);
	_IQK_RestoreRF_8197F(pDM_Odm, Backup_RF_REG, RF_backup);
	...
	)
{
	...
	_IQCalibrate_8197F_Init(pDM_VOID);
	...
	if (!pDM_Odm->RFCalibrateInfo.bIQKInProgress) {
		...
		phy_IQCalibrate_8197F(pDM_Odm, pHalData->CurrentChannel);
		/*DBG_871X("%s,%d, do IQK %u ms\n", __func__, __LINE__, rtw_get_passing_time_ms(time_iqk));*/
	#else
		phy_IQCalibrate_8197F(pDM_Odm, *pDM_Odm->pChannel);
	#endif
