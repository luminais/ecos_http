...
)
{
	...
	u2Byte	macid, phydm_macid;
	PSTA_INFO_T	pEntry;

	ODM_RT_TRACE(pDM_Odm, ODM_COMP_COMMON, ODM_DBG_LOUD, ("Current Station ID = 0x%x\n", pDM_Odm->curr_station_id));
...
)
{
	...
	if ((pDM_Odm->SupportICType & ODM_RTL8822B) && (path <= ODM_RF_PATH_B)) {
		for (rate_idx = 0; rate_idx <= 0x53; rate_idx++) {
			...
			txagc = config_phydm_read_txagc_8822b(pDM_Odm, (ODM_RF_RADIO_PATH_E) path, rate_idx);
...
)
{
	...
	if (pDM_Odm->SupportICType & ODM_RTL8822B) {
		if (dm_value[1] <= 1) {
				...
				if (phydm_write_txagc_1byte_8822b(pDM_Odm, dm_value[3], (ODM_RF_RADIO_PATH_E) dm_value[1], (u1Byte)dm_value[2]))
				...
				for (i = 0; i < 0x53; i+=4)
					status = (status & config_phydm_write_txagc_8822b(pDM_Odm, power_index, (ODM_RF_RADIO_PATH_E) dm_value[1], i));
...
)
{
...
#if (RTL8822B_SUPPORT == 1)
	{
		if (pDM_Odm->SupportICType & ODM_RTL8822B) {
			...
			config_phydm_switch_channel_bw_8822b(pDM_Odm, central_ch, primary_ch_idx, (ODM_BW_E) bandwidth);
...
#if (RTL8822B_SUPPORT == 1)
	{
		if (pDM_Odm->SupportICType & ODM_RTL8822B) {
			...
			if (bEnableDbgMode) {
				pDM_Odm->bDisablePhyApi = FALSE;
				config_phydm_trx_mode_8822b(pDM_Odm, (ODM_RF_PATH_E) TxPath, (ODM_RF_PATH_E) RxPath, bTx2Path);
				pDM_Odm->bDisablePhyApi = TRUE;
...
)
{
	...
	if(function == RATE_DECISION) {
		if(dbg_num == 0) {
			...
			if(content_0 == 1) {
				...
				ODM_RT_TRACE(pDM_Odm, ODM_FW_DEBUG_TRACE,ODM_DBG_LOUD,("[FW] RSSI=(( %d ))\n", content_1));
			}
		}
			...
			if(content_0 == 3) {
				ODM_RT_TRACE(pDM_Odm, ODM_FW_DEBUG_TRACE, ODM_DBG_LOUD, ("[FW] RER_CNT   PCR_ori =(( %d )),  ratio_ori =(( %d )),  total =(( %d )), pcr_var_diff =(( %d ))\n", content_1, content_2, content_3, content_4));
			} else if(content_0 == 5) {
	...
	/*--------------------------------------------*/
		...
		if (dbg_num == 1) {
			...
			if (content_0 == 1) {
				ODM_RT_TRACE(pDM_Odm, ODM_FW_DEBUG_TRACE, ODM_DBG_LOUD, ("[FW][Try Done]  ((try fail )) macid=((%d)), Try_Done_cnt=((%d)),  multi_try_rate=((%d)),  try_rpt=((%d))\n", content_1, content_2, content_3, content_4));
				/**/
			}
		}
	} 
