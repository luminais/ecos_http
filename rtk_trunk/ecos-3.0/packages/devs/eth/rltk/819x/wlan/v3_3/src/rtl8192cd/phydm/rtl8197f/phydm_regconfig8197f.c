...
)
{
	...
	if (bbGainIdx > 0x3c) {
		if (rfGainIdx == pDM_DigTable->rfGainIdx) {
			pDM_DigTable->bigJumpLmt[agcTableIdx] = bbGainIdx - 2;
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_DIG, ODM_DBG_TRACE, ("===> ODM_ConfigBBWithHeaderFile: [AGC_TAB] bigJumpLmt [%d] = 0x%x\n", agcTableIdx, pDM_DigTable->bigJumpLmt[agcTableIdx]));
		}
	}
...
{
	odm_UpdateAgcBigJumpLmt_8197F(pDM_Odm, Data);
