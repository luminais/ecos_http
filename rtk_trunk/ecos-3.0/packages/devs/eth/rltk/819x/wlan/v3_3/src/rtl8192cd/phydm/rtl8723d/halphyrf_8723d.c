	...
	)
{
	...
	else if(bIQKOK)
	{
		...
		if(bTxOnly)
		{
			ODM_RT_TRACE(pDM_Odm,ODM_COMP_CALIBRATION, ODM_DBG_LOUD,  ("_PHY_PathAFillIQKMatrix_8723D only Tx OK\n"));		
			return;
		}
...
//			RT_DISP(FINIT, INIT_IQK, ("phy_IQCalibrate_8723D()==>0x%08x\n",bbvalue));
...
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
		PathAOK = phy_PathA_IQK_8723D(pAdapter, is2T);
#else
		PathAOK = phy_PathA_IQK_8723D(pDM_Odm, is2T);
#endif
//		if(PathAOK == 0x03){
		if(PathAOK == 0x01){
...
//path A RXIQK
	...
	for(i = 0 ; i < retryCount ; i++){
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
		PathAOK = phy_PathA_RxIQK_8723D(pAdapter, is2T);
#else
		PathAOK = phy_PathA_RxIQK_8723D(pDM_Odm, is2T);
#endif
		if(PathAOK == 0x03){
...
//path B Tx IQK
		...
		for(i = 0 ; i < retryCount ; i++){
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
			PathBOK = phy_PathB_IQK_8723D(pAdapter);
#else
			PathBOK = phy_PathB_IQK_8723D(pDM_Odm);
#endif
//		if(PathBOK == 0x03){
		if(PathBOK == 0x01){
...
for(i = 0 ; i < retryCount ; i++){
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
		PathBOK = phy_PathB_RxIQK_8723D(pAdapter, is2T);
#else
		PathBOK = phy_PathB_RxIQK_8723D(pDM_Odm, is2T);
#endif
		if(PathBOK == 0x03){
...
//	 		phy_IQCalibrate_8723D(pAdapter, result, i, FALSE);

#else

//	 		phy_IQCalibrate_8723D(pDM_Odm, result, i, FALSE);
...
//	RT_TRACE(COMP_INIT,DBG_LOUD,("Release Mutex in IQCalibrate \n"));
	...
	{
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
		_PHY_PathAFillIQKMatrix_8723D(pAdapter, bPathAOK, result, final_candidate, (RegEA4 == 0));
#else
		_PHY_PathAFillIQKMatrix_8723D(pDM_Odm, bPathAOK, result, final_candidate, (RegEA4 == 0));
#endif
	}
...
//		{
			_PHY_PathBFillIQKMatrix_8723D(pAdapter, bPathBOK, result, final_candidate, (RegEC4 == 0));
//		}
//	}
#endif
...
#if !(DM_ODM_SUPPORT_TYPE & ODM_AP)
	if(IS_92C_SERIAL( pHalData->VersionID)){
		phy_APCalibrate_8723D(pAdapter, delta, TRUE);
	}
	else
#endif
	{
	...
	)
{
	...
	if(IS_2T2R( pHalData->VersionID)){
		return phy_QueryRFPathSwitch_8723D(pAdapter, TRUE);
	}
	else
#endif		
	{
