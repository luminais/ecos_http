...
static __inline__ void tx_sum_up(struct rtl8192cd_priv *priv, struct stat_info *pstat, int pktlen, struct tx_insn* txcfg, unsigned char *da)
...
#ifdef CONFIG_RTL8672
static __inline__ void rx_sum_up(struct rtl8192cd_priv *priv, struct stat_info *pstat, int pktlen, int retry, unsigned char *da)
#elif defined(TRX_DATA_LOG)
static __inline__ void rx_sum_up(struct rtl8192cd_priv *priv, struct stat_info *pstat, int pktlen, int retry, int data)
#else
...
#ifdef TRX_DATA_LOG
		if (data && !priv->pmib->miscEntry.func_off)
