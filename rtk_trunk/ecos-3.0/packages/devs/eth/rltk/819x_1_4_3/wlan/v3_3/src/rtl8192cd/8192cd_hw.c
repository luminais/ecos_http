...
unsigned int       all_max_power[64]; 

#if defined(CONFIG_WLAN_HAL_8814AE)

//#define MAX_CONFIG_FILE_SIZE (64*1024) // for 8814, added to 64k
#define MAX_CONFIG_FILE_SIZE (10*1024) // for 8192,modify by fh 删除了大容量txt文本中的注释，目前最大的为9K

/*使用应用层内存分配函数，解决从Misc mpool分配内存不足导致死机*/
#define kmalloc(x,y) wifi_kmalloc(x,y)
#define kfree(x) wifi_kfree(x)
#else
...
int PHY_ConfigTXLmtWithParaFile_new(struct rtl8192cd_priv *priv)
{
	...
	if((mem_ptr = (unsigned char *)kmalloc(MAX_CONFIG_FILE_SIZE, GFP_ATOMIC)) == NULL) {
		panic_printk("PHY_ConfigMACWithParaFile(): not enough memory\n");
		return -1;
	}
...
int PHY_ConfigTXLmtWithParaFile(struct rtl8192cd_priv *priv)
{
	...
	if ((mem_ptr = (unsigned char *)kmalloc(MAX_CONFIG_FILE_SIZE, GFP_ATOMIC)) == NULL) {
		panic_printk("PHY_ConfigTXLmtWithParaFile(): not enough memory\n");
		return -1;
	}
...
int PHY_ConfigTXPwrTrackingWithParaFile(struct rtl8192cd_priv *priv)
{
	...
	if ((mem_ptr = (unsigned char *)kmalloc(MAX_CONFIG_FILE_SIZE, GFP_ATOMIC)) == NULL) {
		panic_printk("PHY_ConfigTXPwrTrackingWithParaFile(): not enough memory\n");
		return -1;
	}
				  ...
				  char *OFDMTxAgcOffset_B, char *CCKTxAgc_A, char *CCKTxAgc_B)
{
	...
	{
		if ((mem_ptr = (unsigned char *)kmalloc(MAX_CONFIG_FILE_SIZE, GFP_ATOMIC)) == NULL) {
			panic_printk("PHY_ConfigBBWithParaFile(): not enough memory\n");
			return -1;
		}
			...
			if (file_format == TWO_COLUMN) {
				...
				if (reg_file == PHYREG_PG) {
					if (file_format == THREE_COLUMN){				
						...
						//diag_printf("reg=%x, mask=%x, power=%x\n",u4bRegOffset,u4bRegMask,u4bRegValue);
						temp = u4bRegValue;
						while(temp){
							result = temp & 0xff;
							//diag_printf("%d  ",result);
							all_max_power[kk++] = result;
							temp = temp >> 8;
						}				
...
#endif
//panic_printk("channel %d , priv->pshare->current_pwr_pg_index %d\n",channel,priv->pshare->current_pwr_pg_index);
...
#if defined(CONFIG_WLAN_HAL_8197F) || defined(CONFIG_WLAN_HAL_8192EE) || defined(CONFIG_RTL_88E_SUPPORT) || defined(CONFIG_RTL_8723B_SUPPORT)	// CCK shaping filter
		if(GET_CHIP_VER(priv) == VERSION_8192E || GET_CHIP_VER(priv) == VERSION_8197F || GET_CHIP_VER(priv) == VERSION_8188E || GET_CHIP_VER(priv) == VERSION_8723B){
			if (priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G
#ifdef HIGH_POWER_EXT_LNA		
...
//eric-8822 ?? odm ability
	...
	//eric-8822
	if(GET_CHIP_VER(priv) == VERSION_8822B){
		if(priv->pmib->dot11RFEntry.rfe_type == 0)
			BoardType = ODM_BOARD_DEFAULT;
		else
			BoardType = (ODM_BOARD_EXT_LNA|ODM_BOARD_EXT_LNA_5G|ODM_BOARD_EXT_PA|ODM_BOARD_EXT_PA_5G);
	}
	...
	{		
		if(priv->pmib->dot11RFEntry.rfe_type == 2) {
			...
			panic_printk("PHY paratemters: RFE type 7 ALNA5+GLNA3\n");
		...
		ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_GPA, TYPE_GPA0); //eric-8822
		ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_APA, TYPE_APA0);		
		ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_GLNA, TYPE_GLNA0);		
		ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_ALNA, TYPE_ALNA0);	
	}
	...
	if(GET_CHIP_VER(priv) == VERSION_8822B)
		ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_PACKAGE_TYPE, 1); /*QFN*/
	else
	...
	if(GET_CHIP_VER(priv) == VERSION_8197F){
		...
		if(priv->pmib->dot11RFEntry.rfe_type == 0){
			...
			PHY_SetBBReg(priv,0x4c,BIT24,0x0);//GPIO setting
			PHY_SetBBReg(priv,0x64,BIT28,0x0);//GPIO setting
			PHY_SetBBReg(priv,0x40,0xf000000,0x0);//GPIO setting
			panic_printk("[97F] RFE type0 pin control\n");	
		}else{
			//BoardType = (ODM_BOARD_EXT_LNA|ODM_BOARD_EXT_PA);
			if(priv->pmib->dot11RFEntry.rfe_type == 1){
				panic_printk("[97F] RFE type 1 PHY paratemters: GPA0+GLNA0\n");
				ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_GPA, 0x0);
				ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_GLNA, 0x0);
				PHY_SetBBReg(priv,0x4c,BIT24,0x1);
				PHY_SetBBReg(priv,0x64,BIT28,0x1);
				PHY_SetBBReg(priv,0x40,0xf000000,0x5);
				panic_printk("[97F] RFE type1 pin control\n");
			}else if(priv->pmib->dot11RFEntry.rfe_type == 2){
				panic_printk("[97F] RFE type 2 PHY paratemters: 2L internal\n");
				BoardType = ODM_BOARD_EXT_PA_5G;	/*Special setting for 2L internal*/
				ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_BOARD_TYPE, BoardType);
				PHY_SetBBReg(priv,0x4c,BIT24,0x0);
				PHY_SetBBReg(priv,0x64,BIT28,0x0);
				PHY_SetBBReg(priv,0x40,0xf000000,0x0);
				panic_printk("[97F] RFE type2 pin control\n");
			}else{
	...
	if(GET_CHIP_VER(priv) == VERSION_8822B) {
		if(priv->pmib->dot11RFEntry.rfe_type == 0){			
			ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_LNA, FALSE);
			ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_5G_EXT_LNA, FALSE);
			ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_EXT_PA, FALSE);
			ODM_CmnInfoInit(ODMPTR, ODM_CMNINFO_5G_EXT_PA, FALSE);
		}
		else
...
//	RTL_W32(0x300, tmp32);
    ...
    //panic_printk("\n>>> 0x546=0x%x\n\n", RTL_R8(NAV_PROT_LEN));
		...
		//RTL_W32(0x19a8, 0x00);	// Disable SoftML
	...
	*/
...
#endif
    {
        ...
        if (IS_HAL_CHIP(priv)) {
	        ...
	        if((GET_CHIP_VER(priv) == VERSION_8822B) && (priv->pmib->dot11RFEntry.rfe_type != 0) && (GET_CHIP_VER_8822(priv)==PHYDM_SUPPORT_8822_CUT_VERSION)){
				val32 = phy_RF6052_Config_ParaFile(priv); //force 8822 c-cut external use Radio txt file
	        }else
#endif
	        {		
 ...
 //fixed USB interface interference issue
	...
	if(IS_D_CUT_8192E(priv) || GET_CHIP_VER(priv) == VERSION_8197F){
		...
		if (priv->pmib->dot11RFEntry.kfree_enable || priv->pshare->rf_ft_var.mp_specific){
			...
			efuse_kfree(priv,0);
			priv->pshare->kfree_on = 1;
		}else{
			/* restore original RF gain offset*/
			efuse_kfree(priv,1);
			priv->pshare->kfree_on = 0;
		}
	}
#endif
#endif
...
static UCHAR CAM_find_usable(struct rtl8192cd_priv *priv)
{
	...
	if(GET_CHIP_VER(priv) >= VERSION_8812E) {
		...
		for_begin = RTL8192CD_NUM_VWLAN + HW_ENC_GROUP_CIPHER_OFFSET + 1;
#else
		for_begin = 0;
#endif
	}
				   ...
				   unsigned long encAlg, unsigned long useDK, unsigned char *pKey)
{
	...
	if ((!memcmp(pucMacAddr, "\xff\xff\xff\xff\xff\xff", 6)) || (useDK
		...
		if(IS_VXD_INTERFACE(priv) || (IS_ROOT_INTERFACE(priv) && (OPMODE & WIFI_STATION_STATE)))
			camIndex = keyId;
		else
#endif			
			camIndex = priv->vap_init_seq+HW_ENC_GROUP_CIPHER_OFFSET;
#else
		camIndex = keyId;
#endif
	}
...
#if defined(CLIENT_MODE)
	if (IS_VXD_INTERFACE(priv) || (IS_ROOT_INTERFACE(priv) && (OPMODE & WIFI_STATION_STATE)))
		set_dk_margin = HW_ENC_GROUP_CIPHER_OFFSET;
	else
#endif		
	set_dk_margin = 0;
#endif
...
#endif				
				GET_HAL_INTERFACE(priv)->CAMReadMACConfigHandler(priv, priv->vap_init_seq+HW_ENC_GROUP_CIPHER_OFFSET, ucTempMAC, &CamEntryCfg);
#else
			GET_HAL_INTERFACE(priv)->CAMReadMACConfigHandler(priv, keyId, ucTempMAC, &CamEntryCfg);
#endif
...
#endif					
					GET_HAL_INTERFACE(priv)->CAMEmptyEntryHandler(priv, priv->vap_init_seq+HW_ENC_GROUP_CIPHER_OFFSET);
#else
				GET_HAL_INTERFACE(priv)->CAMEmptyEntryHandler(priv, keyId);
#endif
...
#endif
				CAM_read_mac_config(priv, priv->vap_init_seq+HW_ENC_GROUP_CIPHER_OFFSET, ucTempMAC, &usTempConfig);
#else
			CAM_read_mac_config(priv, keyId, ucTempMAC, &usTempConfig);
#endif
...
#endif					
					CAM_empty_entry(priv, priv->vap_init_seq+HW_ENC_GROUP_CIPHER_OFFSET);
#else
				CAM_empty_entry(priv, keyId);
#endif
				if (priv->pshare->CamEntryOccupied == 1)
...
*/
void efuse_kfree(struct rtl8192cd_priv *priv, unsigned char action)
{
	unsigned char *hwinfo = &(priv->EfuseMap[EFUSE_INIT_MAP][0]);
	...
	if (!(priv->pmib->efuseEntry.enable_efuse & EFUSE_ENABLE) || !(priv->pmib->efuseEntry.enable_efuse & EFUSE_KFREE))
		return;
	...
	if(action == 0){
		...
		if ( (val & 0xF) == 0xF ) {
			...
			priv->pshare->mp_kfree_reg_backup_pathA = res;
		...
		if ( valb == 0xF ) {
			...
			priv->pshare->mp_kfree_reg_backup_pathB = res;
		...
		/*Restore path A 0x55*/
		if(priv->pshare->mp_kfree_reg_backup_pathA){
			panic_printk("orig RF A 0x55 = 0x%x\n", priv->pshare->mp_kfree_reg_backup_pathA);
			res = PHY_QueryRFReg(priv, RF92CD_PATH_A, REG_RF_BB_GAIN, bMask20Bits, 1);
#if defined(CONFIG_WLAN_HAL_8197F)			
			if(GET_CHIP_VER(priv) == VERSION_8197F){
				/* RF 0x55[19, 16:14]*/
				res &= 0xfff63fff;
				res |= (priv->pshare->mp_kfree_reg_backup_pathA & BIT19);
				res |= (priv->pshare->mp_kfree_reg_backup_pathA & (BIT16|BIT15|BIT14));
			}else
#endif			
			{
				/* RF 0x55[18:15]*/
				res &= 0xfff87fff;			
				res |= (priv->pshare->mp_kfree_reg_backup_pathA & (BIT18|BIT17|BIT16|BIT15));
			}
		...
		/*Restore path B 0x55*/
		if(priv->pshare->mp_kfree_reg_backup_pathB){			
			panic_printk("orig RF B 0x55 = 0x%x\n", priv->pshare->mp_kfree_reg_backup_pathB);
			res = PHY_QueryRFReg(priv, RF92CD_PATH_B, REG_RF_BB_GAIN, bMask20Bits, 1);
#if defined(CONFIG_WLAN_HAL_8197F)			
			if(GET_CHIP_VER(priv) == VERSION_8197F){
				/* RF 0x55[19, 16:14]*/
				res &= 0xfff63fff;
				res |= (priv->pshare->mp_kfree_reg_backup_pathB & BIT19);
				res |= (priv->pshare->mp_kfree_reg_backup_pathB & (BIT16|BIT15|BIT14));
			}else
#endif			
			{
				/* RF 0x55[18:15]*/
				res &= 0xfff87fff;			
				res |= (priv->pshare->mp_kfree_reg_backup_pathB & (BIT18|BIT17|BIT16|BIT15));
			}
...
#if defined(HS2_SUPPORT) || defined(RTK_NL80211) /*survey_dump*/ || defined(DOT11K)|| defined(CH_LOAD_CAL) || defined(UCHAN_SUPPORT)
