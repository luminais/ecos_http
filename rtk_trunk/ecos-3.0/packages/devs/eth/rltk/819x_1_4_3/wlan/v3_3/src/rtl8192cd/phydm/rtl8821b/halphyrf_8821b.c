...
#define		ODM_TXPWRTRACK_MAX_IDX8821B		6
	...
	)
{
	...
	PHY_IQCalibrate_8821B(Adapter, FALSE);
}
	...
	)
{
	...
	ODM_RT_TRACE(pDM_Odm,ODM_COMP_TX_PWR_TRACK, ODM_DBG_LOUD,("===>ODM_TxPwrTrackSetPwr8821B\n"));
	...
	)
{
	...
	pConfig->ODM_TxPwrTrackSetPwr = ODM_TxPwrTrackSetPwr8821B;
	pConfig->DoIQK = DoIQK_8821B;
	pConfig->PHY_LCCalibrate = PHY_LCCalibrate_8821B;
	pConfig->GetDeltaSwingTable = GetDeltaSwingTable_8821B;
}
	...
	case ODM_RF_PATH_A:
       {
		...
		if (TX_finish == 1){
			_IQK_TX_FillIQC_8821B(pDM_Odm, Path, TX_X, TX_Y);
		}
		else{
			_IQK_TX_FillIQC_8821B(pDM_Odm, Path, 0x200, 0x0);
		}
		...
		if (RX_finish == 1){
			_IQK_RX_FillIQC_8821B(pDM_Odm, Path, RX_X, RX_Y);
		}
		else{
			_IQK_RX_FillIQC_8821B(pDM_Odm, Path, 0x200, 0x0);
		}
		}
		break;
	default:
...
//Byte 2, Bit 0 ~ Bit 3 : Bandwidth
	...
	FW8821B_FillH2cCommand(pAdapter, 0x45, 3, IQKcmd);
#else
	FillH2CCommand8821B(pAdapter, 0x45, 3, IQKcmd);
#endif
	...
	)
{
	...
	_IQK_BackupMacBB_8821B(pDM_Odm, MACBB_backup, Backup_MACBB_REG, MACBB_REG_NUM);
	_IQK_BackupAFE_8821B(pDM_Odm, AFE_backup, Backup_AFE_REG, AFE_REG_NUM);
	_IQK_BackupRF_8821B(pDM_Odm, RFA_backup, RFB_backup, Backup_RF_REG, RF_REG_NUM);
	
	_IQK_ConfigureMAC_8821B(pDM_Odm);
	_IQK_Tx_8821B(pDM_Odm, ODM_RF_PATH_A);
	_IQK_RestoreRF_8821B(pDM_Odm, ODM_RF_PATH_A, Backup_RF_REG, RFA_backup, RF_REG_NUM);
	
	_IQK_RestoreAFE_8821B(pDM_Odm, AFE_backup, Backup_AFE_REG, AFE_REG_NUM);
	_IQK_RestoreMacBB_8821B(pDM_Odm, MACBB_backup, Backup_MACBB_REG, MACBB_REG_NUM);

	//_IQK_Exit_8821B(pDM_Odm);
	//_IQK_TX_CheckResult_8821B
...
#endif
	{
		...
		if(pMgntInfo->RegIQKFWOffload)
		{
			if ( ! pRFCalibrateInfo->bIQKInProgress) 
			{
				...
				phy_IQCalibrate_By_FW_8821B(pAdapter);
			}
			else
			{
	...
	)
{
	...
	if((reg0x914 & 0x70000) != 0)	//If contTx, disable all continuous TX. 0x914[18:16]
		...
		ODM_Write1Byte(pDM_Odm, REG_TXPAUSE_8821B, 0xFF);			
...
*/
	...
	*/
	...
	ODM_SetRFReg(pDM_Odm, ODM_RF_PATH_A, RF_CHNLBW, bRFRegOffsetMask, LC_Cal|BIT15);
	...
	*/
	...
	{
		ODM_Write1Byte(pDM_Odm, REG_TXPAUSE_8821B, 0x00);	
	}
	...
	)
{
	...
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("===> PHY_LCCalibrate_8821B\n"));

#if (MP_DRIVER == 1)	
	phy_LCCalibrate_8821B(pDM_Odm, TRUE);
#endif 
	...
	)
{
	...
	return;
		...
		phy_SetRFPathSwitch_8821B(pAdapter, bMain, TRUE);
	...
	)
{
	...
	return phy_QueryRFPathSwitch_8821B(pAdapter, FALSE);
}

#else	/* (RTL8821B_SUPPORT == 1)*/
