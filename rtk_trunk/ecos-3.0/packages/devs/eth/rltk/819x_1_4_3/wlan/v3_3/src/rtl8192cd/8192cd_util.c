...
//		return	(pstat->dot11KeyMapping.keyInCam || isMulticast) ? 0 : 1;
		...
		if (pstat && (pstat->state & WIFI_WDS) && !(pstat->state & WIFI_ASOC_STATE)) {
			...
			|| priv->pmib->dot1180211AuthEntry.dot11PrivacyAlgrthm == _WAPI_SMS4_
				...
				if (pstat && !pstat->dot11KeyMapping.keyInCam)
					return 1;
				else // key is in CAM
					return 0;
				}
...
void checkBandwidth(struct rtl8192cd_priv *priv)
{
	...
	int CCA_counter	= priv->pshare->CCA_total_cnt;
...
/* add by jack, 20/40M autowidth */
    if (priv->pmib->dot11nConfigEntry.td_width_enable) {
        ...
        if (!priv->pshare->rf_ft_var.td_width_good_lmt) {
        ...
        if (FA_counter > priv->pshare->rf_ft_var.bws_Thd
                || CCA_counter > (priv->pshare->rf_ft_var.bws_Thd+400)) {
            priv->pshare->rf_ft_var.td_width_bad++;
        } else if (FA_counter < priv->pshare->rf_ft_var.td_width_good_lmt
            && CCA_counter < priv->pshare->rf_ft_var.td_width_good_lmt) {
            priv->pshare->rf_ft_var.td_width_good++;
        }
        if (priv->pshare->rf_ft_var.td_width_dbg) {
            diag_printf("FA(%d) CCA(%d) bad:%d good:%d all:%d good,bad_lmt(%d,%d) loop=%d nosta(%d)\n",
                FA_counter,CCA_counter,
                priv->pshare->rf_ft_var.td_width_bad,
                priv->pshare->rf_ft_var.td_width_good,
                priv->pshare->rf_ft_var.td_width_all,
        ...
        if (priv->pshare->rf_ft_var.td_width_all > 
            ...
            if (priv->pshare->rf_ft_var.td_width_bad * 10 
                ...
                if ((list_empty(&priv->asoc_list) || force_sw)
