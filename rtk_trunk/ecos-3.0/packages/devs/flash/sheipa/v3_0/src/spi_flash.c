...
extern struct m25p;
extern void m25p_probe();
extern int sheipa_spi_probe(void);
extern int erase_sector(struct m25p *flash, u32 offset);
extern unsigned get_block_size(void);
extern uint64_t get_flash_size(void);
extern int flash_erase_sector(u32 offset);
...
rtl819x_spi_flash_init(struct cyg_flash_dev* dev)
{
	...
	td_flash_size = flash_size;
	
    rtl819x_dev->block_info[0].blocks = flash_size/block_size;
...
rtl819x_spi_flash_hw_program(unsigned int flash_addr_offset, unsigned char *src, unsigned int len)
{
	...
	spi_flw_image(0, flash_addr_offset, src, len);
	return CYG_FLASH_ERR_OK;
}
...
/*
   ...
   */
   result = flash_erase_sector(dest);
    return result;
}
