...
void Cyg_Scheduler::unlock_inner( cyg_ucount32 new_lock )
{
    ...
    do {
        ...
        if( current->state != Cyg_Thread::RUNNING || get_need_reschedule() ) {
		   ...
		   //diag_printf("0x%x 0x%p 0x%p\n",regs.d[29],&cyg_interrupt_stack,&cyg_interrupt_stack_base);
		   if((regs.d[29] < (unsigned int)&cyg_interrupt_stack) && (regs.d[29] >(unsigned int)&cyg_interrupt_stack_base))
		   {
		   	dump_stack();
		   }
		   /*HF debug end*/
		...
		/*HF. kick watch dog here if schedule run*/
