...
//add by linlinglong, 增加互斥体调试信息, 可以跟踪每个线程当前申请及尝试申请lock的情况
...
typedef struct mutex_print
{
	...
	char mutex_locked_arry_over;  //是否溢出
	
	
}mutex_print_t;


extern mutex_print_t g_mutex_print[32];
extern int g_mutex_print_over;



int strcmpmutex(const char * cs,const char * ct)
{
	signed char __resss;

	while (1) {
		if ((__resss = *cs - *ct++) != 0 || !*cs++)
			break;
	}
...
char * strcpymutex(char * dest,const char *src)
{
	char *tmp = dest;	
...
mutex_locked_t * mutex_locke_update(mutex_print_t * mutex_print_temp , char *locked)
{
	...
	for (i = 0; i < 8; i++)
	{
		...
		if (mutex_print_temp->mutex_locked_arry[i].mutex_locked_ptr == locked)
		{
			break;
		}
		else if (!(mutex_print_temp->mutex_locked_arry[i].mutex_locked_ptr) && (j == -1))
		{
			j = i; //记录可以插进去的值
		}
	}
	...
	{//找到对应节点
		...
		return &(mutex_print_temp->mutex_locked_arry[i]);
	}
	else if ((j != -1))
	{//找到新插入的点
			
		mutex_print_temp->mutex_locked_arry[j].mutex_locked_ptr = locked;
		mutex_print_temp->mutex_locked_arry[j].mutex_try = 0;
		mutex_print_temp->mutex_locked_arry[j].mutex_couter = 0;
		return &(mutex_print_temp->mutex_locked_arry[j]);
	}
	else 
	{
		mutex_print_temp->mutex_locked_arry_over++;
		return NULL;
	}
}
...
mutex_locked_t * mutex_unlocke_update(mutex_print_t * mutex_print_temp , char *locked)
{
...
mutex_print_t * g_mutex_print_update(char *pidname)
{
	...
	for ( i = 0; i < 48; i++)
	{
		...
		if (g_mutex_print[i].thread_name_buf[0] && !strcmpmutex(g_mutex_print[i].thread_name_buf, pidname) )
		{
			
			return &(g_mutex_print[i]);
		}
		else if (!g_mutex_print[i].thread_name_buf[0])
		{
			strcpymutex(g_mutex_print[i].thread_name_buf, pidname);
			g_mutex_print[i].mutex_locked_arry_over = 0;
			return &(g_mutex_print[i]);
		}
	}
...
Cyg_Mutex::lock(void)
{
...
#endif        
#ifdef __CONFIG_MUTEX_DEBUG__	
    mutex_print_t * mutex_print_temp = g_mutex_print_update(self->get_name());
    mutex_locked_t * mutex_locked_temp = mutex_locke_update(mutex_print_temp, (char *)&locked);
#endif		
...
#ifdef __CONFIG_MUTEX_DEBUG__		
	  //标识thread 锁在尝试中
	  if (mutex_locked_temp)
	  {
	  	mutex_locked_temp->mutex_try = 1;
	  }	
#endif			
    ...
    if( result )
    {
	  ...
	  if (mutex_locked_temp)
	  {
		mutex_locked_temp->mutex_couter++;
	  }
#endif
...
Cyg_Mutex::unlock(void)
{
	...
	Cyg_Thread *self = Cyg_Thread::self();
	mutex_print_t * mutex_print_temp = g_mutex_print_update(self->get_name());
	mutex_locked_t * mutex_locked_temp = mutex_unlocke_update(mutex_print_temp, (char *)&locked);

	if (mutex_locked_temp)
	{
		mutex_locked_temp->mutex_couter--;
		if (mutex_locked_temp->mutex_couter == 0)
		{
			mutex_locked_temp->mutex_locked_ptr = NULL;
			mutex_locked_temp->mutex_try = 0;
		}
	}	
#endif
