...
/*DNS_REALTEK_LQZ add by lqz for dns in cli or code,if defined DNS_REALTEK connot dns requerst for cli or code*/
#define DNS_REALTEK_LQZ
#undef	DNS_REALTEK

#ifdef DNS_REALTEK_LQZ
#define DNS_SERVER_NUM	3
in_addr_t dnslist[DNS_SERVER_NUM];
#endif
...
free_hent(struct hostent * hent)
{
    ...
    if (hent->h_addr_list) {
        ...
        while (hent->h_addr_list[i]) {
			...
			hent->h_addr_list[i] = NULL;
            i++;
        }
        free(hent->h_addr_list);
		hent->h_addr_list = NULL;
    }
	if (hent) {
	    free(hent);
		hent = NULL;
...
alloc_hent_n(int n)
{
	...
	{
        memset(hent, 0, sizeof(struct hostent));
        hent->h_addr_list = malloc(sizeof(char *)*(n+1));
        if (!hent->h_addr_list) 
		{
            free_hent(hent);
            return NULL;
        }
		memset(hent->h_addr_list, 0, sizeof(char *)*(n+1));
		for(i=0; i<n; i++)	
		{
			hent->h_addr_list[i] = malloc(sizeof(struct in_addr));
...
thread_destructor(CYG_ADDRWORD data)
{
    ...
    {
        free_hent(hent);
/*lq gethostbyname 出现死机情况，进行制空操作*/
...
store_hent(struct hostent *hent)
{
    ...
    /*lq gethostbyname 出现死机情况，进行判NULL 操作*/
	if(hent)
	{
...
/*add by lqz for DNS lock,just if want to update DNS,please keep dns_update ture let gethostbyname quit as soon as possible*/
static unsigned char dns_update = 0;
/*end*/

void set_dns_quit_exit_tag(int enable)
{
	if(1 == enable)
		dns_update = 1;
	else
...
int get_dns_quit_exit()
{
	return dns_update;
}

#if defined(DNS_REALTEK)
...
send_recv(unsigned char * msg, int len, int msglen)
{
    ...
    dns_hdr = (struct dns_header *) msg;
	for (i = 0; finished == false && i<DNS_SERVER_NUM; i++) {
		in_addr_t dns;
		...
		do {	
			...
			if (ret == 1) {
				ret = recvfrom(s, msg, msglen, 0,
					(struct sockaddr *)&dest, (socklen_t*)&tlen);
				if (ret < 0) {
					ret = -1;
					break;
				}
...
cyg_dns_res_init(struct in_addr *dns_server)
{
  ...
  dns_update = 1;
   if (init) {
 	...
 	memcpy(dnslist, dns_server, sizeof(dnslist));
    ptdindex = cyg_thread_new_data_index();  
