...
static struct hostent *
parse_answer_n(unsigned char * msg, short rr_type)
{
    static struct hostent *hent;
    struct dns_header *dns_hdr;
    struct resource_record rr, *rr_p = NULL;
    unsigned char *qname = NULL, *pAnswers = NULL;
    unsigned char *ptr;
	int answers_count = 0,answers_addr_count = 0, i = 0 ,index = 0;
    dns_hdr = (struct dns_header *)msg;
    ...
    if (answers_addr_count > 0) {
        hent = alloc_hent_n(answers_addr_count);
        if (!hent) 
            return NULL;
		}
	...
	{
		qname = pAnswers;
        pAnswers += qname_len(pAnswers);
        rr_p = (struct resource_record *)pAnswers;
        memcpy(&rr, pAnswers, sizeof(rr));
        if ((rr.rr_type == htons(rr_type)) && 
            (rr.class == htons(DNS_CLASS_IN))) {
			...
			case DNS_TYPE_A:
				...
				if(hent->h_addr_list[index])
					memcpy(hent->h_addr_list[index], rr_p->rdata, sizeof(struct in_addr));
			    hent->h_addrtype = AF_INET;
			    hent->h_length = sizeof(struct in_addr);
			 index++;
			 break;
		...
		pAnswers += sizeof(struct resource_record) - sizeof(rr.rdata) + ntohs(rr.rdlength);
    }
	if(index == 0)
	{
		free_hent(hent);
...
do_query(const char * hostname) 
{
    ...
    return parse_answer_n(msg, DNS_TYPE_A);	
#endif
}
