 ...
 */
#define  CYGNUM_NET_SNTP_SERVER_MAX 6	//llm add
#ifdef CYGNUM_NET_SNTP_SERVER_MAX
static struct sockaddr sntp_servers[CYGNUM_NET_SNTP_SERVER_MAX];
#else
...
#ifdef CYGNUM_NET_SNTP_SERVER_MAX
#define SNTP_UPDATEPERIOD       (6*60*60) /* Update period in seconds */
#else
#define SNTP_UPDATEPERIOD       (30) /* Update period in seconds */
#endif
...
//roy+++2010/09/07
extern time_t ntp_to_local2(time_t sntp_time);
extern void update_application_time(void);
extern void  set_sntp_update_time_success(void);

//roy+++

/*
	Modify time zone, or other cases, We need re-new time form server.
	What we need do, just modify perod time.
	wangxinyu.yy@gmail.com
*/
extern void update_check_time_thread();
void sntp_renew_time_from_server(void)
{
	NextTimeUpdate = 0;
...
{
	time_t local_time,sntp_time,new_time;
	if(1 == get_sntp_update_time_success())
	{
		if((0 != strcmp(nvram_safe_get("old_time_zone"),"")
			&& 0 != strcmp(nvram_safe_get("old_time_zone"),nvram_safe_get("time_zone"))))
		{
			local_time = time(NULL);
			sntp_time = local_to_ntp(local_time);
			new_time = ntp_to_local2(sntp_time);
			SNTP_DBG("time_zone is changed!\n");
			cyg_libc_time_settime(new_time);
			update_application_time();
			nvram_unset("old_time_zone");
			nvram_commit();
		}
	}
}
...
static void sntp_fn(cyg_addrword_t data)
{
  ...
  time_t new_time2;//roy add 2010/09/07
  fd_set readfds;
  int n;
#ifdef CYGPKG_NET_INET6
  int fd6 = -1;
  ...
  while (sntp_initialized) {
    ...
    if (is_better(&new_srv,&best_srv)) {
      ...
      //if (diff > 2)
      	{//roy+++
      	   new_time2 = ntp_to_local2(new_time);
          //cyg_libc_time_settime(new_time);
          cyg_libc_time_settime(new_time2);//roy modify
          update_application_time();
		  update_check_time_thread();
		  set_sntp_update_time_success(); // add liuchengchi 2014-11-18
      	}//roy+++
    }
#ifdef CYGPKG_NET_SNTP_UNICAST
...
//roy+++
void cyg_sntp_stop(void){
	int pid;
	if(sntp_initialized){
		sntp_initialized = 0;
		
		pid = oslib_getpidbyname("SNTP");
		if (pid == 0)
			return;
...
#ifdef CYGPKG_NET_SNTP_UNICAST
	  	/* destroy the SNTP mutex */
	  	cyg_mutex_destroy(&sntp_mutex);
#endif	
...
#ifdef CYGNUM_NET_SNTP_SERVER_MAX
			sntp_num_servers = CYGNUM_NET_SNTP_SERVER_MAX <= num_servers ?
				CYGNUM_NET_SNTP_SERVER_MAX : num_servers;

			/* Update servers */
			memset(sntp_servers, 0, sizeof(sntp_servers));
			if (num_servers) {
				memcpy(sntp_servers, server_list, sizeof(struct sockaddr) * num_servers);
