 ...
 * Function name: show_splx
 * Description: The following codes is used to cli command of show_splx
 * Date: 2015.12.24
 */
 // +++++
...
{
	diag_printf("spl_state=0x%x\n", spl_state);
	diag_printf("splx_thread=0x%x\n", splx_thread);
	diag_printf("splx_mutex.owner=%p\n", splx_mutex.owner);
	diag_printf("splx_mutex.locked=0x%x\n", splx_mutex.locked);
	diag_printf("splx_mutex.protocol=%d\n", splx_mutex.protocol);
	diag_printf("splx_mutex.ceiling=%d\n", splx_mutex.ceiling);
	if (cyg_mutex_trylock(&splx_mutex)) {
		diag_printf("splx_mutex: lock success\n");
		cyg_mutex_unlock(&splx_mutex);
	}
	else
		diag_printf("splx_mutex: lock fail\n");
}
...
spl_any( cyg_uint32 which )
{
    ...
    if ( cyg_thread_self() != splx_thread ) {
		...
		// Description: The following code is used to cli command of splx, Ported by zhuhuan on 2015.12.24
		// +++++
		SPLXSAVEARGS;
		// +++++
    }
 ...
 * @ Source Code, from BCM project, Ported by zhuhuan @
 * Function name: show_splx
 * Description: The following function is used to cli command of show_splx
 * Date: 2015.12.24
 */
 // +++++
void show_splx(void)
{
	...
	printf("SPLX: spl_state: %d, function: %s, line: %d, thread id: %d\n",
		spl_state,
		splx_func,
		splx_line,
		splx_thread == 0 ? 0 : cyg_thread_get_id(splx_thread));

	printf("spl_next = %d, spl_gauge = %d\n", spl_next, spl_gauge);

	for (i = 0; i < spl_gauge; i++) {
		/* Print SPL TRACE IN */
		if (spl_event[i].in_file) {
	 		printf("[%s] SPLTRACE IN(tid %d): %s(%d), state:%d\n",
		...
		if (spl_event[i].out_file) {
 			printf("[%s] SPLTRACE OUT(tid %d): %s(%d), state:%d\n",
				i < spl_next ? " " : (i == spl_next ? "*" : "-"),
