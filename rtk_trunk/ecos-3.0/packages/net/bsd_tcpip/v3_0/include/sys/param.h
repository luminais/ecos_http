...
extern __attribute__((nomips16))  void cyg_panic(const char *msg, ...);
#else
...
#else
/*no define CLUSTERSIZE*/
#if defined(CYGPKG_NET_INET6) || defined(CONFIG_RTL_DELAY_REFILL)
#define MSIZE           256             /* size of an mbuf */
#else
#define MSIZE           256             /* size of an mbuf */
#endif
#define MCLBYTES        (0x740) /* size of a m_buf cluster */
#endif
...
#ifdef CYGPKG_HAL_MIPS_RLX
extern __attribute__((nomips16)) void callout_init(struct callout *c);
extern __attribute__((nomips16)) void callout_reset(struct callout *, int, timeout_fun *, void *);
extern __attribute__((nomips16)) void callout_stop(struct callout *);
extern __attribute__((nomips16)) int  callout_pending(struct callout *);
#else
...
do { if ((cyg_net_log_mask & lvl) && buf) diag_dump_buf(buf, len); } while (0)
#define log_(lvl) \
...
#define CALLOUT_PENDING  0x0004
extern void untimeout(timeout_fun *fun, void *arg);
#endif //end of _KERNEL
int extern cyg_arc4random(void);

/***************************lq添加栈前后守卫**********************/
#include <cyg/hal/hal_tables.h>
typedef struct stack_guard{
	char* name;
	char* front_guard;
	char* back_guard;
} CYG_HAL_TABLE_TYPE stack_guard_t;

/*单独进行线程守卫的注册，在使用该宏的时候需要自己增加栈的大小
在栈前预留4个字节，栈后预留2个字节，必须通过修改栈的大小去实现*/
#define STACK_REGISTER(thread_name,thread_front,thread_back)\
stack_guard_t stack_name##_guard CYG_HAL_TABLE_ENTRY(stack_check_tab) = \
	{\
		thread_name,\
		thread_front,\
		thread_back\
...
/*配合下面的初始化线程信息宏进行使用，在初始化线程信息同时注册守卫*/
#define STACK_REGISTER(thread_name,stack_name,size)  \
stack_guard_t stack_name##_guard CYG_HAL_TABLE_ENTRY(stack_check_tab) = \
	{\
		thread_name,\
		(stack_name##_stack),\
		(stack_name##_stack+size+4)\
	};

extern stack_guard_t  __stack_check_tab_start__[],__stack_check_tab_end__;

/*线程栈信息初始化,一次性申请栈空间、线程句柄、线程数据*/
#define INIT_STACK_INFO(thread_name,stack_name,size) \
	     static char stack_name##_stack[size + 6];\
            static cyg_thread stack_name##_thread_data;\
	     static cyg_handle_t stack_name##_thread_handle;\
            STACK_REGISTER(thread_name,stack_name,size)

#define GET_THREAD_STACK(stack_name)     		(stack_name##_stack + 4)
#define GET_THREAD_DATA(stack_name)     		stack_name##_thread_data
#define GET_THREAD_HANDLE_T(stack_name)     	stack_name##_thread_handle
/**********************************添加栈守卫************************/
#endif //_SYS_PARAM_H_
