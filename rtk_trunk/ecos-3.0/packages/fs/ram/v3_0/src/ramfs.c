                       ...
                       )
{
    ...
    while( namelen > 0 )
    {
		...
		// in case d is in a new realloced buffer
		if(dp && (unsigned long)dp != ((unsigned long)d-sizeof(ramfs_dirent))) {
			dp = (ramfs_dirent *)((unsigned long)d-sizeof(ramfs_dirent));
        }	
