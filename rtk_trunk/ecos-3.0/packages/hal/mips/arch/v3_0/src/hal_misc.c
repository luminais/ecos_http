...
/*Our ticks is 5us per ticks since we enlarge WTDOG timeout to about 2.5 seconds*/
#ifdef CONFIG_RTL_819X
	if( usticks == 0 ) {
		ticks = ((us1 * CYGNUM_HAL_RTC_PERIOD * CYGNUM_HAL_RTC_DENOMINATOR) / 1000000);
		if(ticks == 0)
			ticks=1;
	}	
	else
...
#ifdef CYGSEM_HAL_819X_CPU_SLEEP
