...
extern unsigned char *tenda_arp_ip_to_mac(in_addr_t ip);
extern in_addr_t inet_addr(const char *cp);
extern int tenda_arp_is_multicast(unsigned char *mac);
...
int wan_speed_detecting_report()
{
	...
	{
		if_TX_RX_status("vlan2", &last_up_bytes, &last_down_bytes);
		cyg_thread_delay(50);
		if(g_detect_state != DETECTING)
			continue;
		duration--;
		if_TX_RX_status("vlan2", &up_bytes, &down_bytes);
...
void show_ip_list()
{
	...
	for (i = 0; i<MAX_DETECT_NUM; i++)
	{
		if( g_ip_list[i].sin_addr.s_addr != 0){
			printf("%d s_addr:%u %s\n",i+1, g_ip_list[i].sin_addr.s_addr ,tenda_arp_inet_ntoa(g_ip_list[i].sin_addr.s_addr));
		}
	}
}
