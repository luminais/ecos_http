...
//返回给APP的在线设备和离线设备总数不能超过100台
#define APP_MAX_ONLINE_EXPIRE_NUM	100	
...
void app_get_ol_host_info(cJSON *recv_root,cJSON *send_root,void *info)
{
	...
	char value[32] = {0};
	int drop_expired_flag = 0;		//0:返回离线设备  1:不返回离线设备
	...
	if(count > APP_MAX_ONLINE_EXPIRE_NUM)
	{
		count = APP_MAX_ONLINE_EXPIRE_NUM;
		drop_expired_flag = 1;
	}
