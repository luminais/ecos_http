...
#if defined(__CONFIG_WEB_REALM__)
static char_t	websRealm[64] = __CONFIG_WEB_REALM__;	/* Realm name */
#else
...
extern void sys_before_upgrade(void);
 ...
 */
	...
	for (i = 0; i <= retries; i++) {
		...
		websListenSock = socketOpenConnectionv6(NULL, port, websAccept, 0);
#else
		websListenSock = socketOpenConnection(websIpaddr, port, websAccept, 0);
#endif
		if (websListenSock >= 0) {
			break;
		}
...
//远程访问Listen端口
static int WebWanListenSock = -1;
int webs_Wan_OpenListen(int port,char *HostIP)
{
	WebWanListenSock = socketOpenConnection(HostIP, port, websAccept, 0);
	if (WebWanListenSock < 0)
			return 0;

	return 1;
}

void webs_Wan_CloseListen()
{
	if (WebWanListenSock >= 0) {
		socketCloseConnection(WebWanListenSock);
		WebWanListenSock = -1;
	}
}
...
#ifdef __CONFIG_IPV6__
	if (gstrstr(wp->ipaddr, T("127.0.0.1")) ||
			gstrstr(wp->ipaddr, websIpaddr)  ||
			gstrstr(wp->ipaddr, websHost) ||
			gstrstr(wp->ipaddr, T("::1")) ) {
...
extern int HTTPDebugLevel;

//tenda add for upgrade
#include <hndsoc.h>
#include <bcmendian.h>
#include <bcmutils.h>
#include <flashutl.h>
...
/*realtek把这个给改了，坑了我老久，llm add*/
#ifdef RTL_SMALL_MBUF
#pragma message("webs.c,defined RTL_SMALL_MBUF, MCLBYTES=1856")
#define MCLBYTES 1856
#endif

/*flash大小，boot，nvram大小定义，后续可能会改,llm add*/
#ifdef REALTEK
...
/* http数据包里的头大小和boundry信息大小，这是我随便定义的 
   在收到第一个包判断上传的升级文件大小是否太大避免因开辟内存导致出错使用
   但wp->clen包含了boundry等信息长度，导致以前864K软件升级提示文件太大*/
#define MISC_SIZE (1*1024)	
#endif
...
extern void muf_page_free(void* addr);
static int is_upgrade_flag = 0;
static int no_net_mbuf_free = 0;

static char * upgrade_pdate_misc = NULL;

struct upgrade_mem *upgrade_mem_list = NULL;

extern int  get_kern_free_mem();
...
struct upgrade_mem *upgrade_mem_alloc(int totlen)
{
	
	int net_mbuf_free, clust_free, misc_free, mbuf_free;
	struct upgrade_mem *p,*q,*mem_node;
	
	
	net_mbuf_free = net_mbuf_free_mem();
	no_net_mbuf_free = 0;
	upgrade_mem_list = NULL;
	if((net_mbuf_free - 256*1024) > 0){
		//don't change 2048 and 400 here
		net_mbuf_free = 2048*((net_mbuf_free - 256*1024)/2048);//reserve 400KB for system
	}else{
		no_net_mbuf_free = 1;
	}
	
	if(upgrade_mem_list == NULL){
		upgrade_mem_list  =  malloc(sizeof(struct upgrade_mem));
		if(upgrade_mem_list == NULL){
			return NULL;
		}
		upgrade_mem_list->next = NULL;
		upgrade_mem_list->inuse = 0;

		if(!no_net_mbuf_free && net_mbuf_free > 0){
			//get from free memory
			if(net_mbuf_free > totlen){
				//only have one node
				net_mbuf_free = totlen;
			}
			upgrade_mem_list->data = malloc(net_mbuf_free);
			if(upgrade_mem_list->data == NULL){
				//try malloc again
				net_mbuf_free = net_mbuf_free/2;
				upgrade_mem_list->data = malloc(net_mbuf_free);
				if(upgrade_mem_list->data == NULL)
					goto free_alloced;
			}
			upgrade_mem_list->totlen = net_mbuf_free;
		}else{
			//get from cluster,the sizeof one cluster is MCLBYTES
			upgrade_mem_list->data = cluster_malloc();
			if(upgrade_mem_list->data == NULL){
				goto free_alloced;
			}
			upgrade_mem_list->totlen = MCLBYTES;
		}
	}
	
	p = upgrade_mem_list;
	totlen = totlen - upgrade_mem_list->totlen;
	
	clust_free = get_kern_free_mem();
//	printf ("%s %d clust_free=%d= totlen=%d=\n", __FUNCTION__, __LINE__, clust_free, totlen);		
	while((totlen > 0 ) && (clust_free > 5*1024)){
		mem_node = malloc(sizeof(struct upgrade_mem));
		if(mem_node == NULL){
			goto free_alloced;
		}
		mem_node->next = NULL;
		...
		p->totlen = MCLBYTES;
		clust_free = clust_free - MCLBYTES;
		totlen = totlen - p->totlen;
	}
		
	mbuf_free = get_kern_mbuf_free_mem();
//	printf ("%s %d mbuf_free=%d= totlen=%d=\n", __FUNCTION__, __LINE__, mbuf_free, totlen);
	while(totlen > 0 && (mbuf_free > 5*1024))
	{
		mem_node = malloc(sizeof(struct upgrade_mem));
		if(mem_node == NULL)
		{
	...
	{	
		...
		misc_free = get_kern_misc_free_mem() - 50*1024;
	//	printf ("%s %d misc_free=%d= totlen=%d=\n", __FUNCTION__, __LINE__, misc_free, totlen);
		if (misc_free >= totlen)
		{	
			...
			p->next = mem_node;
			p = p->next;
			mem_chain_upgrade_malloc_miscpoll(&(p->data), totlen);
	...
	return upgrade_mem_list;
	
free_alloced:
	p = upgrade_mem_list->next;
	while(p){
		q = p->next;
		if(p->data){
			if(p->totlen == MCLBYTES)
			{
				cluster_free(p->data);
			}
			else
			{
				muf_page_free(p->data);
			}
		}
		free(p);
		p = q;
	...
	if(!no_net_mbuf_free){
		if(upgrade_mem_list->data){
			free(upgrade_mem_list->data);
		}
	}else{
		if(upgrade_mem_list->data){
			if(upgrade_mem_list->totlen == MCLBYTES)
			{
				cluster_free(upgrade_mem_list->data);
			}
			else
			{
				muf_page_free(upgrade_mem_list->data);
			}
		}
	}

	free(upgrade_mem_list);
	upgrade_mem_list = NULL;

	return NULL;
}

void upgrade_mem_free(char *head)
{
	struct upgrade_mem *p,*q;

	p =  (struct upgrade_mem *)head;
	p = p->next;
	...
	while(p){
		...
		if(p->data){
			...
			else if (upgrade_pdate_misc == p->data)
			{
				mem_chain_upgrade_free_miscpoll(upgrade_pdate_misc);
			}
			else 
			{
				muf_page_free(p->data);
...
void upgrade_add_text(char *head, char *text, int nbytes)
{
	struct upgrade_mem *p,*q;
	int copy_bytes;
...
put_remain2:
	...
	if(p){
		//diag_printf("p->totlen=%d  p->inuse = %d nbytes = %d\n",p->totlen,p->inuse,nbytes);
		if((p->totlen - p->inuse) >= nbytes){
			memcpy(&p->data[p->inuse], text,nbytes);
			p->inuse += nbytes;
		}else{
			copy_bytes = p->totlen - p->inuse;
			memcpy(&p->data[p->inuse], text,copy_bytes);
			p->inuse += copy_bytes;
			nbytes -= copy_bytes;
			text = &text[copy_bytes];
			goto put_remain2;
		}
	}else{
		//no full space,put into first node, then the data will be wrong late
		q->inuse = 0;
		goto put_remain1;
	}
}
 ...
 */
		...
		while ((rc = websGetInput(wp, &text, &nbytes)) == 0) {
			...
			if(while_count>100)
			{
				HTTP_DBGPRINT (HTTP_DEBUG_OFF,"websGetInput funtion continue %d count \n",while_count);
				break;
			}
 ...
 */
					...
					if(text) {
						wp->query = brealloc(B_L, wp->query, (len + gstrlen(text) + 2) * sizeof(char_t));
						wp->query[len++] = '&';
 ...
 */
					if (text != NULL)
					{
					...
					#else
						len = wp->clen2 - wp->clen;

						if(len == 0){
							/*first POST packet*/
							if(wp->url && strncmp(wp->url,"/cgi-bin/upgrade",16) == 0){
								diag_printf("--------------websReadEvent-------%d----------\n",__LINE__);
								is_upgrade_flag = 1;
							}
							else if(wp->url && strncmp(wp->url,"/login/Auth",11) == 0){
								is_upgrade_flag = 0;
							}

							if(is_upgrade_flag){
								/*is upgrade? then stop all wan service*/
								sys_before_upgrade();
								#ifdef BCM
								...
								#endif
								diag_printf("upgrade file size:------%x--------\n", wp->clen2);
								#ifdef BCM
								if(wp->clen2 >(flashutl_desc->size - CFE_TRX_NVRAM_SIZE)){
								#endif
								#ifdef REALTEK
								/*llm add*/
								if(wp->clen2 > RTK_TRX_SIZE + MISC_SIZE){
								#endif
								/*can not greater than flash size reduce 160KB*/
									wp->query = NULL;
									/*Modified by wzs.2014-6-19 大文件无法上传成功,导致无法检测升级文件*/
									//goto upgrade_error;	
								}
								else
								{
								/*alloc mem to upgrade*/
									wp->query = (char *)upgrade_mem_alloc(wp->clen2);
								}
							}
							else
							{
								wp->query = ( char_t *)balloc (B_L,(wp->clen2+1)* sizeof(char_t));
							}
						}

						if (wp->query) {
							if(is_upgrade_flag){
								upgrade_add_text(wp->query,text,nbytes);
							}else{
								/*maybe upgrade file has '\0' character, so use memcpy*/
						...
						{
							diag_printf("---------websReadEvent-------%d-------\n",__LINE__);
							/*pxy rm, reference A8 source code*/
						#if 0
							/*if memory allocate error*/
							done++;
							break;
				...
				/*pxy rm, reference A8 source code*/
			#if 0
				//在eCos系统中,如果升级时走到这里,可能是前面
				//内存分配错误了,要不然不会走到这里,如果走到这里
				//后面的程序从wp->query 得不到正确的数据
 ...
 */
	if (wp->state == WEBS_POST_CLEN || ((wp->flags&WEBS_CGI_REQUEST)&&wp->state==WEBS_PROCESSING)) {
	...
	if (len > 0) {
		...
		if (wp->flags & WEBS_SECURE) {
			nbytes = websSSLRead(wp->wsp, text, len);
		} else {
			nbytes = socketRead(wp->sid, text, len);
		}
#else
		nbytes = socketRead(wp->sid, text, len);
#endif
		if (nbytes < 0) {						/* Error */
			websDone(wp, 0);
			bfree(B_L, text);//test by Coverity
			return -1;
 ...
 *			is NULL terminated.
 */
 			websSetTimeMark(wp);       /*modify by kevin*/
			text[nbytes] = '\0';
#if 0
 ...
 */
				...
				if (wp->flags & WEBS_POST_REQUEST) {
					if (wp->flags & WEBS_CLEN) {
						...
						clen = wp->clen;
//roy+++ 2010/09/10,record the orig clen
						wp->clen2 = wp->clen;
//+++
					} else {
						wp->state = WEBS_POST;
...
/*
	Here we need to do , is filtering abnormal request  for us.
	We just to deal it Redirect to xxx.xxx.xxx.x/login.asp
	If we need to add some new requsest , pay more attention here.
	wangxinyu.yy@gmail.com
*/

#define CMP_URL_STR(url, str) strncmp(url, str, strlen(str))
#define TRACE()	//printf("func=%s,line=%d\n",__FUNCTION__, __LINE__)
int checkHttpRequestValid(char *url)
{
	...
	snprintf(tmpBuf, sizeof(tmpBuf) - 1, "%s", url);
	if(p = strchr(tmpBuf, '?'))
		*p = '\0';

	if(websRomPageStat(tmpBuf, &sbuf) >= 0)
	{
		goto normal;
	}
	TRACE();

	//sepical we need pass
	if(0 == CMP_URL_STR(url, "/goform/") ||
		0 == CMP_URL_STR(url, "/login/Auth") ||
		0 == CMP_URL_STR(url, "/cgi-bin/upgrade") ||
		0 == CMP_URL_STR(url, "/cgi-bin/UploadCfg") ||
		0 == CMP_URL_STR(url, "/cgi-bin/DownloadCfg") ||
		0 == CMP_URL_STR(url, "/cgi-bin/DownloadFlash") ||
		0 == CMP_URL_STR(url, "/cgi-bin/DownloadSyslog") ||
		0 == CMP_URL_STR(url, "/favicon.ico") ||
		0 == CMP_URL_STR(url, "/public")
	)
	{
		goto normal;
	}
...
invalid:
	...
	//printf("invalid url Request , just Redirect login.asp\n");
	return -1;
...
/* pxy add, 2013.12.05
 * in oder to prevent exception caused by cell phone, do the following
 */
	if (strlen(url)> WEBS_MAX_URL_2 && strncmp(url,"/goform/",strlen("/goform/")) != 0){
		websError(wp, 400, T("Bad URL request"));
		return -1;
	}
/* pxy add end*/

/* wangxinyu added */
	if(checkHttpRequestValid(url) < 0)
	{
		{
			websRedirect(wp, T("/"));
		}
...
#endif
#ifdef __ECOS
		if (gstrstr(url, CGI_BIN) != NULL || gstrstr(url, ".css") != NULL || gstrstr(url, ".js") != NULL || gstrstr(url, "/goform/") != NULL)
			wp->flags |= WEBS_CGI_REQUEST;
#endif

#ifdef __ECOS
		if (gstrstr(url, CGI_BIN_UPGRADE) != NULL || gstrstr(url, "?upgrade") != NULL)
			wp->flags |= WEBS_HEADER_UPGRADE;
#endif


#ifdef __ECOS //roy +++
			if (! (wp->flags & (WEBS_HEADER_UPGRADE|WEBS_CGI_REQUEST)) /*not upgrade opration*/)
			{
				/*filter query like this to avoid system Exception:
				url=/?dataver=pb
				query=dataver=pb*/

				if(gstrcmp(query,"") != 0 && ! gisdigit(query[0])/* && gstrcmp(query,"v=6.0.0") != 0*/)/*not access to our web*/
				{
					websError(wp, 400, T("Bad URL request"));
					return -1;
				}
...
#endif
		if (gstrcmp(key, T("accept-language")) == 0) {
			wp->accLang = bstrdup(B_L, value);
...
/* RCL : the field ends at a quote or a comma, this fix allows
   ...
   look for an invalid character, we know what those are. */
					/*   while (isgoodchar(*npv)) { *//*remove the old way*/
     					while ( (*npv != ',') && (*npv != '\"') ) {
						npv++;
     					}
...
//roy+++,2010/10/15
		else if(gstrcmp(key, T("host")) == 0){
			//snprintf(wp->host_str, 32,"%s",value);
			wp->host_str = bstrdup(B_L, value);
		}
//roy+++
...
/*
*Login Succes: Response to page main.asp without Cookie
*/
extern char_t g_User[64],g_Pass[64];
//hqw add
extern char cookie_suffix[20][4];
//end
#ifdef __CONFIG_TENDA_MULTI__
extern int de_lang, en_lang, es_lang, fr_lang, it_lang, ko_lang, pl_lang, pt_lang, ru_lang, tr_lang;
#endif
void websResponseWithCookie(webs_t wp, int code, char_t *message, char_t *redirect, int user_id)
{
	char_t	*date;
	char_t	userid[32];
	char_t	password[256];
	...
	strncpy(userid,g_User,sizeof(userid));
	//hqw add
	//strncpy(password,g_Pass,sizeof(password));
	//end
	//hqw add
	//根据coverity分析结果修改，原来为无效的判断:if (wp->ipaddr!=NULL)  2017/1/11 F9项目修改
	if(wp!=NULL)
	{
		sprintf(password,"%s%s",g_Pass,cookie_suffix[((unsigned int)inet_addr(wp->ipaddr))%20]);
	}
	else
	{
		sprintf(password,"%s",g_Pass);
	}
	//end

#ifdef __CONFIG_TENDA_MULTI__
	if(wp->accLang && ( (ko_lang && strncmp(wp->accLang, "ko", 2) == 0)||
											(de_lang && strncmp(wp->accLang, "de", 2) == 0)||
											(en_lang && strncmp(wp->accLang, "en", 2) == 0)||
											(es_lang && strncmp(wp->accLang, "es", 2) == 0)||
											(fr_lang && strncmp(wp->accLang, "fr", 2) == 0)||
											(it_lang && strncmp(wp->accLang, "it", 2) == 0)||
											(pl_lang && strncmp(wp->accLang, "pl", 2) == 0)||
											(pt_lang && strncmp(wp->accLang, "pt", 2) == 0)||
											(ru_lang && strncmp(wp->accLang, "ru", 2) == 0)||
											(tr_lang && strncmp(wp->accLang, "tr", 2) == 0))){

		 	strncpy(language,wp->accLang,2);
		 	language[2] = 0;
	}
	else
	{
 ...
 */
		...
		if(user_id == 1){
			websWrite(wp, T("Set-Cookie: ecos_pw=%s:language=%s; path=/\r\n"),password, language);
		}else{
			websWrite(wp, T("Set-Cookie: language=%s; path=/\r\n"),language);
		}
 ...
 */
	if (gstrstr(url, T("http://")) == NULL) {
		...
		//diag_printf("[websRedirect]:: redirectFmt==%s\n",redirectFmt);
...
//diag_printf("[websRedirect]:: url==%s\n",url);
//diag_printf("[websRedirect]:: msgbufl==%s\n",msgbuf);
//diag_printf("[websRedirect]:: wp->path==%s, wp->lpath==%s\n",wp->path, wp->lpath);
//diag_printf("[websRedirect]--->websResponse\n");
...
int websWriteLongString(webs_t wp, char_t *buf)
{
	int 		rc = 0;
	int 		tLen = 0;
	int 		rLen = 0;
	...
	while(rLen < tLen)
	{
		...
		{
			memcpy(tmpBuf, p, 512);
			rc += websWrite(wp,T("%s"), tmpBuf);
			p += 512;
			rLen += 512;
		}
		else
		{
			rc += websWrite(wp,T("%s"), p);
			break;
		}
	}
