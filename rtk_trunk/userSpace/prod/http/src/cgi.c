 ...
 */
	argpsize = 10;
	argp = (char_t **)balloc(B_L, argpsize * sizeof(char_t *));
	*argp = cgiPath;
	n = 1;
	if (gstrchr(query, '=') == NULL) {
		...
		for (cp = gstrtok(query, T(" ")); cp != NULL; ) {
			*(argp+n) = cp;
			...
			if (n >= argpsize) {
				argpsize *= 2;
				argp = (char_t **)brealloc(B_L, argp, argpsize * sizeof(char_t *));
			}
			cp = gstrtok(NULL, T(" "));
		}
	}
 ...
 */
	...
	envp = (char_t **)balloc(B_L, envpsize * sizeof(char_t *));
	n = 0;
	...
	for (s = symFirst(wp->cgiVars); s != NULL; s = symNext(wp->cgiVars)) {
		if (s->content.valid && s->content.type == string &&
			...
			if (n >= envpsize) {
				envpsize *= 2;
				envp = (char_t **)brealloc(B_L, envp, envpsize * sizeof(char_t *));
			}
		}
	}
	*(envp+n) = NULL;
