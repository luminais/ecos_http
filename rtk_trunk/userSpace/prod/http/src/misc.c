	...
	while ((c = *fmt++) != '\0') {
		...
		//	enum flag f = flag_none;
			...
			if (c == 'd' || c == 'i') {
				...
				if (value >= 0) {
					if (f & flag_plus) {
						put_ulong(&buf, value, 10, 0, T("+"), width, prec, (enum flag)f);
					} else if (f & flag_space) {
						put_ulong(&buf, value, 10, 0, T(" "), width, prec, (enum flag)f);
					} else {
						put_ulong(&buf, value, 10, 0, NULL, width, prec, (enum flag)f);
					}
				} else {
					put_ulong(&buf, -value, 10, 0, T("-"), width, prec, (enum flag)f);
				}
				...
				if (c == 'o') {
					if (f & flag_hash && value != 0) {
						put_ulong(&buf, value, 8, 0, T("0"), width, prec, (enum flag)f);
					} else {
						put_ulong(&buf, value, 8, 0, NULL, width, prec, (enum flag)f);
					}
				} else if (c == 'u') {
					put_ulong(&buf, value, 10, 0, NULL, width, prec, (enum flag)f);
				} else {
					if (f & flag_hash && value != 0) {
						if (c == 'x') {
							put_ulong(&buf, value, 16, 0, T("0x"), width, prec, (enum flag)f);
						} else {
							put_ulong(&buf, value, 16, 1, T("0X"), width, prec, (enum flag)f);
						}
					} else {
                   ...
                   */
						put_ulong(&buf, value, 16, ('X' == c) , NULL, width, prec, (enum flag)f);
					}
				}
				...
				if (value == NULL) {
					put_string(&buf, T("(null)"), -1, width, prec, (enum flag)f);
				} else if (f & flag_hash) {
					put_string(&buf,
						value + 1, (char_t) *value, width, prec, (enum flag)f);
				} else {
					put_string(&buf, value, -1, width, prec, (enum flag)f);
				}
			} else if (c == 'p') {
				void *value = va_arg(arg, void *);
				put_ulong(&buf,
					(unsigned long int) value, 16, 0, T("0x"), width, prec, (enum flag)f);
			} else if (c == 'n') {
				if (f & flag_short) {
...
char_t *ascToUni(char_t *ubuf, char *str, int nBytes)
{
	...
	int addr_ipv4to6_flag = 0;
	char *str1 = strstr(str, "::ffff:");
	if(str1)
	{
	...
	if(addr_ipv4to6_flag)
	{
	#ifdef UNICODE
		if (MultiByteToWideChar(CP_ACP, 0, str+7, nBytes / sizeof(char_t), ubuf,
	...
	#ifdef kUseMemcopy
	   memcpy(ubuf, str+7, nBytes);
	#else
		strncpy(ubuf, str+7, nBytes);
	#endif /*kUseMemcopy*/
	#endif
