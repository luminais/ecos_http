...
//roy modify
		...
		buf += len;
#else
		room = ringqPutBlk(rq, (unsigned char *) buf, len);
		if(room <= 0){
			if (socketFlush(sid) < 0) {
					return -1;
			}
 ...
 */
		if (c == '\n') {
			len = ringqLen(lq);
			if (len > 0) {
#if 0//roy modify,2011/11/24				
				*buf = ballocAscToUni((char *)lq->servp, len);
#else
				buf2 = balloc(B_L, (len + 1) * sizeof(char_t));
				if ( buf2 == NULL) {
					len = -1;//error
					*buf = NULL;
				}else{
					memcpy(buf2, (char *)lq->servp, (len + 1) * sizeof(char_t));
					buf2[len] = 0;
					*buf  = buf2;
				}
#endif
...
int socketFlush(int sid)
{
	...
	HTTP_DBGPRINT(HTTP_DEBUG_ERROR, "%s\n", __FUNCTION__);
...
int socketAlloc(char *host, int port, socketAccept_t accept, int flags)
{
	...
	int			sid;
//roy add
	HTTP_DBGPRINT(HTTP_DEBUG_ERROR, "%s: socketMax = %d\n", __FUNCTION__, socketMax);
	if(socketMax > 20)
		return -1;
//	
