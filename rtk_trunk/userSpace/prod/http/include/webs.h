...
#define WEBS_HEADER_BUFINC 		1024/*512			 Header buffer size */
#define WEBS_ASP_BUFINC			1024/*512			Asp expansion increment */
...
#define WEBS_SOCKET_BUFSIZ		1024/*512			 Bytes read from socket */
#define WEBS_MAX_URL_2			128			/* pxy add, 2013.12.05 */


#define WEBS_HTTP_PORT			T("httpPort")
#define CGI_BIN					T("cgi-bin")
#define CGI_BIN_UPGRADE         T("/cgi-bin/upgrade")
...
#define WEBS_HEADER_UPGRADE		0x80000		/* Already output the HTTP header */
...
#define COOKIE_SUFFIX_NUM	80			/* tenda coockie ???¡§2?¨¢¡§??????¡§oy*/
#endif
/* 
...
#ifdef __CONFIG_IPV6__
	char_t			ipaddr[40];			/* Connecting ipaddress */
#else
	char_t			ipaddr[32];			/* Connecting ipaddress */
#endif
//roy+++,2010/10/15
	char_t			*host_str;//[32];			/* Host: oa.tenda.cn -->oa.tenda.cn*/
//+++
...
#ifdef __CONFIG_MOBILE_WEB__	//roy modify,we don't need userAgent, remove for saving memory
	char_t			*userAgent;			/* User agent (browser) */
#endif
	//remember accept language 
	char_t			*accLang;			/* remember accept language  */	
...
//roy+++ 2010/09/10	
	int				clen2;				/* record the orig clen */
//+++
...
extern int 	websWriteLongString(webs_t wp, char_t *buf);
