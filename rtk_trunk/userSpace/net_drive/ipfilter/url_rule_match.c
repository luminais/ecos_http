...
referer_match_record_t *referer_match_record_new(unsigned int ipaddr, len_string_t *referer_p)
{
	...
	if(referer_match_record_p)
	{
		referer_match_record_p->next = NULL;
		referer_match_record_p->arr[0] = NULL;
		referer_match_record_p->arr[1] = NULL;
		referer_match_record_p->arr[2] = NULL;
...
void url_match_record_free(void)
{
	...
	for(i=0; i<MATCH_REDIRECT_ARRAY_SIZE; i++)
	{
		...
		g_match_record[i] = NULL;
	}
}

void url_match_record_init(void)
{
	int i;
