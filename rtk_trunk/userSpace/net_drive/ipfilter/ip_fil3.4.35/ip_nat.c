...
static const char rcsid[] = "@(#)$Id: ip_nat.c,v 1.10 2010-11-04 04:06:43 Exp $";
#endif
...
extern void nat_check_fcache(nat_t *nat, tcphdr_t *tcp);

nat_t *nat_pool = NULL;
nat_t *free_nat = NULL;
nat_t *nat_alloc __P((void));
void nat_free __P((nat_t *nat));
void speedup_natexpire __P((void));

//roy add NAT_PER_IP_LIMIT related module
#define NAT_PER_IP_LIMIT	//enable limit per ip nat number

#ifdef __CONFIG_NAT_LINKS__
/* Notice: CONFIG_NAT_LINKS should be 4096 or 2048, tenda modify,2011/08/16*/
/* User defined network buffer pool and nat table sizes */
#define USER_NET_MEMPOOL_SIZE	roundup((1024*(__CONFIG_NAT_LINKS__/4)),MSIZE)
unsigned char net_mempool_area[USER_NET_MEMPOOL_SIZE];
int net_mempool_size = sizeof(net_mempool_area);
int nat_table_max = __CONFIG_NAT_LINKS__;

#ifdef NAT_PER_IP_LIMIT
int nat_table_per_ip = __CONFIG_NAT_LINKS__;
#endif

#else
/* User defined network buffer pool and nat table sizes */
#define USER_NET_MEMPOOL_SIZE	roundup((1024*1024),MSIZE)
unsigned char net_mempool_area[USER_NET_MEMPOOL_SIZE];
int net_mempool_size = sizeof(net_mempool_area);
int nat_table_max = 4096;

#ifdef NAT_PER_IP_LIMIT
int nat_table_per_ip = 4096;
#endif
...
#define TENDA_NAT_PORT_START 1030
#define TENDA_NAT_PORT_END 5000

char g_wan_ip[20];
char g_private_ip[20];

extern unsigned short tcp_checksum(struct ip *ip, struct tcphdr *tcp);
...
char nochangeweb[10][32];
int nochangewebnum;

unsigned short tenda_natport;
unsigned short tenda_ipid;
unsigned char tenda_ttl;
char wljbweb1[48];
char wljbweb2[48];
int    wljblen1;
int    wljblen2;
unsigned short tenda_mss = 1400; //初始设置mss为1400

unsigned short chsum_fixup_short(unsigned short sum, unsigned short old_data, unsigned short new_data)
{
...
#if 0
static unsigned short chsum_fixup_long(unsigned short sum, unsigned long old_data, unsigned long new_data)
{
	unsigned long sum_new;
...
/******************************************************************************
得到TCP 的seg 选项的值，如果没有seg 返回NULL
*/
unsigned short * mnat_tcp_get_seg(char *buf, int max_len)
{
  unsigned char *ptmp = (unsigned char *)buf;
	int off_len = 0;
...
unsigned short udp_chsum(unsigned short* addr,char len){
	int sum = 0;
	unsigned short *source = addr;
	...
	if (len > 0) {
		...
		*(unsigned char *) (&tmp) = * (unsigned short *) source;
		sum += tmp;
	}
...
// call from ip_fastnat.c  
int qq_packet_change(struct ip *pIpHdr,char *private_ip,char *wan_ip)
{
#if 1
	struct tcphdr *tcph = NULL;
	char *pdata = NULL,*p = NULL;
	char qq_mail_str1[]={"QQMail"};
	int data_len = 0;
	if(pIpHdr->ip_p != IPPROTO_TCP){
		return -1;
	}
	tcph = (struct tcphdr *) ((char *) pIpHdr + (pIpHdr->ip_hl << 2));
	if(tcph->th_flags & TH_SYN) return 0;
	
    	if (tcph->th_dport==0x2ee0 || tcph->th_dport==0x0050 || tcph->th_dport ==0x1f40) 
    	{//检测目的端口为12000 80 8000的tcp数据包
    		pdata = (char *) tcph + (tcph->th_off<<2);
    		data_len = ntohs(pIpHdr->ip_len) - (int)(pdata - (char*)pIpHdr);
      		if (data_len < 0)
      			return -1;
		p = memstr(qq_mail_str1,pdata,sizeof(qq_mail_str1),data_len);
        	if (p == NULL){
          		return -1;
        	}else{
	    	  	p = memstr(private_ip,p,6,data_len);//在数据包中查找私网ip的前六位
	    	  	if (p == NULL){
	    		 	return -1;
	    	  	}
	    	  	memcpy(p,wan_ip,strlen(wan_ip));
	  
	    	  	tcph->th_sum = 0;
		 	tcph->th_sum = tcp_checksum(pIpHdr,tcph);
       		}
    	}
#endif
	return 0;
...
// call from ip_fastnat.c  
int xunlei_packet_change(struct ip *pIpHdr,char *private_ip,char *wan_ip)
{ 
#if 1
 //	xunlei某些下载数据包中包含私网ip
  //xunlei下载数据包 特征码为  0x00,0x80,0x01,0x14,0x00,0x00,0x00
  //和 0x00,0x00,0x00,0x00,0x01,0x10,0x00,0x00,0x00
  //两个特征码之间相隔24个字节 
  //私网ip和第二个特征码相隔20个字节
    struct tcphdr *tcph = NULL;
    struct udphdr *udph = NULL;
    char  *pdata = NULL,*p =NULL;
    char magic_code1[7]    = {0x00,0x80,0x01,0x14,0x00,0x00,0x00};
    char magic_code2[9]    = {0x00,0x00,0x00,0x00,0x01,0x10,0x00,0x00,0x00};
	...
	if(tcph->th_flags & TH_SYN) return 0;
        ...
        udph = (struct udphdr *) ((char *) pIpHdr + (pIpHdr->ip_hl << 2));
        pdata =  ( char *)udph + sizeof(struct udphdr);
        data_len = ntohs(udph->uh_ulen) -  sizeof(struct udphdr);
    }else{
    	return -1;
    }
  	if(data_len <= 0)
    ...
    p = memstr(magic_code1,pdata,sizeof(magic_code1),data_len);
    if (p == NULL){
        return -1;
    }else{
        if (memcmp(p + sizeof(magic_code1) + 24,magic_code2,9) != 0){
          return -1;
        }else{    
      	 p = memstr(private_ip,p,6,data_len);//在数据包中查找私网ip的前六位 
         if(p == NULL)
          	...
          	memcpy(p,wan_ip,strlen(wan_ip));//固定填写一个ip值
        }
    }//if(!p) else
   	if(pIpHdr->ip_p == IPPROTO_TCP){
		tcph->th_sum = 0;
		...
		udph->uh_sum = 0;
		udph->uh_sum = udp_chsum((unsigned short*)udph,sizeof(struct udphdr));
	}
#endif
	return 0;
}
...
int tenda_change_packet(struct ip *pIpHdr)
{
	if ((pIpHdr->ip_off & 0x1fff)==0)
	{
		pIpHdr->ip_sum = chsum_fixup_short(pIpHdr->ip_sum, pIpHdr->ip_id, htons(tenda_ipid));
		pIpHdr->ip_id = htons(tenda_ipid);
		tenda_ipid++;
		if(tenda_ipid == 0xffff)
		{
			tenda_ipid = 1;
		}
		if(pIpHdr->ip_p == 6)
		{
			struct tcphdr * tc = (struct tcphdr *) ((char *) pIpHdr + (pIpHdr->ip_hl << 2));
			if(tc->th_flags & TH_SYN)
			{
				tc->th_sum = chsum_fixup_short(tc->th_sum, tc->th_win, htons(16400));
				tc->th_win = htons(16400);
				if(tc->th_off >= 7)
				{
					//int oldlen = tc->th_off;
					unsigned char *p = (unsigned char *)tc + 20;
					memset(p, 0, (tc->th_off<<2) - 20);
					*p = 0x02;
					*(p+1) = 0x04;
					*(p+2) = 0x05;
					//*(p+3) = 0x78;
					...
					*(p+7) = 0x02;
					tc->th_sum = 0;
					tc->th_off = 7;
					pIpHdr->ip_sum = chsum_fixup_short(pIpHdr->ip_sum, pIpHdr->ip_len, 48);
					pIpHdr->ip_len = 48;
					tc->th_sum = tcp_checksum(pIpHdr,tc);
				}
			}
			else if(tc->th_flags & TH_RST)
			{
				tc->th_sum = chsum_fixup_short(tc->th_sum, tc->th_win, htons(16400));
				tc->th_win = htons(16400);
			}
			...
			{
				int newwin = htons(tc->th_win) - 49103;
				if(newwin <= 0)
				{
					newwin = 16432;
				}
				tc->th_sum = chsum_fixup_short(tc->th_sum, tc->th_win, htons(newwin));
				tc->th_win = htons(newwin);
			}
		}
	}
#if 0	//in ip_output.c
	if(pIpHdr->ip_ttl != tenda_ttl)
	{
		unsigned short *p1 = (unsigned short *)&pIpHdr->ip_ttl;
		unsigned short p2 = pIpHdr->ip_p*256 + tenda_ttl;
		pIpHdr->ip_sum = chsum_fixup_short(pIpHdr->ip_sum, *p1, p2);
		pIpHdr->ip_ttl = tenda_ttl;
	}
#endif
	return 0;
}

int tenda_change_packet2(struct ip *pIpHdr)
{
		pIpHdr->ip_sum = chsum_fixup_short(pIpHdr->ip_sum, pIpHdr->ip_id, htons(tenda_ipid));
		pIpHdr->ip_id = htons(tenda_ipid);
		tenda_ipid++;
		...
		{
			struct tcphdr * tc = (struct tcphdr *) ((char *) pIpHdr + (pIpHdr->ip_hl << 2));
			if ((tc->th_flags & (TH_SYN|TH_RST)) == 0)
			{
				int newwin = htons(tc->th_win) - 49103;
				if(newwin <= 0)
				{
...
int nat_init()
{
	...
	if (nat_pool == NULL) {
		...
		KMALLOCS(nat_pool, nat_t *, sizeof(nat_t) * nat_table_max);
		if (nat_pool != NULL) {
			bzero((char *)nat_pool, sizeof(nat_t ) * nat_table_max);
			for (i = 0, nat = nat_pool; i < nat_table_max - 1; i++, nat++)
				nat->nat_next = nat + 1;
			free_nat = nat_pool;
		} else
			return -1;
...
#ifdef TENDA_WLJB
	tenda_natport = TENDA_NAT_PORT_START;
	tenda_ipid = 1;
	tenda_ttl = 128;
	strcpy(wljbweb1, "http://61.156.33.1/kdgg/");
	wljblen1 = strlen(wljbweb1);

	strcpy(wljbweb2, "http://61.156.7.170/notice/");
	wljblen2 = strlen(wljbweb2);

	strcpy(g_wan_ip,NSTR(SYS_wan_ip));
	strcpy(g_private_ip,NSTR(SYS_lan_ip));
...
nat_t *nat_alloc()
{
	...
	if (nat_stats.ns_inuse >= /*nat_table_max*/ipf_nattable_max) {//roy modify
		speedup_natexpire();
		return NULL;
	}
	if (free_nat == NULL)
	...
	if (nat) {
		bzero((char *)nat, sizeof(nat_t));
	}
	return nat;
}

void nat_free(nat_t *nat)
{
...
ipnat_t *n;
{
	...
	n->in_prnext = np;
	*np = n;
	if (n->in_flags & IPN_AUTOFW) {	
		n->in_autofwnext = autofw_rules;
		autofw_rules = n;
	}
}
...
int mode;
{
...
#if (BSD >= 199306) && defined(_KERNEL) && !defined(__ECOS)
	...
	case SIOCRUPDATENATMSS :
		for (np = &nat_list; (n = *np); np = &n->in_next)
		{
			if((NAT_MAP == n->in_redir) && (n->in_flags & IPN_AUTOPORTMAP))
				break;
		}
		if(NULL != n)
		{	
			IRCOPY(data, (char *)&(n->in_mssclamp), sizeof(n->in_mssclamp));
		}
		break;
#endif
	case SIOCGNATS :
...
static int check_icmpquerytype4(int icmptype)
{
	/*
...
#ifdef NAT_PER_IP_LIMIT
static int get_nat_num_from_maptable(struct in_addr real)
{
	hostmap_t	*hmp;
	int nat_num = 0;
	hmp = maptable[real.s_addr % HOSTMAP_SIZE];
	while(hmp){
		nat_num+=hmp->hm_ref;
		hmp = hmp->hm_next;
	}
...
void speedup_natexpire_tenda(struct in_addr real)
{
	...
	for (natp = &nat_instances; (nat = *natp); ) {
		if(real.s_addr == nat->nat_inip.s_addr){
			switch (nat->nat_p) {
			case IPPROTO_UDP:
				if (nat->nat_age < fr_defnatage*3/4)
					nat->nat_age = 1;
				
				break;
			case IPPROTO_TCP:
				if ((nat->nat_age > fr_tcptimeout) && (ntohs(nat->nat_oport) > 10000/*xunlei*/ || (nat->nat_age < fr_tcpidletimeout*3/4)))
					nat->nat_age = 1;
				break;
			default:
				break;
...
int direction;
{
	...
	icmphdr_t *icmp = NULL;
	aproxy_t *apr = NULL;
#if 0 //handle in nat_malloc, roy modify
...
#ifdef NAT_PER_IP_LIMIT
		if(get_nat_num_from_maptable(fin->fin_src) >= nat_table_per_ip && nat_table_max < (nat_stats.ns_inuse + 300)/*没有NAT剩余*/){
			speedup_natexpire_tenda(fin->fin_src);
			return NULL;
		}
#endif		
	}
	...
	if (nat == NULL) {
		 ...
		 * conditions arising.
		 */
		if (ipf_nattable_max > /*ipf_nattable_sz*/nat_table_max) {
	...
	if (direction == NAT_OUTBOUND) {
		...
		do {
			...
			if (np->in_redir & NAT_MAPBLK) {
				...
				if ((flags & IPN_TCPUDP) &&
					...
					port = htons(port);
					tenda_natport++;
					if(tenda_natport > TENDA_NAT_PORT_END)
					{
						tenda_natport = TENDA_NAT_PORT_START;
					}
#else
		...
		if (np->in_space > 0)
			np->in_space--;
		/* Scan autofw rules */
		for (tnp = autofw_rules; tnp; tnp = tnp->in_autofwnext) {
			if ((tnp->in_autofwp & nflags) &&
			    (ntohs(dport) >= tnp->in_autofwport[0]) && (ntohs(dport) <= tnp->in_autofwport[1])) {
				tnp->in_autofwuse = IPN_AUTOFW;
				tnp->in_inip = fin->fin_saddr;
				tnp->in_inmsk = 0xffffffff;
				tnp->in_autofwage = 1200;
			}
		}
		/* Setup the NAT table */
	...
	if ((np->in_apr != NULL) && (np->in_dport == 0 ||
		...
		apr = np->in_apr;
	(void) appr_new(fin, ip, nat, apr);

	np->in_use++;
#ifdef	IPFILTER_LOG
	nat_log(nat, (u_int)np->in_redir);
#endif
	nat_set_fcache(fin, nat);
	return nat;
badnat:
	nat_stats.ns_badnat++;
...
int rw;
{
	...
	if (fin != NULL)
		...
		ifp = NULL;
	dst = mapdst.s_addr;
	if (fin && (flags & IPN_TCPUDP)) {
		sport = htons(fin->fin_data[0]);
		dport = htons(fin->fin_data[1]);
	} else if (fin && (flags & IPN_ICMPQUERY)) {
		icmphdr_t *icmp;
		...
		gre = (fin)?((p == IPPROTO_GRE) ? (grehdr_t *)fin->fin_dp : NULL):NULL;
		sport = 0;
		dport = 0;
	}
	...
	for (; nat; nat = nat->nat_hnext[1]) {
		...
		// add by z10312 16-0112 解决移植bcm ipnat组件不能转发问题,realtek 有双接口,所以不能直接比较接口指针
		if ((!ifp ||(!strcmp(((struct ifnet *)ifp)->if_xname, ((struct ifnet *)(nat->nat_ifp))->if_xname)) ) &&
		    nat->nat_oip.s_addr == src.s_addr &&
		    nat->nat_outip.s_addr == dst &&
		    (((p == 0) && (sflags == (nat->nat_flags & IPN_TCPUDPICMP))) || (p == nat->nat_p))) {
			switch (p)
			{
			case IPPROTO_GRE :
				if (gre && (gre->gr_call != nat->nat_gre.gs_call))
					continue;
				break;
			case IPPROTO_TCP :
			case IPPROTO_UDP :
...
int rw;
{
	...
	for (; nat; nat = nat->nat_hnext[0]) {
		...
		if ((!ifp || ifp == nat->nat_ifp) &&
		    ...
		    (((p == 0) && (sflags == (nflags & IPN_TCPUDPICMP))) || (p == nat->nat_p))) {
			switch (p)
			{
			case IPPROTO_GRE :
				if (gre && (gre->gr_call != nat->nat_gre.gs_call))
...
fr_info_t *fin;
{
	...
	if ((fin->fin_p == IPPROTO_ICMP) && !(nflags & IPN_ICMPQUERY) &&
		...
		natadd = 0;
				      ...
				      fin->fin_dst, 1))) {
		if ((nat->nat_flags & (FI_WILDP|FI_WILDA)) != 0) {
...
maskloop:
		...
		for (np = nat_rules[hv]; np; np = np->in_mnext)
		{
			...
			if((np->in_redir == NAT_REDIRECT) && (np->in_pnext != sport))
				continue;
#ifdef TENDA_WLJB
			//diag_printf("%s %d src=%x dst=%x sp=%d dp=%d ttl=%d\r\n", __FILE__, __LINE__,ip->ip_src.s_addr,ip->ip_dst.s_addr,sport,dport,ip->ip_ttl);
			tenda_change_packet(ip);
#endif
...
extern int g_enable_remote_acl,g_remote_port;


static int check_localport(unsigned int dst, unsigned short port, unsigned char proto)
{
	...
	switch (proto) {
	case IPPROTO_TCP:
		inpb = LIST_FIRST(tcbinfo.listhead);
		break;
	case IPPROTO_UDP:
		inpb = LIST_FIRST(udbinfo.listhead);
		break;
	default:
		return 0;
	}
	
	for (; inpb; inpb = LIST_NEXT(inpb, inp_list)) {
		if ((inpb->inp_lport == port) &&
		    ((inpb->inp_laddr.s_addr == 0) || (inpb->inp_laddr.s_addr == dst))) {
		    	if((!g_enable_remote_acl && ntohs(port) == 80) ||  (g_enable_remote_acl && g_remote_port != 80 && ntohs(port) == 80 ))//roy add
					return 0;
			else	
					return 1;
		}
...
fr_info_t *fin;
{
	...
	if ((fin->fin_off == 0) && !(fin->fin_fl & FI_SHORT)) {
		if ((fin->fin_p == IPPROTO_TCP) || (fin->fin_p == IPPROTO_UDP)) {
			tcp = (tcphdr_t *)fin->fin_dp;
			if (check_localport(fin->fin_daddr, tcp->th_dport, fin->fin_p) != 0) {
				fin->fin_misc |= FM_OURPORT;
				return 0;
			}
		}
	}
...
maskloop:
		...
		for (np = rdr_rules[hv]; np; np = np->in_rnext) {
			if ((np->in_ifp && (np->in_ifp != ifp)) ||
				...
				continue;
			if (((np->in_flags & IPN_AUTOFW) ^ np->in_autofwuse))
				continue;
			if (np->in_flags & IPN_FILTER) {
				if (!nat_match(fin, np, ip))
					continue;
...
void ip_natunload()
{
	...
	if (nat_pool != NULL) {
		KFREES(nat_pool, sizeof(nat_t) * nat_table_max);
		nat_pool = NULL;
		free_nat = NULL;
	}
}
...
#ifdef NAT_PER_IP_LIMIT	
	unsigned int host_count = 0,hv;	
	unsigned int optimize_bucket = 0,burst = 0;
	hostmap_t	*hmp;
#endif
...
192.168.1.101 -> 0.0.0.0 (use = 56 hv = 649)
192.168.1.102 -> 0.0.0.0 (use = 3 hv = 653)
192.168.1.102 -> 0.0.0.0 (use = 593 hv = 653)
*/
	for (hv = 0; maptable && hv < ipf_hostmap_sz; hv++) {
		hmp = maptable[hv];			
		if (hmp) {
			host_count++;
		}
	...
	if(host_count > 1){
		optimize_bucket = nat_table_max/host_count;
		if(nat_stats.ns_inuse <= nat_table_max*1/6){
			burst = optimize_bucket>>1;
		}else if(nat_stats.ns_inuse <= nat_table_max*2/6){
			burst = optimize_bucket>>2;
		}else if(nat_stats.ns_inuse <= nat_table_max*3/6){
			burst = optimize_bucket>>3;
		}else if(nat_stats.ns_inuse <= nat_table_max*4/6){
			burst = optimize_bucket>>4;
		}else if(nat_stats.ns_inuse <= nat_table_max*5/6){
			burst = optimize_bucket>>5;
		}else{
			burst = 0;
		}

		optimize_bucket += burst;
		nat_table_per_ip = optimize_bucket;
	}
	else{
		nat_table_per_ip = nat_table_max;
	}
#endif
...
void ip_autofwexpire(void)
{
	ipnat_t *np;
	int s;

	SPL_NET(s);
	for (np = autofw_rules; np; np = np->in_autofwnext) {
		if (np->in_autofwuse) {
			if ((--np->in_autofwage) == 0)
				np->in_autofwuse &= ~IPN_AUTOFW;
		}
	}
	SPL_X(s);
}
...
u_short *csump;
{
	...
	if (hlen > sizeof(*tcp)) {
		...
		while (cp < ep) {
			...
			if (advance == 0) 
			{
				diag_printf("%s:unknown option with zero length\n", __func__);
				break;
			}
