...
static const char rcsid[] = "@(#)$Id: ipnat.c,v 1.5 2010-08-19 08:51:40 Exp $";
#endif
...
#ifdef __ECOS
int nataddrule __P((char *fmt, ...));
int natdelrule __P((char *fmt, ...));

int kmemcpy(char *buf, long pos, int size)
{
	memcpy(buf, (char *)pos, size);
	return 0;
}
...
/* Set NAT rule to kernel */ 
int natupdatemssrule(int mss)
{
	int rc = -1;
	int fd;
	...
	if (fd >= 0) {
		/* update mss */
		rc = ioctl(fd, SIOCRUPDATENATMSS, &mss);
		close(fd);
	}
	return rc;
}
...
int nataddrule(char *fmt, ...)
{
	...
	/* Rebuild nat command */
	va_start (args, fmt);
	vsprintf(natcmd, fmt, args);
	va_end (args);

	/* Parse command */
	np = natparse(natcmd, 0, &status);
	if ((np == NULL) || (status != 0))
		return -1;
	...
	if (fd >= 0) {
		/* Add nat entry */
		rc = ioctl(fd, SIOCADNAT, &np);
		close(fd);
	}
	return rc;
}

/* Delete the given NAT rule from kernel */
...
{
	printf("ipnat: [-CFhlnrsv] [-f filename] [-AD natrule]\n");
}
...
int ipnat_cmd(int argc, char *argv[])
{
	...
	if (opts & (OPT_FLUSH|OPT_CLEAR))
		flushtable(fd, opts);
	if (opts & (OPT_LIST|OPT_STAT|OPT_VERBOSE))
		dostats(nsp, opts);
	close(fd);
exit:
	return rc;
