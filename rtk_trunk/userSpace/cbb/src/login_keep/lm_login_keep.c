...
void get_login_serv_ip(char *ip)
{
	while (1)
	{
		if(0 == gpi_wan_get_connect())
		{
			diag_printf("[%s] wan is not connected, sleep %d sec . . . \n", __FUNCTION__, INTERVAL_TIME_30);
			cyg_thread_delay(INTERVAL_TIME_30*100); // 30s
			continue;
		}
...
int send_version_req(int s)
{
	struct lm_keep_version_req version_req;
	struct lm_login_keep_hdr *hdr = NULL;
	char *char_p = NULL;
	char version_req_buf[128] = {0};
	uint16 version_req_len = 0, len_tmp = 0;
	int ret = -1;

	memset(&version_req, 0x0, sizeof(version_req));
	version_req.hdr.cmd = LM_CMD_KEEP_VERSION_REQ;
	version_req.hdr.cmd = htons(version_req.hdr.cmd);
	version_req.check = htonl(keep_cks^keep_sequence);
	version_req.verlen = strlen(LM_RT_VER);
	version_req.version= LM_RT_VER;
	version_req.typelen= strlen(LM_RT_FIRM);
	version_req.type = LM_RT_FIRM;

	char_p = version_req_buf;

	len_tmp = sizeof(struct lm_login_keep_hdr) + sizeof(version_req.check) + sizeof(version_req.verlen);
	memcpy(char_p, &version_req, len_tmp);
	version_req_len += len_tmp;
	char_p += len_tmp;

	len_tmp = version_req.verlen;
	memcpy(char_p, version_req.version, len_tmp);
	version_req_len += len_tmp;
	char_p += len_tmp;

	len_tmp = sizeof(version_req.typelen);
	memcpy(char_p, &(version_req.typelen), len_tmp);
	version_req_len += len_tmp;
	char_p += len_tmp;

	len_tmp = version_req.typelen;
	memcpy(char_p, version_req.type, len_tmp);
	version_req_len += len_tmp;
	char_p += len_tmp;

	hdr = (struct lm_login_keep_hdr *)version_req_buf;
	hdr->length = htons(version_req_len);

	ret = send(s, (void *)version_req_buf, version_req_len, 0);
	if(ret < 0)
	{
	    perror("send_version_req write error\n");
	    return -1;
	}
	diag_printf("[%s][%d] conf_req_len = %d, ret = %d\n", __FUNCTION__, __LINE__, version_req_len, ret);

	keep_sequence++;
	return 0;
}

int send_router_online_req(int s)
{
	struct lm_router_online_req router_online_req;
	int ret = -1;
	uint16 length = 0;

	memset(&router_online_req, 0x0, sizeof(router_online_req));
	router_online_req.hdr.cmd = LM_CMD_KEEP_ONLINE_REQ;
	router_online_req.hdr.cmd = htons(router_online_req.hdr.cmd);
	router_online_req.check = htonl(keep_cks^keep_sequence);
	length = sizeof(router_online_req.hdr)+sizeof(router_online_req.check);
	router_online_req.hdr.length = htons(length);

	ret = send(s, (void *)(&router_online_req), length, 0);
	if(ret < 0)
	{
		perror("send_router_online_req write error\n");
		return -1;
	}
	//diag_printf("[%s][%d] length = %d, ret = %d\n", __FUNCTION__, __LINE__, length, ret);
...
int keep_router_online(char *online)
{
	struct lm_router_online_ack *online_ack = (struct lm_router_online_ack *)online;

	uint32 online_secs = ntohl(online_ack->secs);

	diag_printf("[%s][%d] online_secs = %u\n", __FUNCTION__, __LINE__, online_secs);
...
int parse_version_struct(struct lm_keep_version *keep_version, char *version_struct)
{
	...
	tmp_len = sizeof(keep_version->action)+sizeof(keep_version->verlen);
...
int login_ack_do(int s, struct lm_login_ack *login_ack)
{
	...
	{
		diag_printf("[%s][%d] do register\n", __FUNCTION__, __LINE__);
		login_keep_id = 0;
		nvram_set("lm_device_id", "0");
		nvram_commit();
...
int router_online_check(int s)
{
	uint64 tm = (uint64)(cyg_current_time());
	int ret = 0;

	if(tm > g_next_online_rqst)
	{
		g_next_online_rqst = tm + (uint64)(LM_CMD_KEEP_ONLINE_REQ_PERIOD);
		ret = send_router_online_req(s);
		if(ret == -1)
		{
			diag_printf("[%s][%d] send_router_online_req failed\n", __FUNCTION__, __LINE__);
		}
	}
					...
					if(ret == 0)
					{
						switch(hdr.cmd)
						{
							case LM_CMD_KEEP_ACK:
								...
								if(0 == ret)
								{
									g_next_online_rqst = (uint64)(cyg_current_time()) + (uint64)(LM_CMD_KEEP_ONLINE_REQ_PERIOD);
								#if 0
									ret = send_router_online_req(sock_fd);
									if(ret == -1)
									{
										diag_printf("[%s][%d] send_router_online_req failed\n", __FUNCTION__, __LINE__);
										goto re_connect_login;
									...
									if(ret == -1)
									{
										diag_printf("[%s][%d] send_version_req failed\n", __FUNCTION__, __LINE__);
										goto re_connect_login;
									}
								}
								else
