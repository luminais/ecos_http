 ...
 *
 * $Id: upnp_device.c 257195 2011-05-04 09:23:20Z simonk $
 */
...
upnp_device_advlist_init(UPNP_CONTEXT *context)
{
	...
	//add by z10312 cbb check
	memset (ifp->advlist , 0, size);

	memcpy(ifp->advlist, ifp->device->advertise_table, size);

	/* Renew UUIDs of this ifp */
	for (src = ifp->advlist; src && src->name; src++) {
		...
		for (dst = ifp->advlist; dst && dst->name; dst++) {
