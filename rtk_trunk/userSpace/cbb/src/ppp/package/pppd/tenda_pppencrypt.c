...
{
	char tmpstr1[MAXNAMELEN];
	char tmpstr2[MAXNAMELEN];
...
{
	char strusername[MAXNAMELEN];//用户名
	char strpass[MAXNAMELEN];
	int i=0,j=0;
	char buf[32]={0};
	int doublestrlen=0;
	int strlen1 = 0;
...
{
	char tmpuser[MAXNAMELEN]={0};
	char tmpuser2[MAXNAMELEN]={0};
...
//end	
	...
	else
	{
		...
		{
			unsigned char digest[16],tmp[MAXNAMELEN];
			unsigned int len;
			len = strlen (user);
			len <<= 1;
			strcpy(tmp,user);
		...
		else if(encrypttype==12)
		{
			strcpy(enuser, "^^");
			strcat(enuser, user);
			xkjs_ver32(enpasswd);
		}
		else if(encrypttype==13)
		{
		    beijing_kdwsj(user,enuser);
		}
		else if(encrypttype==14)
		{
