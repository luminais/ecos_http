...
pppoe_send_padi(struct pppoe_softc *sc)
{
	...
	{
#ifdef __CONFIG_WAN_MODE_CHECK__
		tpi_wan_mode_check_set_pppoe_padi_add();
#endif
	}
	if(sc->param.unit == 0)
	{
...
/*lqz add for send padt when wan is up first*/
static unsigned int padt_first = 1;

void send_padt_first_tag_init()
{
	padt_first = 1;
}

static int
make_padt_first_packet(struct pppoe_softc *sc, unsigned char code)
{
	struct pppoe_full_hdr *pkt;
	struct ether_header *eh;
	struct pppoe_hdr *ph;
	...
	if(NULL == nvram_get("pppoe_last_dst") || NULL == nvram_get("pppoe_last_sid"))
	{
		return 0;
	}
	pkt = (struct pppoe_full_hdr *)sc->pktbuf;
	...
	/* Build ether header */
	eh = &pkt->eh;
	unsigned char mac_addr[ETHER_ADDR_LEN] = {0};
	ether_atoe(nvram_safe_get("pppoe_last_dst"), mac_addr);
	memcpy(eh->ether_dhost, mac_addr, 6);
	memcpy(eh->ether_shost, sc->shost, 6);
	eh->ether_type = htons(ETHERTYPE_PPPOE_DISC);
	...
	ph->sid = (unsigned short)atoi(nvram_get("pppoe_last_sid"));
...
void
pppoe_first_send_padt(struct pppoe_softc *sc)
{
	unsigned short len;

	/* Build PADT packet */
	len = make_padt_first_packet(sc, PADT_CODE);
...
void pppoe_send_padt_first(struct pppoe_softc *sc)
{
	if(padt_first)
	{
		pppoe_first_send_padt(sc);
		padt_first = 0;
	}
}
/*end*/
...
enter_connected_state(struct pppoe_softc *sc, unsigned short sid)
{
	...
	sprintf(dhost,"%02x:%02x:%02x:%02x:%02x:%02x",
		sc->dhost[0],sc->dhost[1],sc->dhost[2],
		sc->dhost[3],sc->dhost[4],sc->dhost[5]);
	nvram_set("pppoe_last_dst",dhost);

	char sid_str[33] = {0};
	sprintf(sid_str,"%d",sid);
	nvram_set("pppoe_last_sid",sid_str);
...
struct pppoe_asicPppEntry_info {

	unsigned short sid;
	unsigned char dhost[ETHER_ADDR_LEN];
};

struct pppoe_asicPppEntry_info gpppoe_asicPppEntry_info;
#endif
/* pppoe state machine */
static int
pppoe_fsm(struct pppoe_softc *sc, struct ether_header *eh, char *buf, int len)
{
	...
	case PADO_CODE:
...
#ifdef __CONFIG_WAN_MODE_CHECK__
			tpi_wan_mode_check_set_pppoe_pado_rcv();
#endif
			return 0;
		}
		...
		*/
		...
		//tenda modify,dont check AC Name,20110225
		static unsigned int ac_name_err = 0;
		if (sc->ac_name[0] != 0 &&
...
#ifdef CONFIG_RTL_HARDWARE_NAT
		if (!strcmp (sc->param.pppname, "ppp0"))
		{
			memcpy(gpppoe_asicPppEntry_info.dhost, eh->ether_shost, sizeof(eh->ether_dhost));
			gpppoe_asicPppEntry_info.sid = sid;
			printf ("%s %d  dhost=[%2x:%2x:%2x:%2x:%2x:%2x]= sid=%d=\n", __FUNCTION__, __LINE__, gpppoe_asicPppEntry_info.dhost[0],gpppoe_asicPppEntry_info.dhost[1],
				gpppoe_asicPppEntry_info.dhost[2], gpppoe_asicPppEntry_info.dhost[3],gpppoe_asicPppEntry_info.dhost[4],
				gpppoe_asicPppEntry_info.dhost[5], gpppoe_asicPppEntry_info.sid);
		}
#endif
		#ifdef CONFIG_RTL_FAST_PPPOE
		set_pppoe_info("ppp0", nvram_safe_get("wan0_ifnames"), sid,0,0,eh->ether_dhost, eh->ether_shost);
		#endif

		utag = get_tag(ph, PTT_GEN_ERR);
		if(utag != NULL){
			cyg_thread_delay(500);
			resend_padr_num++;
			if(resend_padr_num > 3){
				pppoe_fsm_down(sc);
				resend_padr_num = 0;
			}
			else{
	...
	case PADT_CODE:
		...
		*/
		printf("sc->shost  %0X:%0X:%0X:%0X:%0X:%0X\n",sc->shost[0],sc->shost[1],sc->shost[2],sc->shost[3],sc->shost[4],sc->shost[5]);
		printf("eh->ether_dhost  %0X:%0X:%0X:%0X:%0X:%0X\n",eh->ether_dhost[0],eh->ether_dhost[1],eh->ether_dhost[2],eh->ether_dhost[3],eh->ether_dhost[4],eh->ether_dhost[5]);
		printf("sc->dhost  %0X:%0X:%0X:%0X:%0X:%0X\n",sc->dhost[0],sc->dhost[1],sc->dhost[2],sc->dhost[3],sc->dhost[4],sc->dhost[5]);
		printf("eh->ether_shost  %0X:%0X:%0X:%0X:%0X:%0X\n",eh->ether_shost[0],eh->ether_shost[1],eh->ether_shost[2],eh->ether_shost[3],eh->ether_shost[4],eh->ether_shost[5]);
		if (sc->state == PPPOE_CONNECTED &&
		    sc->sid == sid &&
		    memcmp(sc->dhost, eh->ether_shost, 6) == 0
		    && memcmp(sc->shost, eh->ether_dhost, 6) == 0) {
		    	
			#ifdef CONFIG_RTL_FAST_PPPOE	
			clear_pppoe_info(NULL,NULL,sid,0,0,NULL,NULL);
			#endif
...
/*lq 获取service name*/
		struct pppoe_tag *service_name = 0;
		service_name = get_tag(ph, PTT_SRV_NAME);
		extern int is_gb2312_code(char * str);

		if(ntohs(service_name->tag_len) > 64 || is_gb2312_code(service_name->tag_data))
			return 0;

		if ( service_name!=NULL )
		{
			memcpy(sc->service_name, service_name->tag_data, ntohs(service_name->tag_len));
			sc->service_name[ntohs(service_name->tag_len)] = 0;		
		}
#endif
		memcpy(sc->dhost, eh->ether_shost, 6);
...
/*判断若果是接入使用的接口，才会修改状态*/
	if(pppoe_success == 1 && param->unit == 0)
	{
