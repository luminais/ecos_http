...
//	oray_server_addr.s_addr = me.sin_addr.s_addr;
	...
	sprintf(postdata,"<?xml version=\"1.0\" encoding=\"utf-8\"?>\r\n"
					...
					"  <clientversion>9.1.0.0</clientversion>\r\n"
					" </GetMiscInfo>\r\n"
					"</soap:Body>\r\n"
					"</soap:Envelope>\r\n", 
					userid,
	...
	while ((ret = oray_recv(info_sock, buf, sizeof(buf)-1)) > 0) 
	{
		...
		{
			new_ptr = (char*) realloc( (void*) proc_blk, proc_blk_len+ret+1 );
			if(NULL == new_ptr)
			{
				free(proc_blk);
				return -1;
...
//	oray_server_addr.s_addr = me.sin_addr.s_addr;
	...
	while ((ret = oray_recv(info_sock, buf, sizeof(buf)-1)) > 0) 
	{
		...
		{
			proc_blk_new = (char*) realloc( (void*) proc_blk, proc_blk_len+ret+1 );
			if(NULL == proc_blk_new)
			{
				free(proc_blk);
				close(info_sock);
				return -3;
			}
			proc_blk = proc_blk_new;
		}
...
//	unsigned char tempbuf3[50];
	...
	{
		if (strcmp(GetMiscInfoResult, "1002") == 0)
		{
		...
		{
			ORAYDBG("GetMiscInfoResult=%s !=0\n",GetMiscInfoResult);
			LOGIN_FAIL(-1, "GetMiscInfoResult !=0")
		}
	}	
