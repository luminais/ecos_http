...
dhcpc_raise_event(char *ifname, char *event)
{
	...
	res = loipc(event, 0, DHCPC_IPC_PORT+ifidx, 0, 0);
	return res;
}
...
dhcpc_stop(char *ifname)
{
	...
	ret = dhcpc_raise_event(ifname, "FREE");
...
/*lq add send decline message*/
void
dhcpc_decline(char *ifname)
{
	dhcpc_raise_event(ifname, "DECLINE");
	return;
}
void
