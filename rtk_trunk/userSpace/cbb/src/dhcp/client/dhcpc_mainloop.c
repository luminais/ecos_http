...
#ifdef __CONFIG_TENDA_HTTPD_NORMAL__
extern void lan_change_config_reboot(unsigned int wan_ip,unsigned int wan_mask);
#endif
...
/*lq add arp check*/
extern void rtl_dhcpArpRequest(struct arpcom *ac, struct in_addr *sip, struct in_addr *tip, u_char *enaddr);
extern int rtl_checkArpReplyForDhcp();
 int arp_check(char *ifname, struct in_addr *tip, u_char *enaddr)
 {	 
	 struct in_addr sip;
	 ...
	 rtl_dhcpArpRequest(ac, &sip, tip, enaddr);			 
 	int i =0;
#ifdef __CONFIG_A9__
/*lq 后期需要同步到主线，在apclient模式下，等待次数重新设置*/
	if(strcmp(nvram_get("wl0_mode"),"wet"))
	{
		num = 8;
	}
#endif
	while(i < num)
	{
		cyg_thread_delay(25);
		i++;
		if(rtl_checkArpReplyForDhcp())
	 		return 0;
	}	 
	 return 1;
 }
static void new_lease(struct dhcpc_ifc *difp) 
{
	struct dhcp_lease *lease = &difp->lease;
    unsigned int tag = 0;
...
#ifdef __CONFIG_WAN_MODE_CHECK__
	tpi_wan_mode_check_set_dhcp_discover_add();
#endif
	
#ifdef __CONFIG_TENDA_HTTPD_NORMAL__
	discover_num++;

	if(discover_num > DHCP_MAX_CHECK_NUM)
	{	
...
/*lq add send decline package*/
static void
send_decline(struct dhcpc_ifc *difp)
{
	struct dhcphdr *txpkt = &difp->txpkt;
	char op_value[7];
	make_dhcpc_packet(difp, DHCP_DECLINE);
	txpkt->ciaddr = difp->req_ip;
	op_value[0] = OPTION_VALUE_HW;
	int i = 1;
	while(i < 7)
	{
		op_value[i] = difp->macaddr[i-1];
		i++;
	}
	dhcp_add_option(txpkt, DHCP_REQUESTED_IP,  sizeof(difp->req_ip), &difp->req_ip);
	dhcp_add_option(txpkt, DHCP_CLIENT_ID,  sizeof(op_value), op_value);
	dhcp_add_option(txpkt, DHCP_SERVER_ID, sizeof(difp->server), &difp->server);
	dhcpc_send(difp);
}
...
dhcpc_read(struct dhcpc_ifc *difp)
{
	...
	if (FD_ISSET(difp->efd, &fds)) {
		...
		else if (datalen >= sizeof("DECLINE") &&
			memcmp(buf, "DECLINE", sizeof("DECLINE")) == 0) {
			difp->event = EVENT_DECLINE;
		}
	}  /*lq add resv DECLINE message*/
...
static char *
get_static_router(char *dest,unsigned char *src, int dest_len)
{
	...
	while( src[pos] != '\0' ) 
	{
		...
		{
			sprintf(destnet, "%u.%u.0.0", src[pos],src[pos+1]);
			pos += 2;
		}
		else if( netmask <= 24 ) 
		{
			sprintf(destnet, "%u.%u.%u.0", src[pos],src[pos+1],src[pos+2]);
			pos += 3;
		}
		else
		{
			sprintf(destnet, "%u.%u.%u.%u", src[pos],src[pos+1],src[pos+2],src[pos+3]);
			pos += 4;
		}
		sprintf(thegw, "%u.%u.%u.%u", src[pos],src[pos+1],src[pos+2],src[pos+3]);
		pos += 4;
		...
		first_time = 0;
	/*限制静态路由长度及条数小于15条，超过就不进行处理了*/	
        if(strlen(dest) + strlen(value) > dest_len)
        {
            printf("%s [%d] dest len not enough, Return.\n", __FUNCTION__, __LINE__);
            break; 
        }
...
static char *
get_33_static_router(char *dest,unsigned char *src, int dest_len)
{
...
enter_bound_state(struct dhcpc_ifc *difp)
{
	...
	char *dns, *wins;
	char domain[256] = {0};
	char static_router[512] = {0};
	unsigned int static_router_tag = 0;
	char static_router_33[512] = {0};
	char static_router_121[512] = {0};
	char static_router_249[512] = {0};
	...
	DHCPC_DBG("DHCPC_BOUND get %s", env[i-1]);
	/* subnet */
...
//hqw add
	...
	if (static_router_121[0] != 0) 
	{
		...
		get_static_router(static_router,static_router_121, sizeof(static_router));
	}
	if(static_router_249[0] != 0)
	{
		static_router_tag = STATIC_ROUTER_249;
		get_static_router(static_router,static_router_249, sizeof(static_router));
	}
	if(static_router_33[0] != 0)
	{
		static_router_tag = STATIC_ROUTER_33;
		get_33_static_router(static_router,static_router_33, sizeof(static_router));
	}
...
#ifdef __CONFIG_WAN_MODE_CHECK__
	tpi_wan_mode_check_set_dhcp_offer_rcv();
#endif
...
//add by roy+++
			...
			if( 0 != strncmp(difp->ifname, nvram_safe_get("lan_ifname"),strlen(nvram_safe_get("lan_ifname"))))
#endif
			{//only for wan port
...
#ifdef __CONFIG_TENDA_HTTPD_NORMAL__
					lan_change_config_reboot(wan_ip,wan_mask);
#endif			
					return;
				}
#ifdef __CONFIG_GUEST__
				gpi_wifi_check_ip_confilct_by_wan(wan_ip, wan_mask);
#endif
			}		
//end---		
...
dhcpc_state_requesting(struct dhcpc_ifc *difp)
{
	...
	if (server_ip && (msgtype = dhcp_get_option(&difp->rxpkt, DHCP_MESSAGE_TYPE, 0, NULL))) {
		if (DHCP_ACK == *msgtype &&
...
/*lq add do decline event*/
static void
do_decline(struct dhcpc_ifc *difp)
{
	send_decline(difp);

	do_deconfig(difp, DHCPC_DECLINE_DELAY);
...
do_event(struct dhcpc_ifc *difp)
{
	...
	case EVENT_RENEW:
		...
		break;
	case EVENT_DECLINE:
		DHCPC_DBG("DHCPC_DECLINE sending");
		do_decline(difp);
		break;
