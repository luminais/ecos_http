...
/* add by Tenda, define special channel */
#define WL_CHANNEL_1           	  1
#define WL_CHANNEL_2           	  2
#define WL_CHANNEL_6           	  6
#define WL_CHANNEL_11           	  11
#define IS_WL_SPECIAL_CH(ch)	 ((WL_CHANNEL_1 == (ch)) || \
									  (WL_CHANNEL_2 == (ch)))

//#define WL_CH_DEBUG_ON

#ifdef WL_CH_DEBUG_ON
#define WL_CH_DEBUG(fmt, args...) 			do{ \
	printf("[DEBUG_CH:%s(%d)]"fmt, __func__, __LINE__, ## args);	\
}while(0)
#else
#define WL_CH_DEBUG(fmt, args...)
#endif
/* end add */
...
wlconf(char *name)
{
	...
	char cap[WLC_IOCTL_SMLEN] = {0};
	char caps[WLC_IOCTL_SMLEN] = {0};
	int btc_mode;
	uint32 leddc;
	...
	if (!ure_enab) {
		...
		//modify by zliang------修复次SSID MAC	前两位与主SSID MAC 不同的问题
		...
		nvram_set(tmp, ether_etoa((uchar *)vif_addr, eaddr));
		#else
		unsigned int wl_vif_value = 0;
		char auto_vif_addr[WLC_IOCTL_SMLEN];
		memcpy(auto_vif_addr, vif_addr, WLC_IOCTL_SMLEN);

		wl_vif_value = (auto_vif_addr[3]&0xFF);
		wl_vif_value = (wl_vif_value<<8)+ (auto_vif_addr[4]&0xFF);
		wl_vif_value = (wl_vif_value<<8)+ (auto_vif_addr[5]&0xFF);
		wl_vif_value += 1;
		auto_vif_addr[3] = (wl_vif_value>>16)&0xFF;
		auto_vif_addr[4] = (wl_vif_value>>8)&0xFF;
		auto_vif_addr[5] = (wl_vif_value&0xFF);
		
		snprintf(tmp, sizeof(tmp), "wl%d.1_hwaddr", unit);
		WL_BSSIOVAR_SET(name, "cur_etheraddr", 1, auto_vif_addr,
		                ETHER_ADDR_LEN);
		nvram_set(tmp, ether_etoa((uchar *)auto_vif_addr, eaddr));
		...
		snprintf(tmp, sizeof(tmp), "wl%d.2_hwaddr", unit);
		WL_BSSIOVAR_SET(name, "cur_etheraddr", 2, auto_vif_addr,
		                ETHER_ADDR_LEN);
		nvram_set(tmp, ether_etoa((uchar *)auto_vif_addr, eaddr));
		#endif
	}
...
/* Tenda add for anti-interference */
	val = atoi(nvram_safe_get("sgi_tx"));

	if (val != 0)
	{
		WL_IOVAR_SETINT(name, "sgi_tx", val);
	}
	
	val = atoi(nvram_safe_get("sgi_rx"));
	if (val != 0)
	{
		WL_IOVAR_SETINT(name, "sgi_rx", val);
	}
/* end add */
...
legacy_mode:
#endif /* EXT_ACS */
		/* 2015-7-3,make sure obss_coex from wl driver */
		WL_IOVAR_GETINT(name, "obss_coex", &obss_coex);
		/* end add by jack*/
		if (obss_coex || channel == 0) {
			if (WLCONF_PHYTYPE_11N(phytype)) {
				...
				times = 3;
/* add by Tenda for 40MHz */
re_sel_chanspec:				
				chanspec = wlconf_auto_chanspec(name);
				int tmp_chann = CHSPEC_CHANNEL(chanspec);
				uint tmp_sb = 0;
				...
				WL_CH_DEBUG("scaned channel is %d\n", tmp_chann);

				if (IS_WL_SPECIAL_CH(tmp_chann))
				{
					times--;
					WL_CH_DEBUG("scaned channel= %d is not excepted,try again!\n", tmp_chann);
					if (times > 0)
						goto re_sel_chanspec;
				}

#ifdef WL_CH_DEBUG_ON
				int dbg_ch = 0;
				if ((dbg_ch = atoi(nvram_safe_get("ch_test"))) != 0)
				{
					tmp_chann = dbg_ch;
					WL_CH_DEBUG("For test name:%s = %d\n", tmp, tmp_chann);
				}
#endif	

				/* if channel always  special channel, set a constant value for it */
				if (IS_WL_SPECIAL_CH(tmp_chann))
				{
					chanspec = 0;
					WL_CH_DEBUG("unexcepted channel(=%d) decteded!\n", tmp_chann);
					uint nctrlsb = WL_CHANSPEC_CTL_SB_NONE;

					if (WL_CHANNEL_1 == tmp_chann || WL_CHANNEL_2 == tmp_chann)
					{
						tmp_chann += 1;
						tmp_sb = WL_CHANSPEC_CTL_SB_LOWER;
						
					}

					WL_CH_DEBUG("change channel to %d\n", tmp_chann);

					/* Get Ctrl SB for 40MHz channel */
					if (nbw == WL_CHANSPEC_BW_40) 
					{	
						/* Adjust the channel to be center channel */
						if (WL_CHANSPEC_CTL_SB_LOWER == tmp_sb)
						{
							nctrlsb = WL_CHANSPEC_CTL_SB_LOWER;
							tmp_chann = tmp_chann + 2;
						}
						else
						{
							nctrlsb = WL_CHANSPEC_CTL_SB_UPPER;
					...
					/* band | BW | CTRL SB | Channel */
					chanspec |= ((bandtype << WL_CHANSPEC_BAND_SHIFT) |
					             (nbw | nctrlsb | tmp_chann));

				}
#ifdef WL_CH_DEBUG_ON
				else
				{
					/* show auto decteded channel */
					tmp_chann =  CHSPEC_CHANNEL(chanspec);
					...
					WL_CH_DEBUG("auto sel channel is %d\n", tmp_chann);
				}
#endif				
...
/* add by Tenda auto channel set for 20MHz */
				times = 3;
re_sel_channel:				
				/* select a channel */
				val = wlconf_auto_channel(name);

				if (IS_WL_SPECIAL_CH(val))
				{
					times--;
					WL_CH_DEBUG("20MHz special channel= %d,try again!\n", val);
					if (times > 0)
						goto re_sel_channel;
				}
				...
				if (IS_WL_SPECIAL_CH(val))
				{
					if (WL_CHANNEL_1 == val || WL_CHANNEL_2 == val)
					{
						val += 2;
					}
...
{	
	...
	WL_IOCTL("eth1", WLC_SET_MACLIST, buf, sizeof(buf));
	...
	WL_IOCTL("eth1", WLC_SET_MACMODE, &val, sizeof(val));
}
#endif
...
void auto_conn_vif_control(unsigned int on)
{
	...
	setbuf.bsscfg_idx = 2;
	setbuf.enable = on;
	WL_IOVAR_SET("eth1", "bss", &setbuf, sizeof(setbuf));
}
#endif
