...
static int init_dhcp6s_cfg()
{
	...
	{
		fprintf(fp, "%s\n", "interface br0 {");
		if(!dhcp6s_cfg->vltime)
			sprintf(str,"address-pool mynetwork 3600;");
		else
			sprintf(str,"address-pool mynetwork %d;", dhcp6s_cfg->vltime);
		fprintf(fp, "\t%s\n", str);
		fprintf(fp, "%s\n", "};");
		fprintf(fp, "%s\n", "pool mynetwork {");
