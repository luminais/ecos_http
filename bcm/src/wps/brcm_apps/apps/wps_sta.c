...
static int wpssta_do_enrollement_again(wpssta_wksp_t *wksp);
static int wpssta_deinit(void *app_wksp);
...
static int
wpssta_start(wpssta_wksp_t *sta_wksp)
{
	char *val;
...
wpssta_check_timeout(wpssta_wksp_t *sta_wksp)
{
	...
	case WPS_ASSOC_STATE_ASSOCIATING:
		...
		if ((now - assoc_state_time) > 10 /* WPS_ENR_SCAN_JOIN_MAX_WAIT_SEC */) {
			...
			/* start wps */
			if (enroll_again) {
				if (wpssta_do_enrollement_again(sta_wksp) != 0)
					return REG_FAILURE;
			}
			else if (wpssta_start(sta_wksp) != WPS_SUCCESS)
...
static int
wpssta_do_enrollement_again(wpssta_wksp_t *sta_wksp)
{
	char *data;
	int len;
...
wpssta_open_session(wps_app_t *wps_app, char* ifname)
{
	...
	wps_app->close = (int (*)(void *))wpssta_deinit;
