 ...
 *
 * $Id: wps_eap.c 282619 2011-09-08 20:28:23Z hwgwang $
 */
...
wps_eap_process_msg(char *buf, int buflen)
{
	...
	if (wps_app->wksp) {
		...
		if (wps_app->mode != WPSM_STA_ENROLL &&
			wps_app->mode != WPSM_STA_BUILTINREG) {
			/*
	 ...
	 */
		...
		WpsSleep(1);	//broadcom WIN7 WPS solution,liuke add 2011/11/24
