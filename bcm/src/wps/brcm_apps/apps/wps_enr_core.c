 ...
 *
 * $Id: wps_enr_core.c 288198 2011-10-06 12:52:50Z simonk $
 *
 */
...
static int do_enrollement_again(wpssta_wksp_t *wksp);
...
found:
	...
	TUTRACE((TUTRACE_INFO,
		"pbc = %s%s, wpsenr param: ifname = %s, mode= %s, op = %s, sec = %s, "
		"ssid = %s, bssid = %s, pin = %s, oob = %d\n",
		(pbc == 1? "HW_PBC": "SW_PBC"),
		(enr_auto == 1? " Auto": ""),
		ifname,
	...
	if (sta_wksp->mode == WPSM_STA_BUILTINREG) {
		if (wpssta_reg_config_init(sta_wksp, ifname, bssid, oob) != WPS_SUCCESS) {
			printf("wpssta_reg_config_init failed, exit.\n");
			goto exit;
		}
	}
	else {
...
exit:
	wpsenr_deinit(sta_wksp);
	return NULL;
}

static int
wpsenr_start(wpssta_wksp_t *sta_wksp)
{
	char *val;
	char user_bssid = false;
	uint32 len;
	...
	if (sta_wksp->mode == WPSM_STA_BUILTINREG) {
		printf("Start registration for BSSID: %02x:%02x:%02x:%02x:%02x:%02x [Mode:%s]\n",
			bssid[0], bssid[1], bssid[2], bssid[3], bssid[4], bssid[5],
			(sta_wksp->configap == true) ? " config AP":" get AP config");

		if (wps_start_registration(pin, start_time) != WPS_SUCCESS) {
		...
		if (wps_start_enrollment(pin, start_time) != WPS_SUCCESS) {
			printf("Start enrollment failed!\n");
			goto err_exit;
		}
	}
...
static int
wpsenr_process(wpssta_wksp_t *sta_wksp, char *buf, int len, int msgtype)
{
	uint32 buf_len = len;
	char *sendBuf;
	int last_recv_msg;
...
static int
wpsenr_check_timeout(wpssta_wksp_t *sta_wksp)
{
	int state, find_pbc;
	int last_sent_msg;
	uint8 msg_type;
	...
	case WPS_ASSOC_STATE_ASSOCIATING:
		...
		if ((now - assoc_state_time) > 10 /* WPS_ENR_SCAN_JOIN_MAX_WAIT_SEC */) {
			...
			/* start wps */
			if (enroll_again) {
				if (do_enrollement_again(sta_wksp) != 0)
					return REG_FAILURE;
			}
			else if (wpsenr_start(sta_wksp) != WPS_SUCCESS)
...
wpssta_reg_config_init(wpssta_wksp_t *sta_wksp, char *ifname, char *bssid, char oob)
{
	...
	if (sta_wksp->configap == true) {
		...
		ret = wps_enr_reg_config_init(&info, NULL, 0, NULL);
	return ret;
}
...
static int
do_enrollement_again(wpssta_wksp_t *sta_wksp)
{
	char *data;
	int len;
	...
	if (sta_wksp->mode == WPSM_STA_BUILTINREG) {
		ret = wps_start_registration(pin, current_time);
	}
	else {
...
int
wpsenr_open_session(wps_app_t *wps_app, char* ifname)
{
	void *sta_wksp = NULL;

	TUTRACE((TUTRACE_INFO, "Start Enroll\n"));

	sta_wksp = wpsenr_init(ifname);
	if (!sta_wksp) {
	...
	/* update mode */
	wps_app->mode = WPSM_STA_ENROLL;
	wps_app->close = (int (*)(void *))wpsenr_deinit;
	wps_app->process = (int (*)(void *, char *, int, int))wpsenr_process;
	wps_app->check_timeout = (int (*)(void *))wpsenr_check_timeout;
