...
wps_gen_ssid(char *ssid, int ssid_len, char *prefix, char *hwaddr)
{
	...
	/* pxy revise, 2013.12.03
	 * set ssid to be eq Tenda_(10 random char), follow Tenda wps standard edited by 
	 * wangwanshun, if no need, don't change it
	 */
#if 0
	...
	/* pxy revise, 2013.12.03, prefix eq "Tenda" or others
	 * get value of "wps_random_ssid_prefix"
	 */
	if (prefix && (strlen(prefix) + 1 + 12) < SIZE_SSID_LENGTH)
		WPS_snprintf(ssid, ssid_len - 1, "%s_", prefix);
...
wps_gen_key(char *key, int key_len)
{
	...
	/* pxy revise, 2013.12.03
	 * set key_length to be eq 63, follow Tenda wps standard edited by 
	 * wangwanshun, if no need, don't change it
	 */
#if 0
	/* key_length < 16 */
