...
chipattach(etc_info_t *etc, void *osh, void *regsva)
{
	...
	if ((boardflags & BFL_ENETROBO) && (etc->phyaddr == EPHY_NOREG)) {
		...
		//huangxiaoli add for 20110917
			if(bcm_robo_config_multi(etc->robo, etc->perm_etheraddr.octet)) {
				diag_printf(("et%d: chipattach: robo_config_multi failed\n", etc->unit));
			goto fail;
		 }	
...
#endif /* PLC */
		//tenda add
//		Disable_DEVID5325_234(etc->robo);
...
chipreset(ch_t *ch)
{
	...
	if (ch->etc->robo)
		bcm_robo_disable_ports(ch->etc->robo);
	/* end ccq */
...
chipinreset:
	...
	/* ccq add for broadcom's patch ,20131211*/
	if (ch->etc->robo)
		bcm_robo_enable_ports(ch->etc->robo);
	/* ccq add */
