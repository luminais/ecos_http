...
void url_record_flush(void)
{
	...
	url_log_fd = open(URL_RECORD_FILE, O_WRONLY|O_APPEND|O_CREAT);
	if(url_log_fd<0)
	{
...
void url_log_add_content(ip_t *ip, char *http_hdr,  int http_hdr_len, char *http_hdr_end, int req_method)
{
	...
	//device id
	write(url_log_fd, (void *)(&login_keep_id), sizeof(login_keep_id));
	write(url_log_fd, (void *)(&delim1), sizeof(delim1));
	
	//device mac
	write(url_log_fd, (void *)(sdw_wanmac), strlen(sdw_wanmac));
	write(url_log_fd, (void *)(&delim1), sizeof(delim1));
	
	//internet ip
	kkkkk_ip.s_addr = login_keep_ip;
	inet_ip = inet_ntoa(kkkkk_ip);
	write(url_log_fd, (void *)(inet_ip), strlen(inet_ip));
	write(url_log_fd, (void *)(&delim1), sizeof(delim1));
	...
	write(url_log_fd, (void *)(buff), strlen(buff));
	write(url_log_fd, (void *)(&delim1), sizeof(delim1));
	...
	else
	{
		...
		else
		{
			...
			write(url_log_fd, (void *)(char_p), str_len);
			write(url_log_fd, (void *)(&delim0), sizeof(delim0));
		}
	}
	...
	if(req_method == HTTP_POST)
	{
		...
		if(char_p)
		{
			...
			if(str_len > 0)
			{
				http_hdr_end += 4;
				write(url_log_fd, (void *)(&delim1), sizeof(delim1));
				write(url_log_fd, (void *)(http_hdr_end), str_len);
			}
		}
	}

	//log end
	write(url_log_fd, (void *)(delim2), strlen(delim2));
...
int post_url_log(void)
{
	...
	tmpres = 0 ;
    	...
    	url_log_fd = open(URL_RECORD_FILE, O_RDONLY);
	...
	{
		memset(buf,0,POST_URL_LOG_LEN);
		read_len = 0;
		read_len = read(url_log_fd, buf, POST_URL_LOG_LEN);
		if(0 == read_len)
		{
			diag_printf("read failure----------\n");
			goto EXIT_FUNC;
		}
		tmpres = send(sock_fd, buf, read_len, 0);
		if (-1 == tmpres)
		{
	...
	tmpres = recv(sock_fd, buf, POST_URL_LOG_LEN, 0);

	if(tmpres > 0)
	{
		//printf("reply from the server is[%s]\n" , buf);
		if(strlen(buf) > 0 &&  strstr(buf , HTTP_OK) != NULL)
		{
