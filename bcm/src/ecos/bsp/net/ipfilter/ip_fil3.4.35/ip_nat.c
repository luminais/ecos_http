...
#define USER_NET_MEMPOOL_SIZE	roundup((1024*(CONFIG_NAT_LINKS/4)),MSIZE)
unsigned char net_mempool_area[USER_NET_MEMPOOL_SIZE];
int net_mempool_size = sizeof(net_mempool_area);
int nat_table_max = CONFIG_NAT_LINKS;

#ifdef NAT_PER_IP_LIMIT
int nat_table_per_ip = CONFIG_NAT_LINKS;
#endif
...
int rw;
{
	...
	if (flags & IPN_TCPUDP) {
		...
		gre = (p == IPPROTO_GRE) ? (grehdr_t *)fin->fin_dp : NULL;
		sport = 0;
		dport = 0;
	}
