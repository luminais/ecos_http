	...
	struct ether_header *eh, struct mbuf *m)
{
	...
	for (ifp = ifpppoe; ifp; ifp = ifp->next) 
	{
		if (ifp->eth_ifp == eth_ifp)
		{
			...
			{	
				sid = 0x1234;
				char mac_clone_gong[16];
				char *value = NULL;
				sprintf(mac_clone_gong,"%02X:%02X:%02X:%02X:%02X:%02X",ifp->dhost[0],
						ifp->dhost[1],ifp->dhost[2],ifp->dhost[3],ifp->dhost[4],ifp->dhost[5]);

				//printf("`````````%s\n",mac_clone_gong);
				if(!strcmp(nvram_safe_get("wan0_hwaddr"),mac_clone_gong))
				{
					;
				}
				else
				{
					if(strcmp(mac_clone_gong,"00:00:00:00:00:00") !=0)
					{	
						if ((value = nvram_get("auto_clone_flag")) != NULL)
						{
							if(!strcmp(value,"yes"))
							{
								nvram_set("wan0_hwaddr",mac_clone_gong);
								//nvram_set("clnway","1");
								mac_clone_flag_ppp =1;
								nvram_commit();
							}
						}
					}					
