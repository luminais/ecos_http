...
int webCgiGetBoundarySize(char_t *fileBuf)
{
	int count;
	int i;
	int curlen;
...
int tenda_upload_fw2(char *stream, int offset_in,int flash_offset)
{
	struct trx_header *trx,trx1;
	struct upgrade_mem *up_buf,*p;

	int i = 0;
	int buflen,buflen_write,actual_write=0;
	int boundarySize=0;
	...
	trx = &trx1;
	diag_printf("[%s][%d] buflen = %d\n", __FUNCTION__, __LINE__, ltoh32(trx->len));
	buflen = ltoh32(trx->len);
	
	p = up_buf;
	diag_printf("[%s][%d] offset_in = %d, p->inuse = %d\n", __FUNCTION__, __LINE__, offset_in, p->inuse);
	buflen_write = p->inuse- offset_in;
	if(buflen_write > buflen)
	{
		boundarySize = webCgiGetBoundarySize(stream);
		diag_printf("[%s][%d] boundarySize = %d\n", __FUNCTION__, __LINE__, boundarySize);
		buflen_write -= (boundarySize+4);
	}
	buflen -= buflen_write;
	actual_write += buflen_write;
	diag_printf("[%s][%d]<%d> buflen = %d, buflen_write = %d, actual_write = %d\n", __FUNCTION__, __LINE__, i, buflen, buflen_write, actual_write);
	i++;

	while((buflen > 0) && p){
		diag_printf("[%s][%d]<%d> buflen = %d, p->inuse = %d\n", __FUNCTION__, __LINE__, i, buflen, p->inuse);
		buflen_write = (buflen >= p->inuse)?p->inuse:buflen;
		buflen -= buflen_write;
		actual_write += buflen_write;
		diag_printf("[%s][%d]<%d> buflen_write = %d, actual_write = %d\n", __FUNCTION__, __LINE__, i, buflen_write, actual_write);
		p = p->next;
		i++;
	}
...
int tenda_upload_fw(char *stream, int offset_in,int flash_offset)
{
	...
	if (!sysFlashWrite(flash_offset, p->data+offset_in,  buflen_write)) {
		diag_printf("%s: error writing FLASH chunk 0x%lx size %lx\n", __func__,
			(unsigned long)flash_offset,  buflen_write);
		ok = -1;
		goto write_done;
	}

	flash_offset += buflen_write;
	buflen -= buflen_write;
	actual_write += buflen_write;
	
	//we use up_buf->data as a tmp buf next, for reduce write flash times
	p->inuse = 0;	
	p = p->next;
	...
	while((buflen > 0) && p){
		...
		buflen_write = (buflen >= p->inuse)?p->inuse:buflen;
		//we assume that up_buf->inuse+buflen_write always smaller than up_buf->totlen when up_buf->inuse is zero
		if((up_buf->inuse+buflen_write) <= up_buf->totlen){
			memcpy(up_buf->data+up_buf->inuse,p->data,buflen_write);
			up_buf->inuse += buflen_write;
			p = p->next;
			if(p) continue;
		}

		//如果上if不成立或者p为空，就会走到这里
		if (!sysFlashWrite(flash_offset, up_buf->data,  up_buf->inuse)) {
			diag_printf("%s: error writing FLASH chunk 0x%lx size %lx\n", __func__,
				(unsigned long)flash_offset,  up_buf->inuse);
			ok = -1;
			goto write_done;
		}
		buflen_write = up_buf->inuse;
		up_buf->inuse = 0;
		flash_offset += buflen_write;
		buflen -= buflen_write;
		actual_write += buflen_write;

		//不用在这里做下面的动作，需要保留p的指针
		//p = p->next;
	}
...
write_done:
	...
	diag_printf("[%s][%d] actual_write = %d\n", __FUNCTION__, __LINE__, actual_write);
	return ok;
}
#else
//roy modify+++
int tenda_upload_fw(char *stream, int offset_in,int flash_offset)
{
	...
	printf (" %s %d flash_size=%d, buflen=%d\n", __FUNCTION__, __LINE__, flash_size, buflen);
...
void DownloadSyslog(webs_t wp, char_t *path, char_t *query)
{
	...
	while(syslog_get2(log_time,log_type,log_data)!=NULL){
		
		sprintf(buf,"%d %20s %16s %s",num,log_time,log_type,log_data);
...
{	
	...
	if( (offset = webCgiGetUploadFile2(wp, query) ) <= 0 || offset >=wp->clen2) 
	{
		...
		SetErrorArg("CGI get config file failed!","direct_reboot.asp");
		goto error_out;
	}
	...
	if(ok != 0){
		SetErrorArg("CGI write config file failed!","direct_reboot.asp");
		goto error_out;
	}
