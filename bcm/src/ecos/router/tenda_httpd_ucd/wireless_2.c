	...
	if (unit_str) {
		...
		for (v = &wl_variables[0]; v->name && !strncmp(v->name, "wl_", 3); v++){
			...
			if (!strncmp(v->name, "wl_", 3)) {
				...
				/*
				if(strcmp(v->name,"wl_wps_mode") == 0 || strcmp(v->name,"wl_ssid") == 0
						|| strcmp(v->name,"wl_mode") == 0 || strcmp(v->name,"wl_wpa_psk") == 0
							|| strcmp(v->name,"wl_unit") == 0)
					printf("---%s,%s:%s\n",tmp,v->name,val);
					*/
			}
...
wl_unit(char *unit,int from_secu)
{
	...
	for (; v->name && !strncmp(v->name, "wl_", 3); v++){
		...
		if(strcmp(v->name,"wl_wps_mode") == 0 || strcmp(v->name,"wl_ssid") == 0
						|| strcmp(v->name,"wl_mode") == 0 || strcmp(v->name,"wl_wpa_psk") == 0
							|| strcmp(v->name,"wl_unit") == 0)
				printf("------%s:%s----%s\n",tmp,nvram_safe_get(v->name),v->name);
				*/
	}
	//printf("------------------------------------------\n");
}
...
//we only use wl0.1	
	...
	if( (wl_vif_value != NULL))
	{
		 ...
		 || strstr(wl_vif_value, "wl0.1") == NULL
		 #endif
		 )
		 {
		 	 wl_vif_value = NULL;
...
char *get_vif_ssid()
{
	...
	{
		if(strstr(buf, "wl0.1") != NULL)
		{
			return nvram_safe_get("wl0.1_ssid");
		}
		else
			return NULL;
	}
	...
	foreach(name, buf, next) 
	{
		if (strlen(name) >strlen("wl0") && strcmp(name, "wl0.2") != 0)
		{
			ret = nvram_safe_get(name);
			break;
		}
	}
	*/
...
char *get_wl0_mode(char *value)
{
	char *mode,*aporwds;

	_GET_VALUE("wl0_mode",mode);

	if(!strcmp(mode,"ap")){
		/*is ap? then do the following*/
		_GET_VALUE("wl0_aporwds",aporwds);
		if(strcmp(aporwds,"0")==0)
			strcpy(value,"ap");
		else
			strcpy(value,"wds");
	}
	else{
		/*is not ap, set default value */
		strcpy(value,mode);
	}
...
{
	char tmp[100], prefix[] = "wlXXXXXXXXXX_", unit[] = "0.2";
	char *wl_mode = NULL, *wl0_mode = NULL;
	 ...
	 * physical device. 
	 */
	wl_mode = nvram_safe_get("wl_mode");
	wl0_mode = nvram_safe_get("wl0_mode");
	printf("%s:wl_mode=%s,wl0_mode=%s---\n", __func__, wl_mode, wl0_mode);
	if((strcmp(wl0_mode, "ap") == 0) || 
		(strcmp(wl0_mode, "") == 0 && strcmp(wl_mode, "ap") == 0))
	{
		for (; v->name && !strncmp(v->name, "wl_", 3); v++){
			if (( v->ezc_flags & WEB_IGNORE) || ((v->ezc_flags & VIF_IGNORE) && (prefix[3] == '.')) ||
				((v->ezc_flags & NVRAM_SECURITY) && (prefix[3] == '.'))) 
				continue;
			nvram_set(strcat_r(prefix, &v->name[3], tmp), nvram_safe_get(v->name));
		}
	}
	
	nvram_set("wl0.2_bss_enabled", "1");	
	nvram_set("wl0.2_ssid", "Tenda_Extender_1");
	nvram_set("wl0.2_wps_mode", "disabled");
	nvram_set("wl0.2_auth", "0");
	nvram_set("wl0.2_wep", "disabled");
	nvram_set("wl0.2_akm", "psk psk2");
	nvram_set("wl0.2_crypto", "aes");
	nvram_set("wl0.2_wpa_psk", "Extender123456");
	nvram_set("wl0.2_mode", "ap");
	nvram_set("wl0.2_closed", "1");
