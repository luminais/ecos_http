...
#ifdef __CONFIG_WIFI_STATUS_CHECK__
#include "pi_common.h"
#endif
...
void formWirelessRepeatApInit(webs_t wp, char_t *path, char_t *query)
{
	...
	mode = gpi_get_wifi_mode();
	gpi_get_wifi_status_info(&wan_status,&wifi_status);

	if(WIFI_OK == wifi_status)
	{
		/*决策:WISP模式下当获取到IP的时候才显示桥接成功*/
		if(WL_WISP_MODE == mode && WAN_CONNECTED != wan_status)
		{
			string_cat(result,"connect-status", "1");
		}
		else
		{
			string_cat(result,"connect-status", "0");
		}
	}
	else if(WIFI_AUTHENTICATED_FAIL == wifi_status|| WIFI_AUTH_FAIL == wifi_status)
	{
		string_cat(result,"connect-status", "2");
	}
	else
	{
		string_cat(result,"connect-status", "1");
	...
	sprintf(value1, "%d", g_cur_wl_radio_status);
	string_cat(result,"wifi-enable", value1);
	strncat(result, ",", 1);
	#else
	string_cat(result,"wifi-enable", "1");
	strncat(result, ",", 1);
	#endif
