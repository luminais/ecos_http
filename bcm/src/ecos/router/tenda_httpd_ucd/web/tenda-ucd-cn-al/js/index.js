	...
	function showErrorMassage() {
		var $errCheckElem = $("#errcheck"),
			...
			stateObj = ['<span class="text-error">未联网</span>',
				...
				'已联网&nbsp;&nbsp;&nbsp;<a href="http://ipcamera.tenda.com.cn/default.html" style="text-decoration: underline; color:#468847;">上网试试</a>  <a style="color:#D3D3D3;"> &nbsp;|&nbsp; </a> <a href="http://smart.aliyun.com/download.htm" style="text-decoration: underline; color:#468847;">下载阿里小智APP</a>'],
		...
		if(G.wanLink == 0) {
			...
			if(G.conType == 1) {//dhcp  + 
				...
				if(G.netCheck == 2) {
					...
					//此种情况已经不存在！怎么又跑出来了？
					if (G.conStat == 2 && G.internetStat == 1) {
						$('#con-stat').html(stateObj[1]);
					}
		...
		G.validate = $.validate({
			...
			success: function () {
				var formElem = $('#index-form')[0],
					...
					wirelessSsid = document.getElementById('ssid').value,
					wanIp = $('#ipval')[0].val(),
					wanMask = $('#submask')[0].val();
				...
				if(G.data['transmitter'] === '1') {
					if (wirelessPwd !== G.data['ssid-pwd'] && wirelessSsid !== G.data['ssid']) {
						if (!confirm("无线信号名称更改为" + wirelessSsid +", 无线密码更改为" + wirelessPwd +",需用新的无线信号名称和密码重新连接路由器！确认更改吗？")) {
							return;
						}
					}else if (wirelessPwd !== G.data['ssid-pwd'] && !confirm("无线更改密码为" + wirelessPwd +", 需用新密码重新连接路由器！确认更改吗？")) {
						return;
					}else if (wirelessSsid !== G.data['ssid'] && !confirm("无线信号名称更改为" + wirelessSsid +", 需用使用新的无线信号名称连接路由器！确认更改吗？")) {
						return;
					}
				}
				clearTimeout(G.ajaxOn);
