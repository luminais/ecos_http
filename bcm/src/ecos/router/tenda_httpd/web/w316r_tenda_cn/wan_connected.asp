...
var help=new Array('&nbsp;&nbsp;&nbsp;&nbsp;静态IP：如果您的宽带服务商提供的是固定IP，请选择静态IP模式。','&nbsp;&nbsp;&nbsp;&nbsp;动态IP：如果您的 ISP 正在运行 DHCP 服务器，选择自动获取 IP。您的 ISP 会自动分配这些值（包括 DNS 服务器）。','&nbsp;&nbsp;&nbsp;&nbsp;PPPoE： 您可询问您的ISP以确定是否可使用PPPoE。如果他们确定使用PPPoE，请输入您的用户名和密码。','&nbsp;&nbsp;&nbsp;&nbsp;PPTP：输入ISP提供的PPTP服务器IP地址、用户名、密码，对于WAN口IP 地址、子网掩码、默认网关地址您可以选择动态获取或者手动输入ISP提供的信息。MTU：输入网络连接的MTU值，如若不清楚可以使用默认值。','&nbsp;&nbsp;&nbsp;&nbsp;L2TP：输入ISP提供的PPTP服务器IP地址、用户名、密码，对于WAN口IP 地址、子网掩码、默认网关地址您可以选择动态获取或者手动输入ISP提供的信息。MTU：输入网络连接的MTU值，如若不清楚可以使用默认值。');
var pages=new Array(
