...
"use strict";
	...
	function initControler() {
		...
		$('.time-hour').on('blur', function() {
			this.value = /2[0-4]?|[0-1]?[0-9]?/.exec(this.value);
		});
		$('.time-minute').on('blur', function(){
			this.value = /60|[0-5]?[0-9]?/.exec(this.value);
		});
	...
	function initView(data) {
		...
		if(G.data.wanMode !== 'ap') {
			...
			//enableCtrlElem.title = _('Disable Bridge function before you enable this function');
			enableCtrlLabel.title = _('Disable Bridge function before you enable this function');
		} else {
			enableCtrlElem.disabled = false;
			//enableCtrlElem.title = '';
