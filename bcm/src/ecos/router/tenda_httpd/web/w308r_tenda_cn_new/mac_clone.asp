...
<input type=hidden name=WMAC />
	...
	<div class="btn-group">
		...
		<input type="button" id="page-reset" class="btn" value="取 消" />
	</div>
</form>
...
var cln_MAC = "<%aspTendaGetStatus("sys","manmac");%>";
	...
	wl_MAC_list2 = new Array(<%get_wireless_station("macinfo2");%>);
...
(function (window) {
	...
	function initEvent(f) {
		...
		T.Event.on("page-reset", 'click', function () {
			init();
		});
	}
	function init(){
