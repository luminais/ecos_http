...
void wireless_form_define(void){
	...
	websFormDefine(T("getWifiRelay"), formGetWifiRelay);
	websFormDefine(T("getWifiScan"), formGetWifiScan);
	websFormDefine(T("setWifiRelay"), formSetWifiRelay);
#ifdef CONFIG_WL_USE_APSTA
...
int get_cur_bandwidth(char *buf)
{
	uint32 chanspec;
	char_t *name=NULL;
	int bandwidth = 20;
...
void formSetWifiRelay(webs_t wp, char_t *path, char_t *query)
{
	char * extra_mode,*super_mac,*go;
	EXTRA_MODE old_mode;

	extra_mode = websGetVar(wp, T("wifiRelayType"), T("0")); //1wisp 模式还是2apclient模式
	super_mac = websGetVar(wp, T("wifiRelayMAC"), T("0"));
	old_mode = check_extra_mode();

	
	if(old_mode == MODE_NONE && strcmp("ap", extra_mode) == 0)
	{
		websWrite(wp, T("HTTP/1.1 200 OK\nContent-type: text/plain; charset=utf-8\nPragma: no-cache\nCache-Control: no-cache\n\n"));
		websWrite(wp,"%s", "{\"errCode\":\"0\"}");
		websDone(wp, 200);
	}
	else
	{
		if(!strcmp("ap", extra_mode)){
			_SET_VALUE("wl0.1_hwaddr", "");
			set_wireless_ap();	
			//设置为BGN模式
			_SET_VALUE("wl0_nmode", "-1");
			...
			_SET_VALUE(_WAN0_PROTO,"dhcp");
			_SET_VALUE(_WAN0_CHECK,"0");
			_SET_VALUE(WLN0_HT_BW1,"0");//强制为20M带宽
			_SET_VALUE(WLN0_OBSS1,"0");
			
			_SET_VALUE(WLN0_CTL_ENABLE, "0");//无线扩展模式，强制关闭无线定时开关功能
...
static void new_set_extra_ssid(webs_t wp, char *wlname)
{
	...
	ssid = websGetVar(wp, T("wifiRelaySSID"), T(""));
	channel = websGetVar(wp, T("wifiRelayChannel"), T("11"));
...
static void new_set_wireless_security2(webs_t wp)
{
	...
	security_web = websGetVar(wp, T("wifiRelaySecurityMode"), T("NONE"));
	snprintf(security , sizeof(security) , "%s" , security_web);

	/*NONE, OPEN, WEP, WPA-TKIP, WPAWPA2-AES, WPA2-AES*/
	if (NULL != (p_str = strchr(security, '/')))
	{
		*p_str = '\0';
	}
...
{
	char_t *pwd;	
	pwd = websGetVar(wp, T("wifiRelayPwd"), T(""));
...
void new_confWPAGeneral( webs_t wp)
{
	...
	cipher_str = websGetVar(wp, T("wifiRelaySecurityMode"), T("WPA/AES"));
	key_renewal_interval = websGetVar(wp, T("keyRenewalInterval"), T("3600"));

	if (0 == strncmp(cipher_str, "WPA", 3) && NULL != (p = strchr(cipher_str, '/')))
	{
		++p;
	}
	...
	if (0 == strcmp(key_renewal_interval, ""))
	{
		_SET_VALUE(WLN0_REKEY_INTERVAL, "3600");
	}
	else
