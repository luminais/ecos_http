...
struct upgrade_mem *upgrade_mem_alloc(int totlen)
{
	...
	if((net_mbuf_free - 400*1024) > 0){
		//don't change 2048 and 400 here
		net_mbuf_free = 2048*((net_mbuf_free - 400*1024)/2048);//reserve 400KB for system
	}else{
		no_net_mbuf_free = 1;
	}
						...
						if(len == 0){
							...
							if(is_upgrade_flag){
								...
								/*can not greater than flash size reduce 160KB*/
									wp->query = NULL;
									/*Modified by wzs.2014-6-19 大文件无法上传成功，
									导致无法检测升级文件*/
									//goto upgrade_error;
								}else{
...
#endif
		if (gstricmp(key, T("authorization")) == 0) {
/*
...
//	printf("%s %d len=%d\n", __FUNCTION__, __LINE__, strlen(buf));
