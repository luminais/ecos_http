...
//roy +++
							...
							if(is_upgrade_flag){
								...
								if(wp->clen2 >(flashutl_desc->size - CFE_TRX_NVRAM_SIZE)){
									//如果大于__CONFIG_IMAGE_SIZE__ KB,那是不允许的
									wp->query =  NULL;
									goto upgrade_error;
								}else{
...
/*	pxy add, 2013.12.05
 *	in oder to prevent exception caused by cell phone, do the following 解决手机传送URL过长导致段错误的问题
 */
...
extern char cookie_suffix[COOKIE_SUFFIX_NUM][4];
...
void websResponseWithCookie(webs_t wp, int code, char_t *message, char_t *redirect, int user_id)
{
	...
	if (wp->ipaddr!=NULL)
	{
		sprintf(password,"%s%s",g_Pass,cookie_suffix[((unsigned int)inet_addr(wp->ipaddr))%COOKIE_SUFFIX_NUM]);
	}
	else
	{
...
#if 0/*要修改产测*/
		if(user_id == 1){
#if 0	//huangxiaoli add 20111019
			websWrite(wp, T("Set-Cookie: %s:language=%s; path=/\r\n"),userid, language);
#else
			websWrite(wp, T("Set-Cookie: username=%s:language=%s; path=/\r\n"),userid, language);
#endif
		}else if(user_id == 2){//清除username
			websWrite(wp, T("Set-Cookie: username=%s:language=%s; path=/\r\n"),NULL, NULL);
		}else{
			websWrite(wp, T("Set-Cookie: username=%s:language=%s; path=/\r\n"),NULL, language);
		}
