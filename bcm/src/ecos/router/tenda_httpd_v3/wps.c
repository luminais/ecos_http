 ...
 void set_wireless_wps(webs_t wp, char_t *path, char_t *query)
{
	...
	if(0 == strcmp(enable, "enabled"))		//enabled wps
	{
		...
		strcpy(pin, oldpin);
		wps_mode=nvram_get(WLN0_WPS_ENABLE);
		if( 0 == strcmp(wps_mode, "disabled")){
			nvram_set(WLN0_WPS_ENABLE, "enabled");
			diag_printf("%s==========%d\n", __FUNCTION__, __LINE__);
			sys_restart();	//add by stanley 12/6
			cyg_thread_delay(800);	//add by stanley 12/6
		}
#else
		strcpy(pin, oldpin);
#endif
		...
		//nvram_commit();
		//websRedirect(wp, T("wireless_wps.asp"));
		//websRedirect(wp, T("/wireless_security.asp"));
		return ;
	...
	{		
		...
		diag_printf("%s\n", uibuf);
		wps_hal_led_blink(12);
		//tenda_wps_gpio_led_off(tenda_wps_led_gpio_active, tenda_wps_led_assertlvl_active);
...
void set_wps_oobssid(void)
{
	char_t *old_ssid = NULL,*wlunit = NULL;
	char_t *hwaddr = NULL;
	char_t wlanmac[16]={0},tmp[128]={0};
	int i,j,m,n;
	
	old_ssid = nvram_safe_get(WLN0_OLD_SSID0);
	wlunit = nvram_safe_get(WLN0_UNIT);
	
	snprintf(tmp, sizeof(tmp), "wl%s_hwaddr", wlunit);
	hwaddr = nvram_safe_get(tmp);
	if(strchr(wlunit,'.')){
		//vif
		m = 9;n = 6;
	}else{
	...
	if(strlen(hwaddr) >= 11){
		//XX:XX:XX:XX:XX:XX
		for(i=0,j=0;  hwaddr[i+m]!='\0' && j<n; ++i){
			if (hwaddr[i+m] != ':'){		
				wlanmac[j]=hwaddr[i+m];
				++j;
			}
		}
		wlanmac[n]='\0';
		snprintf(tmp, sizeof(tmp), "%s_%s",old_ssid, wlanmac);
		nvram_set(WLN0_SSID0, tmp);
	}else{
		nvram_set(WLN0_SSID0, old_ssid);
	}
...
void start_wps_negotiate()
{
	...
	if(strcmp(wl_mode, "sta") == 0 || strcmp(wl_mode, "wet") == 0
		|| nvram_match("wl0_wps_mode", "disabled") && nvram_match("wl0.1_wps_mode", "enabled"))
	{
