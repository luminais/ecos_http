...
function ipMskChk(mn,str)
{
	...
	{
		m=mn.value.split('.');
		if (m.length!=4) { alert(str+" 無效！") ; return 0; }
	}
    for(i=0;i<4;i++)
	{
		if(!t.test(m[i]))
		{
			alert(str+"包含無效字元！") ; return 0;
		}
	}
...
{
    if (kk=='undefined') { alert("未定義！"); return -1; }
...
function IsReboot()
{
	if(confirm("重新啟動路由器使設定生效，是否立即重新啟動？"))	
		return 1;
	else
		return 0;
}
function IsRebootW()
{
	if(confirm("重新啟動無線模組使設定生效，是否立即重新啟動？"))	
		return 1;
	else
		return 0;
}
...
function ckMacReserve(mac)
{
	...
	*/
	...
	{
		alert("此MAC位址為Multicast位址，請重新輸入。")	;
		return false;
	}
...
//add by stanley
	...
	{
		if(m[0]<255){
			alert(str+" 無效，不支援網路聚集功能！");
			//alert(str+" 無效！") ;
			return 0;
		}
		if(m[1]>0 || m[2]>0 || m[3]>0){
			alert(str+" 無效，不支援子網路分割功能！");
			//alert(str+" 無效！") ;
			return 0;
		}	
