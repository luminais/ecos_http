...
function CheckValue() {
	...
	if (f.elements["wlMode"][1].checked == true) {
		...
		else if(secMode == 3 || secMode == 4 ||secMode == 5)//WPA
		{
			...
			if(!ill_check(document.wireless_mode.passphrase.value,illegal_wl_pass, _("security key"))) return false;
...
function macAcc() {
	...
	for (var r = maxcell; r > 1; r --) {
		...
		if (sc) {
			...
			sec_type = (tbl.rows[r - 1].cells[4].innerHTML).split("/");
			if(sec_type[0] == "NONE"){
				f.elements["sta_security_mode"].value = "Disable";
			}else if(sec_type[0] == "WPA"){
				f.elements["sta_security_mode"].value = "psk";
			}else if(sec_type[0] == "WPA2"){
				f.elements["sta_security_mode"].value = "psk2";
			}else if(sec_type[0] == "WPAWPA2"){
				f.elements["sta_security_mode"].value = "psk psk2";
			}else if(sec_type[0] == "WEP"){
				f.elements["sta_security_mode"].value = "0";
			}
			onChangeSec();
			if(sec_type[0] != "NONE" && sec_type[0] != "WEP"){
				if(sec_type[1] == "AES"){
					f.cipher[0].checked = true;
				}else if(sec_type[1] == "TKIP"){
					f.cipher[1].checked = true;
				}else if(sec_type[1] == "AESTKIP"){
					f.cipher[2].checked = true;
				}
			}
			f.passphrase.value = "";
			//f.elements["sta_security_mode"].selectedIndex = tbl.rows[r - 1].cells[4].innerHTML;
			//f.cipher[0].checked = true;
		}
	}
}
...
function init(){
	makeRequest("/goform/initUniversalRepeater", "something");
}
