...
function CheckValue() {
	...
	if(sid != decodeSSID(SSID0)){
		var con=confirm("无线信号名称(主SSID)已更改为 "+sid+"，请以新的无线信号名称重新连接无线网络!");
		if(con == false){
	...
	if(sid1 != decodeSSID(SSID1)) {
		if(sid1 == "")
			var con=confirm("次无线信号名称(次SSID)已禁用，用次SSID连接无线网络的设备将断开无线网络!");
		else
			var con=confirm("次无线信号名称(次SSID)已更改为 "+sid1+"，请用次SSID连接无线网络的设备以新的无线信号名称重新连接无线网络!");
		if(con == false){
...
<input type="hidden" name="bssid_num" value="1">
	<fieldset>
		...
		<div id="divwieless" style="clear:both"><!-- 为了消除ie7bug 添加clear:both属性 -->
			<div class="control-group"> 
				<label class="control-label">无线信号名称(主SSID)</label>
				<div class="controls"><input type="text" name=ssid id=ssid size=20 maxlength=32 class="text" /></div>
			</div>
			<div class="control-group"> 
				<label class="control-label">次无线信号名称(次SSID)</label>
			...
			<div class="control-group"> 
				<label class="control-label">AP 隔离</label>
				<div class="controls">
					<label class="radio"><input type=radio name=ap_isolate value="1" checked />开启</label>
					<label class="radio"><input type=radio name=ap_isolate value="0" />关闭</label>
				</div>
			</div>
