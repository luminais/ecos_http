...
function CheckValue() {
    var wireless_mode,
	 	submit_ssid_num,
		channel_11a_index,
		sid = document.wireless_basic.ssid.value,
		sid1 = document.wireless_basic.mssid_1.value;
	if (!checkSSID(sid)) {
	...
	/*if(!re.test(sid) || sid == "")
	{
		alert(_("SSID can not contain comma, semicolon, double quotation marks, ampersand, percent, backslash and space.."));
		document.wireless_basic.ssid.focus();
		document.wireless_basic.ssid.select();
		return false;
	}
	if(wl0_mode == "sta")
	{
		if(!re.test(sid1) || sid1 == ""){
			alert(_("SSID can not contain comma, semicolon, double quotation marks, ampersand, percent, backslash backslash and space.."));
			document.wireless_basic.mssid_1.focus();
			document.wireless_basic.mssid_1.select();
			return false;
		}
