...
Tenda.hasCapital = function(e, value, pos) {
	var pattern = /[A-Z]/g,
...
Tenda.Event = {
	...
	on: function (node, eventType, handler, scope) {
		...
		if (document.attachEvent) {
			...
			{handler.apply(scope,arguments)}, false);
		} else {
...
Tenda.dom = {
	...
	addCapTip: function(newField, pasElem) {
		...
		Tenda.Event.on(newField, "keyup", function (e) {
			...
			if (!this.capDetected) {
				...
				massageElm.className = "help-inline text-info none";
				massageElm.id = msgId
				massageElm.innerHTML = "您输入的是大写字母";
				if (pasElem) {
			...
			if (Tenda.hasCapital(e, this.value, pos)) {
				Tenda.dom.removeClass(massageElm, "none");
				repeat = "Tenda.dom.hide('" + msgId +"')";
				setTimeout(repeat, 1000);
			} else {
				Tenda.dom.addClass(massageElm, "none");
			}
		});
	},
	
	toTextType: function (elem, capTip) {
		var newField;
	...
	inputPassword: function (id, text, capTip) {
