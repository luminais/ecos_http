	...
	if (http_request.readyState == 4) {
		if (http_request.status == 200) {
			...
			if(mode == "disabled"){
				...
				document.getElementById("device_pin").style.display="none";
...
function UpdateMBSSIDList() {
	...
	if (wl0_mode[0] === "sta" ||  wl0_mode[0] === "wet") {
...
function parseAllData(str) {
	...
	for (var i=0; i<all_str.length; i++) {
		...
		pin_value = trim(fields_str[16]);
		Cur_ssid_index[i] = trim(fields_str[17]);
		wl0_mode[i] = trim(fields_str[18]);
...
function disWps() {
	var f=document.security_form,
		wpsSta = document.getElementById("wps_status");
	if(wps_config_statue == "enabled"){
		wpsSta.innerHTML = _("Unconfigured");
	} else {
		wpsSta.innerHTML = _("Configured");
	}
	if ((f.security_mode.value == "0")||(f.security_mode.value == "1")||(f.security_mode.value == "psk")|| (f.security_mode.value == "psk2" && f.cipher[1].checked == true)
		|| (f.security_mode.value == "psk psk2" && f.cipher[1].checked == true)) {
		document.getElementById("wl_wan_wps").style.display = "none"; 			
	} else {
		document.getElementById("wl_wan_wps").style.display = ""; 			
	}
...
function LoadFields(){
	...
	sp_select.options[sp_select.length] = new Option(_("WPA2 - PSK"),"psk2");
	...
	document.getElementById("pin_value").innerHTML = pin_value;
	securityMode();
	disWps();
}
function initAll() {	
...
function onwpsuse(x){
	...
	//	f.cipher[2].disabled=true;
		...
		document.getElementById("device_pin").style.display="";
		document.getElementById("rstOOB").disabled=false;
