		...
		<div id="wan_sec">
			...
			<fieldset id="type_ppoe" class="table-field none">
				...
				<div class="control-group">
					...
					<div class="controls">
						...
						<span class="help-block">60-3600 <span>seconds</span>
					</div>
				</div>
				<div class="control-group">
			...
			<table id="type_pptp" class="none">
				<tr><td class="control-label">PPTP <span>Server Address</span></td>
					<td class="controls"><input type="text" class=text name="pptpIP" size="15" maxlength="64"></td></tr>
			...
			<table id="type_l2tp" class="none">
				<tr><td class="control-label">L2TP <span>Server Address</span></td>
					<td class="controls"><input type="text" class=text name="l2tpIP" size="15" maxlength="64"></td></tr>
				<tr><td class="control-label">Username</td>
...
function initWanSec(m) {
	var typeArr = ["type_static", "type_dhcp", "type_ppoe", "type_pptp", "type_l2tp"],
		index = parseInt(m, 10),
		i;
	for (i = 0; i < 5; i++) {
		if(i !== index) {
...
function preSubmit(f) {
	...
	if ( m == 2) { //pppoe
		...
		if(f.PUN.value == "Enter username provided by ISP" || f.PUN.value == "" || f.PPW.value == "" || f.PPW.value == "Enter password provided by ISP") {
		...
		if(!ipWanMaskCheck(f.WANIP.value,f.WANGW.value,f.WANMSK.value)){
			alert(_("The WAN gateway and the IP must be in the same network segment!"));
			return;
		}
		...
		if(!ipWanCheck(f.l2tpWANIP.value,f.l2tpWANGW.value)){
			alert(_("The WAN gateway and the IP must be in the same network segment!"));
			return;
		}
	  ...
	  if(!ill_check(f.elements['pptpPPW'].value,illegal_user_pass,_("Password"))) return;
		...
		if(!ipWanCheck(f.pptpWANIP.value,f.pptpWANGW.value)){
			alert(_("The WAN gateway and the IP must be in the same network segment!"));
			return;
		}
	...
	if(m != 7){
		...
		if(m == 2){
			if(parseInt(mtu, 10) < 576 || parseInt(mtu, 10) > 1492){
				alert(_("Please enter an MTU value between 576 and 1492!"));
				return ;
			}
		}else if(parseInt(mtu, 10) < 576 || parseInt(mtu, 10) > 1500) {
			alert(_("Please enter an MTU value between 576 and 1500!"));
			return ;
		}
	}
