...
function chkPOE(f) {
	...
	if(pun == _("Enter user name provided by ISP") || pun == "" || ppw == "") {
...
function doFinish(f) {
	...
	if(password.length < 8 || password.length == 0 || password.length > 63){
		 alert(_("Security key must contain 8~63 characters!"));
		 return false;
	}
	if(!ill_check(password,illegal_wl_pass,"Security Key")) {
		return false;
	}
	da = new Date();
