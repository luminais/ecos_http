...
int tai_active_submit_start(void)
{
	...
	if(!strcmp("sta",wl_mode)) //wisp
		strcpy(speed_detect_ifname, "eth1");
	else if(!strcmp("ap",wl_mode)) //ap
		strcpy(speed_detect_ifname, "vlan2");
	else if(!strcmp("wet",wl_mode))//apclient
		return 0;
	else
		return 0;
