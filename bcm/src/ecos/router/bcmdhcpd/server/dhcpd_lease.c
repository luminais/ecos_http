...
copyHostname2TendaArp(char *ifname)
{
	...
	/*	if (iterate->hostname) {
			for(i=0;i<255;i++){
				if(!det[i].ip)
					continue;
				if(!memcmp(det[i].mac,iterate->mac,sizeof(iterate->mac))){
					strncpy(det[i].hostname, (char *)iterate->hostname,20);
					break;
				}
			}
		}*/
		if(i>=0&&i<255&&iterate->hostname){
			machost[i].ip=iterate->ipaddr.s_addr;
			memcpy(machost[i].mac, iterate->mac, 6);
			strncpy(machost[i].hostname,(char *)iterate->hostname,63);
			i++;
		}
	}
	
	set_arp_hostname(machost);
