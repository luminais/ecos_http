...
extern void board_reboot(void);  // tenda add
void
...
#ifdef BCM47XX
unsigned char net_mempool_area[NET_MEMPOOL_SIZE] __attribute__((weak));
int net_mempool_size __attribute__((weak)) = sizeof(net_mempool_area);
#else
...
void cyg_kmem_print_stats( void )
{
    ...
    diag_printf( "mbufs %ld, free mbufs %ld\n",
                 mbstat.m_mbufs,	/* mbufs obtained from page pool */
                 mbtypes[MT_FREE]	/* free mbufs in MT_FREE */
        );
    diag_printf( "clusters %ld, free clusters %ld\n",
...
int get_net_clusters_freemem(void)
{
	cyg_mempool_info info;
...
#define CLUS_THRESHOLD 	1200
#define CLUS_FREE		20

int do_hammer_by_clusters_status(void)
{
	if(mbstat.m_clusters >= CLUS_THRESHOLD && mbstat.m_clfree < CLUS_FREE )
	{
		return 1 ;
	}
	return 0 ;
...
cyg_netint(cyg_addrword_t param)
{
    ...
    while (true) {
#ifdef BCM47XX
        cyg_tick_count_t later = cyg_current_time() + CYGNUM_NET_FAST_THREAD_TICKLE_DEVS_DELAY;
        curisr = cyg_flag_timed_wait(&netint_flags, NETISR_ANY, 
                               CYG_FLAG_WAITMODE_OR|CYG_FLAG_WAITMODE_CLR, later);
#else
...
#ifdef BCM47XX
	/* try to send out tx queued buffer */
	if (!(curisr & (1<<NETISR_DSR))) {
	    if (NULL != _netisr_handlers[NETISR_DEVTICK])
                (*_netisr_handlers[NETISR_DEVTICK])();
        }
#endif /* BCM47XX */
        splx(spl);
    }
