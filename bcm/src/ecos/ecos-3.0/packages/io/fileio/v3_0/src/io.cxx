...
#ifdef CYGVAR_KERNEL_THREADS_STANDARD_IO_REDIRECT
#include <stdio.h>
...
#ifdef CYGVAR_KERNEL_THREADS_STANDARD_IO_REDIRECT
	/* Get redirected I/O descriptor */
	rdrfd = cyg_thread_get_rdrfd(cyg_thread_self());

	/*
 	 * If the rdrfd has been set, stdin/out/err 
 	 * should be redirected to the specific rdrfd 
 	 */

	if (rdrfd >= 0 && 
	    ((fd == fileno((FILE *)stdout)) ||
	    (fd == fileno((FILE *)stdin)) ||
	    (fd == fileno((FILE *)stderr)))) {
		fd = rdrfd; 
	} 
#endif /* CYGVAR_KERNEL_THREADS_STANDARD_IO_REDIRECT */
