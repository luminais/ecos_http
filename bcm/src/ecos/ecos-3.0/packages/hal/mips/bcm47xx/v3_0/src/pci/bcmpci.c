...
#ifdef DEBUG
#define DBG(x...) diag_printf(x)
#else
#define DBG(x...)
#endif

LIST_HEAD_DECLARE(pci_root_buses);
LIST_HEAD_DECLARE(pci_devices);
...
err_out:
	diag_printf("PCI: Unable to reserve %s region #%d:%lx@%lx for device %s\n",
...
static LIST_HEAD_DECLARE(pci_drivers);
...
#else
			if (l) {
				diag_printf("PCI: Unable to handle 64-bit address for device %s\n", 
				       dev->slot_name);
...
pci_read_bridge_bases(struct pci_bus *child)
{
	...
	if (dev->transparent) {
		diag_printf("Transparent bridge - %s\n", dev->name);
		for(i = 0; i < 4; i++)
	...
	if ((mem_base_lo & PCI_PREF_RANGE_TYPE_MASK) == PCI_PREF_RANGE_TYPE_64) {
		...
		if (mem_base_hi || mem_limit_hi) {
			diag_printf("PCI: Unable to handle 64-bit address space for %s\n", child->name);
			return;
		}
#endif
	}
...
int pci_setup_device(struct pci_dev * dev)
{
	...
	default:				    /* unknown header */
		diag_printf("PCI: device %s has unknown header type %02x, ignoring.\n",
			dev->slot_name, dev->hdr_type);
		return -1;

	bad:
		diag_printf("PCI: %s: class %x doesn't match header type %02x. Ignoring class.\n",
 ...
 */
struct pci_dev *
pci_scan_device(struct pci_dev *temp)
{
	struct pci_dev *dev;
	u32 l;
...
struct pci_dev *
pci_scan_slot(struct pci_dev *temp)
{
...
int
pci_bus_exists(const struct list_head *list, int nr)
{
...
pci_setup(char *str)
{
	while (str) {
		...
		if (*str && (str = pcibios_setup(str)) && *str) {
			...
			diag_printf("PCI: Unknown option `%s'\n", str);
		}
		str = k;
	}
	return 1;
