/*
 *	$Id: bcmpci.h,v 1.2 2010-04-24 09:29:45 simonk Exp $
 *
...
#include <cyg/hal/pci/ioport.h>
#include <cyg/hal/pci/list.h>
#include <errno.h>
...
{
	void * ret = (void *) malloc(size+16);
	
	if (ret)
	{
		HAL_DCACHE_FLUSH((cyg_uint32)ret, size);
		ret = (void *)(CYGARC_UNCACHED_ADDRESS((cyg_uint32)ret));
		memset(ret, 0, size);
		*dma_handle = (dma_addr_t)CYGARC_PHYSICAL_ADDRESS((cyg_uint32)ret);
	}
	return ret;
}
...
{
	unsigned long addr = CYGARC_CACHED_ADDRESS((cyg_uint32)vaddr);
	free((void *)addr);
}
