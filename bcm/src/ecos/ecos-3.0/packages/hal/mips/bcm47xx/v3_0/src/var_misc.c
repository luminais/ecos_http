...
int mips_idle_sleep = 0;

void hal_variant_idle_thread_action( cyg_uint32 count )
{
	if (mips_idle_sleep) {
		asm ("wait; nop; nop; nop;\n");
	}
}

externC cyg_uint8 *
mem_real_region_top(cyg_uint8 *regionend) __attribute__((weak));

cyg_uint8 *
mem_real_region_top(cyg_uint8 *regionend)
{
	unsigned long mem;
