...
#define SER_16X5X_WRITE_LCR(_base_, _val_) serial_out8((_base_) + REG_lcr, _val_)
#endif
                   ...
                   cyg_serial_info_t *new_config, bool init)
{
    ...
    serial_out8(base+REG_mdl, baud_divisor >> 8);
    serial_out8(base+REG_ldl, baud_divisor & 0xFF);
    ...
    if (init) {
        ...
        if (ser_chan->deviceType == s16450) {
            // Check for a FIFO
            serial_out8(base+REG_fcr, FCR_FE);
        ...
        if (ser_chan->deviceType == s16550a) {
            ...
            serial_out8(base+REG_fcr, _fcr_thresh); 
        }
        else {
            ser_chan->tx_fifo_size = 1;
            serial_out8(base+REG_fcr, 0); // make sure it's disabled
        }
        ...
        serial_out8(base+REG_mcr, MCR_INT|MCR_DTR|MCR_RTS);
    }
#ifdef CYGOPT_IO_SERIAL_SUPPORT_LINE_STATUS
    _ier |= (IER_LS|IER_MS);
#endif
    serial_out8(base+REG_ier, _ier);
