...
#if CFG_RESCURE_MODE 
#define WAIT_TIME_FOR_RESCURE 1  /* check 10 times for 500ms interval */
/* CJ++ for failed-save mode return 1 for button_pressed */
int
rescure_mode_button_check(int wait_time_in_second)
{
	int gpio;
	uint32 gpiomask;
       int check_counter;
	...
	gpiomask = (uint32)1 << gpio;
       ...
       if((si_gpioin(sih) & gpiomask) == 0)
        {
	        while (check_counter < wait_time_in_second) {
		 if ((si_gpioin(sih) & gpiomask) == 0) {
                     printf("CJ++ check_counter = %d\n", check_counter);
                     check_counter++;
			OSL_DELAY(RESET_DEBOUNCE_TIME);
		}
         	if ((si_gpioin(sih) & gpiomask) !=0 )
        ...
        if (check_counter >= wait_time_in_second)
        {
            return 1;
         }
         else
...
{
#if 0
    si_gpioout(sih, 1<<6, 1<<6, GPIO_DRV_PRIORITY);
    
    si_gpioouten(sih, 1<<6, 1<<6, GPIO_DRV_PRIORITY);
#else 
    printf("turn off leds gpioout \n");
    si_gpioout(sih, 0xc000e0, 0xc00000, GPIO_DRV_PRIORITY);
    printf("turn off leds gpioouten \n");
    si_gpioouten(sih, 0xc000e0, 0xc000e0, GPIO_DRV_PRIORITY);
#endif
    return 0;
}

#define SYS_LED_VAL (1<<6)
#define SYS_LED_MASK (1<<6)
int board_sysled_init(void)
{
	printf("System led init!\n");
    si_gpioout(sih, SYS_LED_MASK, SYS_LED_VAL, GPIO_DRV_PRIORITY);
    si_gpioouten(sih, SYS_LED_MASK, SYS_LED_VAL, GPIO_DRV_PRIORITY);
}
...
board_console_init(void)
{
	...
	if (CHIPID(sih->chip) == BCM5357_CHIP_ID) {
		...
		nvstr = nvram_get("sdram_phyctl");
		if (nvstr) {
			phyctl = bcm_strtoul(nvstr, NULL, 0);
			si_dmc_phyctl(sih, phyctl);
		}
	}
...
#if 0
#if 0
    fprobe.flash_nparts = 9;
    fprobe.flash_parts[0].fp_size = 0x40000; /*256K*/
    fprobe.flash_parts[0].fp_name = "boot";
    fprobe.flash_parts[1].fp_size = 0x20000; /*128K*/
    fprobe.flash_parts[1].fp_name = "env";
    fprobe.flash_parts[2].fp_size = 0x20000; /* 128K */
    fprobe.flash_parts[2].fp_name = "jffs2";
    fprobe.flash_parts[3].fp_size = 0x300000; /*3M*/
    fprobe.flash_parts[3].fp_name = "kernel1";
    fprobe.flash_parts[4].fp_size = 0x40000;  /*256K*/
    fprobe.flash_parts[4].fp_name = "cramfs1";
    fprobe.flash_parts[5].fp_size = 0x300000; /* 3M */
    fprobe.flash_parts[5].fp_name = "kernel2";
    fprobe.flash_parts[6].fp_size = 0x40000;  /* 256K */
    fprobe.flash_parts[6].fp_name = "cramfs2";
    fprobe.flash_parts[7].fp_size = 0xE0000;  /* 896KB */
    fprobe.flash_parts[7].fp_name = "log";
    fprobe.flash_parts[8].fp_size = NVRAM_SPACE;  /* 128K */
    fprobe.flash_parts[8].fp_name = "nvram";
    cfe_add_device(drv, 0, 0, &fprobe);
    #endif 
    fprobe.flash_nparts = 9;
    fprobe.flash_parts[0].fp_size = 0x80000;
    fprobe.flash_parts[0].fp_name = "boot";
    fprobe.flash_parts[1].fp_size = 0x20000;
    fprobe.flash_parts[1].fp_name = "env";
    fprobe.flash_parts[2].fp_size = 0x100000;
    fprobe.flash_parts[2].fp_name = "jffs2";
    fprobe.flash_parts[3].fp_size = 0x200000;
    fprobe.flash_parts[3].fp_name = "kernel1";
    fprobe.flash_parts[4].fp_size = 0xd20000;
    fprobe.flash_parts[4].fp_name = "cramfs1";
    fprobe.flash_parts[5].fp_size = 0x200000;
    fprobe.flash_parts[5].fp_name = "kernel2";
    fprobe.flash_parts[6].fp_size = 0xd00000;
    fprobe.flash_parts[6].fp_name = "cramfs2";
    fprobe.flash_parts[7].fp_size = 0x20000;
    fprobe.flash_parts[7].fp_name = "log";
    fprobe.flash_parts[8].fp_size = NVRAM_SPACE;
    fprobe.flash_parts[8].fp_name = "nvram";
    cfe_add_device(drv, 0, 0, &fprobe);
#else
		/* Because CFE can only boot from the beginning of a partition */
...
#endif
#if CFG_RESCURE_MODE 
/* CJ++ for rescure mode  check the button before et init */
    ret = rescure_mode_button_check(1);
    if (ret == 1 )  /* button pressed  */
    {
        nvram_set("rescure_mode", "1");
        nvram_set("vlan1ports"," 0 1 2 3 4 5*");
        printf(" button pressed ... stop in cfe\n ");       
    }
    else 
    {
        /* ensure  that we never get the rescure_mode in normal operations */
        nvram_unset("rescure_mode");
    }
	...
	if (commit) {
		...
		if (restore_defaults) {
			printf("Waiting for reset button release...");
                    /* CJ++ never wait for the reset button */
                    //reset_release_wait();
			printf("done\n");
		}
	}
