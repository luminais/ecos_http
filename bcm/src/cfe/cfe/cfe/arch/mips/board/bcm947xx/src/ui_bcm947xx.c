...
#define UPGRADE_DEFAULT_WAIT_TIME 3
#define UPGRADE_MAX_WAIT_TIME 20
extern int g_upgrade_flag;
extern int g_upgrade_delay_time ;


void upgrade_wait(int time_get)
{
	int64_t timer;
	TIMER_SET(timer,CFE_HZ*time_get);
    while (!TIMER_EXPIRED(timer)) {
	POLL();
    }
	printf("*");
	TIMER_SET(timer,CFE_HZ*g_upgrade_delay_time);
    while (!TIMER_EXPIRED(timer)) {
	POLL();
    }
	printf("*");
	//printf("wait upgrade flag!   >> %d \n",g_upgrade_flag);
	while(g_upgrade_flag){
	POLL();
	}
	printf("upgrade_wait over!\n");
}
...
#endif /* CFG_NFLASH */
extern int rescure_mode_button_check(int wait_time_in_second);
...
ui_cmd_go(ui_cmdline_t *cmd, int argc, char *argv[])
{
	...
	/* get the delay time for upgrade */
	pupgrade_time = nvram_get("upgrade_wait_time");
	//printf("<%s>",pupgrade_time);
	if(pupgrade_time)
	{
		upgrade_time = atoi(pupgrade_time);
		if(upgrade_time >= UPGRADE_MAX_WAIT_TIME)
			upgrade_time = 20;
		else if(upgrade_time <= 0)
			upgrade_time = UPGRADE_DEFAULT_WAIT_TIME;
	...
	printf("upgrade wait time is %ds\n",upgrade_time);
	upgrade_wait(upgrade_time);
	...
	if (trx_failed) {
		...
		/* Load the image */
	val = nvram_get("tftp_boot_cmd");
	if( val )
	{
		sprintf(buf, "load -raw -addr=0x%x -max=0x%p %s", ptr, bufsize, val);
	} 
	else 
	{
		sprintf(buf, "load -raw -addr=0x%x -max=0x%p :", ptr, bufsize);
	 }
	...
	//printf("do command [%s]\n", buf);
#if CFG_RESCURE_MODE 	
	if(nvram_get("rescure_mode") !=NULL)
	    {
	     if (1 == rescure_mode_button_check(1))  /* check the reset gpio status keep for 5 seconds */
	        {
	            gpio_led_off();
	            return ui_docommand("ifconfig eth0 -addr=192.168.1.1");
	        }
            else 
            {
	        return ui_docommand(buf);
