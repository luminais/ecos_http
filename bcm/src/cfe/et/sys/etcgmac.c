...
int disable_eee( ch_t *ch)
{
    int phy_id;
    printf("disable EEE for all ports\n");
    printf("ch->etc->chops->phywr at %p\n", ch->etc->chops->phywr);
    printf ("read reg test return 0x%x\n", ch->etc->chops->phyrd(ch, 0, 0x2));
    for (phy_id =0; phy_id < 4; phy_id++)
    {
        ch->etc->chops->phywr(ch, phy_id, 0x0d, 0x07);
        ch->etc->chops->phywr(ch, phy_id, 0x0e, 0x3c);
        ch->etc->chops->phywr(ch, phy_id, 0x0d, 0x4007);
        ch->etc->chops->phywr(ch, phy_id, 0x0e, 0x00);
    }
...
chipattach(etc_info_t *etc, void *osh, void *regsva)
{
	...
	/* CJ++ fix the GE port problem set fix_geport = 1 for GE port problem */
	var = getvar(ch->vars, "fix_geport");
	if (var != NULL) 
     {
        xprintf("fix the GE phy for W45AP\n");
        OSL_DELAY(50000); /* wait 50ms for phyreset */
        xprintf("set phyaddr =0x%x reg =0x18 value = 0x80e7\n", etc->phyaddr);
        xprintf("set phyaddr =0x%x reg =0x1c value = 0x8c00\n", etc->phyaddr);
        /* clear the 0x18 shadow 111 bit 8  RGMII RXD to RXC Skew  */
        //chipphywr(ch,  etc->phyaddr, 0x18, 0x80e7);
        /* clear the 0x1c shadow 00011 bit9 GTXCLK Clock Delay Enable */
        //chipphywr(ch,  etc->phyaddr, 0x1c, 0x8c00);
                /* clear the 0x18 shadow 111 bit 8  RGMII RXD to RXC Skew  */
        chipphywr(ch,  1, 0x18, 0x80e7);
        /* clear the 0x1c shadow 00011 bit9 GTXCLK Clock Delay Enable */
        chipphywr(ch,  1, 0x1c, 0x8c00);
	}
 /* CJ++ end */
 /* CJ++ for p130ap signal leds */
#define POWER_LEVEL_LED0  (0x1<<20) 
#define POWER_LEVEL_LED1  (0x1<<21)
#define POWER_LEVEL_LED2  (0x1<<22) 
#define POWER_LEVEL_LED3  (0x1<<23)
#define POWER_LEVEL_LED4  (0x1<<11)
#define POWER_LED_MASK    (POWER_LEVEL_LED0|POWER_LEVEL_LED1|POWER_LEVEL_LED2|POWER_LEVEL_LED3|POWER_LEVEL_LED4) 

	var = getvar(ch->vars, "p130ap_leds");
	if (var != NULL)
	{
		si_gpioout(ch->sih, POWER_LED_MASK, POWER_LED_MASK, GPIO_DRV_PRIORITY);
		si_gpioouten(ch->sih, POWER_LED_MASK, POWER_LED_MASK, GPIO_DRV_PRIORITY);
		OSL_DELAY(500000);
		si_gpioout(ch->sih, POWER_LED_MASK, 0, GPIO_DRV_PRIORITY);

	}
 /* CJ++ for p130ap signal leds end */
...
#endif /* ETADM */
#if CFG_RESCURE_MODE 
/* CJ++ for rescure mode turn on all the leds */
if (getvar (ch->vars, "rescure_mode") !=NULL)
{
#if 1  /* for india version fh303 */
    printf("turn on leds gpioout \n");
    //si_gpioout(ch->sih, 0xc000e0, 0xe0, GPIO_DRV_PRIORITY);
    printf("turn on leds gpioouten \n");
    //si_gpioouten(ch->sih, 0xc000e0, 0xc000e0, GPIO_DRV_PRIORITY);
    bcm_robo_turn_on_leds(etc->robo);
 #else
    /* CJ++ for A31 */
     printf("turn on leds gpioout \n");
    si_gpioout(ch->sih, 0x2, 0x0, GPIO_DRV_PRIORITY); 
    printf("turn on leds gpioouten \n");
    si_gpioouten(ch->sih, 0x2, 0x2, GPIO_DRV_PRIORITY);
    //bcm_robo_turn_on_leds(etc->robo);
 #endif
    }
/* CJ++ end */
