...
static cmd_func_t td_wl_stalist; /* add by jack,2015-10-16, for cmd 'stalist' */
#endif
...
#ifdef TENDA_WLAN_DBG
	{ "stalist", td_wl_stalist, WLC_GET_TENDA_STA, -1,
	"AP only: Get the list of sta status ."},
#endif
...
#ifdef WLMEDIA_IPTV
	{ "scb_videotag", wl_iov_mac, WLC_GET_VAR, WLC_SET_VAR,
	"wl sta_videotag <xx:xx:xx:xx:xx:xx>"},
#endif /* WLMEDIA_IPTV */
...
int
tenda_wl_rsn_ie_crypto(bcm_tlv_t *ie)
{
	int i;
	int rsn;
	wpa_ie_fixed_t *wpa = NULL;	
	wpa_suite_t *suite = NULL ;
...
int
tenda_wl_dump_wpa_rsn_crypto(uint8* cp, uint len)
{
	uint8 *parse = cp;
	uint parse_len = len;
	uint8 *wpaie;
	...
	if (wpaie){
		rtn1=tenda_wl_rsn_ie_crypto((bcm_tlv_t*)wpaie);
	}

	rsnie = wlu_parse_tlvs(cp, len, DOT11_MNG_RSN_ID);
	if (rsnie){
		rtn2=tenda_wl_rsn_ie_crypto((bcm_tlv_t*)rsnie);
	}
...
int
tenda_dump_bss_crypto(wl_bss_info_t *bi)
{
	int rtn=0;

	if (dtoh32(bi->ie_length))
		rtn=tenda_wl_dump_wpa_rsn_crypto((uint8 *)(((uint8 *)bi) + dtoh16(bi->ie_offset)),
		                    dtoh32(bi->ie_length));
	return rtn;
}
...
#define WPAPSK 				0x0001
#define WPA2PSK				0x0002
#define MAXWPAWPA2			0x0003
#define WEP					0x0004

int
tenda_wl_rsn_ie_dump(bcm_tlv_t *ie)
{
	int rsn;
	wpa_ie_fixed_t *wpa = NULL;	
	rsn_parse_info_t rsn_info;	
...
int
tenda_wl_dump_wpa_rsn_ies(uint8* cp, uint len)
{
	uint8 *parse = cp;
	uint parse_len = len;
	uint8 *wpaie;
	...
	if (wpaie){
		rtn1=tenda_wl_rsn_ie_dump((bcm_tlv_t*)wpaie);
	}

	rsnie = wlu_parse_tlvs(cp, len, DOT11_MNG_RSN_ID);
	if (rsnie){
		rtn2=tenda_wl_rsn_ie_dump((bcm_tlv_t*)rsnie);
	}
...
int
tenda_dump_bss_info(wl_bss_info_t *bi)
{
	int rtn=0;

	if (dtoh32(bi->ie_length))
		rtn=tenda_wl_dump_wpa_rsn_ies((uint8 *)(((uint8 *)bi) + dtoh16(bi->ie_offset)),
		                    dtoh32(bi->ie_length));
	return rtn;
}
...
int
wl_deauth_sta(char *wl, char *mac)
{
	struct ether_addr ea;
	printf("deauth wl:%s mac:%s\n", wl, mac);
	if(wl == NULL || mac == NULL)
		return -1;
	
	if (!wl_ether_atoe(mac, &ea)) {
		printf("Malformed STA MAC parameter. mac is %s\n", mac);
		return -1;
	}
	
	return wlu_set(wl, WLC_SCB_DEAUTHENTICATE, &ea, ETHER_ADDR_LEN);
}
#endif
...
#ifdef TENDA_WLAN_DBG
static int td_wl_stalist(void *wl, cmd_t *cmd, char **argv)
{
    int ret;
    struct td_sta_list *sta_list = (struct td_sta_list *) buf;
    struct td_sta_info *sta;
    uint i, max;
    uint len;
    ...
    if (!*++argv) {
        ...
        max = (WLC_IOCTL_MAXLEN - sizeof(int)) / (sizeof(struct td_sta_info));
        sta_list->count = htod32(max);
        len = sizeof(struct td_sta_info) * sta_list->count;
        sta_list->sta = (struct td_sta_info *)malloc(len);
    	if (sta_list->sta == NULL) {
    		printf("%s:allocating %d bytes fail!!!\n", len);
    		return -1;
    	}
        if ((ret = wlu_get(wl, cmd->get, sta_list, WLC_IOCTL_MAXLEN)) < 0) {
            free(sta_list->sta);
            return ret;
        }

        printf("idx      MAC_ADDR      state Rssi TX_R RX_R   online age Tx_ucnt Rx_ucnt\n");
        for (i = 0, sta = sta_list->sta; i < sta_list->count && i < max; i++, sta++) {
            sta->state     = dtoh32(sta->state);
            sta->assoctime = dtoh32(sta->assoctime);
            sta->agetime   = dtoh32(sta->agetime);
            sta->rssi      = dtoh32(sta->rssi);
            sta->tx_rate   = dtoh32(sta->tx_rate)/1000; 
            sta->rx_rate   = dtoh32(sta->rx_rate)/1000; 
            sta->tx_cnt    = dtoh32(sta->tx_cnt); 
            sta->rx_cnt    = dtoh32(sta->rx_cnt);
            printf("[%2d] %s  0x%-2x %d %3dM %3dM  %8d %4d %d   %d\n", 
                i,wl_ether_etoa(sta->ea),
                sta->state,
                sta->rssi,
                sta->tx_rate,sta->rx_rate,
                sta->assoctime,sta->agetime,
                sta->tx_cnt,sta->rx_cnt);
        }
        free(sta_list->sta);
    }else {
		if (cmd->set < 0)
...
int
wl_set_vendor_ie(void *wl, const char *cmd, char **argv)
{
	return (wl_vndr_ie(wl, cmd, argv));
}
...
wl_pfn_event_check(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (err < 0) {
		...
		close(fd);//hqw add for tcp 2014.01.24
		return -1;
	}
...
void wl_get_buf(char *getbuf)
{
	if(NULL==getbuf)
		return;
	strncpy(getbuf,buf,2048);
	return;
}
