...
extern int access_attack_add_list(const char *mac);
extern void ali_attack(void);
	...
	void *body, uint len, bool short_preamble, bool dec_attempt)
{
	...
	defined(WLMSG_BTA) || defined(__CONFIG_ALINKGW__)
	char eabuf[ETHER_ADDR_STR_LEN], *sa = bcm_ether_ntoa(&hdr->sa, eabuf);
#endif
	wlc_ap_info_pvt_t *appvt = (wlc_ap_info_pvt_t *) ap;
	wlc_info_t *wlc = appvt->wlc;
...
#ifdef __CONFIG_ALINKGW__
extern int get_ali_channel_service_enable();
extern int set_ali_channel_service_enable();
#endif
static int
...
#ifdef __CONFIG_ALINKGW__//add by ll
				|| get_ali_channel_service_enable()
#endif
		)) 
		{		
...
#ifdef __CONFIG_ALINKGW__//add by ll
			if( g_alinkgw_enable )
				set_ali_channel_service_enable();
#endif	
		}
#endif /* APCS */
...
wlc_ap_ioctl(wlc_ap_info_t *ap, int cmd, void *arg, int len, struct wlc_if *wlcif)
{
	...
	case WLC_SET_SCB_TIMEOUT:
		if (AP_ENAB(pub))
			...
			bcmerror = BCME_NOTAP;
		break;
/* add by jack,2015-10-16,支持用户表状态查看 */
#ifdef TENDA_WLAN_DBG
	case WLC_GET_TENDA_STA:
        ASSERT(arg != NULL);
        struct td_sta_list *sta_list = (struct td_sta_list *)arg;
        struct td_sta_info *sta = sta_list->sta;
		/* returns a list of STAs associated with a specific bsscfg */
		if (len < (int)(sizeof(sta_list->count) + (sta_list->count * sizeof(struct td_sta_info)))) {
		...
		FOREACHSCB(wlc->scbstate, &scbiter, scb) {
			if (scb->bsscfg == bsscfg) {
				val++;
				if (sta_list->count >= (uint)val) {
					bcopy((void*)&scb->ea, (void*)&sta->ea[0],
					    ETHER_ADDR_LEN);
                    sta->assoctime = wlc->pub->now - scb->assoctime;
                    sta->agetime   = wlc->pub->now - scb->used;
                    sta->rssi      = wlc_scb_rssi(scb);
                    sta->state     = scb->state;
#ifdef WLCNTSCB
                    sta->tx_rate   = RSPEC2KBPS(scb->scb_stats.tx_rate);
                    sta->rx_rate   = RSPEC2KBPS(scb->scb_stats.rx_rate);
                    sta->tx_cnt    = scb->scb_stats.tx_pkts;
                    sta->rx_cnt    = scb->scb_stats.rx_ucast_pkts;
#endif
				} else {
					bcmerror = BCME_BUFTOOSHORT;
					break;
