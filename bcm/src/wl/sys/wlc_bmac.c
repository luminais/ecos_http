...
#define USE_BIGHAMMER 1

#ifdef USE_BIGHAMMER // for stuck issue
extern int g_stuck_issue_flag;
#endif
...
/* add by tenda for debug */
extern int ampdu_rsp_timeout_flg;
extern int fifo_overf_flag;
/* end add */
...
wlc_dpc(wlc_info_t *wlc, bool bounded)
{
	...
	if (macintstatus & MI_PHYTXERR) {
		...
		/* add by tenda for debug, defined at  wlc_ampdu.c¡¢wlc.c*/
		//if ((fifo_overf_flag || ampdu_rsp_timeout_flg) && nvram_match("wl_reinit","1"))
		if (fifo_overf_flag || ampdu_rsp_timeout_flg)
		{
			WL_ERROR(("wl%d: ampdu_rsp_timeout fifo_overf_flag, wl reinit!\n",wlc_hw->unit, ));
			wl_init(wlc->wl);
			ampdu_rsp_timeout_flg = 0;
			fifo_overf_flag = 0;
		}
		/* end add */
		
		if( wlc_hw->phyerr_cnt >= 3 ) {
			WL_ERROR(("wl%d: HAMMERING info, CNT = %d\n", wlc_hw->unit , wlc_hw->phyerr_cnt ));
			wl_init(wlc->wl);
#ifdef TENDA_WLAN_DBG
            TD_DBG(wlc,TD_DBG_BIG_HAMMER,"wl%d:phyerr_cnt >= 3,so do big hammer!\n",
                wlc->pub->unit);
#endif
			g_stuck_issue_flag = 1;
		}
	} else {
		wlc_hw->phyerr_cnt = 0;
	}
...
//#if (defined(WLTEST) || defined(WLPKTENG))//delete by ll
