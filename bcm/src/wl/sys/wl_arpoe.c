 ...
 *
 * $Id: wl_arpoe.c,v 1.29 2010-02-09 18:15:32 Exp $
 */
...
BCMATTACHFN(wl_arp_attach)(wlc_info_t *wlc)
{
	...
	/* register module */
	wlc_module_register(wlc->pub, arp_iovars, "arp", arpi, arp_doiovar, arp_watchdog,
		NULL, NULL);
...
wl_arp_send_proc(wl_arp_info_t *arpi, void *sdu)
{
	...
	if (arp->oper == HTON16(ARP_OPC_REQUEST)) {
		...
		WL_ERROR(("wl%d: wl_arp_send_proc: unknown ARP oper 0x%x\n",
		          WLCUNIT(arpi), ntoh16_ua((const void *)arp->oper)));
		return -1;
	}
