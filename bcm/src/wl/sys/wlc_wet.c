 ...
 *
 * $Id: wlc_wet.c,v 1.53 2010-02-09 18:15:34 Exp $
 */
...
struct wlc_wet_info {
	...
	/* true if wireless tunneling operation */
	bool tunneling;
};
...
BCMATTACHFN(wlc_wet_attach)(wlc_pub_t *pub)
{
	...
	/* register module */
	wlc_module_register(pub, wet_iovars, "wet", weth, wet_doiovar, NULL, NULL, NULL);
#ifdef BCMDBG
...
static void
wet_sta_clean_all(wlc_wet_info_t *weth)
{
	int i;

	bzero(weth->sta, sizeof(wet_sta_t)*WET_NUMSTAS);
	bzero(weth->stahash_mac, sizeof(wet_sta_t *)*WET_STA_HASH_SIZE);
	bzero(weth->stahash_ip, sizeof(wet_sta_t *)*WET_STA_HASH_SIZE);
	for (i = 0; i < WET_NUMSTAS - 1; i++)
		weth->sta[i].next = &weth->sta[i + 1];
	weth->stafree = &weth->sta[0];
}
...
void
wlc_wet_parse_tunnel_ie(wlc_info_t *wlc, uint8 *tlvs, int tlvs_len)
{
	wlc_wet_info_t *weth = wlc->weth;
	uint8 *ie;
	brcm_prop_ie_t *wet_tunnel_ie;
	bool tunneling = 0;

	/* dhcp_relay enabled, ignore beacon parse and disable WET tunneling. */
	if (!IPV4_ADDR_NULL(weth->ip)) {
		if (weth->tunneling) {
			/* clear station table */
			wet_sta_clean_all(weth);
	...
	while ((ie = (uint8 *)bcm_parse_tlvs(tlvs, tlvs_len, DOT11_MNG_PROPR_ID))) {
		wet_tunnel_ie = (brcm_prop_ie_t *)ie;
		if ((wet_tunnel_ie->len >= BRCM_PROP_IE_LEN) &&
		    (!bcmp(&wet_tunnel_ie->oui[0], BRCM_PROP_OUI, DOT11_OUI_LEN)) &&
	...
	/* validate that the wet tunnel capability */
	if (ie) {
		if (ntohs(wet_tunnel_ie->cap))
			tunneling = 1;
	}
	if (weth->tunneling != tunneling) {
		/* clear station table */
		wet_sta_clean_all(weth);

		weth->tunneling = tunneling;
	}
}

/* Check if multicast frame forwarded back by AP-WET Tunnel. */
int
wlc_wet_multicast_forwarded_back(wlc_info_t *wlc, uint8 *eaddr)
{
	wlc_wet_info_t *weth = wlc->weth;
	wet_sta_t *sta;
	int i;

	/* check tunneling capability */
	if (!weth->tunneling)
		return 0;

	/* find station mac */
	i = WET_STA_HASH_MAC(eaddr);
	for (sta = weth->stahash_mac[i]; sta; sta = sta->next_mac) {
		if (bcmp(&sta->mac, eaddr, ETHER_ADDR_LEN))
			continue;
		return 1;
