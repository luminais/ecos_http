 ...
 *
 * $Id: wlc_high_stubs.c,v 1.103.2.44 2011-02-02 10:31:37 Exp $
 */
...
wlc_rpc_bmac_dispatch(wlc_rpc_ctx_t *rpc_ctx, struct rpc_buf* buf)
{
	...
	case WLRPC_WLC_PHY_ANT_RXDIV_GET_ID: {
		...
		wlc_phy_ant_rxdiv_get(((wlc_hw_info_t *)wlc_hw)->band->pi, &pval);
	...
	case WLRPC_WLC_PHY_ANT_RXDIV_SET_ID: {
		...
		wlc_phy_ant_rxdiv_set(((wlc_hw_info_t *)wlc_hw)->band->pi, val);
		break;
	}
	...
	case WLRPC_WLC_PHY_TXPOWER_SET_ID: {
		...
		ret = wlc_phy_txpower_set(((wlc_hw_info_t*)wlc_hw)->band->pi, qdbm, override);
	...
	case WLRPC_WLC_PHY_RADAR_DETECT_ENABLE_ID: {
		...
		wlc_phy_radar_detect_enable(((wlc_hw_info_t *)wlc_hw)->band->pi, on);
#endif
		break;
	}
...
#if defined(AP) && defined(RADAR)
		ret = wlc_phy_radar_detect_run(((wlc_hw_info_t *)wlc_hw)->band->pi);
#else
		ret = 0;
#endif
		err = bcm_xdr_pack_int32(&retb, ret);
	...
	case WLRPC_WLC_BMAC_COPYFROM_VARS_ID: {
		...
		wlc_bmac_copyfrom_vars(wlc_hw, &bufl, &len);
		...
		err = bcm_xdr_pack_opaque_varlen(&retb, len, bufl);
		ASSERT(!err);
...
#define HANDLE_BOUNDARY(b, p, prev_p)						\
	...
	case WLRPC_WLC_PHY_TXPOWER_GET_CURRENT_ID: {
		...
		wlc_phy_txpower_get_current(((wlc_hw_info_t *)wlc_hw)->band->pi, power, channel);
	...
	case WLRPC_WLC_PHY_BAND_FIRST_CHANSPEC_ID: {
		...
		pi = ((wlc_hw_info_t *)wlc_hw)->bandstate[band]->pi;
	...
	case WLRPC_WLC_PHY_TXPOWER_LIMIT_SET_ID: {
		...
		wlc_phy_txpower_limit_set(((wlc_hw_info_t *)wlc_hw)->band->pi, &txpwr, chanspec);
		break;
	}
	...
	case WLRPC_WLC_PHY_BSSINIT_ID: {
		...
		wlc_phy_BSSinit(((wlc_hw_info_t *)wlc_hw)->band->pi, bonlyap, rssi);
	...
	case WLRPC_WLC_PHY_BAND_CHANNELS_ID: {
		...
		wlc_phy_chanspec_band_validch(((wlc_hw_info_t *)wlc_hw)->band->pi, band, &channels);
	...
	case WLRPC_WLC_BMAC_BTC_PERIOD_GET_ID: {
		...
		wlc_bmac_btc_period_get(wlc_hw, &val, &btactive);
...
#if !(defined(WLC_LOW_ONLY) && defined(WLMCHAN))
		ASSERT(ea_p);
#endif
	...
	case WLRPC_WLC_PHY_CAL_CACHE_INIT_ID: {
		...
		ret = wlc_phy_cache_cals_nphy_init(((wlc_hw_info_t *)wlc_hw)->band->pi);
	...
	case WLRPC_WLC_PHY_CAL_CACHE_DEINIT_ID: {
		...
		wlc_phy_cache_cals_nphy_deinit(((wlc_hw_info_t *)wlc_hw)->band->pi);
		break;
	}
	...
	case WLRPC_WLC_BMAC_DNGL_WD_SET_ID: {
		rpc_buf_t *rpc_buf;
		bcm_xdr_buf_t retb;
		uint32 dngl_wd_exptime, dngl_wd;

		err = bcm_xdr_unpack_uint32(&b, &dngl_wd);
		ASSERT(!err);
		err = bcm_xdr_unpack_uint32(&b, &dngl_wd_exptime);
		ASSERT(!err);

		wlc_bmac_dngl_wd_set(wlc_hw, (bool)dngl_wd, dngl_wd_exptime);

		rpc_buf = wlc_rpc_buf_alloc(rpc, &retb, 0, WLRPC_WLC_BMAC_DNGL_WD_SET_ID);
		ASSERT(rpc_buf != NULL);
	...
	case WLRPC_WLC_BMAC_SET_EPA_DEFAULT_STATE_ID: {
		bcm_xdr_buf_t retb;
		rpc_buf_t *rpc_buf;
		int ret;
		...
		ret = wlc_bmac_set_epa_default_state(wlc_hw);
...
wlc_update_txppr_offset(wlc_info_t *wlc, int8 *txpwr)
{
	...
	len = TXPOWER_XDR_SZ + TXPOWER_HT_XDR_SZ;

	rpc_buf = wlc_rpc_buf_alloc(rpc, &b, len, WLRPC_WLC_BMAC_TXPPR_ID);
	ASSERT(rpc_buf != NULL);

	err = wlc_rpc_pack_txpwr_limits(&b, (txppr_t *)txpwr);
	ASSERT(!err);
...
wlc_p2p_int_proc(wlc_info_t *wlc, uint8 *p2p_interrupts, uint32 tsf_l, uint32 tsf_h)
{
	...
	rpc_buf = wlc_rpc_buf_alloc(rpc, &b,
		ROUNDUP((sizeof(uint8) * M_P2P_BSS_MAX) + sizeof(uint32) * 2, sizeof(uint32)),
