 ...
 *
 * $Id: wlc_cac.c,v 1.120.2.7 2011-01-14 22:07:58 Exp $
 */
...
BCMATTACHFN(wlc_cac_attach)(wlc_info_t *wlc)
{
	...
	wlc_module_register(wlc->pub, cac_iovars, "cac", cac,
		wlc_cac_iovar, wlc_cac_watchdog, NULL, wlc_cac_down);
#ifdef BCMDBG
	...
	uint8 dialog_token, int reason_code, void **pkt, struct scb *scb)
{
	...
	*pkt = NULL;
	...
	if ((*pkt = wlc_frame_get_action(wlc, FC_ACTION, da, &wlc->pub->cur_etheraddr,
 ...
 */
	...
	if ((*pkt = wlc_frame_get_action(wlc, FC_ACTION, &scb->ea, &wlc->pub->cur_etheraddr,
	        &wlc->cfg->BSSID, len, &body, DOT11_ACTION_NOTIFICATION)) == NULL)
		return BCME_ERROR;
