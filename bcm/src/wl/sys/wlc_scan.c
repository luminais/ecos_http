 ...
 *
 * $Id: wlc_scan.c,v 1.192.2.32 2010-12-23 19:16:08 Exp $
 */
...
#if defined(WLLMAC_ONLY) || defined(BRINGUP_BUILD)
/* LMAC doesn't wouldn't have the support routines */
#undef WLC_SCAN_IOVARS
#endif /* definded (WLLMAC_ONLY) || defined(BRINGUP_BUILD) */
...
BCMATTACHFN(wlc_scan_attach)(wlc_info_t *wlc, void *wl, osl_t *osh, uint unit)
{
	...
	err = wlc_module_register(wlc->pub, iovars, "scan",
	                          scan_info, iovar_fn, watchdog_fn, NULL, wlc_scan_down);
	if (err) {
...
wlc_scan_prepare_off_channel(wlc_info_t *wlc)
{
	...
	if (AP_ACTIVE(wlc)) {
		...
		wlc_stop_ap(wlc, TRUE, NULL, WLC_STOP_AP_SCAN);
	}
}
...
wlc_scan_prepare_pm_mode(wlc_info_t *wlc, wlc_scan_info_t *wlc_scan_info)
{
	...
	if (wlc->exptime_cnt == 0 &&
	    !SCAN_TYPE_FOREGROUND((scan_info_t *)wlc_scan_info->scan_priv)) {
		...
		FOREACH_AS_STA(wlc, idx, cfg) {
			if (cfg->pm->PMenabled)
				continue;
#ifdef WLMCHAN
			/* For mchan operation, only enable PS for STA on our home channel */
			if (MCHAN_ENAB(wlc->pub) &&
			    (cfg->chan_context->chanspec != wlc->home_chanspec)) {
				WL_MCHAN(("wl%d.%d: %s: skip pmpend for cfg on other channel\n",
				          wlc->pub->unit, WLC_BSSCFG_IDX(cfg), __FUNCTION__));
				continue;
			}
#endif
			WL_SCAN(("wl%d.%d: wlc_scan_prepare_pm_mode: entering PM mode...\n",
...
wlc_scantimer(void *arg)
{
	...
	if (scan_info->pass >= 1 && scan_info->pass <= passes) {
		chanspec_t scan_chanspec = chanspec;

		/* If the scan chanspec has the same control channel as our current chanspec,
		 * change scan chanspec to our current chanspec to avoid an actual channel
		 * change in the phy but still allow wlc_set_chanspec to be called.
		 */
		if (wf_chspec_ctlchan(chanspec) == wf_chspec_ctlchan(WLC_BAND_PI_RADIO_CHANSPEC)) {
			scan_chanspec = WLC_BAND_PI_RADIO_CHANSPEC;
		}
		...
		wlc_set_chanspec(wlc, scan_chanspec);
		wlc_enable_mac(wlc);

		WL_SCAN(("wl%d: %s: switched to channel %d tsf %u\n",
			wlc->pub->unit, __FUNCTION__, CHSPEC_CHANNEL(scan_chanspec),
			tsf_l));

		wlc_scan_do_pass(scan_info, scan_chanspec);
		goto exit;
	}
	 ...
	 */
	if (WLC_AUTOCOUNTRY_ENAB(wlc) && wlc->awaiting_cntry_info)
...
wlc_scan_return_home_channel(wlc_scan_info_t *wlc_scan_info)
{
	...
	if (AP_ACTIVE(wlc)) {
		...
		wlc_stop_ap(wlc, FALSE, NULL, WLC_STOP_AP_SCAN);
	}
	...
	FOREACH_AS_STA(wlc, idx, cfg) {
		...
		if ((cfg->pm->PM != PM_MAX || cfg->pm->WME_PM_blocked) &&
		    cfg->pm->PMenabled) {
#ifdef WLMCHAN
			/* For mchan operation, only disable PS for STA on our home channel */
			if (MCHAN_ENAB(wlc->pub) &&
			    (cfg->chan_context->chanspec != wlc->home_chanspec)) {
				WL_MCHAN(("wl%d.%d: %s: skip pmclr for cfg on other channel\n",
				          wlc->pub->unit, WLC_BSSCFG_IDX(cfg), __FUNCTION__));
				continue;
			}
#endif
	...
	int channel_max)
{
	...
	for (i = 0; i < NBANDS(wlc); i++, band = wlc->bandstate[OTHERBANDUNIT(wlc)]) {
		...
		if (!wlc_channel_get_chanvec(wlc, wlc->autocountry_default,
		     band->bandtype, &chanvec))
			return 0;
		maxchannel = BAND_2G(band->bandtype) ? WLC_MAX_2G_CHANNEL : MAXCHANNEL;
		for (j = 0; j < sizeof(chanvec_t); j++)
			sup_chanvec.vec[j] &= chanvec.vec[j];

		maxchannel = BAND_2G(band->bandtype) ? (WLC_MAX_2G_CHANNEL + 1) : MAXCHANNEL;
		...
		if (ACT_FRAME_IN_PROGRESS(wlc->scan)) {
			if (wlc->scan->action_frame) {
				WL_SCAN(("wl%d: sending action frame to %d ms, channel %d\n",
				         wlc->pub->unit, channel_dwelltime,
...
bool
wlc_scan_recv_parse_bcn_prb(wlc_scan_info_t *si, bool beacon, wlc_bss_info_t *bi,
	wlc_d11rxhdr_t *wrxh, uint8 *body, int body_len)
{
	...
	if (P2P_ENAB(wlc->pub) &&
	    wlc_p2p_recv_parse_bcn_prb(wlc->p2p, scan_info->bsscfg, beacon, &bi->rateset,
	                          wrxh, body, body_len) != BCME_OK) {
