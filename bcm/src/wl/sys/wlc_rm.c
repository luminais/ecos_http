 ...
 *
 * $Id: wlc_rm.c,v 1.5.2.5 2010-07-23 17:09:30 Exp $
 */
...
{
	rm_info_t *rm_info;
	if ((rm_info = (rm_info_t *)MALLOC(wlc->osh, sizeof(rm_info_t))) == NULL) {
		WL_ERROR(("wl%d: wlc_rm_attach: out of mem, malloced %d bytes\n",
			wlc->pub->unit, MALLOCED(wlc->osh)));
		goto fail;
	}
	bzero((char *)rm_info, sizeof(rm_info_t));
	rm_info->wlc = wlc;
	if ((rm_info->rm_state = (wlc_rm_req_state_t*)MALLOC(wlc->osh,
	...
	bzero((char *)rm_info->rm_state, sizeof(wlc_rm_req_state_t));
	...
	if (wlc_module_register(wlc->pub, rm_iovars, "rm", rm_info,
		wlc_rm_doiovar, NULL, NULL, wlc_rm_down)) {
...
wlc_rm_start(wlc_info_t *wlc)
{
	...
	for (i = 0; i < rm_state->req_count; i++) {
		...
		WL_INFORM(("RM REQ token 0x%02x (%2d) type %2d%s channel %3d tsf 0x%x:%08x dur %4d"
			" TUs\n",
...
{
	wlc_rm_req_state_t* rm_state = rm_info->rm_state;
	chanspec_t cur_chanspec, rm_chanspec;
	wlc_rm_req_t *req;
	int i;
	...
	for (i = 0; i < rm_state->req_count; i++) {
		...
		if (!wlc_valid_chanspec_db(rm_info->wlc->cmi, req->chanspec)) {
			req->flags |= WLC_RM_FLAG_INCAPABLE;
			continue;
		}
...
wlc_rm_timer(void *arg)
{
	...
	case WLC_RM_ABORT:
		...
		break;
	case WLC_RM_IDLE:
		WL_ERROR(("wl%d: wlc_rm_timer: error, in timer with state WLC_RM_IDLE\n",
			wlc->pub->unit));
		break;
	default:
		break;
...
wlc_rm_begin(rm_info_t* rm_info)
{
	...
	if (!blocked &&
		...
		if (rm_chanspec != 0 && rm_chanspec != WLC_BAND_PI_RADIO_CHANSPEC)
			WL_ERROR(("wl%d: wlc_rm_begin: fail to set channel "
	...
	for (req_idx = rm_state->cur_req; req_idx < rm_state->req_count; req_idx++) {
		...
		case WLC_RM_TYPE_RPI:
			...
			break;
		default:
				WL_ERROR(("wl%d: wlc_rm_begin: unknown measurement "
					"request type %d in request token %d\n",
					wlc->pub->unit, req->type, rm_state->token));
			break;
		}
	}
...
wlc_rm_report_ioctl(rm_info_t *rm_info, wlc_rm_req_t *req_block, int count)
{
	...
	rep_block = (uint8*)MALLOC(rm_info->wlc->osh, len);
	if (rep_block == NULL) {
		WL_ERROR(("wl%d: wlc_rm_report_ioctl: out of memory, malloced %d bytes\n",
			rm_info->wlc->pub->unit, MALLOCED(rm_info->wlc->osh)));
		return;
	}
	bzero(rep_block, len);
...
wlc_rm_req_ioctl(rm_info_t *rm_info, wl_rm_req_t *req_set, int len)
{
	...
	if (rm_state->step != WLC_RM_IDLE) {
		WL_ERROR(("wl%d: wlc_rm_req_ioctl: Radio Measurement request "
	...
	if (wl_req_set.count == 0) {
		WL_ERROR(("wl%d: wlc_rm_req_ioctl: invalid args, "
	...
	rm_req = (wlc_rm_req_t*)MALLOC(wlc->osh, rm_req_size);
	if (rm_req == NULL) {
		WL_ERROR(("wl%d: wlc_rm_req_ioctl: out of memory, malloced %d bytes\n",
			wlc->pub->unit, MALLOCED(wlc->osh)));
		return BCME_NORESOURCE;
	}
	bzero(rm_req, rm_req_size);
