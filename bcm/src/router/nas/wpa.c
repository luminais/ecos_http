 ...
 *
 * $Id: wpa.c 278326 2011-08-18 15:10:40Z palter $
 */
...
typedef struct tpsk_list
{
	...
	int dur;    /* TPSK duration */
	int index;
	struct tpsk_list *next;
}Alilink_tpsk_list, *pAlilink_tpsk_list;

extern Alilink_tpsk_list * alilink_tpsk_list; //tpsklist，可能有多个，全0的为临时tpsk
extern int gTpskDebug;
extern char alilink_tmp_tpsk[64]; 	//临时tpsk，只有一个
extern int g_needReportTpskList;
extern int create_and_add_to_list(char *tpsk, char *mac, int dur, int save);
extern void clean_tmp_tpsk(void);
...
void show_tpsklist()
{
	Alilink_tpsk_list *entry;
	int i = 0;
	printf("############show_tpsklist############\n");
	printf("alilink_tmp_tpsk:%s\n",alilink_tmp_tpsk);
	
	printf("alilink_tpsk_list:%p\n",alilink_tpsk_list);
	for(entry = alilink_tpsk_list;entry != NULL; entry = entry->next)
	{
		i++;
		printf("num: %d entry: %p\n",i,entry);
		printf("\ttpsk:%s\n",entry->tpsk);
		printf("\tmac:%s\n",entry->mac);
		printf("\tdur:%d\n",entry->dur);
		printf("\tindex:%d\n",entry->index);
	}
	printf("##########show_tpsklist end###########\n");
}
#endif 	
#endif 	
...
wpa_check_mic(nas_sta_t *sta, eapol_header_t *eapol)
{
	...
	uint8 old_pmk[PMK_LEN] = {0};
	int old_pmk_len = 0;
	int find = 0,isTmpTpsk = 0, isTpskList = 0;
	char tpsk[TPSK_LEN]={0};
#endif 		
	...
	bzero((char*)digest, sizeof(digest));
	...
	if (bcmp(digest, mic, MIC_KEY_LEN) == 0) {
		dbg(sta->nas, "match");
 //       printf("In %s:%d, bcmp(digest, mic) ture\n", __FILE__, __LINE__);
		ret = TRUE;
	} else {
		dbg(sta->nas, "fail");
		ret = FALSE;
...
// 增加阿里tpsk 认证过程    2015-02-12  
	
	//printf("In %s:%d,ret=%d state = %d\n", __FILE__, __LINE__,ret, sta->suppl.state);
	//add to check tpsk
#ifdef __CONFIG_ALINKGW__
	if (g_alinkgw_enable && ret == FALSE && sta->suppl.state == WPA_PTKINITNEGOTIATING) 
	{
	     if(gTpskDebug)
	     	show_tpsklist();
            memset(eabuf, 0, sizeof(eabuf));
            ether_etoa((unsigned char *)&sta->ea, eabuf);
            //printf("In %s:%d, sta.mac = %s, state = %d\n", __FILE__, __LINE__, eabuf, sta->suppl.state);
		//查找永久tpsklist
		
		for(entry = alilink_tpsk_list; entry != NULL; entry = entry->next)
		{
			//printf("In %s:%d, entry.mac = %s entry.tpsk = %s\n", __FILE__, __LINE__, entry->mac,entry->tpsk);
			if (strlen(entry->mac) > 0 && !strncmp(entry->mac, eabuf, STR_MAC_LEN))
			{
				find = 1;
				isTpskList = 1;
				strncpy(tpsk, entry->tpsk, sizeof(tpsk));
				if(gTpskDebug)
					printf("find tpsk from list,tpsk is %s\n",entry->tpsk);
				break;
			}
		}
		
use_tmp_tpsk:	             
		//永久list 没有表项，以临时tpsk 计算PMK
		if(!find)
		{
			if(strlen(alilink_tmp_tpsk) > 0)  //证明阿里云下发了临时TPSK
			{       
				strncpy(tpsk, alilink_tmp_tpsk, sizeof(tpsk));
				find = 1;
				isTmpTpsk = 1;
				 if(gTpskDebug)
					printf("use alilink_tmp_tpsk:%s\n",alilink_tmp_tpsk);
			}
		}
		
            // printf("In %s:%d, sta->nas->ssid = %s, tpsk = %s find:%d\n", __FILE__, __LINE__, (uchar *)sta->nas->ssid, tpsk, find);
		...
		/*通过tpsk来计算PMK*/
		if (passhash(tpsk, strlen(tpsk), (uchar *)sta->nas->ssid,
					 strlen(sta->nas->ssid), output)) {
			 if(gTpskDebug)
			 	printf("In %s:%d, passhash false goto done\n", __FILE__, __LINE__);
			//ret = FALSE;
			goto done;
		}
		else {
		    old_pmk_len = sta->suppl.pmk_len;
                    memcpy(old_pmk, sta->suppl.pmk, PMK_LEN);
			sta->suppl.pmk_len = PMK_LEN;
			memcpy(sta->suppl.pmk, output, PMK_LEN);
		}
              
		wpa_calc_ptk(sta->nas->wpa, sta);
		...
		/* compare against the MIC contents */
         ...
         //       printf("In %s:%d, bcmp(digest, mic) ture\n", __FILE__, __LINE__);
                ...
                ret = TRUE;
                //由临时tpsk认证成功需上报所有永久tpsklist
                if(isTmpTpsk)
                {
                	if(gTpskDebug)
                    		printf("use tmp tpsk check mic ok ,need to add to list\n");
                    if (create_and_add_to_list(tpsk, eabuf, 0, 1))//测试实际上报时间是否影响用户接入
                    {
                        g_needReportTpskList = 1;
			   untimeout((timeout_fun *)clean_tmp_tpsk,NULL);
			   if(gTpskDebug)
			  	 printf("sta used tmp tpsk, now clean tmp tpsk...\n ");
                        clean_tmp_tpsk();     
                    }
		...
		{
			 if(gTpskDebug)
				 printf("use tpsklist check mic ok !\n");
		}
            }
            else 
            {
            	  if(gTpskDebug)
                	printf("check mic err.\n");
		  if(isTpskList == 1 && isTmpTpsk == 0)
		  {
		  	find = 0;
			goto use_tmp_tpsk;
		  }
                sta->suppl.pmk_len = old_pmk_len;
                memcpy(sta->suppl.pmk, old_pmk, PMK_LEN);
                wpa_calc_ptk(sta->nas->wpa, sta);
            }	
	}
...
#endif
       //printf("In %s:%d, result = %s\n", __FILE__, __LINE__, (ret ? "match" : "fail"));
	return ret;
// 增加阿里tpsk 认证过程    2015-02-12
...
wpa_send_eapol(wpa_t *wpa, nas_sta_t *sta)
{
	...
	unsigned char mic[SHA1HashSize] = {0};	/* size of largest digest */
...
#ifdef __CONFIG_ALINKGW__
extern int access_attack_report(const char *mac);
#endif
static int
...
#ifdef __CONFIG_ALINKGW__			
		if( g_alinkgw_enable && sta->retries == 0)//多次check mic错误只上报一次
		{
			access_attack_report(ether_etoa((uchar *)&sta->ea, eabuf));
		}
#endif
		return 0;
	}
