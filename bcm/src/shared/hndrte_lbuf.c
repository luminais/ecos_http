 ...
 *
 * $Id: hndrte_lbuf.c,v 1.48.8.1 2010-08-17 17:54:19 Exp $
 */
...
#ifdef BCMPKTPOOL
		/* do not free the orig, if it is from pktpool */
		if (!lb_pool(orig))
#endif
		{
			lb_free_one(orig);
...
lb_resetpool(struct lbuf *lb, uint16 len)
{
	...
	lb->len = len;
	lb->data = lb->end - ROUNDUP(len, sizeof(int));
	lb_setpool(lb, TRUE, pool);
}
