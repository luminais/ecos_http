 ...
 *
 * $Id: hndrte.c,v 1.234.2.7 2011-01-27 17:03:39 Exp $
 */
...
extern void traptest(uint32, uint, char *[]);
...
#define dbg_throttle()	1
#else
#define dbg_throttle()	0
#endif
...
hndrte_print_memuse(void)
{
	...
	{
		int n, m;
		int tot, tot_overhead;

		n = pktpool_len(&pktpool_shared);
		m = pktpool_avail(&pktpool_shared);
		tot = pktpool_plen(&pktpool_shared) * n;
		tot_overhead = (pktpool_plen(&pktpool_shared) + LBUFSZ) * n;
		...
		printf("In use - pool: %d(%dK), w/oh: %d(%dK)\n",
			inuse_size - tot, (inuse_size - tot)/1024,
			(inuse_size + inuse_overhead) - tot_overhead,
			((inuse_size + inuse_overhead) - tot_overhead)/1024);
	}
#endif /* BCMPKTPOOL */
	}
#ifdef HNDRTE_PT_GIANT
...
BCMATTACHFN(hndrte_pktpool_init)(void)
{
	 ...
	 */
	n = 1;
	pktpool_init(hndrte_osh, &pktpool_shared, &n, PKTBUFSZ, FALSE);
...
#endif
#ifdef BCMTRAPTEST
	hndrte_cons_addcmd("tr", traptest, 0);
#endif
...
{
	uintptr addr = (uintptr)__builtin_return_address(0);

	printf("ASSERT in file %s line %d at 0x%x\n", file, line, addr);
...
extern uint8 _overlayerrfn_start[];
extern uint8 _r0overlaymem_start[];
extern uint8 _r0overlaymem_end[];
extern uint8 _r1overlaymem_start[];
extern uint8 _r1overlaymem_end[];
...
#define OVERLAY_REGION_INUSE		1
#define OVERLAY_REGION_RESERVED	2

overlay_region_t overlay_region[] = {
	{ _r0overlaymem_start, _r0overlaymem_end, 0, 0 },
	{ _r1overlaymem_start, _r1overlaymem_end, 0, 0 }
};

/*
 * copy len bytes in overlay param to an offset within the overlay region
 */
int
hndrte_overlay_copy(uint32 overlay_idx, uint8 *overlay, int offset, int len)
{

	if (overlay_idx > ((sizeof(overlay_region) / sizeof(overlay_region_t)) - 1))
		return BCME_RANGE;

	if (offset + len > (uint32) overlay_region[overlay_idx].end)
		return BCME_BADLEN;

	bcopy(overlay, overlay_region[overlay_idx].start + offset, len);

	return BCME_OK;
}

/* 
 * fill overlay region with nop's so if a caller calls a nonresident overlay fn
 * execution will fall through the nop's and reach the branch instruction at the 
 * end of the overlay region, which, when called, prints a msg and asserts
 */
/* #ifdef BCMDBG_ASSERT so it is not included in release builds ?? */
int
hndrte_overlay_invalidate(uint32 overlay_idx)
{
	uint32 *curr;
	...
	curr = (uint32*)overlay_region[overlay_idx].start;

	/* fill with nop's, except for the last 4 bytes */
	while (curr < (uint32*)overlay_region[overlay_idx].end - 1)
		*curr++ = NOP_UINT32;

	/* last 4 bytes is a branch instr to the nonresident_overlay_access() routine */
	*curr = overlay_region[overlay_idx].branch_instr;
	return BCME_OK;
}

/* compute branch instructions to nonresident_overlay_access(); invalidate overlay sections */
void
BCMATTACHFN(hndrte_overlay_prep)(void)
{
	int region_ct = (sizeof(overlay_region) / sizeof(overlay_region_t));
	int i;

	for (i = 0; i < region_ct; ++i) {
		/* linker script calculations may not result in proper alignment */
		if ((uint32)(overlay_region[i].start) & 0x3)
			overlay_region[i].start =
			    (uint8*) ROUNDUP((uint32)(overlay_region[i].start), 4);
		if ((uint32)(overlay_region[i].end) & 0x3)
			overlay_region[i].end =
			    (uint8*)ROUNDUP((uint32)(overlay_region[i].end), 4);

		/* compute the branch instruction from end-of-overlay-region to the error routine */
		overlay_region[i].branch_instr =
		    hndrte_overlayerrfn_branch_instr((int32)overlay_region[i].end,
		                                     (int32)_overlayerrfn_start);
		hndrte_overlay_invalidate(i);
	}
}

int nonresident_overlay_access(void);
/* invoked when there is a call into an overlay region and the overlay code is not present */ 
int
BCMOVERLAYERRFN(nonresident_overlay_access)(void)
{
	printf("ASSERT: call to nonresident overlay from 0x%x!\n", __builtin_return_address(0));
	hndrte_die(__LINE__);
	return BCME_NONRESIDENT;
}
