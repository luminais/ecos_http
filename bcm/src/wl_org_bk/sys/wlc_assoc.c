 ...
 *
 * $Id: wlc_assoc.c,v 1.126.2.85 2011-02-08 00:07:28 Exp $
 */
	...
	wl_join_assoc_params_t *assoc_params, int assoc_params_len)
{
	...
	wlc_join_start(bsscfg, scan_params, assoc_params);
}
	...
	const struct ether_addr *bssid, const chanspec_t *chanspec_list, int channel_num)
{
	...
	if (ASSOC_CACHE_ASSIST_ENAB(wlc) && wlc->scan_results->count > 0) {
		...
		err = wlc_scan(wlc->scan, bss_type, bssid, 1, &ssid, scan_type, nprobes,
			(as->flags & AS_F_SPEEDY_RECREATE) ? WLC_SCAN_RECREATE_TIME : active_time,
			passive_time, home_time, chanspec_list, channel_num, 0, TRUE,
...
wlc_assoc_scan_complete(void *arg, int status, wlc_bsscfg_t *dummy)
{
	...
	/* SAM 11.10 */
			//if (status != WLC_E_STATUS_ABORT)
				WL_ERROR(("wl%d: WLC_E_STATUS_SUPPRESS\n", WLCWLUNIT(wlc)));
...
wlc_join_wsec_filter(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
	...
	if ((cfg->wsec & MFP_REQUIRED) && !(bi->wpa2.flags & RSN_FLAGS_MFPC)) {
		/* We're configured for MFP mandatory, but peer doesn't have MFP enabled */
	...
	if ((bi->wpa2.flags & RSN_FLAGS_MFPR) && !(cfg->wsec & MFP_CAPABLE)) {
		/* Peer is configured for for MFP mandatory, but we don't have MFP enabled */
...
wlc_join_attempt(wlc_bsscfg_t *cfg)
{
	...
	for (; wlc->join_targets_last > 0; wlc->join_targets_last--) {
		...
		if (bc != NULL) {
			WL_ASSOC(("wl%d.%d: JOIN: Skipping BSS %s, associated by bsscfg %d\n",
			          WLCWLUNIT(wlc), WLC_BSSCFG_IDX(cfg),
			          bcm_ether_ntoa(&bi->BSSID, eabuf), WLC_BSSCFG_IDX(bc)));
			continue;
		}
...
#ifdef BCMWAPI_WPI
			wsec |= cfg->wsec &
				~(AES_ENABLED | TKIP_ENABLED | WEP_ENABLED | SMS4_ENABLED);
#else
			wsec |= cfg->wsec & ~(AES_ENABLED | TKIP_ENABLED | WEP_ENABLED);
#endif /* BCMWAPI_WPI */
...
wlc_join_check_ap_need_csa(wlc_info_t *wlc, wlc_bsscfg_t *cfg, chanspec_t chanspec, uint state)
{
	...
	if ((as->state != state) &&
		...
		/* reenable the ap temporarily */
		wlc_stop_ap(wlc, FALSE, cfg, -1);
		if (wlc_join_ap_do_csa(wlc, chanspec)) {
			...
			/* stop the ap */
			WL_ASSOC(("wl%d.%d: %s: not doing ap CSA, no up ap on curr channel\n",
			          WLCWLUNIT(wlc), WLC_BSSCFG_IDX(cfg), __FUNCTION__));
			wlc_stop_ap(wlc, TRUE, cfg, -1);
		}
	}
...
wlc_join_ap_do_csa(wlc_info_t *wlc, chanspec_t tgt_chanspec)
{
	...
	FOREACH_UP_AP(wlc, apidx, apcfg) {
		...
		if (WLC_BAND_PI_RADIO_CHANSPEC == apcfg->current_bss->chanspec) {
			wlc_csa_t *csa = apcfg->csa;

			/* Stop the current queue with flow control */
			wlc_txflowcontrol_override(wlc, apcfg->wlcif->qi,
			                           ON, TXQ_STOP_FOR_PKT_DRAIN);
			csa->csa.mode = DOT11_CSA_MODE_ADVISORY;
			csa->csa.chspec = tgt_chanspec;
			csa->csa.reg = wlc_get_regclass(wlc->cmi, tgt_chanspec);
			csa->csa.count = apcfg->current_bss->dtim_period + 1;
			wlc_ap_apply_channel_switch(apcfg);
			ap_do_csa = TRUE;
	...
	if (bi->infra == 0) {
		...
		if (BSS_P2P_ENAB(wlc, cfg))
			wlc_rateset_filter(&target_bss->rateset, &scb->rateset, FALSE,
			                   WLC_RATES_OFDM, RATE_MASK, bss_ht);
		else
#endif
		wlc_rateset_filter(&target_bss->rateset, &scb->rateset, FALSE,
		...
		if (!(scb->state & (PENDING_AUTH | PENDING_ASSOC))) {
			...
			if (WL_P2P_ON()) {
				int bss = wlc_p2p_d11cb_idx(wlc->p2p, cfg);
				uint16 state = wlc_p2p_read_shm(wlc->p2p, M_P2P_BSS_ST(bss));
				uint16 next_noa = wlc_p2p_read_shm(wlc->p2p, M_P2P_BSS_N_NOA(bss));
				uint16 hps = wlc_p2p_read_shm(wlc->p2p, M_P2P_HPS);
...
wlc_assoc_recreate_timeout(wlc_bsscfg_t *cfg)
{
	...
	wlc_set_dtim_programmed(cfg, FALSE);
...
wlc_join_recreate(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
	...
	if (as->type == AS_RECREATE && as->state == AS_LOSS_ASSOC) {
		...
		wlc_set_dtim_programmed(bsscfg, FALSE);
...
{
	uint wake_up_tbtt_in_bcns = 1;
	uint roam_time_thresh, roam_time_left, uatbtt_time_thresh, bcn_per;
	wlc_info_t *wlc = cfg->wlc;
	wlc_roam_t *roam = cfg->roam;
	wlc_bss_info_t *current_bss = cfg->current_bss;
	wlc_pm_st_t *pm = cfg->pm;

	/* Calculates (max) tbtt thresholds for unaligned bcns and roaming. */
	/* determine the max wakeinterval, based on the assumption dongle sleeps */
	if (pm->PM != PM_OFF && cfg->dtim_programmed) {

		if (wlc->bcn_li_dtim || (wlc->bcn_li_bcn == 0)) {
			if (wlc->bcn_li_dtim)
				wake_up_tbtt_in_bcns = wlc->bcn_li_dtim;
			else
#ifdef WLP2P
			/* P2P ucode generate TBTT interrupt for every beacon interval */
			if (!P2P_ENAB(wlc->pub))
#endif
				wake_up_tbtt_in_bcns *= current_bss->dtim_period;
		}
		else
			wake_up_tbtt_in_bcns =  wlc->bcn_li_bcn;
	}

	/*
	 * if not set roam_time_thresh defaults to a min of 2 sec or half of the bcn_timeout
	 * to make sure linkdown doesn't happen even before roam starts in normal case
	 */

	roam_time_thresh = wlc->pub->roam_time_thresh;

	if (roam_time_thresh == 0) {
		/* bcntime out should be nonzero */
		ASSERT(roam->bcn_timeout);
		roam_time_thresh = MIN(2000, (roam->bcn_timeout * 1000)/2);
	}

	bcn_per = current_bss->beacon_period;
	if (!bcn_per)
		bcn_per = 100;

	if (!wake_up_tbtt_in_bcns) {
		ASSERT(wake_up_tbtt_in_bcns != 0);
		/* This is to prevent divide by zero below */
		wake_up_tbtt_in_bcns = 1;
	}

	/* at 3/5th of the total roam threhsold time, check for unaligned bcn case */
	uatbtt_time_thresh = (roam_time_thresh * 3) / 5;

	roam->uatbtt_tbtt_thresh = (uint8) (uatbtt_time_thresh / (bcn_per * wake_up_tbtt_in_bcns));

	if (!roam->uatbtt_tbtt_thresh)
		roam->uatbtt_tbtt_thresh = 1;

	/* calculate the remaining time account for rounding of the uatbtt */
	roam_time_left = roam_time_thresh -
	                  (roam->uatbtt_tbtt_thresh * bcn_per * wake_up_tbtt_in_bcns);
	roam->tbtt_thresh =  (uint8) ((roam_time_left / bcn_per) + roam->uatbtt_tbtt_thresh);

	WL_INFORM(("wl%d: wlc_roam_thresh_upd: roam_time_thresh %d, "
	          "dtim_prog %d, >PM %d, uatbtt_thresh %d,roam_thresh %d\n", WLCWLUNIT(wlc),
	          wlc->pub->roam_time_thresh, cfg->dtim_programmed,
	          pm->PM, roam->uatbtt_tbtt_thresh, roam->tbtt_thresh));
}
...
*/
static bool
wlc_adopt_cie_check(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, bool is_normal_ap)
{
	if (wlc->awaiting_cntry_info ||
	    is_normal_ap ||
	    !wlc->autocountry_adopted_from_ap) {
		return TRUE;
	}
	return FALSE;
}
	...
	struct dot11_bcn_prb *bcn, int bcn_len)
{
	...
	if (target_bss->infra == 1) {
		...
		if (tim) {
			...
			wlc_set_dtim_programmed(cfg, TRUE);
		}
	}
	...
	if ((BSS_WL11H_ENAB(wlc, cfg) || WLC_AUTOCOUNTRY_ENAB(wlc)) &&
		...
		bool is_normal_ap = TRUE;

		country_ie = bcm_parse_tlvs(tlvs, tlvs_len, DOT11_MNG_COUNTRY_ID);
#ifdef WLP2P
		is_normal_ap = (bcm_find_p2pie(tlvs, tlvs_len) == NULL);
#endif

		WL_REGULATORY(("wl%d.%d: %s: is_normal_ap = %d, country_ie %s\n",
		               wlc->pub->unit, WLC_BSSCFG_IDX(cfg), __FUNCTION__,
		               is_normal_ap, ((country_ie) ? "PRESENT" : "ABSENT")));

		if (country_ie) {
			if (wlc_adopt_cie_check(wlc, cfg, is_normal_ap)) {
				WL_REGULATORY(("wl%d: %s: Adopting Country IE from associated AP\n",
					wlc->pub->unit, __FUNCTION__));
				if (wlc_adopt_country_ie(wlc, country_ie) && is_normal_ap) {
					wlc->autocountry_adopted_from_ap = TRUE;
				}
			}
		}
	}
...
#ifdef MFP
	if (scb) {
		/* when we reach here, checking for all of the invalid combinnations are done */
		scb->flags2 &= ~SCB2_MFP;
		if ((cfg->wsec & MFP_CAPABLE) && (target_bss->wpa2.flags & RSN_FLAGS_MFPC)) {
			scb->flags2 |= SCB2_MFP;
			WL_ASSOC(("wl%d: %s: turn MFP on\n",
			               wlc->pub->unit, __FUNCTION__));
		}
	}
#endif
	...
	uint body_len, struct scb *scb)
{
...
#ifdef WLP2P
	if (BSS_P2P_ENAB(wlc, cfg)) {
		wlc_p2p_process_assocresp(wlc->p2p, scb, (uint8 *)&assoc[1],
	...
	if (BSS_WME_ENAB(wlc, cfg)) {
		...
		if (wme_ie) {
			...
			} else if (BSSCFG_STA(cfg) && WME_AUTO(wlc) &&
			           (scb->flags & SCB_BRCM)) {
	...
	if ((MBSS_ENAB(wlc->pub) || cfg != wlc->cfg) &&
	...
	uint body_len)
{
	...
	if (cfg == NULL) {
		WL_ASSOC(("wl%d: %s: FT_RESP: no bsscfg for BSS\n", WLCWLUNIT(wlc), __FUNCTION__));
	...
	if ((ft_resp->action != DOT11_FT_ACTION_FT_RES) ||
	    ...
	    !(scb = wlc_scblookup(wlc, (struct ether_addr *) &target_bss->BSSID))) {
...
#if defined(BCMSUP_PSK)
	if (!wlc_sup_ft_authresp(cfg->sup, (uint8 *)&ft_resp[1],
...
wlc_disassociate_client(wlc_bsscfg_t *cfg, bool send_disassociate, pkcb_fn_t fn, void *arg)
{
	...
	if (P2P_ENAB(wlc->pub) && cfg->BSS)
		wlc_p2p_assoc_upd(wlc->p2p, cfg, FALSE);
#endif
				...
				if (fn != NULL) {
					...
					(current_bss->chanspec == WLC_BAND_PI_RADIO_CHANSPEC)) {
						...
						if (wlc_pkt_callback_register(wlc,
							...
							WL_ERROR(("wl%d: wlc_disassociate_client"
							"out of pkt callbacks\n", wlc->pub->unit));
						} else {
							mute_mode = OFF;
						}
				} else {
	...
	if (!cfg->associated) {
		...
		cfg->prot_cfg->shortpreamble = FALSE;
	}
	...
	uint auth_status, uint auth_type)
{
	...
	/* Clear pending bits */
	scb = addr? wlc_scbfind(wlc, addr): NULL;
	if (scb)
		wlc_scb_clearstatebit(scb, PENDING_AUTH | PENDING_ASSOC);
...
wlc_mac_request_entry(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int req)
{
	...
	case WLC_ACTION_ACTFRAME:
		...
		*/
		...
		/* use the scan engine for Action Frame */
		if (err == BCME_OK) {
			wlc->scan->usage = SCAN_ENGINE_USAGE_AF;
		}
		break;
	...
	case WLC_ACTION_REASSOC:
		if (QUIET_STATE(wlc) & SILENCE) {
			...
			WL_INFORM(("e/iscan aborting due to another kind of scan request\n"));
			wlc_scan_abort(wlc->scan, WLC_E_STATUS_NEWSCAN);
		} else
#endif /* STA */
		if (SCAN_IN_PROGRESS(wlc->scan)) {
		...
		/* use the scan engine for ESCAN */
		if (err == BCME_OK) {
			if (req == WLC_ACTION_ESCAN)
				wlc->scan->usage = SCAN_ENGINE_USAGE_ESCAN;
			else
				wlc->scan->usage = SCAN_ENGINE_USAGE_NORM;
		}
