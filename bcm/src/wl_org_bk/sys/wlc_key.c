 ...
 *
 * $Id: wlc_key.c,v 1.168.8.17 2010-12-23 19:15:13 Exp $
 */
...
wlc_key_rotation_wapi_update(wlc_info_t *wlc, struct scb *scb, uint32 key_algo, uint32 key_id)
{
	...
	if ((key->algo == key_algo) && (key->algo == CRYPTO_ALGO_SMS4)) {
		if (key->id != key_id) {
			...
			if (scb->prev_key)
				wlc_key_delete(wlc, scb->bsscfg, scb->prev_key);
...
wlc_rcmta_add_bssid(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	if (cfg->rcmta_bssid_idx >= RCMTA_SIZE) {
		WL_ERROR(("wl%d.%d: %s: RCMTA entry not allocated\n",
		          wlc->pub->unit, WLC_BSSCFG_IDX(cfg), __FUNCTION__));
		return BCME_ERROR;
	}
...
wlc_rcmta_del_bssid(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	if (wlc->pub->up) {
		...
		wlc_p2p_d11ab_unset(wlc->p2p, cfg);
	}
	}
	else
#endif /* WLP2P */
...
wlc_key_scb_delete(wlc_info_t *wlc, struct scb *scb)
{
	...
	if (key) {
		...
		wlc_key_delete(wlc, scb->bsscfg, key);
	...
	uint indx, bool ismulti)
{
	...
	if (!ismulti && WSEC_SCB_KEY_VALID(scb)) {
		...
		if (key->algo == CRYPTO_ALGO_SMS4) {
			...
			} else if (scb->prev_key && (scb->prev_key->id == indx)) {
	...
	for (i = 0; i < hw_keys; i++) {
		...
		if (key) {
			...
			if (ETHER_ISNULLADDR(&key->ea)) {
				...
				cfg = scb ? SCB_BSSCFG(scb) : wlc->cfg;
			}
			wlc_key_hw_init(wlc, i, cfg);
		}
	}
}
...
wlc_key_update(wlc_info_t *wlc, int indx, wlc_bsscfg_t *bsscfg)
{
	...
	if (indx >= WLC_MAX_WSEC_HW_KEYS(wlc)) {
...
wlc_key_hw_init(wlc_info_t *wlc, int indx, wlc_bsscfg_t *bsscfg)
{
	...
	ASSERT(indx < WLC_MAX_WSEC_HW_KEYS(wlc));
	...
	if (skl_index < M_SECKINDXALGO_BLK_SZ) {
		WL_WSEC(("wl%d.%d: %s: write wsec key skl_index %d, v = %d\n",
		         wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg), __FUNCTION__,
		         skl_index, v));
	...
	if (key) {
		...
		if (WSEC_HW_TKMIC_KEY(wlc, key, bsscfg)) {
			...
			ASSERT(wlc->hw_wapi_capable || wlc->hw_wapi_capable);
