 ...
 *
 * $Id: wlc_phy_lcn.c 280942 2011-08-31 21:35:40Z luisgm $
 */
...
#define wlc_lcnphy_iqcal_active(pi)	\
	(phy_reg_read((pi), LCNPHY_iqloCalCmd) & \
	(LCNPHY_iqloCalCmd_iqloCalCmd_MASK | LCNPHY_iqloCalCmd_iqloCalDFTCmd_MASK))

#define txpwrctrl_off(pi) (0x3 != ((phy_reg_read(pi, LCNPHY_TxPwrCtrlCmd) & 0xC000) >> 14))
...
#define wlc_lcnphy_tx_gain_override_enabled(pi) \
	(0 != (phy_reg_read((pi), LCNPHY_AfeCtrlOvr) & LCNPHY_AfeCtrlOvr_dacattctrl_ovr_MASK))
...
{  38, 5190, 0x09, 0x98, 0x99, 0x0D, 0x0F, 0x0F, 0xED, 0x0F },
{  42, 5210, 0x09, 0x87, 0x99, 0x0A, 0x0F, 0x0C, 0xDB, 0x0F },
{  46, 5230, 0x08, 0x87, 0x99, 0x0A, 0x0F, 0x0C, 0xDB, 0x0F },
{  36, 5180, 0x09, 0x98, 0x99, 0x0D, 0x0F, 0x0F, 0xED, 0x0F },
{  40, 5200, 0x09, 0x87, 0x99, 0x0A, 0x0F, 0x0C, 0xDB, 0x0F },
{  44, 5220, 0x08, 0x87, 0x99, 0x0A, 0x0F, 0x0C, 0xDB, 0x0F },
{  48, 5240, 0x08, 0x87, 0x99, 0x0A, 0x0F, 0x0C, 0xDB, 0x0F },
{  52, 5260, 0x08, 0x87, 0x99, 0x0A, 0x0F, 0x0C, 0xDB, 0x0F },
{  56, 5280, 0x08, 0x77, 0x99, 0x0A, 0x0F, 0x0C, 0xDB, 0x0E },
{  60, 5300, 0x07, 0x77, 0x99, 0x0A, 0x0F, 0x0C, 0xDB, 0x0E },
{  64, 5320, 0x07, 0x77, 0x99, 0x0A, 0x0F, 0x0C, 0xDB, 0x0E },
{ 100, 5500, 0x05, 0x66, 0xAA, 0x05, 0x08, 0x06, 0x75, 0x0B },
{ 104, 5520, 0x04, 0x66, 0xAA, 0x05, 0x08, 0x06, 0x75, 0x0B },
{ 108, 5540, 0x04, 0x66, 0xAA, 0x05, 0x08, 0x06, 0x75, 0x0B },
{ 112, 5560, 0x04, 0x66, 0xAA, 0x05, 0x08, 0x06, 0x75, 0x0A },
{ 116, 5580, 0x04, 0x66, 0xAA, 0x05, 0x08, 0x06, 0x75, 0x0A },
{ 120, 5600, 0x04, 0x66, 0xAA, 0x05, 0x08, 0x06, 0x75, 0x0A },
{ 124, 5620, 0x04, 0x55, 0xAA, 0x05, 0x08, 0x06, 0x75, 0x09 },
{ 128, 5640, 0x03, 0x55, 0xAA, 0x05, 0x08, 0x06, 0x75, 0x09 },
{ 132, 5660, 0x03, 0x55, 0xBB, 0x05, 0x08, 0x06, 0x75, 0x09 },
{ 136, 5680, 0x03, 0x55, 0xBB, 0x05, 0x08, 0x06, 0x75, 0x09 },
{ 140, 5700, 0x03, 0x55, 0xBB, 0x02, 0x04, 0x03, 0x32, 0x08 },
{ 149, 5745, 0x02, 0x44, 0xBB, 0x02, 0x04, 0x03, 0x32, 0x07 },
{ 153, 5765, 0x02, 0x44, 0xBB, 0x02, 0x04, 0x03, 0x32, 0x07 },
{ 157, 5785, 0x02, 0x44, 0xBB, 0x02, 0x04, 0x03, 0x32, 0x06 },
...
#define wlc_lcnphy_set_start_tx_pwr_idx(pi, idx) \
	PHY_REG_MOD(pi, LCNPHY, TxPwrCtrlCmd, pwrIndex_init, idx*2)
...
#define wlc_lcnphy_get_tx_pwr_npt(pi) \
	((phy_reg_read(pi, LCNPHY_TxPwrCtrlNnum) & \
...
#define wlc_lcnphy_get_current_tx_pwr_idx_if_pwrctrl_on(pi) \
	(phy_reg_read(pi, LCNPHY_TxPwrCtrlStatusExt) & 0x1ff)

#define wlc_lcnphy_get_target_tx_pwr(pi) \
	((phy_reg_read(pi, LCNPHY_TxPwrCtrlTargetPwr) & \
...
#define tempsense_done(pi) ((phy_reg_read(pi, LCNPHY_TxPwrCtrlStatusTemp) & 0x8000) == 0x8000)
...
static bool wlc_lcnphy_rx_iq_est(phy_info_t *pi, uint16 num_samps, uint8 wait_time,
	uint8 wait_for_crs, phy_iq_est_t *iq_est);
...
static void wlc_lcnphy_papd_idx_sel(phy_info_t *pi, uint32 amam_cmp, uint8 num_symbols,
	uint16 bbmult_step, lcnphy_txcalgains_t *txgains);
...
static void wlc_phy_cal_cache_dbg_lcnphy(ch_calcache_t *ctx);
#endif
...
{
	return phy_reg_read(pi, LCNPHY_TxPwrCtrlCmd) &
...
wlc_lcnphy_get_current_tx_pwr_idx(phy_info_t *pi)
{
	...
	if (txpwrctrl_off(pi))
		...
		indx = (int8)(wlc_lcnphy_get_current_tx_pwr_idx_if_pwrctrl_on(pi)/2);
	else
		indx = pi_lcn->lcnphy_current_index;
	return indx;
}
...
wlc_lcnphy_measure_digital_power(phy_info_t *pi, uint16 nsamples)
{
	...
	if (!wlc_lcnphy_rx_iq_est(pi, nsamples, 32, 0, &iq_est))
		return 0;
...
wlc_lcnphy_crsuprs(phy_info_t *pi, int channel)
{
	...
	afectrlovr = phy_reg_read(pi, LCNPHY_AfeCtrlOvr);
	afectrlovrval = phy_reg_read(pi, LCNPHY_AfeCtrlOvrVal);
...
wlc_lcnphy_toggle_afe_pwdn(phy_info_t *pi)
{
	...
	save_AfeCtrlOvrVal = phy_reg_read(pi, LCNPHY_AfeCtrlOvrVal);
	save_AfeCtrlOvr = phy_reg_read(pi, LCNPHY_AfeCtrlOvr);
...
static void
wlc_phy_papd_txiqlo_rxiq_enable(phy_info_t *pi, bool enable)
{
	phy_info_lcnphy_t *pi_lcn = pi->u.pi_lcnphy;
	...
	if (enable) {
		...
		if (ctx->valid) {
#else
		if (pi_lcn->lcnphy_full_cal_channel) {
#endif
...
wlc_phy_chanspec_set_lcnphy(phy_info_t *pi, chanspec_t chanspec)
{
	...
	uint32 freq = wlc_phy_channel2freq(channel);
	...
	if (wlc_phy_no_cal_possible(pi)) {
		...
		*/
		if (((pi_lcn->lcnphy_full_cal_channel > 14) &&
			...
			(CHSPEC_CHANNEL(pi->radio_chanspec) <= 14)))
			wlc_phy_papd_txiqlo_rxiq_enable(pi, 1);
		else
			wlc_phy_papd_txiqlo_rxiq_enable(pi, 0);
		return;
	}
	...
	if ((CHIPID(pi->sh->chip) == BCM4313_CHIP_ID) &&
	    pi_lcn->lcnphy_papd_cal_done_at_init && (!pi_lcn->ePA))	{
	...
	else if ((CHIPID(pi->sh->chip) == BCM4313_CHIP_ID) && (pi_lcn->ePA)) {
...
wlc_lcnphy_set_dac_gain(phy_info_t *pi, uint16 dac_gain)
{
	...
	dac_ctrl = (phy_reg_read(pi, LCNPHY_AfeDACCtrl) >> LCNPHY_AfeDACCtrl_dac_ctrl_SHIFT);
...
wlc_lcnphy_get_pa_gain(phy_info_t *pi)
{
	...
	pa_gain = (phy_reg_read(pi, LCNPHY_txgainctrlovrval1) &
...
wlc_lcnphy_set_tssi_mux(phy_info_t *pi, lcnphy_tssi_mode_t pos)
{
	...
	if (LCNPHY_TSSI_POST_PA == pos) {
		...
		if (RADIOID(pi->pubpi.radioid) == BCM2064_ID) {
			...
			write_radio_reg(pi, RADIO_2064_REG07F, 0);
		} else {
			mod_radio_reg(pi, RADIO_2064_REG086, 0x4, 0x4);
			write_radio_reg(pi, RADIO_2064_REG07D, 0);
		}
		if (CHIPID(pi->sh->chip) == BCM4313_CHIP_ID) {
...
wlc_lcnphy_pwrctrl_rssiparams(phy_info_t *pi)
{
	...
	/* for tempsense */
	if (RADIOID(pi->pubpi.radioid) != BCM2064_ID)
		mod_radio_reg(pi, RADIO_2064_REG082, (1 << 1), (1 << 1));
	else
		mod_radio_reg(pi, RADIO_2064_REG082, (1 << 5), (1 << 5));

	/* for vbat */
	if (RADIOID(pi->pubpi.radioid) != BCM2064_ID)
		mod_radio_reg(pi, RADIO_2064_REG07F, (1 << 4), (1 << 4));
	else
...
wlc_lcnphy_tssi_ucode_setup(phy_info_t *pi, uint8 update_lut)
{
	...
	uint16 phy_restore_LUT[26]; /* Max 13 Phy Regs supported by ucode */
	uint16 phy_setup_LUT[26];
	int phy_count = 0;

	uint16 radio_restore_LUT[30]; /* Max 15 Radio Regs supported by ucode */
	uint16 radio_setup_LUT[30];
	int radio_count = 0;
	int i;
	...
	/* Fill up LUT with setup values */
		...
		phy_reg_gen(pi, LCNPHY_TxPwrCtrlPwrIndex,
		...
		/* Power Up Tx Chain */
		/* Force on DAC */
		phy_reg_gen(pi, LCNPHY_AfeCtrlOvr,
		...
		phy_reg_gen(pi, LCNPHY_AfeCtrlOvrVal,
		...
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			/* Force on the Gband-ePA and force off the Aband-ePA */
				phy_reg_gen(pi, LCNPHY_RFOverride0,
			...
			phy_reg_gen(pi, LCNPHY_RFOverrideVal0,
			...
			phy_reg_gen(pi, LCNPHY_rfoverride3,
			...
			phy_reg_gen(pi, LCNPHY_rfoverride3_val,
		...
		gen_radio_reg(pi, RADIO_2064_REG007, 0x1, 1,
			&radio_restore_LUT[radio_count], &radio_restore_LUT[radio_count+1],
			&radio_setup_LUT[radio_count], &radio_setup_LUT[radio_count+1]);
		radio_count += 2;

		gen_radio_reg(pi, RADIO_2064_REG0FF, 0x10, 1 << 4,
			&radio_restore_LUT[radio_count], &radio_restore_LUT[radio_count+1],
			&radio_setup_LUT[radio_count], &radio_setup_LUT[radio_count+1]);
		radio_count += 2;


		gen_radio_reg(pi, RADIO_2064_REG11F, 0x4, 1 << 2,
			&radio_restore_LUT[radio_count], &radio_restore_LUT[radio_count+1],
			&radio_setup_LUT[radio_count], &radio_setup_LUT[radio_count+1]);
		radio_count += 2;
		...
		phy_reg_gen(pi, LCNPHY_rfoverride4,
		...
		phy_reg_gen(pi, LCNPHY_rfoverride4val,
		...
		if (RADIOID(pi->pubpi.radioid) != BCM2064_ID) {
			gen_radio_reg(pi, RADIO_2064_REG131, 1<<0, 1<<0,
				&radio_restore_LUT[radio_count], &radio_restore_LUT[radio_count+1],
				&radio_setup_LUT[radio_count], &radio_setup_LUT[radio_count+1]);
			radio_count += 2;
			...
			if (pi_lcn->ePA) {
				if (CHSPEC_IS2G(pi->radio_chanspec)) {
					gen_radio_reg(pi, RADIO_2064_REG07D, 0x3, 3,
					...
					gen_radio_reg(pi, RADIO_2064_REG07D, 0x3, 2,
						&radio_restore_LUT[radio_count],
						&radio_restore_LUT[radio_count+1],
						&radio_setup_LUT[radio_count],
						&radio_setup_LUT[radio_count+1]);
				...
				gen_radio_reg(pi, RADIO_2064_REG028, ((1<<5) | 0xf), ((1<<5) | 1),
					&radio_restore_LUT[radio_count],
					&radio_restore_LUT[radio_count+1],
					&radio_setup_LUT[radio_count],
					&radio_setup_LUT[radio_count+1]);
				...
				gen_radio_reg(pi, RADIO_2064_REG028,
			...
			gen_radio_reg(pi, RADIO_2064_REG03A, 0x3, 0x3,
				&radio_restore_LUT[radio_count], &radio_restore_LUT[radio_count+1],
				&radio_setup_LUT[radio_count], &radio_setup_LUT[radio_count+1]);
			radio_count += 2;

			gen_radio_reg(pi, RADIO_2064_REG11A, 0x1, 1 << 0,
				&radio_restore_LUT[radio_count], &radio_restore_LUT[radio_count+1],
				&radio_setup_LUT[radio_count], &radio_setup_LUT[radio_count+1]);
			radio_count += 2;

			/* Power off TX Mixer because of leakage seen in Uno3 */
			gen_radio_reg(pi, RADIO_2064_REG12E, 1 << 2, 1 << 2,
				&radio_restore_LUT[radio_count], &radio_restore_LUT[radio_count+1],
				&radio_setup_LUT[radio_count], &radio_setup_LUT[radio_count+1]);
			radio_count += 2;

			gen_radio_reg(pi, RADIO_2064_REG090, 0xff, 0,
				&radio_restore_LUT[radio_count], &radio_restore_LUT[radio_count+1],
				&radio_setup_LUT[radio_count], &radio_setup_LUT[radio_count+1]);
			radio_count += 2;
		}
		else {
			gen_radio_reg(pi, RADIO_2064_REG03A, 1, 0x1,
				&radio_restore_LUT[radio_count], &radio_restore_LUT[radio_count+1],
				&radio_setup_LUT[radio_count], &radio_setup_LUT[radio_count+1]);
			radio_count += 2;

			gen_radio_reg(pi, RADIO_2064_REG11A, 0x8, 0x8,
				&radio_restore_LUT[radio_count], &radio_restore_LUT[radio_count+1],
				&radio_setup_LUT[radio_count], &radio_setup_LUT[radio_count+1]);
			radio_count += 2;

			gen_radio_reg(pi, RADIO_2064_REG005, 0x8, 1 << 3,
				&radio_restore_LUT[radio_count], &radio_restore_LUT[radio_count+1],
				&radio_setup_LUT[radio_count], &radio_setup_LUT[radio_count+1]);
			radio_count += 2;

			gen_radio_reg(pi, RADIO_2064_REG03A, 0x4, 1 << 2,
				&radio_restore_LUT[radio_count], &radio_restore_LUT[radio_count+1],
				&radio_setup_LUT[radio_count], &radio_setup_LUT[radio_count+1]);
			radio_count += 2;
		...
		phy_reg_gen(pi, LCNPHY_TxPwrCtrlRfCtrlOverride0,
		...
		if (RADIOID(pi->pubpi.radioid) != BCM2064_ID) {
			gen_radio_reg(pi, RADIO_2064_REG082, (1 << 1), (1 << 1),
				&radio_restore_LUT[radio_count], &radio_restore_LUT[radio_count+1],
				&radio_setup_LUT[radio_count], &radio_setup_LUT[radio_count+1]);
			radio_count += 2;
		} else {
			gen_radio_reg(pi, RADIO_2064_REG082, (1 << 5), (1 << 5),
				&radio_restore_LUT[radio_count], &radio_restore_LUT[radio_count+1],
				&radio_setup_LUT[radio_count], &radio_setup_LUT[radio_count+1]);
			radio_count += 2;
		}
		...
		phy_reg_gen(pi, LCNPHY_TxPwrCtrlRfCtrlOverride1,
	...
	#if 0
		/* Force External TSSI on 4336B0 */
		/* set reg(RF_REG07F) 0x1 */
		/* set reg(RF_REG028.o_wrf_jtag_iqcaltssi_sel_sw) 0x1 */
		write_radio_reg(pi, RADIO_2064_REG07F, 0x1);
		mod_radio_reg(pi, RADIO_2064_REG028, 0x1e, 0x1 << 1);
	#endif
...
#elif LCN_BCK_TSSI_DEBUG_LUT_DIRECT
	{
		...
		for (i = 0; i < phy_count; i++) {
			phy_reg_write(pi, phy_setup_LUT[i], phy_setup_LUT[i+1]);
			i = i+1;
		}

		for (i = 0; i < radio_count; i++) {
			write_radio_reg(pi, radio_setup_LUT[i], radio_setup_LUT[i+1]);
			i = i+1;
		}
		...
		tempsenseval1 = phy_reg_read(pi, LCNPHY_TxPwrCtrlStatusTemp);
		tempsenseval2 = phy_reg_read(pi, LCNPHY_TxPwrCtrlStatusTemp1);
		vbat = phy_reg_read(pi, LCNPHY_TxPwrCtrlStatusVbat);
		...
		/* Get tempsense back */
		PHY_REG_MOD(pi, LCNPHY, TxPwrCtrlRfCtrlOverride0, amuxSelPortOverride, 0);
		PHY_REG_MOD(pi, LCNPHY, TxPwrCtrlRfCtrlOverride1, afeAuxpgaSelVmidOverride, 0);
		PHY_REG_MOD(pi, LCNPHY, TxPwrCtrlRfCtrlOverride1, afeAuxpgaSelGainOverride, 0);
		OSL_DELAY(3);
		PHY_REG_MOD(pi, LCNPHY, TxPwrCtrlRangeCmd, force_vbatTemp, 1);
		OSL_DELAY(6);
		tempsenseval1 = phy_reg_read(pi, LCNPHY_TxPwrCtrlStatusTemp);
		...
		for (i = 0; i < phy_count; i++) {
			phy_reg_write(pi, phy_restore_LUT[i], phy_restore_LUT[i+1]);
			i = i+1;
		}

		for (i = 0; i < radio_count; i++) {
			write_radio_reg(pi, radio_restore_LUT[i], radio_restore_LUT[i+1]);
			i = i+1;
		}
...
extern uint8
wlc_lcnphy_modify_max_txpower(phy_info_t *pi, uint8 maxtxpwr)
{
	phy_info_lcnphy_t *pi_lcn = pi->u.pi_lcnphy;
	if (!pi_lcn->ePA)
		maxtxpwr = (maxtxpwr > 4) ? (maxtxpwr - 4) : 0;
	else
...
{
	uint32 rate_table[WL_NUM_RATES_CCK];
	uint32 cck_rate_offset_iPA[4] = {14, 14, 14, 14};
	uint32 cck_rate_offset_ePA[4] = {6, 6, 6, 6};
	...
	for (i = 0; i < (int)ARRAYSIZE(rate_table); i++) {
		temp = (int32)(-pi->tx_power_offset[i]);
		temp += cck_rate_offset_ptr[i];
		pi->tx_power_offset[i] = (uint8)(-temp);
	}
}
...
wlc_lcnphy_set_tssi_pwr_limit(phy_info_t *pi, uint8 mode)
{
	...
	phy_info_lcnphy_t *pi_lcn = pi->u.pi_lcnphy;
	int32 lcnphy_tssi_maxpwr_limit = (1<<30)-1;
	int32 lcnphy_tssi_minpwr_limit = (1<<31);
	uint8 tssi_ladder_offset_maxpwr, tssi_ladder_offset_minpwr;

#if TWO_POWER_RANGE_TXPWR_CTRL
	int32 lcnphy_tssi_maxpwr_limit_second = (1<<30)-1;
	int32 lcnphy_tssi_minpwr_limit_second = (1<<31);
#endif
...
{
	uint32 cck_rate_offset[4] = {6, 6, 6, 6};
	uint32 ofdm_offset;
	int i;
	uint16 index2;
	phytbl_info_t tab;
...
wlc_lcnphy_tempcompensated_txpwrctrl(phy_info_t *pi, bool calc_cap_index)
{
	...
	if (pi->tx_power_min != 0) {
		delta_brd = (chMeasPower - pi->tx_power_min);
	} else {
		delta_brd = 0;
	}
...
wlc_lcnphy_set_tx_pwr_ctrl(phy_info_t *pi, uint16 mode)
{
	...
	if (old_mode != mode) {
		...
		phy_reg_mod(pi, LCNPHY_TxPwrCtrlCmd,
			(LCNPHY_TxPwrCtrlCmd_txPwrCtrl_en_MASK |
			LCNPHY_TxPwrCtrlCmd_hwtxPwrCtrl_en_MASK |
		...
		if (mode == LCNPHY_TX_PWR_CTRL_TEMPBASED) {
			...
			pi_lcn->lcnphy_current_index = (int8)
				((phy_reg_read(pi, LCNPHY_TxPwrCtrlBaseIndex) & 0xFF)/2);
		}

		if ((!pi_lcn->ePA) && (CHIPID(pi->sh->chip) == BCM4313_CHIP_ID)) {
...
wlc_lcnphy_cw_tx_pwr_ctrl(phy_info_t *pi, int32 targetpwr)
{
	...
	if (pi->phy_tx_tone_freq) {
		...
		OSL_DELAY(5);
		wlc_lcnphy_start_tx_tone(pi, (4000 * 1000), 88, 0);
	} else {
		wlc_lcnphy_start_tx_tone(pi, (4000 * 1000), 88, 0);
	}
	...
	while (1) {
		...
		if (((ABS(tone_pwr - targetpwr)) <= 1) || (count >= 10))
			break;
		count++;
	}
	wlc_lcnphy_stop_tx_tone(pi);
...
wlc_lcnphy_tone_power(phy_info_t *pi, bool tssi_setup)
{
	...
	*/
	wlc_phy_get_paparams_for_band(pi, &a1, &b0, &b1);
	estPower = wlc_lcnphy_tssi2dbm(tssi_idx, a1, b0, b1) >> 3;
...
wlc_lcnphy_tssi_loopback(phy_info_t *pi, uint16 *values_to_save)
{
	...
	for (i = 0; i < ARRAYSIZE(tone_power_rf_reg); i++) {
		values_to_save[i] = read_radio_reg(pi, tone_power_rf_reg[i]);
	}

	write_radio_reg(pi, RADIO_2064_REG007, 1);
	...
	if (RADIOID(pi->pubpi.radioid) == BCM2064_ID) {
		mod_radio_reg(pi, RADIO_2064_REG03A, 1, 1);
		mod_radio_reg(pi, RADIO_2064_REG03A, 4, 1<<2);
		mod_radio_reg(pi, RADIO_2064_REG11A, 8, 1<<3);
	} else {
		mod_radio_reg(pi, RADIO_2064_REG03A, 2, 1<<1);
		mod_radio_reg(pi, RADIO_2064_REG086, 4, 1<<2);
	}

	mod_radio_reg(pi, RADIO_2064_REG005, 8, 1<<3);
	mod_radio_reg(pi, RADIO_2064_REG11A, 1, 1);
...
wlc_lcnphy_tssi_loopback_cleanup(phy_info_t *pi, uint16 *values_to_save)
{
	...
	for (i = 0; i < ARRAYSIZE(tone_power_rf_reg); i++) {
		write_radio_reg(pi, tone_power_rf_reg[i], values_to_save[i]);
	}
	...
	bool keep_tone)
{
	...
	save_sslpnRxFeClkEnCtrl = phy_reg_read(pi, LCNPHY_sslpnRxFeClkEnCtrl);
	save_sslpnCalibClkEnCtrl = phy_reg_read(pi, LCNPHY_sslpnCalibClkEnCtrl);
	...
	save_txpwrctrlrfctrl2 = phy_reg_read(pi, LCNPHY_TxPwrCtrlRfCtrl2);
	...
	if ((CHIPID(pi->sh->chip) == BCM4313_CHIP_ID)) {
		...
		PHY_REG_WRITE(pi, LCNPHY, iqloCalCmdGctl, 0xaa4);
	}
...
wlc_lcnphy_idle_tssi_est(wlc_phy_t *ppi)
{
	...
	idleTssi = phy_reg_read(pi, LCNPHY_TxPwrCtrlStatus);
	suspend = !(R_REG(pi->sh->osh, &((phy_info_t *)pi)->regs->maccontrol) & MCTL_EN_MAC);
	if (!suspend)
		wlapi_suspend_mac_and_wait(pi->sh->physhim);
	wlc_lcnphy_set_tx_pwr_ctrl(pi, LCNPHY_TX_PWR_CTRL_OFF);
	...
	if (pi_lcn->lcnphy_twopwr_txpwrctrl_en) {
		...
		wlc_phy_do_dummy_tx(pi, TRUE, OFF);
		idleTssi1 = PHY_REG_READ(pi, LCNPHY, TxPwrCtrlStatusNew2, estPwr1);
		/* avgTssi value is in 2C (S9.0) format */
		idleTssi1_2C = PHY_REG_READ(pi, LCNPHY, TxPwrCtrlStatusNew4Ext, avgTssi1);
		/* Convert idletssi1_2C from 2C to OB format by toggling MSB OB value */
		/* ranges from 0 to (2^9-1) = 511, 2C value ranges from -256 to (2^9-1-2^8) = 255 */
		/* Convert 9-bit idletssi1_2C to 9-bit idletssi1_OB. */
		if (idleTssi1_2C >= 256)
...
wlc_lcnphy_idle_tssi_est_iovar(phy_info_t *pi, bool type)
{
	...
	return (phy_reg_read(pi, LCNPHY_TxPwrCtrlIdleTssi));
}
...
wlc_lcnphy_vbat_temp_sense_setup(phy_info_t *pi, uint8 mode)
{
	...
	for (i = 0; i < ARRAYSIZE(tempsense_phy_regs); i++)
		values_to_save[i] = phy_reg_read(pi, tempsense_phy_regs[i]);
	suspend = !(R_REG(pi->sh->osh, &pi->regs->maccontrol) & MCTL_EN_MAC);
	if (!suspend)
		wlapi_suspend_mac_and_wait(pi->sh->physhim);
	...
	for (i = 0; i < ARRAYSIZE(tempsense_phy_regs); i++)
		phy_reg_write(pi, tempsense_phy_regs[i], values_to_save[i]);
...
wlc_lcnphy_set_radio_loft(phy_info_t *pi,
	...
	uint8 fq0)
{
	write_radio_reg(pi, RADIO_REG_EI(pi), lcnphy_iqlocc_write(pi, ei0));
	write_radio_reg(pi, RADIO_REG_EQ(pi), lcnphy_iqlocc_write(pi, eq0));
	write_radio_reg(pi, RADIO_REG_FI(pi), lcnphy_iqlocc_write(pi, fi0));
	write_radio_reg(pi, RADIO_REG_FQ(pi), lcnphy_iqlocc_write(pi, fq0));
}
...
wlc_lcnphy_get_tx_gain(phy_info_t *pi,  lcnphy_txgains_t *gains)
{
	...
	dac_gain = phy_reg_read(pi, LCNPHY_AfeDACCtrl) >>
...
wlc_lcnphy_frmTypBasedIqLoCoeff(phy_info_t *pi)
{
	...
	*/
	if ((wlc_lcnphy_tx_dig_iir_dc_err_comp(pi) || pi_lcn->use_per_modulation_loft_cal) &&
		(pi_lcn->txgaintbl != 1) && (CHSPEC_IS2G(pi->radio_chanspec)))
		return TRUE;
	else
		return FALSE;
}
	...
	if (wlc_lcnphy_frmTypBasedIqLoCoeff(pi)) {
		...
		*/
		PHY_REG_MOD(pi, LCNPHY, dlo_coeff_di_cck, dlo_coeff_di_cck,
			(phy_reg_read(pi, LCNPHY_dlo_coeff_di_ofdm) & 0xff));
		PHY_REG_MOD(pi, LCNPHY, dlo_coeff_dq_cck, dlo_coeff_dq_cck,
			(phy_reg_read(pi, LCNPHY_dlo_coeff_dq_ofdm) & 0xff));
	}
...
wlc_lcnphy_stop_tx_tone(phy_info_t *pi)
{
	...
	playback_status = phy_reg_read(pi, LCNPHY_sampleStatus);
	...
	int8 iqlocalidxoffs)
{
	if (iqlocalidx != -1) {
		...
		if (iqlocalpwr == 0)
			txpwrindex = (int) wlc_lcnphy_cw_tx_pwr_ctrl(pi, (int32)pi->tx_power_min);
		if (iqlocalpwr != -1) {
...
wlc_lcnphy_txpwrtbl_iqlo_cal(phy_info_t *pi)
{
	...
	if (LCNREV_GE(pi->pubpi.phy_rev, 2))
		save_TxPwrCtrlRfCtrlOverride0 = phy_reg_read(pi, LCNPHY_TxPwrCtrlRfCtrlOverride0);
	...
	if (((CHIPID(pi->sh->chip) == BCM4313_CHIP_ID) && LCNREV_IS(pi->pubpi.phy_rev, 1)) ||
		...
		if (pi_lcn->ePA && (CHIPID(pi->sh->chip) != BCM4313_CHIP_ID)) {
		...
		else if (CHSPEC_IS2G(pi->radio_chanspec) && (pi_lcn->txgaintbl == 1)) {
			int8 tx_idx;
			...
			uint16 didq;
			/* OFDM coeffs */
			mod_radio_reg(pi, RADIO_2064_REG012, 0x1,
			PHY_REG_READ(pi, LCNPHY, lpfbwlutreg1, lpf_ofdm_tx_byp));
			wlc_lcnphy_tx_iqlo_cal(pi, NULL, (pi_lcn->lcnphy_recal ?
				LCNPHY_CAL_RECAL : LCNPHY_CAL_FULL), FALSE);
			didq = wlc_lcnphy_get_tx_locc(pi);
	...
	if (!CHSPEC_IS2G(pi->radio_chanspec) || (pi_lcn->txgaintbl != 1))
	...
	uint16 LPGN_Q)
{
	...
	*/
	...
	SPINWAIT(phy_reg_read(pi, LCNPHY_papd_control) & LCNPHY_papd_control_papd_cal_run_MASK,
		1 * 1000 * 1000);
}
...
wlc_lcnphy_temp_sense_vbatTemp_on(phy_info_t *pi, int mode)
{
	...
	save_afectrlovrval = phy_reg_read(pi, LCNPHY_AfeCtrlOvrVal);
	save_TxPwrCtrlRfCtrlOverride0 = phy_reg_read(pi, LCNPHY_TxPwrCtrlRfCtrlOverride0);
	save_TxPwrCtrlRfCtrlOverride1 = phy_reg_read(pi, LCNPHY_TxPwrCtrlRfCtrlOverride1);
	...
	mod_radio_reg(pi, RADIO_2064_REG007, 0x1, (1<<0));
	mod_radio_reg(pi, RADIO_2064_REG0FF, 0x10, (1<<4));
	mod_radio_reg(pi, RADIO_2064_REG11F, 0x4, (1<<2));
	...
	if (mode == TEMPSENSE) {
		tempsenseval1 = phy_reg_read(pi, LCNPHY_TxPwrCtrlStatusTemp) & 0x1FF;
		tempsenseval2 = phy_reg_read(pi, LCNPHY_TxPwrCtrlStatusTemp1) & 0x1FF;
		...
		vbat = phy_reg_read(pi, LCNPHY_TxPwrCtrlStatusVbat) & 0x1FF;
		if (vbat > 255)
			avg = (int16)(vbat - 512);
		else
			avg = (int16)vbat;
...
{
	uint16 tempsensereg1, tempsensereg2;
	uint16 tempsenseval1, tempsenseval2;
	int16 temp2, temp1;
	int16 avg = 0;
	...
	tempsensereg1 = phy_reg_read(pi, LCNPHY_TxPwrCtrlStatusTemp);
	tempsensereg2 = phy_reg_read(pi, LCNPHY_TxPwrCtrlStatusTemp1);

	tempsenseval1 = tempsensereg1 & 0x1FF;
	tempsenseval2 = tempsensereg2 & 0x1FF;
...
wlc_lcnphy_vbatsense(phy_info_t *pi, bool mode)
{
	...
	if (CHIPID(pi->sh->chip) == BCM4313_CHIP_ID) {
		...
		vbatsenseval = phy_reg_read(pi, LCNPHY_TxPwrCtrlStatusVbat) & 0x1FF;
		if (vbatsenseval > 255)
	...
	uint8 CurTxGain)
{
	...
	for (bsCnt = 0; bsCnt < bsDepth; bsCnt++) {
		...
		if (cal_type == PHY_PAPD_CAL_CW) {
			lgI = ((int16) phy_reg_read(pi, LCNPHY_papd_loop_gain_cw_i)) << 6;
			lgI = lgI >> 6;
			lgQ = ((int16) phy_reg_read(pi, LCNPHY_papd_loop_gain_cw_q)) << 6;
			lgQ = lgQ >> 6;

			cwLpGn2 = (lgI * lgI) + (lgQ * lgQ);

			PHY_PAPD(("LCNPHY_papd_loop_gain_cw_i %x LCNPHY_papd_loop_gain_cw_q %x\n",
				phy_reg_read(pi, LCNPHY_papd_loop_gain_cw_i),
				phy_reg_read(pi, LCNPHY_papd_loop_gain_cw_q)));
	...
	int8 *minUpdtIdx)
{
	...
	papd_lut_index_updt_63_48 = phy_reg_read(pi, LCNPHY_papd_lut_index_updated_63_48);
	papd_lut_index_updt_47_32 = phy_reg_read(pi, LCNPHY_papd_lut_index_updated_47_32);
	papd_lut_index_updt_31_16 = phy_reg_read(pi, LCNPHY_papd_lut_index_updated_31_16);
	papd_lut_index_updt_15_0  = phy_reg_read(pi, LCNPHY_papd_lut_index_updated_15_0);
...
wlc_lcnphy_save_papd_calibration_results(phy_info_t *pi)
{
	...
	cache->analog_gain_ref =
		phy_reg_read(pi, LCNPHY_papd_tx_analog_gain_ref);
	cache->lut_begin =
		phy_reg_read(pi, LCNPHY_papd_track_pa_lut_begin);
	cache->lut_step  =
		phy_reg_read(pi, LCNPHY_papd_track_pa_lut_step);
	cache->lut_end	 =
		phy_reg_read(pi, LCNPHY_papd_track_pa_lut_end);
	cache->rxcompdbm =
		phy_reg_read(pi, LCNPHY_papd_rx_gain_comp_dbm);
	cache->papdctrl  =
		phy_reg_read(pi, LCNPHY_papd_control);
	cache->sslpnCalibClkEnCtrl =
		phy_reg_read(pi, LCNPHY_sslpnCalibClkEnCtrl);
#else
	...
	uint16 bbmult_step)
{
	...
	/* Save lcnphy filt bw */
	lpfbwlut0 = phy_reg_read(pi, LCNPHY_lpfbwlutreg0);
	lpfbwlut1 = phy_reg_read(pi, LCNPHY_lpfbwlutreg1);
	...
	if ((CHIPID(pi->sh->chip) == BCM4313_CHIP_ID) && !pi_lcn->ePA) {
		PHY_REG_LIST_START
	...
	AphyControl_old = phy_reg_read(pi, LCNPHY_AphyControlAddr);
	Core1TxControl_old = phy_reg_read(pi, LCNPHY_Core1TxControl);
	lcnCtrl3_old = phy_reg_read(pi, LCNPHY_sslpnCtrl3);
	lcnCalibClkEnCtrl_old = phy_reg_read(pi, LCNPHY_sslpnCalibClkEnCtrl);
	...
	for (j = 0; j < ARRAYSIZE(savereg2g_addr); j++)
		savereg2g_data[j] = (uint8)read_radio_reg(pi, savereg2g_addr[j]);

#ifdef BAND5G
	if (CHSPEC_IS5G(pi->radio_chanspec)) {
		for (j = 0; j < ARRAYSIZE(savereg5g_addr); j++)
			savereg5g_data[j] = (uint8)read_radio_reg(pi, savereg5g_addr[j]);
	}
#endif
	...
	if (CHSPEC_IS2G(pi->radio_chanspec))
		...
		mod_radio_reg(pi, RADIO_2064_REG128, 0x8, (1<<3));
		mod_radio_reg(pi, RADIO_2064_REG0E7, 0x1, (0<<0));
	}
#endif
	...
	if (CHSPEC_IS2G(pi->radio_chanspec)) {
		...
		if (pi_lcn->lcnphy_papd_4336_mode) {
			if (CHIPID(pi->sh->chip) != BCM43362_CHIP_ID)
				...
				write_radio_reg(pi, RADIO_2064_REG097, 0xf0);
			else
				write_radio_reg(pi, RADIO_2064_REG097, 0xf9);
		}
		else
			/* atten for papd cal path */
			write_radio_reg(pi, RADIO_2064_REG097, 0xfa);
	} else {
#ifdef BAND5G
		/* rx chain pwr ups */
		write_radio_reg(pi, RADIO_2064_REG06A, 0x80);
		mod_radio_reg(pi, RADIO_2064_REG0D8, 0x80, (0<<7));
		mod_radio_reg(pi, RADIO_2064_REG0DA, 0x80, (0<<7));
		mod_radio_reg(pi, RADIO_2064_REG0DC, 0x1, (1<<0));
		mod_radio_reg(pi, RADIO_2064_REG127, 0x10, (1<<4));
		mod_radio_reg(pi, RADIO_2064_REG0E2, 0x1, (1<<0));
		mod_radio_reg(pi, RADIO_2064_REG127, 0x8, (1<<3));

		/* loopback: */
		write_radio_reg(pi, RADIO_2064_REG0ED, 0xc);
		mod_radio_reg(pi, RADIO_2064_REG128, 0x2, (1<<1));
		mod_radio_reg(pi, RADIO_2064_REG128, 0x1, (1<<0));

		/* don't force PA off */
		mod_radio_reg(pi, RADIO_2064_REG11A, 0x1, (0<<0));
#endif /* BAND5G */
	}
	...
	if (CHSPEC_IS2G(pi->radio_chanspec))
		write_radio_reg(pi, RADIO_2064_REG02E, 0x10); /* logen Rx path enable */
#ifdef BAND5G
	else
		mod_radio_reg(pi, RADIO_2064_REG004, 0x4, (1<<2));
#endif
...
#ifdef BAND5G
		if (CHSPEC_IS5G(pi->radio_chanspec)) {
			mod_radio_reg(pi, RADIO_2064_REG121, 0x8, (1<<3));
			mod_radio_reg(pi, RADIO_2064_REG0D7, 0x8, (0<<3));
		}
		else
#endif
		{
			mod_radio_reg(pi, RADIO_2064_REG120, 0x40, (1<<6));
		}
	}
	if (LCNREV_LT(pi->pubpi.phy_rev, 2))
	...
	for (j = 0; j < ARRAYSIZE(savereg2g_addr); j++)
		write_radio_reg(pi, savereg2g_addr[j], savereg2g_data[j]);

#ifdef BAND5G
	if (CHSPEC_IS5G(pi->radio_chanspec)) {
		for (j = 0; j < ARRAYSIZE(savereg5g_addr); j++)
			write_radio_reg(pi, savereg5g_addr[j], savereg5g_data[j]);
	}
#endif
	...
	phy_iq_est_t *iq_est)
{
	...
	PHY_REG_LIST_START
		PHY_REG_MOD_ENTRY(LCNPHY, IQEnableWaitTimeAddress, iqstart, 1)
	PHY_REG_LIST_EXECUTE(pi);

	/* Wait for IQ estimation to complete */
	while (phy_reg_read(pi, LCNPHY_IQEnableWaitTimeAddress) &
	...
	/* Save results */
	iq_est->iq_prod = ((uint32)phy_reg_read(pi, LCNPHY_IQAccHiAddress) << 16) |
		(uint32)phy_reg_read(pi, LCNPHY_IQAccLoAddress);
	iq_est->i_pwr = ((uint32)phy_reg_read(pi, LCNPHY_IQIPWRAccHiAddress) << 16) |
		(uint32)phy_reg_read(pi, LCNPHY_IQIPWRAccLoAddress);
	iq_est->q_pwr = ((uint32)phy_reg_read(pi, LCNPHY_IQQPWRAccHiAddress) << 16) |
		(uint32)phy_reg_read(pi, LCNPHY_IQQPWRAccLoAddress);
...
wlc_lcnphy_set_lna_freq_comp(phy_info_t *pi)
{
	...
	write_radio_reg(pi, RADIO_2064_REG0AD, lna_freq_tune);
}
	...
	uint8 wait_for_crs, phy_iq_est_t* est)
{
	...
	rf_override_2 = phy_reg_read(pi, LCNPHY_rfoverride2);
	lna_freq_tune = read_radio_reg(pi, RADIO_2064_REG0AD);
	...
	PHY_REG_MOD(pi, LCNPHY, rfoverride2val, hpf1_ctrl_ovr_val, 7);
	PHY_REG_MOD(pi, LCNPHY, rfoverride2val, hpf2_ctrl_ovr_val, 7);
	PHY_REG_MOD(pi, LCNPHY, rfoverride2val, lpf_lq_ovr_val, 0);
	PHY_REG_MOD(pi, LCNPHY, rfoverride2, hpf1_ctrl_ovr, 1);
	PHY_REG_MOD(pi, LCNPHY, rfoverride2, hpf2_ctrl_ovr, 1);
	PHY_REG_MOD(pi, LCNPHY, rfoverride2, lpf_lq_ovr, 1);
	...
	while (counter < retry_count)
	{
		...
		do {
			...
			for (count = 0; count < 4; count++) {
				(void)wlc_lcnphy_rx_iq_est(pi, num_samps, wait_time,
					wait_for_crs, est);
				i_pwr = i_pwr + est->i_pwr;
				q_pwr = q_pwr + est->q_pwr;
			}
	...
	wlc_lcnphy_rx_gain_override_enable(pi, FALSE);
	wlc_lcnphy_clear_trsw_override(pi);
	phy_reg_write(pi, LCNPHY_rfoverride2, rf_override_2);
	write_radio_reg(pi, RADIO_2064_REG0AD, lna_freq_tune);
...
#define LCNPHY_MIN_RXIQ_PWR 2
	...
	/* Save original c0 & c1 */
	a0_new = ((phy_reg_read(pi, LCNPHY_RxCompcoeffa0) & LCNPHY_RxCompcoeffa0_a0_MASK) >>
		LCNPHY_RxCompcoeffa0_a0_SHIFT);
	b0_new = ((phy_reg_read(pi, LCNPHY_RxCompcoeffb0) & LCNPHY_RxCompcoeffb0_b0_MASK) >>
	...
	if (!(result = wlc_lcnphy_rx_iq_est(pi, num_samps, 32, 0, &iq_est)))
		goto cleanup;
	...
	int module, int tx_gain_idx)
{
	...
	if (module == 1) {
		...
		RFOverride0_old = phy_reg_read(pi, LCNPHY_RFOverride0);
		RFOverrideVal0_old = phy_reg_read(pi, LCNPHY_RFOverrideVal0);
		rfoverride2_old = phy_reg_read(pi, LCNPHY_rfoverride2);
		rfoverride2val_old = phy_reg_read(pi, LCNPHY_rfoverride2val);
		rfoverride3_old = phy_reg_read(pi, LCNPHY_rfoverride3);
		rfoverride3val_old = phy_reg_read(pi, LCNPHY_rfoverride3_val);
		rfoverride4_old = phy_reg_read(pi, LCNPHY_rfoverride4);
		rfoverride4val_old = phy_reg_read(pi, LCNPHY_rfoverride4val);
		afectrlovr_old = phy_reg_read(pi, LCNPHY_AfeCtrlOvr);
		afectrlovrval_old = phy_reg_read(pi, LCNPHY_AfeCtrlOvrVal);
		old_sslpnCalibClkEnCtrl = phy_reg_read(pi, LCNPHY_sslpnCalibClkEnCtrl);
		old_sslpnRxFeClkEnCtrl = phy_reg_read(pi, LCNPHY_sslpnRxFeClkEnCtrl);
		old_rxlnaandgainctrl1ovrval = phy_reg_read(pi, LCNPHY_rxlnaandgainctrl1ovrval);
		...
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			...
			write_radio_reg(pi, RADIO_2064_REG0ED, 0x3);
			write_radio_reg(pi, RADIO_2064_REG0EC, (1 << 4));
		}
#endif
		...
		do {
		  ...
		  while ((lna2_gain >= 0) && !set_gain) {
			...
			if (!wlc_lcnphy_rx_iq_est(pi, k_lcnphy_rx_iq_est_n_samps, 32, 0, &iq_est_l))
			{
			...
			write_radio_reg(pi, RADIO_2064_REG112, 0);
			if (!wlc_lcnphy_rx_iq_est(pi, k_lcnphy_rx_iq_est_n_samps, 32, 0, &iq_est_h))
			{
		...
		PHY_REG_WRITE(pi, LCNPHY, RFOverride0, RFOverrideVal0_old);
...
cal_done:
	...
	PHY_INFORM(("wl%d: %s: Rx IQ cal complete, coeffs: A0: %d, B0: %d\n",
		pi->sh->unit, __FUNCTION__,
		(int16)((phy_reg_read(pi, LCNPHY_RxCompcoeffa0) & LCNPHY_RxCompcoeffa0_a0_MASK)
		>> LCNPHY_RxCompcoeffa0_a0_SHIFT),
		(int16)((phy_reg_read(pi, LCNPHY_RxCompcoeffb0) & LCNPHY_RxCompcoeffb0_b0_MASK)
		>> LCNPHY_RxCompcoeffb0_b0_SHIFT)));
	return result;
}
...
wlc_lcnphy_periodic_cal(phy_info_t *pi)
{
	...
	if (pi_lcn->lcnphy_papd_4336_mode && !pi_lcn->ePA) {
		...
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			...
			if (pi_lcn->pacalamamlim_2g != 0) pacalamamlim = pi_lcn->pacalamamlim_2g;
		}
		else {
			...
			if (pi_lcn->pacalamamlim_5g != 0) pacalamamlim = pi_lcn->pacalamamlim_5g;
		}
		/* Check if the idx or txpwr is passed via NVRAM */
		if (pacalidx != -1)
			txgains.index = (uint8)pacalidx;
		else {
			if (pacalpwr == 0)
				txgains.index =
				(uint8) wlc_lcnphy_cw_tx_pwr_ctrl(pi, (int32)pi->tx_power_min);
			if (pacalpwr != -1)
				txgains.index =
				(uint8) wlc_lcnphy_cw_tx_pwr_ctrl(pi, (int32)pacalpwr);
		}
		if (pacalamamlim != 0) {
			wlc_lcnphy_papd_idx_sel(pi, pacalamamlim, 6, 32767, &txgains);
			txgains.index = wlc_lcnphy_get_current_tx_pwr_idx(pi);
		}
		wlc_lcnphy_papd_cal(pi, PHY_PAPD_CAL_CW, &txgains, 0, 0, 0, 0, 219, 1, 0);
		wlc_lcnphy_set_tx_pwr_by_index(pi, indx);
	}
	...
	uint16 bbmult_step, lcnphy_txcalgains_t *txgains)
{
	...
	tab.tbl_len = 1;
	tab.tbl_offset = 63;
	amam = ((tmp_i*tmp_i) + (tmp_j*tmp_j));
	amam_cmp *= amam_cmp;
	wlc_lcnphy_papd_cal(pi, PHY_PAPD_CAL_CW, txgains, 0, 0, 0, 0, num_symbols, 1, bbmult_step);

	while (amam > amam_cmp) {
		tmp_tx_idx = wlc_lcnphy_get_current_tx_pwr_idx(pi);
		wlc_lcnphy_read_table(pi, &tab);
		tmp_i = (val & 0x00fff000) << 8;
		tmp_j = (val & 0x00000fff) << 20;
		tmp_i = tmp_i >> 20;
		tmp_j = tmp_j >> 20;
		amam = ((tmp_i*tmp_i) + (tmp_j*tmp_j));
		if (amam < amam_cmp) {
			...
			/* set gain index */
			tmp_tx_idx += 5;
			txgains->index = tmp_tx_idx;
			wlc_lcnphy_papd_cal(pi, PHY_PAPD_CAL_CW, txgains, 0, 0, 0, 0,
				num_symbols, 1, bbmult_step);
		}
	}
...
wlc_lcnphy_get_tssi(phy_info_t *pi, int8 *ofdm_pwr, int8 *cck_pwr)
{
	...
	*ofdm_pwr = 0;
	...
	if (wlc_lcnphy_tssi_based_pwr_ctrl_enabled(pi))	{
		if (phy_reg_read(pi, LCNPHY_TxPwrCtrlStatus)
			& LCNPHY_TxPwrCtrlStatus_estPwrValid_MASK)
			*ofdm_pwr =
				(int8)(PHY_REG_READ(pi, LCNPHY, TxPwrCtrlStatus, estPwr) >> 1);
		else if (phy_reg_read(pi, LCNPHY_TxPwrCtrlStatusNew2)
...
static gain_table_tweak_t lcnphy_5G_wlbga_low_temp_adj[] = {
	{ 54, 0xf0000000, 0x18, 15, 0x1186},
	{ 56,          0, 0x19, 16, 0x5186},
	{ 58, 0x10000000, 0x19, 17, 0x9186}
};

static gain_table_tweak_t lcnphy_5G_wlbga_dflt_adj[] = {
	{ 54, 0xf0000000, 0x18, 15, 0x0d47},
	{ 56,          0, 0x19, 16, 0x1147},
	{ 58, 0x10000000, 0x19, 17, 0x5147}
};

static void
wlc_lcnphy_apply_gain_table_tweaks(phy_info_t *pi, uint8 n, gain_table_tweak_t* p_tweaks)
{
	phytbl_info_t tab;
	uint8 i;
	uint32 data[2];
	...
	i = 0;
	while (i < n) {
	  data[0] = p_tweaks->gain_idx_lo;
	  data[1] = p_tweaks->gain_idx_hi;
	  tab.tbl_len = 2;
	  tab.tbl_id = LCNPHY_TBL_ID_GAIN_IDX;
	  tab.tbl_offset = p_tweaks->gain_idx_offset;
	  wlc_lcnphy_write_table(pi, &tab);

	  data[0] = p_tweaks->gain;
	  tab.tbl_len = 1;
	  tab.tbl_id = LCNPHY_TBL_ID_GAIN_TBL;
	  tab.tbl_offset = p_tweaks->gain_offset;
	  wlc_lcnphy_write_table(pi, &tab);
...
wlc_lcnphy_set_chanspec_tweaks(phy_info_t *pi, chanspec_t chanspec)
{
	...
	if (CHIPID(pi->sh->chip) == BCM4313_CHIP_ID) {
		 ...
		 * at low temperatures and high channels 
		 */
		if (CHSPEC_IS5G(pi->radio_chanspec) &&
		    !(BOARDFLAGS(GENERIC_PHY_INFO(pi)->boardflags) &  BFL_EXTLNA) &&
		    (pi->sh->chippkg == BCM4330_WLBGA_PKG_ID)) {
			if ((wlc_lcnphy_tempsense_degree(pi, 1) > -15) ||
			   (channel < 100))
				wlc_lcnphy_apply_gain_table_tweaks(pi,
				  sizeof(lcnphy_5G_wlbga_dflt_adj)/sizeof(gain_table_tweak_t),
				  lcnphy_5G_wlbga_dflt_adj);
			else
				wlc_lcnphy_apply_gain_table_tweaks(pi,
				  sizeof(lcnphy_5G_wlbga_low_temp_adj)/sizeof(gain_table_tweak_t),
				  lcnphy_5G_wlbga_low_temp_adj);
		}
#endif /* BAND5G */
	}
...
wlc_lcnphy_tx_iqlo_loopback(phy_info_t *pi, uint16 *values_to_save)
{
	...
	if (RADIOID(pi->pubpi.radioid) != BCM2064_ID) {
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			...
			or_radio_reg(pi, RADIO_2064_REG0C0, 1 << 0);
			or_radio_reg(pi, RADIO_2064_REG0CA, 1 << 0);
			wlc_lcnphy_set_pa_gain(pi, 0x10);
		}
	}
	else
		and_radio_reg(pi, RADIO_2064_REG03A, 0xFB);
	...
	if (RADIOID(pi->pubpi.radioid) == BCM2064_ID) {
		...
		or_radio_reg(pi, RADIO_2064_REG11A, 0x10);
	}
	else {
		or_radio_reg(pi, RADIO_2064_REG036, 0x01);
		or_radio_reg(pi, RADIO_2064_REG0C5, 0x01);
		or_radio_reg(pi, RADIO_2064_REG11A, 0x08);
	}
	OSL_DELAY(20);

	if (CHSPEC_IS2G(pi->radio_chanspec) && pi->u.pi_lcnphy->ePA &&
		(CHIPID(pi->sh->chip) != BCM4313_CHIP_ID)) {
		mod_radio_reg(pi, RADIO_2064_REG036, 1<<0, 0);		/* tap at PA output */
		mod_radio_reg(pi, RADIO_2064_REG03A, 1<<1, 1<<1);
...
#ifdef BAND5G
	else if (CHSPEC_IS5G(pi->radio_chanspec) && pi->u.pi_lcnphy->ePA) {
	...
	if (RADIOID(pi->pubpi.radioid) == BCM2064_ID) {
		...
		or_radio_reg(pi, RADIO_2064_REG0CA, 1 << 1);

	OSL_DELAY(20);

	if (CHSPEC_IS2G(pi->radio_chanspec)) {
		mod_radio_reg(pi, RADIO_2064_REG03A, 0x1, 1 << 0);
	} else {
		mod_radio_reg(pi, RADIO_2064_REG03A, 0x1, 0 << 0);
		mod_radio_reg(pi, RADIO_2064_REG0CA, 0x4, 1 << 2);
	}
	if (RADIOID(pi->pubpi.radioid) == BCM2064_ID)
		or_radio_reg(pi, RADIO_2064_REG11A, 0x8);
	/* iqcal block */
	write_radio_reg(pi, RADIO_2064_REG025, 0xE);
	or_radio_reg(pi, RADIO_2064_REG025, 1 << 0);
...
wlc_lcnphy_samp_cap(phy_info_t *pi, int clip_detect_algo, uint16 thresh, int16* ptr, int mode)
{
	...
	sslpnCalibClkEnCtrl = phy_reg_read(pi, LCNPHY_sslpnCalibClkEnCtrl);
...
wlc_lcnphy_set_cc(phy_info_t *pi, int cal_type, int16 coeff_x, int16 coeff_y)
{
	...
	switch (cal_type) {
		...
		case 3:
			...
			x = 8 - k;
			data_rf = (x * 16 + y);
			write_radio_reg(pi, RADIO_REG_EI(pi), data_rf);
			k = wlc_lcnphy_calc_floor(coeff_y, 0);
			y = 8 + k;
			k = wlc_lcnphy_calc_floor(coeff_y, 1);
			x = 8 - k;
			data_rf = (x * 16 + y);
			write_radio_reg(pi, RADIO_REG_EQ(pi), data_rf);
			break;
		case 4:
			k = wlc_lcnphy_calc_floor(coeff_x, 0);
			y = 8 + k;
			...
			x = 8 - k;
			data_rf = (x * 16 + y);
			write_radio_reg(pi, RADIO_REG_FI(pi), data_rf);
			k = wlc_lcnphy_calc_floor(coeff_y, 0);
			y = 8 + k;
			k = wlc_lcnphy_calc_floor(coeff_y, 1);
			x = 8 - k;
			data_rf = (x * 16 + y);
			write_radio_reg(pi, RADIO_REG_FQ(pi), data_rf);
			break;
	}
}
...
wlc_lcnphy_tx_iqlo_soft_cal(phy_info_t *pi, int cal_type, int num_levels, int step_size_lg2)
{
	...
	wlc_lcnphy_tx_iqlo_loopback(pi, values_to_save);
	OSL_DELAY(500);
	save_rfoverride4 = phy_reg_read(pi, LCNPHY_rfoverride4);
	save_txpwrctrlrfctrloverride0 = phy_reg_read(pi, LCNPHY_TxPwrCtrlRfCtrlOverride0);
	save_txpwrctrlrfctrloverride1 = phy_reg_read(pi, LCNPHY_TxPwrCtrlRfCtrlOverride1);
	...
	for (level = 0; grid_size != 0 && level < num_levels; level++) {
		...
		/* gain control */
		thresh = phy_reg_read(pi, LCNPHY_iqloCalGainThreshD2);
		thresh = 2*thresh;
		prev_clipped = 0;
...
wlc_lcnphy_tx_iqlo_loopback_cleanup(phy_info_t *pi, uint16 *values_to_save)
{
	...
	if (pi->u.pi_lcnphy->ePA && (CHIPID(pi->sh->chip) != BCM4313_CHIP_ID)) {
		wlc_lcnphy_epa_pd(pi, 0);
	}
	...
	const lcnphy_tx_gain_tbl_entry * gain_table)
{
	...
	for (j = 0; j < 128; j++) {
		...
		if (pi_lcn->lcnphy_papd_4336_mode && !pi_lcn->ePA &&
			CHSPEC_IS2G(pi->radio_chanspec) && (pi_lcn->txgaintbl != 1))
			gm_gain = 15;
		else if ((CHIPID(pi->sh->chip) == BCM4313_CHIP_ID) && !pi_lcn->ePA)
			gm_gain = 15;
		else if (pi_lcn->ePA &&	(CHSPEC_IS5G(pi->radio_chanspec)))
			gm_gain = 7;
		else
...
WLBANDINITFN(wlc_lcnphy_sw_ctrl_tbl_init)(phy_info_t *pi)
{
	...
	if ((CHIPID(pi->sh->chip) == BCM4330_CHIP_ID)) {
		...
		phy_reg_mod(pi, LCNPHY_swctrlconfig,
...
wlc_lcnphy_rx_gain_tbl_init(phy_info_t *pi)
{
	...
	if (LCNREV_GE(pi->pubpi.phy_rev, 2)) {
		...
		if (RADIOID(pi->pubpi.radioid) == BCM2064_ID) {
			/* BCM43362 */
			for (idx = 0; idx < dot11lcnphytbl_radio2064_rx_gain_info_sz_rev3; idx++)
			...
			if (CHSPEC_IS2G(pi->radio_chanspec)) {
			  for (idx = 0; idx < dot11lcnphytbl_2G_rx_gain_info_sz_rev2; idx++)
			    if (BOARDFLAGS(GENERIC_PHY_INFO(pi)->boardflags) &
			      BFL_EXTLNA) {
			        if (pi->sh->chippkg == BCM4330_WLBGA_PKG_ID) {
			          wlc_lcnphy_write_table(pi,
			            &dot11lcnphytbl_2G_ext_lna_aci_rx_gain_info_rev2[idx]);
			        } else {
			          wlc_lcnphy_write_table(pi,
				...
				if (pi->sh->chippkg == BCM4330_WLBGA_PKG_ID) {
				  wlc_lcnphy_write_table(pi,
				     &dot11lcnphytbl_2G_rx_gain_info_rev2[idx]);
				} else {
				  wlc_lcnphy_write_table(pi,
				     &dot11lcnphytbl_2G_aci_rx_gain_info_rev2[idx]);
				}
			     }
			}
#ifdef BAND5G
			else {
				for (idx = 0; idx < dot11lcnphytbl_5G_rx_gain_info_sz_rev2; idx++)
					if (BOARDFLAGS(GENERIC_PHY_INFO(pi)->boardflags) &
						BFL_EXTLNA_5GHz) {
						wlc_lcnphy_write_table(pi,
						&dot11lcnphytbl_5G_ext_lna_rx_gain_info_rev2[idx]);
					}
					else
						wlc_lcnphy_write_table(pi,
						&dot11lcnphytbl_5G_rx_gain_info_rev2[idx]);
			}
#endif /* BAND5G */
		}
	} else {
			...
			if (CHIPID(pi->sh->chip) == BCM4313_CHIP_ID) {
				if (!(BOARDFLAGS(GENERIC_PHY_INFO(pi)->boardflags) & BFL_FEM_BT) &&
					...
					for (idx = 0; idx <
						...
						idx++) {
						wlc_lcnphy_write_table(pi,
							&dot11lcnphytbl_2G_ext_lna_rx_gain_info_rev1
							[idx]);
					}
				}
			}
	...
	if ((CHIPID(pi->sh->chip) == BCM4313_CHIP_ID) && LCNREV_IS(pi->pubpi.phy_rev, 1)) {
		for (idx = 0; idx < dot11lcnphytbl_gain_tbl_info_sz_rev1; idx++) {
		}
	}
	#endif
}
...
WLBANDINITFN(wlc_lcnphy_tbl_init)(phy_info_t *pi)
{
	...
	if (CHSPEC_IS2G(pi->radio_chanspec)) {
		...
		if (pi_lcn->ePA)
			...
			if (pi_lcn->txgaintbl == 0)
				wlc_lcnphy_load_tx_gain_table(pi, dot11lcnphy_2GHz_gaintable_rev0);
			else
				wlc_lcnphy_load_tx_gain_table
				(pi, dot11lcnphy_2GHz_gaintable_1_rev0);
		}
	}
#ifdef BAND5G
	else {
...
void
wlc_phy_lcn_updatemac_rssi(phy_info_t *pi, int8 rssi)
{
	phy_info_lcnphy_t *pi_lcn = pi->u.pi_lcnphy;
	if (rssi > 10)
		pi_lcn->Rssi = rssi;
...
WLBANDINITFN(wlc_lcnphy_agc_temp_init)(phy_info_t *pi)
{
	...
	temp = (int16) phy_reg_read(pi, LCNPHY_gainidxoffset);
	...
	temp = (int16)(phy_reg_read(pi, LCNPHY_InputPowerDB)
			& LCNPHY_InputPowerDB_inputpwroffsetdb_MASK);
	if (temp > 127)
		temp -= 256;
	pi_lcn->lcnphy_input_pwr_offset_db = (int8)temp;

	pi_lcn->lcnphy_Med_Low_Gain_db = (phy_reg_read(pi, LCNPHY_LowGainDB)
					& LCNPHY_LowGainDB_MedLowGainDB_MASK)
					>> LCNPHY_LowGainDB_MedLowGainDB_SHIFT;
	pi_lcn->lcnphy_Very_Low_Gain_db = (phy_reg_read(pi, LCNPHY_VeryLowGainDB)
...
wlc_lcnphy_set_tr_attn(phy_info_t *pi, bool override_en, uint8 override_val)
{
	...
	if (tr_attn_idx != 0xff) {
		...
		PHY_REG_MOD(pi, LCNPHY, radioTRCtrlCrs1, gainReqTrAttOnEnByCrs, 0);
	}
...
WLBANDINITFN(wlc_lcnphy_bu_tweaks)(phy_info_t *pi)
{
	...
	if (NORADIO_ENAB(pi->pubpi))
		return;
	/* dmdStExitEn */
	phy_reg_or(pi, LCNPHY_pktfsmctrl, 0x1);
	/* DSSS threshold tightening to avoid cross detections */
	PHY_REG_MOD(pi, LCNPHY, DSSSConfirmCnt, DSSSConfirmCntHiGain, 0x5);
	if (LCNREV_GE(pi->pubpi.phy_rev, 2))
		PHY_REG_MOD(pi, LCNPHY, SyncPeakCnt, Kthresh, 0x1F);
	PHY_REG_MOD(pi, LCNPHY, SyncPeakCnt, MaxPeakCntM1, 0x6);
	phy_reg_write(pi, LCNPHY_dsssPwrThresh0, 0x1e10);
	phy_reg_write(pi, LCNPHY_dsssPwrThresh1, 0x0640);

	/* cck gain table tweak to shift the gain table to give uptil */
	/* 98 dBm rx fe power */
	phy_reg_mod(pi, LCNPHY_gainidxoffset,
	...
	if ((CHIPID(pi->sh->chip) == BCM4313_CHIP_ID)) {
		...
		if (LCNREV_IS(pi->pubpi.phy_rev, 1)) {
			...
			/* CCK Power offset adjustment. PR:86052  */
			if (!pi_lcn->ePA) {
	  ...
	  if (CHSPEC_IS2G(pi->radio_chanspec)) {
{
			...
			if (pi->sh->chippkg == BCM4330_WLBGA_PKG_ID)
				phy_reg_mod(pi, LCNPHY_gainMismatch,
				LCNPHY_gainMismatch_GainmisMatchPktRx_MASK,
				15 << LCNPHY_gainMismatch_GainmisMatchPktRx_SHIFT);
...
#ifdef BAND5G
	    if (BOARDFLAGS(GENERIC_PHY_INFO(pi)->boardflags) & BFL_EXTLNA_5GHz) {
		phy_reg_mod(pi, LCNPHY_crsMiscCtrl0, LCNPHY_crsMiscCtrl0_cfoCalcEn_MASK,
		...
		PHY_REG_LIST_START_WLBANDINITDATA
		 ...
		 PHY_REG_MOD_ENTRY(LCNPHY, veryLowGainEx, veryLowGain_2DB, 37)
		 PHY_REG_MOD_ENTRY(LCNPHY, veryLowGainEx, veryLowGain_3DB, 37)
		...
		PHY_REG_LIST_START_WLBANDINITDATA
		 PHY_REG_MOD_ENTRY(LCNPHY, HiGainDB, HiGainDB, 70)
		 PHY_REG_MOD_ENTRY(LCNPHY, ClipCtrThresh, clipCtrThreshLoGain, 30)
		 PHY_REG_MOD_ENTRY(LCNPHY, LowGainDB, MedLowGainDB, 40)
		 PHY_REG_MOD_ENTRY(LCNPHY, HiGainDB, MedHiGainDB, 49)
		 PHY_REG_MOD_ENTRY(LCNPHY, VeryLowGainDB, veryLowGainDB, 23)
		 ...
		 PHY_REG_MOD_ENTRY(LCNPHY, gaindirectMismatch, medGainGmShftVal, 2)
	         PHY_REG_MOD_ENTRY(LCNPHY, gainMismatch, GainmisMatchPktRx, 9)
		 PHY_REG_MOD_ENTRY(LCNPHY, veryLowGainEx, veryLowGain_2DB, 29)
		 PHY_REG_MOD_ENTRY(LCNPHY, clipCtrThreshLowGainEx, clipCtrThreshLoGain2, 20)
		 PHY_REG_MOD_ENTRY(LCNPHY, veryLowGainEx, veryLowGain_3DB, 29)
		 PHY_REG_MOD_ENTRY(LCNPHY, clipCtrThreshLowGainEx, clipCtrThreshLoGain3, 20)
...
WLBANDINITFN(wlc_radio_2064_init)(phy_info_t *pi)
{
	...
	if ((RADIOID(pi->pubpi.radioid) == BCM2064_ID) &&
		(CHIPID(pi->sh->chip) != BCM4313_CHIP_ID)) {
		/* disable dac gpio */
		write_radio_reg(pi, RADIO_2064_REG073, 0);
	}
	...
	if (CHSPEC_IS5G(pi->radio_chanspec)) {
		if (CHIPID(pi->sh->chip) == BCM4313_CHIP_ID)
			...
			write_radio_reg(pi, RADIO_2064_REG032, 0x6a);
	} else
#endif
		if (pi_lcn->lcnphy_papd_4336_mode)
			write_radio_reg(pi, RADIO_2064_REG032, 0x6a);
	...
	if (pi_lcn->rfreg033)
		write_radio_reg(pi, RADIO_2064_REG033, pi_lcn->rfreg033);

	write_radio_reg(pi, RADIO_2064_REG0C2, 0x6f);
	...
	if ((CHIPID(pi->sh->chip) == BCM4313_CHIP_ID) && !pi_lcn->ePA)
		...
		if (CHSPEC_IS2G(pi->radio_chanspec) && (pi_lcn->dacrate == 160) && !pi_lcn->ePA)
			ofdm_tx_lpf_bw = 4;
		else
			ofdm_tx_lpf_bw = 2;
	}
	...
	/* 4313 IPA tuning */
	if ((CHIPID(pi->sh->chip) == BCM4313_CHIP_ID) && !pi_lcn->ePA) {
		write_radio_reg(pi, RADIO_2064_REG02C, 0x7);
		write_radio_reg(pi, RADIO_2064_REG032, 0x6f);
		write_radio_reg(pi, RADIO_2064_REG08F, 0xc);
		write_radio_reg(pi, RADIO_2064_REG033, 0x39);
		write_radio_reg(pi, RADIO_2064_REG003, 0x66);
		write_radio_reg(pi, RADIO_2064_REG00D, 0x0);
		write_radio_reg(pi, RADIO_2064_REG00C, 0x55);
		write_radio_reg(pi, RADIO_2064_REG00A, 0x73);
		write_radio_reg(pi, RADIO_2064_REG008, 0xff);
		write_radio_reg(pi, RADIO_2064_REG01F, 0x0);
		write_radio_reg(pi, RADIO_2064_REG01D, 0x7);
		mod_radio_reg(pi, RADIO_2064_REG01E, 0x1c, 0x0);
	...
	if (pi_lcn->xtal_mode[0]) {
		mod_radio_reg(pi, RADIO_2064_REG11D, 0x04, 0x04);
		mod_radio_reg(pi, RADIO_2064_REG09F, 0x3f, pi_lcn->xtal_mode[1]);
		mod_radio_reg(pi, RADIO_2064_REG09E, 0x3f, pi_lcn->xtal_mode[2]);
		mod_radio_reg(pi, RADIO_2064_REG077, 0x0f, pi_lcn->xtal_mode[3]);
	}
	if (CHSPEC_IS5G(pi->radio_chanspec)) {
		if (pi_lcn->loidacmode_5g == 1) {
			write_radio_reg(pi, RADIO_2064_REG0B4, 0x24);
			write_radio_reg(pi, RADIO_2064_REG0B7, 0x24);
			write_radio_reg(pi, RADIO_2064_REG0B8, 0x3);
		}
	}
}
...
wlc_lcnphy_rcal(phy_info_t *pi)
{
	...
	and_radio_reg(pi, RADIO_2064_REG129, 0xFD);
}
...
wlc_lcnphy_rc_cal(phy_info_t *pi)
{
	...
	if (0 && pi_lcn->lcnphy_papd_4336_mode && LCNREV_GE(pi->pubpi.phy_rev, 2)) {
		PHY_REG_LIST_START
...
static const char BCMATTACHDATA(rstr_pacalalim2g)[] = "pacalalim2g";
static const char BCMATTACHDATA(rstr_pacalalim5g)[] = "pacalalim5g";
...
BCMATTACHFN(wlc_phy_txpwr_srom_read_lcnphy)(phy_info_t *pi, int bandtype)
{
	...
	pi_lcn->init_txpwrindex_2g = (uint32)PHY_GETINTVAR(pi, rstr_initxidx2g);
	...
	/* VBATSENSE */
	if (PHY_GETVAR(pi, rstr_vbatsmf))
		pi_lcn->lcnphy_vbat_vf = (uint8)PHY_GETINTVAR(pi, rstr_vbatsmf);
	else
		pi_lcn->lcnphy_vbat_vf = 0;

	if (PHY_GETVAR(pi, rstr_vbatsmc))
		pi_lcn->lcnphy_vbat_vc = (uint8)PHY_GETINTVAR(pi, rstr_vbatsmc);
	else
		pi_lcn->lcnphy_vbat_vc = 8;

	if (PHY_GETVAR(pi, rstr_vbatsav))
		pi_lcn->lcnphy_vbat_gs = (uint8)PHY_GETINTVAR(pi, rstr_vbatsav);
	else
		pi_lcn->lcnphy_vbat_gs = 2;


	/* TEMPSENSE */
	if (PHY_GETVAR(pi, rstr_tempsmf))
		pi_lcn->lcnphy_temp_vf = (uint8)PHY_GETINTVAR(pi, rstr_tempsmf);
	else
		pi_lcn->lcnphy_temp_vf = 4;

	if (PHY_GETVAR(pi, rstr_tempsmc))
		pi_lcn->lcnphy_temp_vc = (uint8)PHY_GETINTVAR(pi, rstr_tempsmc);
	else
		pi_lcn->lcnphy_temp_vc = 8;

	if (PHY_GETVAR(pi, rstr_tempsav))
		pi_lcn->lcnphy_temp_gs = (uint8)PHY_GETINTVAR(pi, rstr_tempsav);
	else
		pi_lcn->lcnphy_temp_gs = 2;
	...
	pi_lcn->noise_offset_5gm = (int16)PHY_GETINTVAR_DEFAULT(pi, rstr_noiseoff5gm, -28);
	pi_lcn->noise_offset_5gh = (int16)PHY_GETINTVAR_DEFAULT(pi, rstr_noiseoff5gh, -32);

	freq_corr = chan_info_2066_lcnphy_lna_corr;
	for (i = 0; i < ARRAYSIZE(chan_info_2066_lcnphy_lna_corr); i++) {
		sprintf(str1, "rxgainfreqcorr%d", chan_info_2066_lcnphy_lna_corr[i].chan);
	...
	/* Max tx power */
	txpwr = (int8)PHY_GETINTVAR(pi, rstr_maxp2ga0);
	...
	cckpo = (uint16)PHY_GETINTVAR(pi, rstr_cck2gpo);
	...
	for (i = TXP_FIRST_SISO_MCS_20; i <= TXP_LAST_SISO_MCS_20;  i++) {
		pi->tx_srom_max_rate_2g[i-8] = txpwr -
			((offset_mcs & 0xf) * 2);
		offset_mcs >>= 4;
	}
...
#ifdef BAND5G
	/* Max tx power for 5G */
	txpwr = (int8)PHY_GETINTVAR(pi, rstr_maxp5ga0);
	pi->tx_srom_max_5g_mid = txpwr;

	/* Extract offsets for 8 OFDM mid rates */
	offset_ofdm = (uint32)PHY_GETINTVAR(pi, rstr_ofdm5gpo);
	for (i = TXP_FIRST_OFDM; i <= TXP_LAST_OFDM; i++) {
	...
	/* mcs2gpo(x) are 16 bit numbers */
	offset_mcs =
		((uint16)PHY_GETINTVAR(pi, rstr_mcs5gpo1) << 16) |
		(uint16)PHY_GETINTVAR(pi, rstr_mcs5gpo0);
	for (i = TXP_FIRST_SISO_MCS_20; i <= TXP_LAST_SISO_MCS_20;  i++) {
		pi->tx_srom_max_rate_5g_mid[i-8] = txpwr -
			((offset_mcs & 0xf) * 2);
		offset_mcs >>= 4;
	}

	txpwr = (int8)PHY_GETINTVAR(pi, rstr_maxp5gla0);
	pi->tx_srom_max_5g_low = txpwr;

	/* Extract offsets for 8 OFDM low rates */
	offset_ofdm = (uint32)PHY_GETINTVAR(pi, rstr_ofdm5glpo);
	for (i = TXP_FIRST_OFDM; i <= TXP_LAST_OFDM; i++) {
	...
	/* mcs2gpo(x) are 16 bit numbers */
	offset_mcs =
		((uint16)PHY_GETINTVAR(pi, rstr_mcs5glpo1) << 16) |
		(uint16)PHY_GETINTVAR(pi, rstr_mcs5glpo0);
	for (i = TXP_FIRST_SISO_MCS_20; i <= TXP_LAST_SISO_MCS_20; i++) {
		pi->tx_srom_max_rate_5g_low[i-8] = txpwr -
			((offset_mcs & 0xf) * 2);
		offset_mcs >>= 4;
	}

	txpwr = (int8)PHY_GETINTVAR(pi, rstr_maxp5gha0);
	pi->tx_srom_max_5g_hi = txpwr;

	/* Extract offsets for 8 OFDM high rates */
	offset_ofdm = (uint32)PHY_GETINTVAR(pi, rstr_ofdm5ghpo);
	for (i = TXP_FIRST_OFDM; i <= TXP_LAST_OFDM; i++) {
	...
	/* mcs2gpo(x) are 16 bit numbers */
	offset_mcs =
		((uint16)PHY_GETINTVAR(pi, rstr_mcs5ghpo1) << 16) |
		(uint16)PHY_GETINTVAR(pi, rstr_mcs5ghpo0);
	for (i = TXP_FIRST_SISO_MCS_20; i <= TXP_LAST_SISO_MCS_20; i++) {
		pi->tx_srom_max_rate_5g_hi[i-8] = txpwr -
			((offset_mcs & 0xf) * 2);
		offset_mcs >>= 4;
	}
#endif /* #ifdef BAND5G */
	...
	for (i = 1; i <= CH_MAX_2G_CHANNEL; i++) {
		sprintf(str1, rstr_openlpgainidxbN, i);
	...
	pi_lcn->pacalamamlim_2g = (int16)PHY_GETINTVAR_DEFAULT(pi, rstr_pacalalim2g, 0);

	/* PA cal amam threshold 5g */
	pi_lcn->pacalamamlim_5g = (int16)PHY_GETINTVAR_DEFAULT(pi, rstr_pacalalim5g, 0);
	...
	pi_lcn->init_txpwrindex_5g = (uint32)PHY_GETINTVAR(pi, rstr_initxidx5g);
#endif /* BAND5G */
	...
	pi_lcn->txgaintbl = (bool) PHY_GETINTVAR_DEFAULT(pi, rstr_txgaintbl, FALSE);
	pi_lcn->txgaintbl5g = (uint8) PHY_GETINTVAR_DEFAULT(pi, rstr_txgaintbl5g, 0);
...
wlc_lcnphy_radio_2064_channel_tune_4313(phy_info_t *pi, uint8 channel)
{
	...
	if ((CHIPID(pi->sh->chip) == BCM4313_CHIP_ID) &&
		...
		write_radio_reg(pi, RADIO_2064_REG03C, 0x27);
		write_radio_reg(pi, RADIO_2064_REG03D, 0x51);
	}
...
wlc_lcnphy_radio_2064_channel_tune(phy_info_t *pi, uint8 channel)
{
	...
	void *chi;
	uint8 pll_pwrup, pll_pwrup_ovr;
	fixed qFxtal, qFref, qFvco = 0, qFcal, qVco, qVal;
	...
	/* Tx Tuning */
	write_radio_reg(pi, RADIO_2064_REG091, 0);
	write_radio_reg(pi, RADIO_2064_REG038, 7);
	if (CHIPID(pi->sh->chip) != BCM4313_CHIP_ID)
		write_radio_reg(pi, RADIO_2064_REG088, 0xa);
	...
	if (RADIOID(pi->pubpi.radioid) == BCM2066_ID) {
		...
		if (((CHIPID(pi->sh->chip) == BCM4336_CHIP_ID) &&
			...
			if (CHIPID(pi->sh->chip) != BCM43362_CHIP_ID)
				...
				write_radio_reg(pi, RADIO_2064_REG038, 0x8);
	...
	if ((LCNREV_GE(pi->pubpi.phy_rev, 1) &&
		...
		(CHIPID(pi->sh->chip) == BCM43362_CHIP_ID)) {
		write_radio_reg(pi, RADIO_2064_REG02A, 0xc);
		write_radio_reg(pi, RADIO_2064_REG02C, 0x1);
	}

	if (pi->u.pi_lcnphy->logen_mode[0]) {
		write_radio_reg(pi, RADIO_2064_REG05E, pi->u.pi_lcnphy->logen_mode[1]);
		write_radio_reg(pi, RADIO_2064_REG02A, pi->u.pi_lcnphy->logen_mode[2]);
		write_radio_reg(pi, RADIO_2064_REG02B, pi->u.pi_lcnphy->logen_mode[3]);
		write_radio_reg(pi, RADIO_2064_REG02C, pi->u.pi_lcnphy->logen_mode[4]);
		write_radio_reg(pi, RADIO_2064_REG02D, pi->u.pi_lcnphy->logen_mode[5]);
	}

#ifdef BAND5G
	if (CHSPEC_IS5G(pi->radio_chanspec))
		mod_radio_reg(pi, RADIO_2064_REG0B2, 0x38, 7 << 3);
#endif

	/* a band ipa tuning */
	if ((!pi->u.pi_lcnphy->ePA) && CHSPEC_IS5G(pi->radio_chanspec))
		write_radio_reg(pi, RADIO_2064_REG0BF, 0xee);
	...
	if (1) {
		...
		mod_radio_reg(pi, RADIO_2064_REG0F7, 0x4, 1 << 2);
		mod_radio_reg(pi, RADIO_2064_REG0F1, 0x3, 0);
	...
	if (rfpll_doubler)
		...
		mod_radio_reg(pi, RADIO_2064_REG09D, 0x4, 0x0 << 2);
...
#define QFACTOR	9
	...
	write_radio_reg(pi, RADIO_2064_REG042, ((temp << 3) | ((temp1 & 0x1c) >> 2)));
	write_radio_reg(pi, RADIO_2064_REG043, ((temp1 & 0x1f) | ((temp1 & 3) << 5)));
	...
	write_radio_reg(pi, RADIO_2064_REG040, ((temp << 4) | (temp1 & 0xf)));
	...
	temp1 = (((2640 << (2*QFACTOR))/((h30*h29) >> QFACTOR)) - (420 << QFACTOR)
		+ (46 << (QFACTOR-1)))/(46 << QFACTOR);
	write_radio_reg(pi, RADIO_2064_REG041, ((temp << 4) | (temp1 & 0xf)));
	...
	if (((CHSPEC_IS2G(pi->radio_chanspec)) &&
		...
		qVal = wlc_lcnphy_qdiv_roundup(250 * qFref, qFvco, 16)
				* (cp_current + 4) * (kpd_scale + 1);
		mod_radio_reg(pi, RADIO_2064_REG09E, 0xc0, 0x2<<6);
	}
	else
		qVal = wlc_lcnphy_qdiv_roundup(100 * qFref, qFvco, 16)
				* (cp_current + 4) * (kpd_scale + 1);
	...
	mod_radio_reg(pi, RADIO_2064_REG03D, 0x3f, offset_current);

	/*  PLL_ioff_scale2 */
	mod_radio_reg(pi, RADIO_2064_REG03D, 1 << 6, ioff_scale << 6);
	...
	/* PLL_cal_xt_sdiv */
	if (fref > 52000000)
		mod_radio_reg(pi, RADIO_2064_REG057, 1 << 4, 1 << 4);
	else
		mod_radio_reg(pi, RADIO_2064_REG057, 1 << 4, 0 << 4);
...
BCMATTACHFN(wlc_phy_attach_lcnphy)(phy_info_t *pi, int bandtype)
{
	...
	pi->pi_fptr.phywritetable = wlc_lcnphy_write_table;
	pi->pi_fptr.phyreadtable = wlc_lcnphy_read_table;
...
wlc_lcnphy_get_receive_power(phy_info_t *pi, int8* gain_val)
{
	...
	rf_override_2 = phy_reg_read(pi, LCNPHY_rfoverride2);
	PHY_REG_MOD(pi, LCNPHY, rfoverride2val, hpf1_ctrl_ovr_val, 5);
	PHY_REG_MOD(pi, LCNPHY, rfoverride2val, hpf2_ctrl_ovr_val, 5);
	PHY_REG_MOD(pi, LCNPHY, rfoverride2val, lpf_lq_ovr_val, 1);
	PHY_REG_MOD(pi, LCNPHY, rfoverride2, hpf1_ctrl_ovr, 1);
	PHY_REG_MOD(pi, LCNPHY, rfoverride2, hpf2_ctrl_ovr, 1);
	PHY_REG_MOD(pi, LCNPHY, rfoverride2, lpf_lq_ovr, 1);
...
wlc_lcnphy_rx_signal_power(phy_info_t *pi, int32 gain_index)
{
	...
	nominal_power_db = phy_reg_read(pi, LCNPHY_VeryLowGainDB) >>
...
#define k_noise_cal_max_negative_po_adj  3
...
wlc_lcnphy_noise_log_ucode_data_ok(phy_info_t *pi)
{
	...
	pi_lcn->noise.dbg_info[idx][PHY_NOISE_DBG_NOISE_GAIN] =
		((phy_reg_read(pi, LCNPHY_HiGainDB) & LCNPHY_HiGainDB_HiGainDB_MASK) >>
...
wlc_lcnphy_noise_log_state(phy_info_t *pi)
{
	...
	pi_lcn->noise.dbg_info[idx][PHY_NOISE_DBG_STATE] =
	  (uint16)((phy_reg_read(pi, LCNPHY_sslpnCalibClkEnCtrl) &
	    LCNPHY_sslpnCalibClkEnCtrl_iqEstClkEn_MASK) >>
	  LCNPHY_sslpnCalibClkEnCtrl_iqEstClkEn_SHIFT) |
	  (uint16)(((phy_reg_read(pi, LCNPHY_crsgainCtrl) &
...
wlc_lcnphy_noise_set_input_pwr_offset(phy_info_t *pi)
{
	...
	if (input_pwr_offset != 0xff) {
		...
		phy_reg_mod(pi, LCNPHY_InputPowerDB,
			LCNPHY_InputPowerDB_inputpwroffsetdb_MASK,
...
wlc_lcnphy_noise_init(phy_info_t *pi)
{
	...
	if (gain_tbl_adj > 0) {
		...
		tbl_offset = phy_reg_read(pi, LCNPHY_gainidxoffset);
		...
		phy_reg_write(pi, LCNPHY_gainidxoffset, tbl_offset);
...
{
	phy_info_lcnphy_t *pi_lcn = pi->u.pi_lcnphy;
	int16 high_gain, high_gain_old, input_pwr_offset;
	int8 log_metric, adj;
	bool done = TRUE;
	bool adj_en = pi_lcn->noise.adj_en;

	/* Get current settings */
	high_gain = ((phy_reg_read(pi, LCNPHY_HiGainDB) &
		LCNPHY_HiGainDB_HiGainDB_MASK) >> LCNPHY_HiGainDB_HiGainDB_SHIFT);
	high_gain_old = high_gain;
	input_pwr_offset = ((phy_reg_read(pi, LCNPHY_InputPowerDB) &
		LCNPHY_InputPowerDB_inputpwroffsetdb_MASK) >>
	...
	if (adj_en && (high_gain != high_gain_old)) {
		...
		PHY_REG_WRITE(pi, LCNPHY, nfSubtractVal,
			(pi_lcn->noise.nf_substract_val + 4*(high_gain - pi_lcn->noise.high_gain)));
	...
	if (input_pwr_offset < (pi_lcn->noise.input_pwr_offset - k_noise_cal_max_negative_po_adj))
	  input_pwr_offset = (pi_lcn->noise.input_pwr_offset - k_noise_cal_max_negative_po_adj);
...
#define k_noise_spb_pad_samples 8
	...
	phy_reg_write(pi, LCNPHY_TableAddress, 0x5400);
	if ((phy_reg_read(pi, LCNPHY_TabledataLo) & 0xf) == 5)
	  phy_reg_write(pi, LCNPHY_TableAddress, 0x5400);
	else {
	  phy_reg_write(pi, LCNPHY_TableAddress, 0x5401);
	  n -= 2;
	}
	...
	*/
	...
	while (n > 0) {
		...
		x = ((int32)phy_reg_read(pi, LCNPHY_TabledataLo) >> 4) & 0xfff;
		x |= ((int32)phy_reg_read(pi, LCNPHY_TabledataHi) << 12);
...
wlc_lcnphy_noise_measure_disable(phy_info_t *pi, uint32 flag, uint32* p_flag)
{
	...
	if (p_flag) {
		if (wlc_lcnphy_noise_log_dump_active(pi))
			...
			*p_flag = ((uint32)pi_lcn->noise.enable) |
			(((uint32)pi_lcn->noise.global_adj_en)<<1);
			*p_flag |= (0xffff & wlapi_bmac_read_shm(pi->sh->physhim, M_JSSI_AUX))<<16;
		}
		return;
	}
	...
	if (!wlc_lcnphy_noise_log_ioctl(pi, (flag >> 3))) {
		...
		pi_lcn->noise.global_adj_en = ((flag & 2) ? TRUE : FALSE);
		if (!pi_lcn->noise.enable) {
...
wlc_lcnphy_btc_adjust(phy_info_t *pi, bool btactive)
{
	if ((CHIPID(pi->sh->chip) == BCM4313_CHIP_ID) &&
		...
		if (btactive && !pi->bt_active) {
			...
			write_radio_reg(pi, RADIO_2064_REG060, 0x70);
			write_radio_reg(pi, RADIO_2064_REG061, 0x7f);
			write_radio_reg(pi, RADIO_2064_REG062, 0x75);
...
wlc_lncphy_iq_samp_cap(phy_info_t *pi, int n, uint32* ptr, int mode)
{
	...
	/* configure phy to do a timer based capture of mode 2(adc o/p) */
	phy_reg_write(pi, LCNPHY_sslpnphy40_soft_reset, 3);
	phy_reg_write(pi, LCNPHY_sslpnCalibClkEnCtrl, (uint32)(old_sslpnCalibClkEnCtrl | 0x2008));
	phy_reg_write(pi, LCNPHY_sslpnphy_gpio_ctrl, 5);
	phy_reg_write(pi, LCNPHY_sslpnphy_gpio_sel, (uint16)(mode | mode << 6));
	phy_reg_write(pi, LCNPHY_sslpnphy_gpio_out_en_34_32, 7);
	phy_reg_write(pi, LCNPHY_dbg_samp_coll_start_mac_xfer_trig_timer_15_0, 0);
	phy_reg_write(pi, LCNPHY_dbg_samp_coll_start_mac_xfer_trig_timer_31_16, 0);
	phy_reg_write(pi, LCNPHY_dbg_samp_coll_end_mac_xfer_trig_timer_15_0, (2*n));
	phy_reg_write(pi, LCNPHY_dbg_samp_coll_end_mac_xfer_trig_timer_31_16, 0);
	phy_reg_write(pi, LCNPHY_dbg_samp_coll_ctrl, 0x201);

	phy_reg_write(pi, LCNPHY_sslpnphy40_soft_reset, 0);
	phy_reg_write(pi, LCNPHY_sslpnphy_gpio_test_reg_17_2,  0xdead);
	phy_reg_write(pi, LCNPHY_sslpnphy_gpio_test_reg_33_18, 0xbeef);
	...
	do {
		OSL_DELAY(10);
		timer++;
	} while ((phy_reg_read(pi, LCNPHY_dbg_samp_coll_ctrl) & 1) && (timer < 500));


	if (phy_reg_read(pi, LCNPHY_dbg_samp_coll_ctrl) & 1) {
		return 0;
	}

	/* get data from spb */
	phy_reg_write(pi, LCNPHY_sslpnphy_gpio_test_reg_17_2, phy_reg_read(pi, 0x593));
	phy_reg_write(pi, LCNPHY_sslpnphy_gpio_test_reg_33_18, phy_reg_read(pi, 0x580));
	n_samps_out = 0;
	phy_reg_write(pi, LCNPHY_TableAddress, 0x5400);
	if ((phy_reg_read(pi, LCNPHY_TabledataLo) & 0xf) == 5)
		phy_reg_write(pi, LCNPHY_TableAddress, 0x5400);
	...
	while (n > 0) {
		...
		lo = phy_reg_read(pi, LCNPHY_TabledataLo);
		hi = phy_reg_read(pi, LCNPHY_TabledataHi);
	...
	phy_reg_write(pi, LCNPHY_sslpnCalibClkEnCtrl, old_sslpnCalibClkEnCtrl);
...
wlc_lcnphy_iovar_samp_cap(wlc_phy_t *ppi, int32 gain, int32 *ret)
{
	...
	if (ret == NULL) {
		...
		swctrl_override = phy_reg_read(pi, LCNPHY_swctrlOvr);
		rf_override_0 = phy_reg_read(pi, LCNPHY_RFOverride0);
		rf_override_2 = phy_reg_read(pi, LCNPHY_rfoverride2);
		rf_override_3 = phy_reg_read(pi, LCNPHY_rfoverride3);
		rf_override_4 = phy_reg_read(pi, LCNPHY_rfoverride4);
		phy_reg_write(pi, LCNPHY_swctrlOvr, 0);
		phy_reg_write(pi, LCNPHY_RFOverride0, 0);
		phy_reg_write(pi, LCNPHY_rfoverride2, 0);
		phy_reg_write(pi, LCNPHY_rfoverride3, 0);
		phy_reg_write(pi, LCNPHY_rfoverride4, 0);
		...
		phy_reg_write(pi, LCNPHY_swctrlOvr, swctrl_override);
		phy_reg_write(pi, LCNPHY_RFOverride0, rf_override_0);
		phy_reg_write(pi, LCNPHY_rfoverride2, rf_override_2);
		phy_reg_write(pi, LCNPHY_rfoverride3, rf_override_3);
		phy_reg_write(pi, LCNPHY_rfoverride4, rf_override_4);
