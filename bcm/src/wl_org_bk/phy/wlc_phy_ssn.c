 ...
 *
 * $Id: wlc_phy_ssn.c 279629 2011-08-25 02:53:36Z csm $
 */
...
#if (defined(WLNINTENDO2) || defined(WLTEST)) && defined(ROMTERMPHY)
	if (sdna_board_flag) {
		wlc_sslpnphy_set_crs(pi, SSLPNPHY_CRS_DESENSE_VAL);
	}
#endif /* (WLNINTENDO2 || WLTEST) && ROMTERMPHY */
...
wlc_sslpnphy_radio_2063_channel_tune(phy_info_t *pi, uint8 channel)
{
	...
	qFxtal = wlc_lpphy_qdiv_roundup(PHY_XTALFREQ(pi->xtalfreq), PLL_2063_MHZ, 16);
...
#else
	wlc_info_t * wlc_pi = pi->wlc;
#if defined(WLNINTENDO2) || defined(WLTEST)
	bool sdna_board_flag =
		(BOARDTYPE(GENERIC_PHY_INFO(pi)->sih->boardtype) == BCM94319SDNA_SSID);
#endif /* WLNINTENDO2 || WLTEST */
#endif /* !ROMTERMPHY */
